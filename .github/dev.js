...
const DASH_DASH_ARGS = process.argv
...
if (DASH_DASH_ARGS.includes('search')) {
  ...
  COMMAND_GHOST.env['sodoSearch__url'] =
    'http://localhost:5370/umd/sodo-search.min.js';
  COMMAND_GHOST.env['sodoSearch__styles'] =
    'http://localhost:5370/umd/main.css';
}
...
(async () => {
  if (DASH_DASH_ARGS.includes('stripe')) {
    ...
    try {
      ...
      console.error(
        'Failed to fetch Stripe secret token, do you need to connect Stripe CLI?',
        err
      );
      return;
    }
