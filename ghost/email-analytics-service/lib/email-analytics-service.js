...
module.exports = class EmailAnalyticsService {
  ...
  async fetchLatest({ maxEvents = Infinity } = {}) {
    ...
    if (end <= begin) {
      // Skip for now
      logging.info(
        '[EmailAnalytics] Skipping fetchLatest because end (' +
          end +
          ') is before begin (' +
          begin +
          ')'
    ...
    return await this.#fetchEvents(this.#fetchLatestData, {
      begin,
      end,
      maxEvents,
    });
  ...
  async fetchMissing({ maxEvents = Infinity } = {}) {
    ...
    const begin =
      ...
      this.#fetchMissingData?.lastBegin ??
      new Date(Date.now() - TRUST_THRESHOLD_MS * 3);
    ...
    return await this.#fetchEvents(this.#fetchMissingData, {
      begin,
      end,
      maxEvents,
    });
  ...
  async fetchScheduled({ maxEvents = Infinity } = {}) {
    ...
    if (end <= begin) {
      // Skip for now
      logging.info(
        '[EmailAnalytics] Ending fetchScheduled because end is before begin'
      );
      this.#fetchScheduledData = null;
      return 0;
    }

    const count = await this.#fetchEvents(this.#fetchScheduledData, {
      begin,
      end,
      maxEvents,
    });
  ...
  async #fetchEvents(fetchData, { begin, end, maxEvents = Infinity }) {
    ...
    const processBatch = async (events) => {
      ...
      if (
        Date.now() - lastAggregation > 5 * 60 * 1000 ||
    ...
    try {
      for (const provider of this.providers) {
        await provider.fetchLatest(processBatch, {
          begin,
          end,
          maxEvents,
        });
    ...
    if (
      ...
      logging.info(
        '[EmailAnalytics] Reached end of new events, increasing lastEventTimestamp with one second'
      );
      fetchData.lastEventTimestamp = new Date(
        fetchData.lastEventTimestamp.getTime() + 1000
      );
    }
  ...
  async processEventBatch(events, result, fetchData) {
    ...
    if (time > 1000) {
      ...
      logging.warn(
        `[EmailAnalytics] Processing event batch took ${(time / 1000).toFixed(
          1
        )}s`
      );
    }
  ...
  async aggregateStats({ emailIds = [], memberIds = [] }) {
    ...
    logging.info(
      `[EmailAnalytics] Aggregating for ${memberIds.length} members`
    );
