...
const {
  ...
  TierNameChangeEvent,
} = require('@tryghost/tiers');
const OfferCreatedEvent = require('@tryghost/members-offers').events
  .OfferCreatedEvent;
...
class PaymentsService {
  ...
  async getCustomerForMember(member) {
    ...
    for (const row of rows) {
      try {
        const customer = await this.stripeAPIService.getCustomer(
          row.customer_id
        );
        if (!customer.deleted) {
          return customer;
  ...
  async getProductForTier(tier) {
    const rows = await this.StripeProductModel.where({
    ...
    for (const row of rows) {
      try {
        const product = await this.stripeAPIService.getProduct(
