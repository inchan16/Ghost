...
function printBuildErrorDetails() {
  if (stdOutChunks && stdOutChunks.length > 0) {
    const failDetails = Buffer.concat(
      stdOutChunks.slice(4, stdOutChunks.length - 1)
    )
      .toString()
      .replace(/^(?=\n)$|\s*$|\n\n+/gm, '');
...
function printBuildComplete(code) {
  if (code === 0) {
    if (!showVerbose) {
      ...
      log();
      log(
        chalk.bold.greenBright.bgBlackBright(
          `${'-'.repeat(25)}Build Success${'-'.repeat(25)}`
        )
      );
    }
  } else {
    if (!showVerbose) {
      ...
      log(
        chalk.bold.redBright.bgBlackBright(
          `${'-'.repeat(25)}Build finished: Failed${'-'.repeat(25)}`
        )
      );
    }
  }
...
function printBuildStart() {
  if (showVerbose) {
    log(
      chalk.bold.greenBright.bgBlackBright(
        `${'-'.repeat(32)}Building${'-'.repeat(32)}`
      )
    );
    log();
  } else {
    spinner = ora(
      chalk.magentaBright.bold('Bundling files, hang on...')
    ).start();
  }
}
...
function startDevServer() {
      ...
      rewrites: [
        {
          ...
          destination: 'umd/announcement-bar.min.js',
        },
        {
          source: '/announcement-bar.min.js.map',
          destination: 'umd/announcement-bar.min.js.map',
        },
      ],
      headers: [
        {
  ...
  server.listen(port, () => {
    log(
      chalk.whiteBright(
        `Announcement-Bar dev server is running on http://localhost:${port}`
      )
    );
    watchFiles();
  });
