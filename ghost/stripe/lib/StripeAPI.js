...
module.exports = class StripeAPI {
  ...
  configure(config) {
    ...
    this._config = config;
    this._testMode =
      config.secretKey && config.secretKey.startsWith('sk_test_');
    if (this._testMode) {
    ...
    try {
      ...
      if (webhook.api_version !== STRIPE_API_VERSION) {
        throw new VersionMismatchError({
          message: 'Webhook has incorrect api_version',
        });
      }
  ...
  parseWebhook(body, signature, secret) {
    ...
    try {
      ...
      debug(
        `parseWebhook(${body}, ${signature}, ${secret}) -> Success ${event.type}`
      );
      return event;
    } catch (err) {
  ...
  async createCheckoutSetupSession(customer, options) {
    ...
    const session = await this._stripe.checkout.sessions.create({
      ...
      payment_method_types: ['card'],
      success_url:
        options.successUrl || this._config.checkoutSetupSessionSuccessUrl,
      cancel_url:
        options.cancelUrl || this._config.checkoutSetupSessionCancelUrl,
  ...
  async getSubscription(id, options = {}) {
    ...
    try {
      await this._rateLimitBucket.throttle();
      const subscription = await this._stripe.subscriptions.retrieve(
        id,
        options
      );
      ...
      debug(
        `getSubscription(${id}, ${JSON.stringify(options)}) -> ${err.type}`
      );
      throw err;
    }
  }
  ...
  async updateSubscriptionItemPrice(subscriptionId, id, price) {
    await this._rateLimitBucket.throttle();
    const subscription = await this._stripe.subscriptions.update(
      subscriptionId,
      {
