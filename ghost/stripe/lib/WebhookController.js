...
module.exports = class WebhookController {
  ...
  async handle(req, res) {
    ...
    let event;
    try {
      event = this.webhookManager.parseWebhook(
        req.body,
    ...
    if (session.mode === 'setup') {
      ...
      if (setupIntent.metadata.subscription_id) {
        const updatedSubscription =
          await this.api.updateSubscriptionDefaultPaymentMethod(
      ...
      const activeSubscriptions = subscriptions.models.filter(
        (subscription) => {
      ...
      for (const subscription of activeSubscriptions) {
        if (
          subscription.get('customer_id') === setupIntent.metadata.customer_id
        ) {
          const updatedSubscription =
            await this.api.updateSubscriptionDefaultPaymentMethod(
      ...
      if (!member) {
        ...
        if (metadataNewsletters) {
          try {
            ...
            logging.error(
              `Ignoring invalid newsletters data - ${metadataNewsletters}.`
            );
          }
        }
        ...
        member = await this.deps.memberRepository.create(
          memberDataWithStripeCustomer
        );
      } else {
        const payerName = _.get(
          customer,
