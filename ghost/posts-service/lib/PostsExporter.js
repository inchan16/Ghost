...
class PostsExporter {
  ...
  async export({ filter, order, limit }) {
    ...
    const membersTrackSources =
      membersEnabled && this.#settingsCache.get('members_track_sources');
    const paidMembersEnabled =
      membersEnabled && this.#settingsHelpers.arePaidMembersEnabled();
    const trackOpens = this.#settingsCache.get('email_track_opens');
    const trackClicks = this.#settingsCache.get('email_track_clicks');
    const hasNewslettersWithFeedback = !!newsletters.find((newsletter) =>
      newsletter.get('feedback_enabled')
    );
    ...
    const mapped = posts.data.map((post) => {
      ...
      const feedbackEnabled =
        email && email.get('feedback_enabled') && hasNewslettersWithFeedback;
      const showEmailClickAnalytics =
        trackClicks && email && email.get('track_clicks');
      ...
      return {
        ...
        email_recipients: email
          ? this.humanReadableEmailRecipientFilter(
        ...
        newsletter_name:
          newsletters.length > 1 && post.get('newsletter_id') && email
            ? newsletters
                .find(
                  (newsletter) =>
                    newsletter.get('id') === post.get('newsletter_id')
                )
                ?.get('name')
            : null,
        ...
        free_signups:
          membersTrackSources && published
            ? post.get('count__signups') ?? 0
            : null,
        paid_conversions:
          membersTrackSources && paidMembersEnabled && published
            ? post.get('count__paid_conversions') ?? 0
            : null,
        feedback_more_like_this: feedbackEnabled
          ? post.get('count__positive_feedback') ?? 0
          : null,
        feedback_less_like_this: feedbackEnabled
          ? post.get('count__negative_feedback') ?? 0
          : null,
      };
    });
  ...
  postAccessToString(post) {
    ...
    if (visibility === 'tiers') {
      ...
      return (
        'Specific tiers: ' + tiers.map((tier) => tier.get('name')).join(', ')
      );
    }
