...
const messages = {
  ...
  invalidEmailSegment:
    "The email segment parameter doesn't contain a valid filter",
...
class PostsService {
  ...
  async editPost(frame) {
    ...
    if (frame.options.newsletter && frame.options.email_segment) {
      if (frame.options.email_segment !== 'all') {
        ...
        try {
          await this.models.Member.findPage({
            filter: frame.options.email_segment,
            limit: 1,
          });
        } catch (err) {
    ...
    if (model.get('newsletter_id')) {
      const sendEmail =
        model.wasChanged() &&
        this.shouldSendEmail(model.get('status'), model.previous('status'));
  ...
  async bulkEdit(data, options) {
    ...
    if (data.action === 'access') {
      if (
        !['public', 'members', 'paid', 'tiers'].includes(data.meta.visibility)
      ) {
  ...
  async bulkDestroy(options) {
    ...
    const postRows = await this.models.Post.getFilteredCollectionQuery({
      ...
      .select('posts.id', 'emails.id as email_id');
    ...
    return await this.models.Post.bulkDestroy(deleteIds, 'posts', {
      transacting: options.transacting,
      throwErrors: true,
    });
  }
  ...
  async getProductsFromVisibilityFilter(visibilityFilter) {
    try {
      ...
      const productsData =
        (visibilityFilterJson.product
          ? [visibilityFilterJson]
          : visibilityFilterJson.$or) || [];
  ...
  shouldSendEmail(currentStatus, previousStatus) {
    return (
      ['published', 'sent'].includes(currentStatus) &&
  ...
  handleCacheInvalidation(model) {
    ...
    if (
      ...
      (model.get('status') === 'draft' &&
        model.previous('status') === 'published')
    ) {
