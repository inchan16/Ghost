...
class LinkClickTrackingService {
  ...
  async #updateLinks(data, options) {
    ...
    const transformedOldUrl = this.#urlUtils.absoluteToTransformReady(
      redirectUrl.href
    );
    ...
    const linkIds = await this.#linkRedirectService.getFilteredIds(
    ...
    return await this.#postLinkRepository.updateLinks(
      linkIds,
      updateData,
      bulkUpdateOptions
    );
