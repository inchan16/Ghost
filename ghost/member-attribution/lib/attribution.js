...
class Attribution {
  ...
  getResource(model) {
    ...
    const updatedUrl = this.#urlTranslator.getUrlByResourceId(this.id, {
      absolute: true,
    });
    ...
    return {
      ...
      title:
        model.get('title') ??
        model.get('name') ??
        this.#urlTranslator.getUrlTitle(this.url),
      referrerSource: this.referrerSource,
      referrerMedium: this.referrerMedium,
      referrerUrl: this.referrerUrl,
    };
  ...
  async fetchResource() {
    ...
    // Fetch model
    const model = await this.#urlTranslator.getResourceById(
      this.id,
      this.type,
      { absolute: true }
    );
...
class AttributionBuilder {
  ...
  async getAttribution(history) {
    ...
    const referrerData = this.referrerTranslator.getReferrerDetails(
      history
    ) || {
      referrerSource: null,
      referrerMedium: null,
