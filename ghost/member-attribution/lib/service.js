...
class MemberAttributionService {
  ...
  async getAttributionFromContext(context) {
    ...
    if (['import', 'api', 'admin'].includes(source)) {
      ...
      if (context?.integration?.id) {
        try {
          const integration = await this.models.Integration.findOne({
            id: context.integration.id,
          });
          attribution.referrerSource = `Integration: ${integration?.get(
            'name'
          )}`;
        } catch (error) {
  ...
  addPostAttributionTracking(url, post) {
    ...
    if (
      url.searchParams.has('attribution_id') ||
      url.searchParams.has('attribution_type')
    ) {
      // Don't overwrite
      return url;
    }
  ...
  async getMemberCreatedAttribution(memberId) {
    const memberCreatedEvent = await this.models.MemberCreatedEvent.findOne(
      { member_id: memberId },
  ...
  async getSubscriptionCreatedAttribution(subscriptionId) {
    const subscriptionCreatedEvent =
      await this.models.SubscriptionCreatedEvent.findOne(
