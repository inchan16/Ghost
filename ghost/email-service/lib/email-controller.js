...
class EmailController {
  ...
  async _getFrameData(frame) {
    ...
    let post;
    if (frame.options.id) {
      post = await this.models.Post.findOne(
      ...
      post = await this.models.Post.findOne(
        ...
        { ...frame.options, withRelated: ['posts_meta', 'authors'] }
      );
    }
    ...
    if (slug) {
      ...
      newsletter =
        (await post.getLazyRelation('newsletter')) ??
        (await this.models.Newsletter.getDefaultNewsletter());
    }
    return {
      post,
      newsletter,
  ...
  async retryFailedEmail(frame) {
    const email = await this.models.Email.findOne(frame.data, {
      require: false,
    });
