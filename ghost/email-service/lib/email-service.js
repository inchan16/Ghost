...
class EmailService {
  ...
  async createEmail(post) {
    ...
    const emailCount = await this.#emailSegmenter.getMembersCount(
      newsletter,
    ...
    if (email) {
      ...
      if (member) {
        ...
        if (segment === 'status:-free' && member.get('status') !== 'free') {
          ...
          const subscriptions = (
            await member.getLazyRelation('stripeSubscriptions')
          ).toJSON();
  ...
  async previewEmail(post, newsletter, segment) {
    ...
    let { html, plaintext, replacements } =
      await this.#emailRenderer.renderBody(post, newsletter, segment, {
    ...
    return {
      ...
      plaintext: this.replaceDefinitions(
        plaintext,
        replacements,
        exampleMember
      ),
