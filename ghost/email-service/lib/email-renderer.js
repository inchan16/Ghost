...
class EmailRenderer {
  ...
  getSubject(post) {
    return (
      post.related('posts_meta')?.get('email_subject') || post.get('title')
    );
  }

  getFromAddress(_post, newsletter) {
    let senderName = this.#settingsCache.get('title')
      ? this.#settingsCache.get('title').replace(/"/g, '\\"')
      : '';
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (
        /@localhost$/.test(fromAddress) ||
        /@ghost.local$/.test(fromAddress)
      ) {
        const localAddress = 'localhost@example.com';
        logging.warn(
          `Rewriting bulk email from address ${fromAddress} to ${localAddress}`
        );
  ...
  getSegments(post) {
    ...
    const segments = [...new Set(allSegments)].filter((segment) =>
  ...
  renderPostBaseHtml(post) {
    ...
    let html;
    if (post.get('lexical')) {
      html = this.#renderers.lexical.render(post.get('lexical'), {
        target: 'email',
        postUrl,
      });
    } else {
  ...
  async renderBody(post, newsletter, segment, options) {
    ...
    const isPaidPost =
      post.get('visibility') === 'paid' || post.get('visibility') === 'tiers';
    ...
    if (options.clickTrackingEnabled) {
      html = await this.#linkReplacer.replace(html, async (url) => {
        ...
        url = await this.#linkTracking.service.addTrackingToUrl(
          url,
          post,
          '--uuid--'
        );
  ...
  getMemberStatusText(member) {
    ...
    if (expires) {
      ...
      return tpl(messages.subscriptionStatus.complimentaryExpires, {
        date,
      });
    }
    ...
    const baseDefinitions = [
      {
        id: 'unsubscribe_url',
        getValue: (member) => {
          return this.createUnsubscribeUrl(member.uuid, {
            newsletterUuid,
          });
        },
      },
      ...
      },
      {
        ...
        getValue: (member) => {
          ...
          return member.createdAt
            ? formatDateLong(member.createdAt, timezone)
            : '';
        },
      },
      {
      ...
      if (match) {
        ...
        if (definition) {
          replacements.push({
            ...
            token: new RegExp(
              escapeRegExp(replacementMatch).replace(
                /(?:"|&quot;)/g,
                '(?:"|&quot;)'
              ),
              'g'
  ...
  async renderTemplate(data) {
    ...
    if (this.#labs.isSet('makingItRain')) {
      const cssPartialSource = await fs.readFile(
        path.join(__dirname, './email-templates/partials/', `styles.hbs`),
        'utf8'
      );
      this.#handlebars.registerPartial('styles', cssPartialSource);
    } else {
      const cssPartialSource = await fs.readFile(
        path.join(__dirname, './email-templates/partials/', `styles-old.hbs`),
        'utf8'
      );
      this.#handlebars.registerPartial('styles', cssPartialSource);
    }

    const paywallPartial = await fs.readFile(
      path.join(__dirname, './email-templates/partials/', `paywall.hbs`),
      'utf8'
    );
    this.#handlebars.registerPartial('paywall', paywallPartial);

    const feedbackButtonPartial = await fs.readFile(
      path.join(
        __dirname,
        './email-templates/partials/',
        `feedback-button.hbs`
      ),
      'utf8'
    );
    ...
    const feedbackButtonMobilePartial = await fs.readFile(
      path.join(
        __dirname,
        './email-templates/partials/',
    ...
    this.#handlebars.registerPartial(
      ...
      feedbackButtonMobilePartial
    );

    const latestPostsPartial = await fs.readFile(
      path.join(__dirname, './email-templates/partials/', `latest-posts.hbs`),
      'utf8'
    );
    ...
    if (this.#labs.isSet('makingItRain')) {
      const htmlTemplateSource = await fs.readFile(
        path.join(__dirname, './email-templates/', `template.hbs`),
        'utf8'
      );
      this.#renderTemplate = this.#handlebars.compile(
        Buffer.from(htmlTemplateSource).toString()
      );
    } else {
      const htmlTemplateSource = await fs.readFile(
        path.join(__dirname, './email-templates/', `template-old.hbs`),
        'utf8'
      );
      this.#renderTemplate = this.#handlebars.compile(
        Buffer.from(htmlTemplateSource).toString()
  ...
  truncateHtml(text, maxLength, maxLengthMobile) {
    ...
    if (text && text.length > maxLength) {
      if (text.length <= maxLengthMobile) {
        return (
          escapeHtml(text.substring(0, maxLength - 1)) +
          '<span class="mobile-only">' +
          escapeHtml(text.substring(maxLength - 1, maxLengthMobile - 1)) +
          '</span>' +
  ...
  async getTemplateData({ post, newsletter, html, addPaywall }) {
    ...
    try {
      adjustedAccentColor =
        accentColor &&
        darkenToContrastThreshold(accentColor, '#ffffff', 2).hex();
      adjustedAccentContrastColor =
        accentColor && textColorForBackgroundColor(adjustedAccentColor).hex();
    } catch (e) {
      logging.error(e);
      accentColor = '#15212A';
    }
    ...
    const backgroundIsDark =
      textColorForBackgroundColor(backgroundColor).hex().toLowerCase() ===
      '#ffffff';
    const borderColor = this.#getBorderColor(newsletter, accentColor);
    const secondaryBorderColor = textColorForBackgroundColor(backgroundColor)
      .alpha(0.12)
      .toString();
    ...
    const secondaryTextColor = textColorForBackgroundColor(backgroundColor)
      .alpha(0.5)
      .toString();
    const linkColor = backgroundIsDark ? '#ffffff' : accentColor;

    const { href: headerImage, width: headerImageWidth } =
      await this.limitImageWidth(newsletter.get('header_image'));
    const {
    ...
    if (postAuthors?.models) {
      if (postAuthors.models.length <= 2) {
        authors = postAuthors.models
          .map((author) => author.get('name'))
          .join(' & ');
      } else {
        authors = `${postAuthors.models[0].get('name')} & ${
          postAuthors.models.length - 1
        } others`;
      }
    }
    ...
    // Audience feedback
    const positiveLink = this.#audienceFeedbackService
      .buildLink('--uuid--', post.id, 1)
      .href.replace('--uuid--', '%%{uuid}%%');
    const negativeLink = this.#audienceFeedbackService
      .buildLink('--uuid--', post.id, 0)
      .href.replace('--uuid--', '%%{uuid}%%');
    ...
    const hasEmailOnlyFlag =
      post.related('posts_meta')?.get('email_only') ?? false;
    ...
    if (newsletter.get('show_latest_posts')) {
      ...
      for (const latestPost of data) {
        ...
        } = await this.limitImageWidth(
          latestPost.get('feature_image'),
          100,
          100
        );
        const {
    ...
    const data = {
      ...
      post: {
        ...
        feature_image_caption: post
          .related('posts_meta')
          ?.get('feature_image_caption'),
      },
      ...
      classes: {
        title:
          'post-title' +
          (newsletter.get('title_font_category') === 'serif'
            ? ` post-title-serif`
            : ``) +
          (newsletter.get('title_alignment') === 'left'
            ? ` post-title-left`
            : ``),
        titleLink:
          'post-title-link' +
        ...
        meta:
          ...
          (newsletter.get('title_alignment') === 'left'
            ...
            : ` post-meta-center`),
        body:
          newsletter.get('body_font_category') === 'sans_serif'
      ...
      try {
        ...
        if (this.#storageUtils.isLocalImage(href)) {
          ...
          return {
            href: href.replace(
              /\/content\/images\//,
              '/content/images/size/w' +
                visibleWidth * 2 +
                (visibleHeight ? 'h' + visibleHeight * 2 : '') +
                '/'
            ),
            width: size.width,
            height: size.height,
