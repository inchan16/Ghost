...
class BatchSendingService {
  ...
  async emailJob({ emailId }) {
    ...
    const retryCutOffTime = new Date(
      Date.now() + this.#BEFORE_RETRY_CONFIG.maxTime
    );
    ...
    if (!email) {
      logging.error(
        `Tried sending email that is not pending or failed ${emailId}`
      );
      return;
    }
    ...
    try {
      ...
      await this.retryDb(
        ...
        {
          ...this.#AFTER_RETRY_CONFIG,
          description: `email ${emailId} -> submitted`,
        }
      );
    } catch (e) {
      ...
      await this.retryDb(
          ...
          await email.save(
            {
              status: 'failed',
              error:
                e.message || 'Something went wrong while sending the email',
            },
            { patch: true, autoRefresh: false }
          );
        },
        {
          ...this.#AFTER_RETRY_CONFIG,
          description: `email ${emailId} -> failed`,
        }
      );
    }
  }
  ...
  async sendEmail(email) {
    ...
    const newsletter = await this.retryDb(
      async () => {
        return await email.getLazyRelation('newsletter', {
          require: true,
        });
      },
      {
    ...
    const post = await this.retryDb(
      async () => {
        return await email.getLazyRelation('post', {
          require: true,
          withRelated: ['posts_meta', 'authors'],
        });
      },
      {
        ...this.#getBeforeRetryConfig(email),
    ...
    let batches = await this.retryDb(
      ...
      {
        ...this.#getBeforeRetryConfig(email),
        description: `getBatches for email ${email.id}`,
      }
    );
  ...
  async getBatches(email) {
    ...
    return await this.#models.EmailBatch.findAll({
  ...
  async createBatches({ email, post, newsletter }) {
    ...
    for (const segment of segments) {
      ...
      while (!members || lastId) {
        ...
        members = await this.#models.Member.getFilteredCollectionQuery({
          filter,
        })
          .orderByRaw('id DESC')
          ...
          const batch = await this.retryDb(
            async () => {
              return await this.createBatch(
                email,
                segment,
                members.slice(0, BATCH_SIZE)
              );
            },
            {
              ...this.#getBeforeRetryConfig(email),
              description: `createBatch email ${email.id} segment ${segment}`,
            }
          );
          batches.push(batch);
        }
    ...
    if (email.get('email_count') !== totalCount) {
      logging.error(
        `Email ${email.id} has wrong stored email_count ${email.get(
          'email_count'
        )}, did expect ${totalCount}. Updating the model.`
      );
  ...
  async createBatch(email, segment, members, options) {
    if (!options || !options.transacting) {
      return this.#models.EmailBatch.transaction(async (transacting) => {
        return this.createBatch(email, segment, members, {
          transacting,
        });
      });
    }
  ...
  async sendBatch({
    ...
    if (!batch) {
      logging.error(
        `Tried sending email batch that is not pending or failed ${originalBatch.id}`
      );
      return true;
    }
    ...
    try {
      ...
      const response = await this.retryDb(
        ...
        },
        {
          ...
          description: `Sending email batch ${originalBatch.id}`,
        }
      );
      succeeded = true;
    ...
    await this.retryDb(
      async () => {
        await this.#models.EmailRecipient.where({
          batch_id: batch.id,
        }).save(
  ...
  async updateStatusLock(Model, id, status, allowedStatuses) {
    ...
    await Model.transaction(async (transacting) => {
      model = await Model.findOne(
        { id },
        { require: true, transacting, forUpdate: true }
      );
