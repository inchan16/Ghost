...
const messages = {
  ...
  unallowedValueForSetting:
    "Unallowed value for '{key}'. Allowed values: {allowedValues}.",
  invalidValueForSetting:
    "Invalid value for '{key}'. The value must follow this format: {format}.",
};
...
module.exports = class CustomThemeSettingsService {
  ...
  async activateTheme(name, theme) {
    ...
    if (this._activatingPromise) {
      ...
      const isSameSettings =
        JSON.stringify(theme.customSettings) === this._activatingSettings;
   ...
   */
  listSettings() {
    const settingObjects = Object.entries(this._activeThemeSettings).map(
      ([key, setting]) => {
  ...
  async updateSettings(settings) {
    ...
    const firstUnknownSetting = settings.find(
      (setting) => !this._activeThemeSettings[setting.key]
    );
      ...
      switch (definition.type) {
        case 'select':
          if (!definition.options.includes(setting.value)) {
            throw new ValidationError({
              message: tpl(messages.unallowedValueForSetting, {
                key: setting.key,
        ...
        case 'color':
          if (!/^#[0-9a-f]{6}$/i.test(setting.value)) {
            throw new ValidationError({
              message: tpl(messages.invalidValueForSetting, {
                key: setting.key,
                format: '#1234AF',
              }),
            });
  ...
  async _syncRepositoryWithTheme(name, theme) {
    ...
    const settingsCollection = await this._repository.browse({
      filter: `theme:'${name}'`,
    });
    ...
    for (const knownSetting of knownSettings) {
      ...
      const hasChangedType =
        themeSetting && themeSetting.type !== knownSetting.type;

      if (hasBeenRemoved || hasChangedType) {
        debug(
          `Removing custom theme setting '${name}.${knownSetting.key}' - ${
            hasBeenRemoved ? 'not found in theme' : 'type changed'
          }`
        );
      ...
      if (
        ...
        !themeSetting.options.includes(knownSetting.value)
      ) {
        debug(
          `Resetting custom theme setting value '${name}.${themeSetting.key}' - "${knownSetting.value}" is not a valid option`
        );
        await this._repository.edit(
          { value: themeSetting.default },
          { id: knownSetting.id }
        );
      }
    }
    ...
    knownSettings = knownSettings.filter(
      (setting) => !removedIds.includes(setting.id)
    );
    ...
    const updatedSettingsCollection = await this._repository.browse({
      filter: `theme:'${name}'`,
    });
