...
module.exports = class MembersCSVImporter {
  ...
  async prepare(inputFilePath, headerMapping, defaultLabels) {
    ...
    if (pathExists) {
      throw new errors.DataImportError({
        message: tpl(messages.filenameCollision),
      });
    }

    // completely rely on explicit user input for header mappings
    const rows = await membersCSV.parse(
      inputFilePath,
      headerMapping,
      defaultLabels
    );
  ...
  async perform(filePath) {
    ...
    const result = await rows.reduce(
      async (resultPromise, row) => {
        ...
        const trx = await this._knex.transaction(undefined, {
        ...
        try {
          ...
          const createdAt = moment(row.created_at).isAfter(moment())
            ? moment().toDate()
            : row.created_at;
          const memberValues = {
            email: row.email,
          ...
          const existingMember = await membersRepository.get(
          ...
          let member;
          if (existingMember) {
            const existingLabels = existingMember.related('labels')
              ? existingMember.related('labels').toJSON()
              : [];
  ...
  async process({
    ...
    if ((job.batches <= 500 && !job.metadata.hasStripeData) || forceInline) {
      ...
      const importLabelModel = result.imported
        ? await LabelModel.findOne(importLabel)
        : null;
      ...
      this._addJob({
        job: async () => {
          ...
          try {
            ...
            logging.error(
              'Error in members import job when testing import threshold'
            );
            logging.error(e);
          }
        },
