...
const validate = (config, attrs) => {
  ...
  _.each(attrs, (value, key) => {
    ...
    if (config?.[key]) {
      const allowedValues = Array.isArray(config[key])
        ? config[key]
        : config[key].values;
      ...
      if (allowedValues) {
        ...
        if (unallowedValues.length) {
          ...
          if (key === 'include') {
            attrs.include = valuesAsArray.filter((x) =>
              allowedValues.includes(x)
            );
            return;
          }
  ...
  add(apiConfig, frame) {
    ...
    if (!['posts', 'tags'].includes(apiConfig.docName)) {
      if (
        ...
        return Promise.reject(
          new BadRequestError({
            message: tpl(messages.noRootKeyProvided, {
    ...
    if (apiConfig.data) {
      ...
      _.each(apiConfig.data, (value, key) => {
        if (
          jsonpath.query(frame.data[apiConfig.docName][0], key).length === 0
        ) {
