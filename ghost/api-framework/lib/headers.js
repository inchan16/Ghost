...
module.exports = {
  ...
  async get(result, apiConfigHeaders = {}, frame) {
    ...
    if (apiConfigHeaders.disposition) {
      const dispositionHeader = await disposition[
        apiConfigHeaders.disposition.type
      ](result, apiConfigHeaders.disposition);
    ...
    const hasFrameData =
      frame?.method === 'add' && result[frame.docName]?.[0]?.id;

    if (!locationHeaderDisabled && hasFrameData) {
      const protocol =
        frame.original.url.secure === false ? 'http://' : 'https://';
