...
module.exports = class MailgunClient {
  ...
  async send(message, recipientData, replacements) {
    ...
    try {
      ...
      replacements.forEach((replacement) => {
        messageContent[replacement.format] = messageContent[
          replacement.format
        ].replace(replacement.regexp, `%recipient.${replacement.id}%`);
      });
      ...
      startTime = Date.now();
      const response = await mailgunInstance.messages.create(
  ...
  async fetchEvents(
    ...
    try {
      let page = await mailgunInstance.events.get(
        mailgunConfig.domain,
        mailgunOptions
      );
      ...
      let events = (page?.items?.map(this.normalizeEvent) || []).filter(
        (e) => !!e && e.timestamp <= startDate
      );
      debug(
        `fetchEvents: finished fetching first page with ${events.length} events`
      );
      ...
      while (events.length !== 0) {
        ...
        events = (page?.items?.map(this.normalizeEvent) || []).filter(
          (e) => !!e && e.timestamp <= startDate
        );
        debug(
          `fetchEvents: finished fetching next page with ${events.length} events`
        );
      }
    } catch (error) {
  ...
  async removeSuppression(type, email) {
    ...
    try {
      await instance.suppressions.destroy(config.domain, type, email);
      return true;
    } catch (err) {
      logging.error(err);
      return false;
  ...
  normalizeEvent(event) {
    ...
    if (
      !providerId &&
      !(event['user-variables'] && event['user-variables']['email-id'])
    ) {
      ...
      error:
        ...
        typeof (
          ...
          ? {
              ...
              enhancedCode:
                event['delivery-status']['enhanced-code']
                  ?.toString()
                  ?.substring(0, 50) ?? null,
            }
          : null,
  ...
  #getConfig() {
    ...
    const mailgunConfig = hasMailgunConfig
      ? bulkEmailConfig.mailgun
      : bulkEmailSetting;
