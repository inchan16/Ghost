...
class OfferDuration extends ValueObject {
  ...
  static create(type, months) {
    ...
    if (
      ...
      throw new InvalidOfferDuration({
        message:
          'Offer `duration` must be one of "once", "repeating", "forever" or "trial.',
      });
    }
    if (type !== 'repeating') {
    ...
    if (typeof months !== 'number') {
      throw new InvalidOfferDuration({
        message:
          'Offer `duration` must have include `duration_in_months` when "repeating".',
      });
    }
    if (!Number.isInteger(months)) {
      throw new InvalidOfferDuration({
        message:
          'Offer `duration_in_months` must be an integer greater than 0.',
      });
    }
    if (months < 1) {
      throw new InvalidOfferDuration({
