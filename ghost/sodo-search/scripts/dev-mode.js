...
function startDevServer() {
  const server = http.createServer((request, response) => {
    return handler(request, response, {
      rewrites: [
        {
          source: '/sodo-search',
          destination: 'umd/sodo-search.min.js',
        },
        {
          source: '/sodo-search.min.js.map',
          destination: 'umd/sodo-search.min.js.map',
        },
      ],
      headers: [
        {
  ...
  server.listen(port, () => {
    log(
      chalk.whiteBright(
        `Sodo-Search dev server is running on http://localhost:${port}`
      )
    );
    watchFiles();
  });
