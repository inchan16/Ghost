...
function HighlightedSection({ text = '', highlight = '', isExcerpt }) {
  ...
  if (isExcerpt && highlightIndexes?.[0]) {
    ...
    if (startIdx > 50) {
      ...
      const { parts: updatedParts } = getHighlightParts({
        text,
        highlight,
      });
      parts = updatedParts;
...
function AuthorAvatar({ name, avatar }) {
  ...
  return (
    <div className="rounded-full bg-neutral-200 w-7 h-7 mr-2 flex items-center justify-center font-bold">
...
function SearchResultBox() {
  ...
  const hasResults =
    filteredPosts?.length || filteredAuthors?.length || filteredTags?.length;
...
function Results({ posts, authors, tags }) {
  ...
  useEffect(() => {
    ...
    return () => {
      containeRefNode?.ownerDocument?.removeEventListener(
        'keyup',
        keyUphandler
      );
    };
