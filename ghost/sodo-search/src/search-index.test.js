...
describe('search index', function () {
  test('initializes search index', async () => {
    ...
    const scope = nock('http://localhost/ghost/api/content')
      .get(
        '/posts/?key=69010382388f9de5869ad6e558&limit=10000&fields=id%2Cslug%2Ctitle%2Cexcerpt%2Curl%2Cupdated_at%2Cvisibility&order=updated_at%20DESC'
      )
      .reply(200, {
        posts: [],
      })
      .get(
        '/authors/?key=69010382388f9de5869ad6e558&limit=10000&fields=id,slug,name,url,profile_image&order=updated_at%20DESC'
      )
      .reply(200, {
        authors: [],
      })
      .get(
        '/tags/?key=69010382388f9de5869ad6e558&&limit=10000&fields=id,slug,name,url&order=updated_at%20DESC&filter=visibility%3Apublic'
      )
      .reply(200, {
        tags: [],
      });
  ...
  test('allows to search for indexed posts and authors', async () => {
        ...
        posts: [
          {
            ...
            excerpt:
              'We are sitting by the pool and smashing out search features. Barcelona life is great!',
    ...
    expect(searchResults.posts[0].url).toEqual(
      'http://localhost/ghost/awesome-barcelona-life/'
    );

    expect(searchResults.authors.length).toEqual(1);
    expect(searchResults.authors[0].name).toEqual('Barcelona Author');
    expect(searchResults.authors[0].url).toEqual(
      'http://localhost/ghost/authors/barcelona-author/'
    );
    expect(searchResults.authors[0].profile_image).toEqual(
      'https://url_to_avatar/barcelona.png'
    );

    expect(searchResults.tags.length).toEqual(1);
    expect(searchResults.tags[0].name).toEqual('Barcelona Tag');
    expect(searchResults.tags[0].url).toEqual(
      'http://localhost/ghost/tags/barcelona-tag/'
    );
