...
export default class EmbeddedRelationAdapter extends BaseAdapter {
  find(store, type, id, snapshot) {
    return this.ajax(
      this.buildIncludeURL(store, type.modelName, id, snapshot, 'find'),
      'GET'
    );
  }
  ...
  findRecord(store, type, id, snapshot) {
    return this.ajax(
      this.buildIncludeURL(store, type.modelName, id, snapshot, 'findRecord'),
      'GET'
    );
  }
  ...
  query(store, type, query) {
    return super.query(
      store,
      type,
      this.buildQuery(store, type.modelName, query)
    );
  }

  queryRecord(store, type, query) {
    return super.queryRecord(
      store,
      type,
      this.buildQuery(store, type.modelName, query)
    );
