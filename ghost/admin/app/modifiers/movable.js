...
export default class MovableModifier extends Modifier {
    ...
    if (!this.didSetup) {
      ...
      if (named.adjustOnResize) {
        this._resizeObserver = new ResizeObserver(() => {
          ...
          const { x, y } = named.adjustOnResize(element, {
            x: this.currentX,
            y: this.currentY,
          });
  ...
  drag(e) {
    ...
    if (!this.active) {
      if (
        Math.abs(Math.abs(this.initialX - eventX) - Math.abs(this.xOffset)) >
          this.moveThreshold ||
        Math.abs(Math.abs(this.initialY - eventY) - Math.abs(this.yOffset)) >
          this.moveThreshold
      ) {
