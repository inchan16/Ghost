...
export default class PostsRoute extends AuthenticatedRoute {
  ...
  constructor() {
    ...
    this.router.on('routeWillChange', (transition) => {
      if (
        transition.to &&
        (this.routeName === 'posts' || this.routeName === 'pages')
      ) {
        let toThisRoute = transition.to.find(
          (route) => route.name === this.routeName
        );
        if (
          transition.from &&
  ...
  setupController(controller, model) {
    ...
    if (
      !this.session.user.isAuthorOrContributor &&
      !controller._hasLoadedAuthors
    ) {
