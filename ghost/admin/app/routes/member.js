...
export default class MembersRoute extends AdminRoute {
  ...
  setupController(controller, member, transition) {
    ...
    if (transition.from?.name === 'posts.analytics') {
      ...
      const oldParams =
        transition.router?.oldState?.params['posts.analytics'] ?? {};
      ...
      transition.from?.name === 'members.index' &&
      transition.from?.parent?.name === 'members'
    ) {
      const fromAnalytics =
        transition.from?.parent?.metadata.fromAnalytics ?? null;
      controller.fromAnalytics = fromAnalytics;
      this.fromAnalytics = fromAnalytics;
    } else {
