...
export default class MigrateService extends Service {
  ...
  async apiToken() {
    ...
    const ghostIntegrationsUrl =
      this.ghostPaths.url.api('integrations') + '?include=api_keys';
    return this.ajax
      .request(ghostIntegrationsUrl)
      .then(async (response) => {
        const ssmIntegration = response.integrations.find(
          (r) => r.slug === 'self-serve-migration'
        );
  ...
  get isStripeConnected() {
    return this.settings.stripeConnectAccountId &&
      ...
      this.settings.stripeConnectLivemode
      ? true
      : false;
  }
