...
let isViewEqual = function (viewA, viewB) {
  return (
    viewA.route === viewB.route && isFilterEqual(viewA.filter, viewB.filter)
  );
};
  ...
  *saveViewTask(view) {
    ...
    let duplicateView = viewList.find((existingView) => {
      return (
        ...
        existingView.name.trim().toLowerCase() ===
    ...
    let matchingView = viewList.find((existingView) =>
      isViewEqual(existingView, view)
    );
    if (matchingView) {
  ...
  *deleteViewTask(view) {
  ...
  findView(routeName, queryParams) {
    ...
    return this.viewList.find((view) => {
      return (
        view.route === _routeName && isFilterEqual(view.filter, queryParams)
      );
    });
  }
  ...
  async _saveViewSettings(viewList) {
    let sharedViews = viewList
      .reject((view) => view.isDefault)
