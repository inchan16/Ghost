...
export default class CustomThemeSettingsServices extends Service {
  ...
  get isDirty() {
    const dirtySetting = this.settings.find(
      (setting) => setting.hasDirtyAttributes
    );
    return !!dirtySetting;
  }
  ...
  *saveTask() {
  ...
  _buildSettingGroups(settings) {
    ...
    const siteWideSettings = settings.filter(
      (setting) => !groupKeys.includes(setting.group)
    );
    ...
    this.KNOWN_GROUPS.forEach((knownGroup) => {
      const groupSettings = settings.filter(
        (setting) => setting.group === knownGroup.key
      );
