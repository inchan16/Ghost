...
export default class MembersUtilsService extends Service {
  ...
  get hasMultipleTiers() {
    return (
      this.paidMembersEnabled && this.paidTiers && this.paidTiers.length > 1
    );
  }

  get hasActiveTiers() {
    return (
      this.paidMembersEnabled && this.paidTiers && this.paidTiers.length > 0
    );
  }
  ...
  get isStripeEnabled() {
    ...
    const hasDirectKeys =
      !!this.settings.stripeSecretKey && !!this.settings.stripePublishableKey;
    const hasConnectKeys =
      !!this.settings.stripeConnectSecretKey &&
      !!this.settings.stripeConnectPublishableKey;
  ...
  getPortalPreviewUrl(overrides) {
    ...
    const allowSelfSignup =
      membersSignupAccess === 'all' && (!this.isStripeEnabled || isFreeChecked);
    ...
    settingsParam.append(
      'signupButtonText',
      encodeURIComponent(signupButtonText)
    );
    settingsParam.append('membersSignupAccess', membersSignupAccess);
    settingsParam.append('allowSelfSignup', allowSelfSignup);
    settingsParam.append(
      'signupTermsHtml',
      this.settings.portalSignupTermsHtml || ''
    );
    settingsParam.append(
      'signupCheckboxRequired',
      this.settings.portalSignupCheckboxRequired
    );
    ...
    if (this.settings.accentColor === '' || this.settings.accentColor) {
      settingsParam.append(
        'accentColor',
        encodeURIComponent(`${this.settings.accentColor}`)
      );
    }
    if (this.settings.portalButtonStyle) {
      settingsParam.append(
        'buttonStyle',
        encodeURIComponent(this.settings.portalButtonStyle)
      );
    }
  ...
  getOfferPortalPreviewUrl(overrides) {
    ...
    settingsParam.append(
      'display_description',
      encodeURIComponent(displayDescription)
    );
    ...
    settingsParam.append(
      'duration_in_months',
      encodeURIComponent(durationInMonths)
    );
