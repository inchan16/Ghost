...
export default class DashboardMocksService extends Service {
  ...
  _updateGrow(settings) {
    const change = Math.round(
      Math.random() * (settings.growRate - settings.shrinkOffset)
    );
    ...
    if (settings.growPeriod) {
      ...
      if (settings.growCount > settings.growLength) {
        ...
        settings.growLength =
          Math.floor(Math.random() * settings.maxPeriod) + 20;
      }
    } else {
      settings.growCount += 1;
      if (settings.growCount > settings.growLength) {
  ...
  updateMockedData({ days }) {
    ...
    for (let index = 0; index < generateDays; index++) {
      ...
      let paidSubscribed1 = Math.max(
        0,
        this._updateGrow(paidSubscribedGrowthTier1)
      );
      const paidCanceled1 = Math.min(
        previous.tier1,
        Math.max(0, this._updateGrow(paidCanceledGrowthTier1))
      );

      const paidSubscribed2 = Math.max(
        0,
        this._updateGrow(paidSubscribedGrowthTier2)
      );
      const paidCanceled2 = Math.min(
        previous.tier2,
        Math.max(0, this._updateGrow(paidCanceledGrowthTier2))
      );
      ...
      const tier1 = Math.max(
        0,
        previous.tier1 + paidSubscribed1 - paidCanceled1
      );
      const tier2 = Math.max(
        0,
        previous.tier2 + paidSubscribed2 - paidCanceled2
      );
      ...
      if (!showEmptyState) {
        this.memberAttributionStats.push({
          ...
          source:
            attributionSources[
              Math.floor(Math.random() * attributionSources.length)
            ],
          signups: hasFreeSignups ? Math.floor(Math.random() * 50) : 0,
          paidConversions: hasPaidConversions
    ...
    this.subscriptionCountStats = stats.map((data) => {
      ...
      return {
        ...
        cancellations: Math.floor(signups * 0.3)
          ? Math.floor(signups * 0.3)
          : 0,
      };
    });
    ...
    this.newsletterSubscribers = {
      ...
      total:
        Math.floor(currentCounts.paid * 0.9) +
        Math.floor(currentCounts.free * 0.5),
    };

    this.emailsSent30d = Math.floor((days * 123) / 90);

    this.membersLastSeen7d = Math.round(
      (Math.random() * currentCounts.free) / 2
    );
    this.membersLastSeen30d =
      this.membersLastSeen7d +
      Math.round((Math.random() * currentCounts.free) / 2);
