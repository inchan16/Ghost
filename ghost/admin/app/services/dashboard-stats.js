...
export default class DashboardStatsService extends Service {
  ...
  get memberSourceAttributionCounts() {
    ...
    return this.memberAttributionStats
      .filter((stat) => {
        if (this.chartDays === 'all') {
          return true;
        }
        return (
          stat.date >=
          moment().add(-this.chartDays, 'days').format('YYYY-MM-DD')
        );
      })
      .reduce((acc, stat) => {
  ...
  *_loadSiteStatus() {
    ...
    const hasPaidTiers =
      this.membersUtils.paidMembersEnabled &&
      this.activePaidTiers &&
      this.activePaidTiers.length > 0;
  ...
  *_loadSubscriptionCountStats() {
    ...
    if (this.dashboardMocks.enabled) {
      ...
      this.paidMembersByCadence = {
        ...this.dashboardMocks.paidMembersByCadence,
      };
    ...
    for (const cadence of result.meta.cadences) {
      paidMembersByCadence[cadence] = result.meta.totals.reduce(
        (sum, total) => {
  ...
  *_loadLastSeen() {
    ...
    const [result30d, result7d] = yield Promise.all([
      this.membersCountCache.count(
        'last_seen_at:>' + start30d.toISOString() + extraFilter
      ),
      this.membersCountCache.count(
        'last_seen_at:>' + start7d.toISOString() + extraFilter
      ),
    ]);
  ...
  *_loadEmailsSent() {
    ...
    const start30d = new Date(Date.now() - 30 * 86400 * 1000);
    const result = yield this.store.query('email', {
      limit: 100,
      filter: 'submitted_at:>' + start30d.toISOString(),
    });
