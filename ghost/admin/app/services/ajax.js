...
class ajaxService extends AjaxService {
  ...
  async _makeRequest(hash) {
    ...
    const retryErrorChecks = [
      this.isServerUnreachableError,
      this.isMaintenanceError,
    ];
    ...
    while (retryingMs <= maxRetryingMs && !success) {
      try {
        ...
        if (attempts !== 0 && this.config.sentry_dsn) {
          captureMessage('Request took multiple attempts', {
            extra: getErrorData(),
          });
        }
        ...
        if (
          ...
          await timeout(
            retryPeriods[attempts] || retryPeriods[retryPeriods.length - 1]
          );
          attempts += 1;
        } else if (attempts > 0 && this.config.sentry_dsn) {
          captureMessage('Request failed after multiple attempts', {
            extra: getErrorData(),
          });
          throw error;
        } else {
  ...
  normalizeErrorResponse(status, headers, payload) {
    if (payload && typeof payload === 'object') {
      let errors =
        payload.error || payload.errors || payload.message || undefined;
