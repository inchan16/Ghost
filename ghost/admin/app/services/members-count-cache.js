...
export default class MembersCountCacheService extends Service {
  ...
  async countString(filter = '', { knownCount, newsletter } = {}) {
    ...
    if (this.hasMultipleNewsletters === null) {
      const allNewsletters = await this.store.query('newsletter', {
        status: 'active',
        limit: 'all',
      });
    ...
    const nounSingular =
      newsletter && this.hasMultipleNewsletters ? 'subscriber' : 'member';
    const nounPlural = nounSingular + 's';
    const suffix =
      newsletter && this.hasMultipleNewsletters ? ' of ' + newsletter.name : '';

    const basicFilter = newsletter
      ? filter
          .replace(newsletter.recipientFilter, '')
          .replace(/^\+\((.*)\)$/, '$1')
      : filter;
    const filterParts = basicFilter.split(',');
    const isFree = filterParts.length === 1 && filterParts[0] === 'status:free';
    const isPaid =
      filterParts.length === 1 && filterParts[0] === 'status:-free';
    const isAll =
      !filter ||
      (filterParts.includes('status:free') &&
        filterParts.includes('status:-free'));
    ...
    const recipientCount =
      knownCount !== undefined ? knownCount : await this.count(filter);
