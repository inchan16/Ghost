...
export default class MembersStatsService extends Service {
  ...
  fetch() {
    ...
    let staleData =
      this._lastFetched && new Date() - this._lastFetched > ONE_MINUTE;
  ...
  fetchCounts() {
    let staleData =
      this._lastFetchedCounts &&
      new Date() - this._lastFetchedCounts > ONE_MINUTE;
  ...
  fetchMemberCount() {
    let staleData =
      this._lastFetchedMemberCounts &&
      new Date() - this._lastFetchedMemberCounts > ONE_MINUTE;
    ...
    if (
      ...
      !this._forceRefresh &&
      !staleData &&
      this._fetchMemberCountsTask.last
    ) {
  ...
  fetchNewsletterStats() {
    let staleData =
      this._lastFetchedNewsletterStats &&
      new Date() - this._lastFetchedNewsletterStats > ONE_MINUTE;
    ...
    if (
      ...
      !this._forceRefresh &&
      !staleData &&
      this._fetchNewsletterStatsTask.last
    ) {
  ...
  fillDates(data = []) {
    ...
    let initialDateInRangeVal =
      firstDateInRangeIndex > 0 ? data[firstDateInRangeIndex - 1] : null;
    if (firstDateInRangeIndex === 0 && !initialDateInRangeVal) {
      initialDateInRangeVal = data[firstDateInRangeIndex];
    }
    if (
      data.length > 0 &&
      !initialDateInRangeVal &&
      firstDateInRangeIndex !== 0
    ) {
  ...
  fillCountDates(data = {}) {
    ...
    let lastVal = {
      ...
      total: initialDateInRangeVal
        ? initialDateInRangeVal.paid +
          initialDateInRangeVal.free +
          initialDateInRangeVal.comped
        : 0,
    };
    while (currentRangeDate.isBefore(endDate)) {
      let dateStr = currentRangeDate.format('YYYY-MM-DD');
  ...
  fetchMRR() {
    let staleData =
      this._lastFetchedMRR && new Date() - this._lastFetchedMRR > ONE_MINUTE;
  ...
  *_fetchNewsletterStatsTask() {
    ...
    if (data.length < limit) {
      ...
      const lastSubmittedAt =
        data.length > 0 ? data[results.length - 1].submittedAtUTC : moment();
      for (let i = 0; i < pad; i++) {
        paddedResults.push({
          subject: '',
          submittedAt: moment(lastSubmittedAt)
            .subtract(i + 1, 'days')
            .format('YYYY-MM-DD'),
          openRate: 0,
        });
