...
function updateBodyClasses(transition) {
  let { body } = document;
  let { oldClasses, newClasses } = collectMetadataClasses(
    transition,
    'bodyClasses'
  );
...
export default class UiService extends Service {
  ...
  get adjustedAccentColor() {
    ...
    if (this.feature.nightShift) {
      adjustedAccentColor = lightenToContrastThreshold(
        accentColor,
        backgroundColor,
        2
      );
    } else {
      adjustedAccentColor = darkenToContrastThreshold(
        accentColor,
        backgroundColor,
        2
      );
  ...
  constructor() {
    ...
    this.router.on('routeDidChange', (transition) => {
      ...
      let { newClasses: mainClasses } = collectMetadataClasses(
        transition,
        'mainClasses'
      );
