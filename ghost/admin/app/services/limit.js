...
export default class LimitsService extends Service {
  ...
  constructor() {
    ...
    if (
      this.config.hostSettings?.billing?.enabled === true &&
  ...
  decorateWithCountQueries(limits) {
    ...
    if (limits.newsletters) {
      limits.newsletters.currentCountQuery = bind(
        this,
        this.getNewslettersCount
      );
    }
  ...
  async getStaffUsersCount() {
    return RSVP.hash({
      ...
      const staffUsers = data.users.filter(
        (u) =>
          u.get('status') !== 'inactive' && u.role.get('name') !== 'Contributor'
      );
      const staffInvites = data.invites.filter(
        (i) => i.role.get('name') !== 'Contributor'
      );
  ...
  async getNewslettersCount() {
    const activeNewsletters = await this.store.query('newsletter', {
