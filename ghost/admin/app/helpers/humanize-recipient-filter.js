...
export default helper(function humanizeRecipientFilter(
  [filter = ''] /*, hash*/
) {
  ...
  if (productsArrayRegex.test(filter)) {
    ...
    const products = [...filter.matchAll(productRegex)].map(
      ([, product]) => product
    );
    outputParts.push(
      `${pluralize(products.length, 'Product', {
