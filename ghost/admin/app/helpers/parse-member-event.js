...
export default class ParseMemberEventHelper extends Helper {
  ...
  getIcon(event) {
    ...
    if (this.feature.get('suppressionList')) {
      ...
      if (
        event.type === 'email_delivered_event' ||
        event.type === 'email_sent_event'
      ) {
        icon = 'received-email';
      }
    }
    ...
    if (
      ...
      event.type === 'aggregated_click_event'
    ) {
      icon = 'click';
    }
  ...
  getAction(event, hasMultipleNewsletters) {
    ...
    if (event.type === 'email_failed_event') {
      return this.feature.get('suppressionList')
        ? 'bounced email'
  ...
  getURL(event) {
    if (
      ['comment_event', 'click_event', 'feedback_event'].includes(event.type)
    ) {
      if (event.data.post) {
