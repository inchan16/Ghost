...
export default class PublishOptions {
  ...
  toggleScheduled(shouldSchedule) {
    ...
    if (
      shouldSchedule &&
      (!this.scheduledAtUTC ||
        this.scheduledAtUTC.isBefore(this.defaultScheduledAt))
    ) {
  ...
  get selectedPublishTypeOption() {
    return this.publishTypeOptions.find(
      (pto) => pto.value === this.publishType
    );
  }
  ...
  get recipientFilter() {
    if (this.selectedRecipientFilter === undefined) {
      return (
        (this.post.newsletter && this.post.emailSegment) ||
        this.defaultRecipientFilter
      );
    } else {
  ...
  *fetchRequiredDataTask() {
    ...
    if (this.user.isAdmin) {
      promises.push(
        this.store.query('member', { limit: 1 }).then((res) => {
