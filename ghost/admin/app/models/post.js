...
function statusCompare(postA, postB) {
  ...
  if (
    status1 === 'scheduled' &&
    (status2 === 'draft' || status2 === 'published')
  ) {
    return -1;
  }
  ...
  if (
    status2 === 'scheduled' &&
    (status1 === 'draft' || status1 === 'published')
  ) {
    return 1;
  }
...
export default Model.extend(Comparable, ValidationEngine, {
  ...
  postRevisions: hasMany('post_revisions', {
    embedded: 'always',
    async: false,
  }),
  ...
  showEmailOpenAnalytics: computed(
    'hasBeenEmailed',
    'isSent',
    'isPublished',
    function () {
  ...
  showEmailClickAnalytics: computed(
    'hasBeenEmailed',
    'isSent',
    'isPublished',
    'email',
  ...
  fullRecipientFilter: computed(
    'newsletter.recipientFilter',
    'emailSegment',
    function () {
      if (!this.newsletter) {
  ...
  _getPublishedAtBlogTZ() {
    ...
    if (
      !publishedAtUTC &&
      isBlank(publishedAtBlogDate) &&
      isBlank(publishedAtBlogTime)
    ) {
      return null;
    }

    if (publishedAtBlogDate && publishedAtBlogTime) {
      let publishedAtBlog = moment.tz(
  ...
  _setPublishedAtBlogTZ: on(
    'init',
    observer('publishedAtUTC', 'settings.timezone', function () {
      let publishedAtUTC = this.publishedAtUTC;
