...
export default class LexicalEditorController extends Controller {
  ...
  async createSnippet(data) {
    ...
    if (existingSnippet) {
      await this.confirmUpdateSnippet(existingSnippet, {
        lexical: data.value,
        mobiledoc: '{}',
      });
    } else {
  ...
  *backgroundLoaderTask() {
  ...
  async willTransition(transition) {
    ...
    let fromNewToEdit =
      this.router.currentRouteName === 'lexical-editor.new' &&
  ...
  _hasDirtyAttributes() {
    ...
    if (scratch || lexical) {
      if (scratch !== lexical) {
        this._leaveModalReason = {
          reason: 'lexical is different',
          context: { current: lexical, scratch },
        };
        return true;
      }
    }
