...
export default class MembersController extends Controller {
  ...
  get selectedOrder() {
    return this.availableOrders.find(
      (order) => order.value === this.orderParam
    );
  }
  ...
  get availableLabels() {
    let labels = this._availableLabels
      ...
      .sort((labelA, labelB) =>
        labelA.name.localeCompare(labelB.name, undefined, {
  ...
  get selectedPaidParam() {
    return (
      this.paidParams.findBy('value', this.paidParam) || {
        value: '!unknown',
      }
    );
  }
  ...
  includeTierQuery() {
    const availableFilters = this.filters.length
      ? this.filters
      : this.softFilters;
  ...
  *fetchMembersTask(params) {
    ...
    let { label, paidParam, searchParam, orderParam, filterParam } =
    ...
    if (
      !forceReload &&
      this._startDate &&
      !(this._startDate - startDate > 1 * 60 * 1000)
    ) {
      return this.members;
    }
