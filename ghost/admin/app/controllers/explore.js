...
export default class ExploreController extends Controller {
  ...
  get visibilityClass() {
    return this.explore.isIframeTransition
      ? 'explore iframe-explore-container'
      : ' explore fullscreen-explore-container';
  }
  ...
  submitExploreSite() {
    ...
    if (this.explore.isIframeTransition) {
      this.explore.sendRouteUpdate({
        path: this.explore.submitRoute,
      ...
      const destination = new URL(
        `${this.explore.exploreUrl}${this.explore.submitRoute}`
      );
