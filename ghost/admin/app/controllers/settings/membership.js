...
import {
  ...
  getCurrencyOptions,
  getSymbol,
} from 'ghost-admin/utils/currency';
...
export default class MembersAccessController extends Controller {
  ...
  get hasChangedPrices() {
    if (this.tier) {
      ...
      if (
        monthlyPrice?.amount &&
        parseFloat(this.stripeMonthlyAmount) !== monthlyPrice.amount / 100
      ) {
        return true;
      }
      if (
        yearlyPrice?.amount &&
        parseFloat(this.stripeYearlyAmount) !== yearlyPrice.amount / 100
      ) {
        return true;
      }
    }
  ...
  validateStripePlans({ updatePortalPreview = true } = {}) {
    ...
    try {
      ...
      if (
        ...
        throw new TypeError(
          `Subscription amount must be at least ${symbol}1.00`
        );
      }
  ...
  updatePortalPreview({ forceRefresh } = { forceRefresh: false }) {
    ...
    const newUrl = new URL(
      this.membersUtils.getPortalPreviewUrl({
        button: false,
        monthlyPrice,
        yearlyPrice,
        portalTiers,
  ...
  portalPreviewInserted(iframe) {
    ...
    if (!this.portalMessageListener) {
      this.portalMessageListener = (event) => {
        ...
        if (
          resizeEvents.includes(event.data.type) &&
          event.data.payload?.height &&
          this.portalPreviewIframe?.parentNode
        ) {
