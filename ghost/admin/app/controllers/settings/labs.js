...
export default class LabsController extends Controller {
  ...
  init() {
    ...
    this.yamlAccept = [
      ...this.yamlMimeType,
      ...Array.from(this.yamlExtension, (extension) => '.' + extension),
    ];
  }
  ...
  onUpload(file) {
    ...
    return this._validate(file)
      ...
      .catch((response) => {
        if (
          isUnsupportedMediaTypeError(response) ||
          isRequestEntityTooLargeError(response)
        ) {
          this.set('importErrors', [response]);
        } else if (
          response &&
          response.payload.errors &&
          isEmberArray(response.payload.errors)
        ) {
