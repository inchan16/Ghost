...
export default class EditorController extends Controller {
  ...
  leaveEditor() {
    ...
    if (!transition) {
      this.notifications.showAlert(GENERIC_ERROR_MESSAGE, {
        type: 'error',
      });
      return;
    }
  ...
  *saveTask(options = {}) {
    ...
    if (options.backgroundSave) {
      ...
      if (this.get('post.pastScheduledTime')) {
        status =
          !this.willSchedule && !this.willPublish ? 'draft' : 'published';
      } else {
  ...
  *beforeSaveTask(options = {}) {
    ...
    this.set(
      'post.mobiledoc',
      JSON.parse(JSON.stringify(this.post.scratch || null))
    );
    ...
    this.set(
      ...
      this.get('post.twitterDescriptionScratch')
    );
  ...
  *_savePostTask(options = {}) {
    ...
    try {
      ...
      if (isServerUnreachableError(error)) {
        const [prevStatus, newStatus] = this.post.changedAttributes()
  ...
  afterSave(post) {
    ...
    let bodiesMatch =
      JSON.stringify(post.get('scratch')) ===
      JSON.stringify(post.get('mobiledoc'));
  ...
  *backgroundLoaderTask() {
  ...
  async willTransition(transition) {
    ...
    let fromNewToEdit =
      this.router.currentRouteName === 'editor.new' &&
    ...
    let deletedWithoutChanges =
      state.isDeleted && (state.isSaving || !state.hasDirtyAttributes);
    ...
    if (scratch || mobiledoc) {
      ...
      if (scratchJSON !== mobiledocJSON) {
        this._leaveModalReason = {
          ...
          context: { current: mobiledocJSON, scratch: scratchJSON },
        };
        return true;
      }
    }
  ...
  _showSaveNotification(prevStatus, status, delayed) {
    ...
    notifications.showNotification(message, {
      type: 'success',
      actions: actions && htmlSafe(actions),
      delayed,
    });
  }

  async _showScheduledNotification(delayed) {
    let { publishedAtUTC, previewUrl, emailOnly, newsletter } = this.post;
    ...
    if (newsletter) {
      const recipientCount = await this.membersCountCache.countString(
        this.post.fullRecipientFilter,
        { newsletter }
      );
      description.push(
        `${
          !emailOnly ? 'and delivered ' : ''
        }to <span><strong>${recipientCount}</strong></span>`
      );
    }

    description.push(
      `on <span><strong>${publishedAtBlogTZ.format('MMM Do')}</strong></span>`
    );
    description.push(
      `at <span><strong>${publishedAtBlogTZ.format('HH:mm')}</strong>`
    );
    if (publishedAtBlogTZ.utcOffset() === 0) {
      description.push('(UTC)</span>');
    } else {
      description.push(
        `(UTC${publishedAtBlogTZ
          .format('Z')
          .replace(/([+-])0/, '$1')
          .replace(/:00/, '')})</span>`
      );
    ...
    let actions = htmlSafe(
      `<a href="${previewUrl}" target="_blank">View Preview</a>`
    );
