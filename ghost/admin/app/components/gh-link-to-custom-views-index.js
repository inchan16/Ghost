...
export default class GhCustomViewsIndexLinkComponent extends Component {
  ...
  handleRouteWillChange({ from, to }) {
    ...
    if (
      from &&
      from.name === this.args.route &&
      normalizedToRoute !== this.args.route
    ) {
      if (
        this.customViews.activeView &&
        this.customViews.activeView.route === this.args.route
      ) {
  ...
  get isActive() {
    ...
    let currentRouteName = this.router.currentRouteName.replace(
      /_loading$/,
      ''
    );
    ...
    this._lastIsActive =
      currentRouteName === this.args.route && !this.customViews.activeView;
