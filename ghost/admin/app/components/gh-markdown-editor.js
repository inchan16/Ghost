...
export default class GhMarkdownEditor extends Component.extend(ShortcutsMixin) {
  ...
  didReceiveAttrs() {
    ...
    if (
      !isEmpty(uploadedImageUrls) &&
      uploadedImageUrls !== this._uploadedImageUrls
    ) {
    ...
    if (
      this.autofocus &&
      this._editor &&
      this.markdown !== this._editor.value()
    ) {
  ...
  setEditor(editor) {
    ...
    this._editor.codemirror.setOption(
      'specialChars',
      /[\u0000-\u001f\u007f-\u009f\u00ad\u061c\u200b-\u200f\u2028\u2029\ufeff\xa0]/g
    );
  ...
  toggleSplitScreen() {
    ...
    if (isSplitScreen) {
      ...
      if (this._editor.isPreviewActive()) {
        let preview = this._editor.toolbar.find(
          (button) => button.name === 'preview'
        );
  ...
  _connectSplitPreview() {
    ...
    let previewContent = this.element.querySelector(
  ...
  _disconnectSplitPreview() {
    ...
    this._editorPane.removeEventListener(
      'scroll',
      this._onEditorPaneScroll,
      false
    );
  ...
  _toggleHemingway() {
    ...
    if (this._isHemingwayMode) {
      notificationText =
        '<span class="gh-notification-title">Hemingway Mode On:</span> Write now; edit later. Backspace disabled.';
      extraKeys.Backspace = function () {};
    } else {
      notificationText =
        '<span class="gh-notification-title">Hemingway Mode Off:</span> Normal editing restored.';
    ...
    this.notifications.showNotification(htmlSafe(notificationText), {
