...
export default class ModalPostPreviewEmailComponent extends Component {
  ...
  get selectedSegment() {
    return this.segments.find(
      (segment) => segment.value === this.memberSegment
    );
  }
  ...
  *sendPreviewEmailTask() {
    try {
      ...
      const data = {
        ...
        newsletter: this.newsletter.slug,
      };
      const options = {
        data,
        dataType: 'json',
  ...
  async _fetchEmailData() {
    ...
    if (
      html &&
      subject &&
      memberSegment === this._lastMemberSegment &&
      newsletter.slug === this._lastNewsletterSlug
    ) {
      return { html, subject };
    }
  ...
  @task
  *loadNewslettersTask() {
    const newslettersList = yield this.store.query('newsletter', {
