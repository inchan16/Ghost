...
export default class PublishManagement extends Component {
  ...
  async openPublishFlow(event, { skipAnimation } = {}) {
    ...
    if (
      (isValid && !this.publishFlowModal) ||
      this.publishFlowModal?.isClosing
    ) {
  ...
  *saveButtonTimeoutTask() {
    yield timeout(
      envConfig.environment === 'test' ? 1 : SHOW_SAVE_STATUS_DURATION
    );
  }
  ...
  *revertToDraftTask() {
    try {
      ...
      this.notifications.showNotification(
        `${postType} successfully reverted to a draft.`,
        { type: 'success' }
      );
