...
export default ModalComponent.extend({
  ...
  _uploadSuccess(importResponse) {
    ...
    let errorCsvName = importResponse.meta.import_label
      ? `${importResponse.meta.import_label.name} - Errors.csv`
  ...
  _uploadError(error) {
    ...
    if (isUnsupportedMediaTypeError(error)) {
      ...
      isHostLimitError(error) &&
      error?.payload?.errors?.[0]?.code === 'EMAIL_VERIFICATION_NEEDED'
    ) {
      message = htmlSafe(error.payload.errors[0].message);
