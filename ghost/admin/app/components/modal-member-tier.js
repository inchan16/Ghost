...
export default class ModalMemberTier extends ModalComponent {
  ...
  *fetchTiers() {
  ...
  @task({ drop: true })
  *addTier() {
    const url = `${this.ghostPaths.url.api(
      `members/${this.member.get('id')}`
    )}?include=tiers`;
    ...
    for (let i = 0; i < this.activeSubscriptions.length; i++) {
      ...
      const cancelUrl = this.ghostPaths.url.api(
        `members/${this.member.get('id')}/subscriptions/${subscription.id}`
      );
