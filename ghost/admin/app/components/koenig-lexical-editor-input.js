...
class ErrorHandler extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <p className="koenig-react-editor-error">
...
const fetchKoenig = function () {
  ...
  const fetchPackage = async () => {
    ...
    const GhostAdmin =
      window.GhostAdmin ||
      window.Ember.Namespace.NAMESPACES.find((ns) => ns.name === 'ghost-admin');
    const urlTemplate =
      GhostAdmin.__container__.lookup('config:main').editor?.url;
    const urlVersion =
      GhostAdmin.__container__.lookup('config:main').editor?.version;
...
const KoenigComposer = (props) => {
  const { KoenigComposer: _KoenigComposer, MINIMAL_NODES: _MINIMAL_NODES } =
    editorResource.read();
...
const KoenigComposableEditor = (props) => {
  const {
    KoenigComposableEditor: _KoenigComposableEditor,
    MINIMAL_TRANSFORMERS: _MINIMAL_TRANSFORMERS,
  } = editorResource.read();
  return (
    <_KoenigComposableEditor
      markdownTransformers={_MINIMAL_TRANSFORMERS}
      {...props}
    />
  );
};
...
export default class KoenigLexicalEditorInput extends Component {
  ...
  ReactComponent = (props) => {
    return (
      <div
        className={['koenig-react-editor', this.args.className]
          .filter(Boolean)
          .join(' ')}
      >
        <ErrorHandler>
          <Suspense
            fallback={
              <p className="koenig-react-editor-loading">Loading editor...</p>
            }
          >
            <KoenigComposer
