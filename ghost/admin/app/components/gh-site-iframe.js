...
export default class GhSiteIframeComponent extends Component {
  ...
  onLoad(event) {
    ...
    if (
      this.args.invisibleUntilLoaded &&
      typeof this.args.invisibleUntilLoaded === 'boolean'
    ) {
    ...
    if (typeof this.args.invisibleUntilLoaded === 'string') {
      this.messageListener = (event) => {
        ...
        if (originURL.origin === srcURL.origin) {
          if (
            event.data === this.args.invisibleUntilLoaded ||
            event.data.type === this.args.invisibleUntilLoaded
          ) {
            this.makeVisible.perform();
          }
        }
