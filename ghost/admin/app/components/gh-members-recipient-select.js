...
export default class GhMembersRecipientSelect extends Component {
  ...
  get renderInPlace() {
    return this.args.renderInPlace === undefined
      ? true
      : this.args.renderInPlace;
  }

  get baseFilters() {
    const filterItems = (this.args.filter || '').split(',');
    const filterItemsArray = filterItems.filter((item) =>
      BASE_FILTERS.includes(item?.trim())
    );
  ...
  get specificFilters() {
    const filterItems = (this.args.filter || '').split(',');
    const filterItemsArray = filterItems.reject(
      (item) => isBlank(item) || BASE_FILTERS.includes(item?.trim())
    );
    return new Set(filterItemsArray);
  }
  ...
  get selectedSpecificOptions() {
    return flattenGroupedOptions(this.specificOptions).filter((o) =>
      this.specificFilters.has(o.segment)
    );
  }
  ...
  toggleFilter(filter) {
    ...
    newBaseFilters.has(filter)
      ? newBaseFilters.delete(filter)
      : newBaseFilters.add(filter);
  ...
  *fetchSpecificOptionsTask() {
    ...
    const tiers = yield this.store.query('tier', {
      filter: 'type:paid',
      limit: 'all',
    });
