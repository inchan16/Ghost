...
export default class Analytics extends Component {
  ...
  get totalFeedback() {
    return (
      this.post.count.positive_feedback + this.post.count.negative_feedback
    );
  }

  get feedbackChartData() {
    const values = [
      this.post.count.positive_feedback,
      this.post.count.negative_feedback,
    ];
  ...
  updateLinkData(linksData) {
    ...
    if (this.links?.length) {
      updatedLinks = this.links.map((link) => {
        let linkData = linksData.find(
          (l) => l.link.link_id === link.link.link_id
        );
        if (linkData) {
          return {
            ...linkData,
  ...
  *_updateLinks(linkId, newLink) {
    ...
    let bulkUpdateUrl =
      this.ghostPaths.url.api(`links/bulk`) +
    ...
    let statsUrl =
      this.ghostPaths.url.api(`links/`) +
      `?filter=${encodeURIComponent(linksFilter)}`;
  ...
  *_fetchReferrersStats() {
    let statsUrl = this.ghostPaths.url.api(
      `stats/referrers/posts/${this.post.id}`
    );
    let result = yield this.ajax.request(statsUrl);
  ...
  *_fetchMentions() {
  ...
  get showSources() {
    return (
      this.feature.get('sourceAttribution') &&
      this.post.showAttributionAnalytics
    );
  }
  ...
  get isLoaded() {
    return (
      this.links !== null && this.souces !== null && this.mentions !== null
    );
  }
}
