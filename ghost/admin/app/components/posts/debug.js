...
export default class Debug extends Component {
  ...
  async updateEmail() {
    try {
      this.latestEmail = await this.store.findRecord(
        'email',
        this.post.email.id,
        { reload: true }
      );
  ...
  get emailError() {
    ...
    let failedBatches =
      this.emailBatchesData?.filter((batch) => {
    ...
    let details =
      this.loading || !totalBatches
        ? ''
        : `${failedBatches} of ${ghPluralize(
            totalBatches,
            'batch'
          )} failed to send, check below for more details.`;
    return {
  ...
  get emailSettings() {
    return {
      ...
      createdAt: this.email?.createdAtUTC
        ? moment(this.email.createdAtUTC).format('DD MMM, YYYY, HH:mm:ss')
        : '',
      submittedAt: this.email?.submittedAtUTC
        ? moment(this.email.submittedAtUTC).format('DD MMM, YYYY, HH:mm:ss')
        : '',
    ...
    return this.emailBatches?.map((batch) => {
      return {
        ...
        createdAt: batch.created_at
          ? moment(batch.created_at).format('DD MMM, YYYY, HH:mm:ss')
          : '',
      ...
      .map((failure) => {
        return {
          ...
          failedAt: failure.failed_at
            ? moment(failure.failed_at).format('DD MMM, YYYY, HH:mm:ss')
            : '',
          processedAt: failure.email_recipient.processed_at
            ? moment(failure.email_recipient.processed_at).format(
          ...
          recipient: {
            ...
            initials: this.getInitials(
              failure.email_recipient?.member_name ||
                failure.email_recipient?.member_email
            ),
          },
          member: {
  ...
  *_fetchEmailBatches() {
    ...
    let statsUrl = this.ghostPaths.url.api(
      `emails/${this.post.email.id}/batches`
    );
  ...
  *_fetchRecipientFailures() {
    ...
    let statsUrl = this.ghostPaths.url.api(
      `/emails/${this.post.email.id}/recipient-failures`
    );
    let result = yield this.ajax.request(statsUrl, { data });
  ...
  *_fetchAnalyticsStatus() {
    let statsUrl = this.ghostPaths.url.api(
      `/emails/${this.post.email.id}/analytics`
    );
