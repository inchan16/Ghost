...
export default class InstallThemeModal extends Component {
  ...
  get installFailure() {
    return (
      !this.installSuccess &&
      (this.validationErrors.length || this.fatalValidationErrors.length)
    );
  }
  ...
  get willOverwriteExisting() {
    return (
      !this.isDefaultTheme &&
      this.themes.findBy('name', this.themeName.toLowerCase())
    );
  }

  get hasWarningsOrErrors() {
    return (
      this.validationWarnings.length > 0 || this.validationErrors.length > 0
    );
  }
  ...
  *installThemeTask() {
    try {
      ...
      const url =
        this.ghostPaths.url.api('themes/install') +
        `?source=github&ref=${this.themeRef}`;
      ...
      if (result.themes) {
        ...
        this.installedTheme = this.store.peekRecord(
          'theme',
      ...
      if (isThemeValidationError(error)) {
        ...
        this.validationWarnings =
          error.payload.errors[0].details.warnings || [];
        return false;
      }
