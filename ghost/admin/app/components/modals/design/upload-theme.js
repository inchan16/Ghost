...
export default class UploadThemeModal extends Component {
  ...
  get themeName() {
    ...
    return themePackage
      ? `${themePackage.name} - ${themePackage.version}`
      : name;
  }
      ...
      return {
        payload: {
          errors: [
            {
              message:
                'Sorry, the default Casper theme cannot be overwritten.<br>Please rename your zip file to continue.',
            },
          ],
        },
      };
  ...
  uploadFailed(errorResponse) {
    if (isThemeValidationError(errorResponse)) {
      ...
      this.validationWarnings =
        errorResponse.payload.errors[0].details.warnings || [];
    }
  }
