...
export default class EditNewsletterPreview extends Component {
  ...
  get backgroundColorIsDark() {
    return (
      textColorForBackgroundColor(this.backgroundColor).hex().toLowerCase() ===
      '#ffffff'
    );
  }
  ...
  get secondaryBorderColor() {
    return textColorForBackgroundColor(this.backgroundColor)
      .alpha(0.12)
      .toString();
  }
  ...
  get showHeader() {
    return (
      (this.args.newsletter.showHeaderIcon && this.settings.icon) ||
      this.headerTitle
    );
  }
  ...
  get showCommentCta() {
    return (
      this.args.newsletter.showCommentCta &&
      this.settings.commentsEnabled !== 'off'
    );
  }
  ...
  get headerSubtitle() {
    if (
      ...
      this.args.newsletter.showHeaderName
    ) {
      return this.args.newsletter.name;
    }
