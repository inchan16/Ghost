...
export default class CustomViewFormModal extends Component {
  ...
  @task
  *saveTask() {
    const view = yield this.customViews.saveViewTask.perform(
      this.args.data.customView
    );
    this.args.close();
    return view;
  }
  ...
  @task
  *deleteTask() {
    const view = yield this.customViews.deleteViewTask.perform(
      this.args.data.customView
    );

    const routeName = this.router.currentRouteName;
    this.router.transitionTo(routeName, {
      queryParams: resetQueryParams(routeName),
    });
