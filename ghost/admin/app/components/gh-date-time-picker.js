...
export default class GhDateTimePicker extends Component {
  ...
  didReceiveAttrs() {
    ...
    if (
      (date && date.valueOf()) !== (this._lastDate && this._lastDate.valueOf())
    ) {
      this._resetScratchDate();
    }
    ...
    if (minDate === 'now') {
      this.set(
        '_minDate',
        moment(moment().tz(blogTimezone).format(DATE_FORMAT)).toDate()
      );
    } else if (!isBlank(minDate)) {
      this.set(
        '_minDate',
        moment(moment.tz(minDate, blogTimezone).format(DATE_FORMAT)).toDate()
      );
    } else {
    ...
    if (maxDate === 'now') {
      ...
      this.set(
        '_maxDate',
        moment(moment.tz(maxDate, blogTimezone).format(DATE_FORMAT)).toDate()
      );
    } else {
