...
export default class StripeSettingsForm extends Component {
  ...
  currencies = currencies
    .slice(NO_OF_TOP_CURRENCIES, currencies.length)
    .map((currency) => {
      return {
        value: currency.isoCode.toLowerCase(),
        label: `${currency.isoCode} - ${currency.name}`,
  ...
  get selectedCurrency() {
    return (
      this.currencies.findBy('value', this.stripePlans.monthly.currency) ||
  ...
  get stripePlans() {
    ...
    const yearly = plans.find(
      (plan) => plan.interval === 'year' && plan.name !== 'Complimentary'
    );
  ...
  updateStripeDirect() {
    // Allow disabling stripe direct keys if stripe is still enabled, while the config is disabled
    this.stripeDirect =
      this.config.stripeDirect ||
      (this.membersUtils.isStripeEnabled &&
        !this.settings.stripeConnectAccountId);
  }
      ...
      while (pollTimeout < RETRY_PRODUCT_SAVE_MAX_POLL) {
        ...
        try {
          ...
          if (
            ...
            error.payload.errors[0].code === 'STRIPE_NOT_CONFIGURED'
          ) {
