...
export default class BulkUnsubscribeMembersModal extends Component {
  ...
  get hasMultipleNewsletters() {
    ...
    const activeNewsletters = newsletters.filter(
      (newsletter) => newsletter.status !== 'archived'
    );
  ...
  *bulkUnsubscribeTask() {
    try {
      ...
      const removeLabelUrl = `${this.ghostPaths.url.api(
        'members/bulk'
      )}?${query}`;
      const response = yield this.ajax.put(removeLabelUrl, {
        data: {
          bulk: {
            action: 'unsubscribe',
            newsletter: this.selectedNewsletterId
