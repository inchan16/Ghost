...
const FILTER_PROPERTIES = FILTER_GROUPS.flatMap((group) =>
  group.filters.map((f) => {
    f.group = group.name;
    return f;
  })
...
class Filter {
  ...
  constructor(options) {
    ...
    let defaultValue = '';
    if (
      options.properties.valueType === 'options' &&
      options.properties.options.length > 0
    ) {
    ...
    if (
      this.properties.valueType === 'date' &&
      typeof this.value === 'string'
    ) {
      // Convert string to Date
      this.value = moment(
        moment.tz(moment.utc(options.value), this.timezone).format('YYYY-MM-DD')
      ).toDate();
    }
    ...
    if (options.properties.valueType === 'options') {
      if (
        !options.properties.options.find((option) => option.name === this.value)
      ) {
  ...
  get isResourceFilter() {
    return (
      typeof this.properties.resource === 'string' &&
      this.properties.valueType === 'string'
    );
  }
...
export default class MembersFilter extends Component {
  ...
  get filterProperties() {
    ...
    if (this.newsletters?.length > 1) {
      ...
      availableFilters = availableFilters.filter(
        (prop) => prop.name !== 'subscribed'
      );
      // find the index of the 'basic' group and insert the 'multiple newsletters' filter after it
      const indexes = availableFilters
        .map((obj, index) => (obj.group === 'Basic' ? index : null))
        .filter((i) => i !== null);
      const lastIndex = indexes.pop();
      availableFilters.splice(
        lastIndex + 1,
        0,
        ...NEWSLETTERS_FILTER(this.newsletters)
      );
    }
    ...
    availableFilters = availableFilters.filter(
      (prop) => !prop.feature || this.feature[prop.feature]
    );
    availableFilters = availableFilters.filter(
      (prop) => !prop.setting || this.settings[prop.setting]
    );
    availableFilters = availableFilters.filter(
      (prop) => !prop.excludeForFeature || !this.feature[prop.excludeForFeature]
    );
  ...
  get availableFilterProperties() {
    ...
    if (!this.settings.paidMembersEnabled) {
      availableFilters = availableFilters.reject(
        (prop) => prop.group === 'Subscription'
      );
    }
  ...
  generateNqlFilter(filters) {
    ...
    filters.forEach((filter) => {
      const filterProperty = this.filterProperties.find(
        (prop) => prop.name === filter.type
      );
      ...
      if (filterProperty.valueType === 'array' && filter.value?.length) {
        ...
        let tzMoment = moment.tz(
          moment(filter.value).format('YYYY-MM-DD'),
          this.settings.timezone
        );

        if (relationStr === '>') {
          tzMoment = tzMoment.set({
            hour: 23,
            minute: 59,
            second: 59,
          });
  ...
  parseNqlFilterKey(nqlFilter) {
    ...
    if (relation && value) {
      const properties = this.filterProperties.find(
        (prop) => key === prop.name
      );
  ...
  *fetchTiers() {
  ...
  @task({ drop: true })
  *fetchNewsletters() {
    const response = yield this.store.query('newsletter', {
  ...
  @task({ drop: true })
  *fetchOffers() {
    const response = yield this.store.query('offer', {
      filter: 'status:active',
    });
  ...
  *fetchFilterResourcesTask() {
    ...
    if (ids.length > 0) {
      const posts = yield this.store.query('post', {
        limit: 'all',
        filter: `id:[${ids.join(',')}]`,
      });
