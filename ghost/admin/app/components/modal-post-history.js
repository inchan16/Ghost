...
export default ModalComponent.extend({
  ...
  comparisonRevision: computed(
    'selectedRevisionIndex',
    'revisionList.[]',
    function () {
      return (
        this.revisionList[this.selectedRevisionIndex + 1] ||
  ...
  restoreRevision: action(function (index) {
    ...
    this.modals.open(RestoreRevisionModal, {
      ...
      updateEditor: () => {
        const state = this.editorAPI.editorInstance.parseEditorState(
          revision.lexical
        );
  ...
  diffCards(div) {
    ...
    for (const card of cards) {
      const hasChanges =
        !!card.querySelectorAll('del').length ||
        !!card.querySelectorAll('ins').length;
      if (hasChanges) {
  ...
  updateDiff() {
    if (this.comparisonEditor && this.selectedEditor) {
      let comparisonState =
        this.comparisonEditor.editorInstance.parseEditorState(
          this.comparisonRevision.lexical
        );
      let selectedState = this.selectedEditor.editorInstance.parseEditorState(
        this.selectedRevision.lexical
      );
    ...
    let previous = document.querySelector(
      '.gh-post-history-hidden-lexical.previous'
    );
    let current = document.querySelector(
      '.gh-post-history-hidden-lexical.current'
    );
      ...
      if (previousDone && currentDone) {
        this.set(
          ...
          this.calculateHTMLDiff(
            this.stripInitialPlaceholder(previous.innerHTML),
            this.stripInitialPlaceholder(current.innerHTML)
          )
        );
        this.set(
          'selectedHTML',
