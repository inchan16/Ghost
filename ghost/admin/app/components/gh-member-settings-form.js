...
export default class extends Component {
    ...
    let tiers = subscriptions
      ...
      .filter((value, index, self) => {
        ...
        return (
          ...
          self.findIndex(
            (element) =>
              (element.tier_id || element.id) === (value.tier_id || value.id)
          ) === index
        );
      });
      ...
      .map((sub) => {
        const data = {
          ...
          startDate: sub.start_date
            ? moment(sub.start_date).format('D MMM YYYY')
            : '-',
          validUntil: sub.current_period_end
            ? moment(sub.current_period_end).format('D MMM YYYY')
            : '-',
        ...
        if (sub.trial_end_at) {
          const inTrialMode = moment(sub.trial_end_at).isAfter(
            new Date(),
            'day'
          );
          if (inTrialMode) {
  ...
  *cancelSubscriptionTask(subscriptionId) {
    let url = this.ghostPaths.url.api(
      'members',
      this.member.get('id'),
      'subscriptions',
      subscriptionId
  ...
  *continueSubscriptionTask(subscriptionId) {
  ...
  @task({ drop: true })
  *fetchTiers() {
    this.tiersList = yield this.store.query('tier', {
  ...
  @task({ drop: true })
  *fetchNewsletters() {
    this.newslettersList = yield this.store.query('newsletter', {
      filter: 'status:active',
    });
    if (this.member.get('isNew')) {
      const defaultNewsletters = this.newslettersList.filter((newsletter) => {
        return (
          newsletter.subscribeOnSignup && newsletter.visibility === 'members'
        );
      });
