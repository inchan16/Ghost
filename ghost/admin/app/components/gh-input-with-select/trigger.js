...
export default class GhSearchInputTrigger extends Component {
  ...
  handleFocus() {
    ...
    if (
      this.args.extra.showSearchMessage === false &&
  ...
  handleBlur(event) {
    if (event?.relatedTarget) {
      const thisInputTrigger = this.inputElem.closest(
        '.ember-basic-dropdown-trigger'
      );
      const relatedInputTrigger = event.relatedTarget.closest(
        '.ember-basic-dropdown-trigger'
      );
    ...
    if (
      this.args.extra?.value &&
      this.args.select.searchText === this.args.extra.value
    ) {
      this.args.select.actions.search('');
      this.close();
    }
