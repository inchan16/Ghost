...
export default class PaidMix extends Component {
  ...
  get areTiersAllZero() {
    if (
      this.dashboardStats.paidMembersByTier === null ||
      this.dashboardStats.paidMembersByTier.length === 0
    ) {
      return true;
    }
    const data = this.dashboardStats.paidMembersByTier.map(
      (stat) => stat.members
    );
  ...
  get chartData() {
    const totalCadence =
      this.dashboardStats.paidMembersByCadence.month +
      this.dashboardStats.paidMembersByCadence.year;
    const monthlyPercentage = Math.round(
      (this.dashboardStats.paidMembersByCadence.month / totalCadence) * 100
    );
    const annualPercentage = Math.round(
      (this.dashboardStats.paidMembersByCadence.year / totalCadence) * 100
    );
    ...
    // if it's for tiers...
    const labels = this.dashboardStats.paidMembersByTier.map(
      (stat) => stat.tier.name
    );
    const data = this.dashboardStats.paidMembersByTier.map(
      (stat) => stat.members
  ...
  get chartOptions() {
    ...
    let totalCadence =
      this.dashboardStats.paidMembersByCadence.month +
      this.dashboardStats.paidMembersByCadence.year;
    let minTickValue = -Math.round(
      (this.dashboardStats.paidMembersByCadence.month / totalCadence) * 100
    );
    let maxTickValue = Math.round(
      (this.dashboardStats.paidMembersByCadence.year / totalCadence) * 100
    );
    ...
    if (this.mode === 'cadence') {
      ...
      if (!this.areTiersAllZero) {
        let data = this.dashboardStats.paidMembersByTier.map(
          (stat) => stat.members
        );
        ...
        let negativeTierPercentage = Math.round(
          (data[0] / totalTiersAmount) * 100
        );
      ...
      tooltips: {
        ...
        callbacks: {
          label: (tooltipItems, data) => {
            const tooltipTextEl = document.querySelector(
              '#gh-dashboard-mix-tooltip .gh-dashboard-tooltip-value'
            );
            ...
            tooltipTextEl.innerHTML = `<span class="indicator solid" style="background-color: ${
              data.datasets[tooltipItems.datasetIndex].backgroundColor
            }"></span><span class="value">${value}</span><span class="metric">${label}</span>`;
          },
          title: () => {
            return null;
          },
