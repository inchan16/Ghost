...
export default class Anchor extends Component {
  ...
  get selectedDisplayOption() {
    return (
      this.displayOptions.find((d) => d.value === this.selectedChartDisplay) ??
  ...
  get loading() {
    return (
      this.dashboardStats.memberCountStats === null ||
      (this.hasPaidTiers && this.dashboardStats.mrrStats === null) ||
      this.resizing
    );
  }
  ...
  get hasTrends() {
    return (
      this.dashboardStats.memberCounts !== null &&
  ...
  get totalMembersTrend() {
    if (this.dashboardStats.memberCountsTrend) {
      return this.calculatePercentage(
        this.dashboardStats.memberCountsTrend.total,
        this.dashboardStats.memberCounts.total
      );
    }
    return 0;
  }
  ...
  get paidMembersTrend() {
    if (this.dashboardStats.memberCountsTrend) {
      return this.calculatePercentage(
        this.dashboardStats.memberCountsTrend.paid,
        this.dashboardStats.memberCounts.paid
      );
    }
    return 0;
  }
  ...
  get freeMembersTrend() {
    if (this.dashboardStats.memberCountsTrend) {
      return this.calculatePercentage(
        this.dashboardStats.memberCountsTrend.free,
        this.dashboardStats.memberCounts.free
      );
    }
    return 0;
  }
  ...
  get mrrTrend() {
    if (this.dashboardStats.currentMRRTrend) {
      return this.calculatePercentage(
        this.dashboardStats.currentMRRTrend,
        this.dashboardStats.currentMRR
      );
    }
    return 0;
  }
  ...
  get mrrCurrencySymbol() {
    ...
    const firstCurrency = this.dashboardStats.mrrStats[0]
      ? this.dashboardStats.mrrStats[0].currency
      : 'usd';
  ...
  get chartOptions() {
    ...
    let barColor = this.feature.nightShift
      ? 'rgba(200, 204, 217, 0.25)'
      : 'rgba(200, 204, 217, 0.65)';
      ...
      tooltips: {
        ...
        callbacks: {
          label: (tooltipItems, data) => {
            if (this.selectedChartDisplay === 'mrr') {
              const value = `${that.mrrCurrencySymbol}${ghPriceAmount(
              ...
              document.querySelector(
                '#gh-dashboard-anchor-tooltip .gh-dashboard-tooltip-value .value'
              ).innerHTML = value;
            } else {
          ...
          title: (tooltipItems) => {
            if (this.selectedChartDisplay === 'mrr') {
              ...
              document.querySelector(
                '#gh-dashboard-anchor-tooltip .gh-dashboard-tooltip-label'
              ).innerHTML = value;
            } else {
              const value = moment(tooltipItems[0].xLabel).format(DATE_FORMAT);
              document.querySelector(
              ...
              callback: function (value, index, values) {
                if (index === 0) {
                  document.getElementById(
                    'gh-dashboard-anchor-date-start'
                  ).innerHTML = moment(value).format(DATE_FORMAT);
                }
                if (index === values.length - 1) {
                  document.getElementById(
                    'gh-dashboard-anchor-date-end'
                  ).innerHTML = moment(value).format(DATE_FORMAT);
                }
  ...
  get emptyData() {
    return {
      ...
      data: [
        ...
        5799, 6133, 6403, 6677, 7053, 7111, 7242, 7292, 7596, 7713, 7856, 7886,
        8065, 8172, 8361, 8407, 8447, 8496,
      ],
    };
  }
