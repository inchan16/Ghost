...
export default class ControlPanel extends Component {
  ...
  loadState() {
    try {
      ...
      if (savedStatus) {
        ...
        if (parsed) {
          this.dashboardMocks.siteStatus = {
            ...this.dashboardMocks.siteStatus,
            ...parsed,
          };
        }
      ...
      if (enabledStr) {
        ...
        if (
          ...
          parsed !== this.dashboardMocks.enabled
        ) {
  ...
  get selectedStateOption() {
    return this.stateOptions.find(
      (option) => option.value.days === this.state.days
    );
  }
