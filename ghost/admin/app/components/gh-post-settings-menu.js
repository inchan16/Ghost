...
export default class GhPostSettingsMenu extends Component {
  ...
  willDestroyElement() {
    ...
    if (
      errors.has('publishedAtBlogDate') ||
      errors.has('publishedAtBlogTime')
    ) {
  ...
  setPublishedAtBlogDate(date) {
    ...
    let dateString = moment
      .tz(date, this.settings.get('timezone'))
      .format('YYYY-MM-DD');
  ...
  setHeaderInjection(code) {
    ...
    return post
      .validate({ property: 'codeinjectionHead' })
      .then(() => this.savePostTask.perform());
  }
  ...
  setFooterInjection(code) {
    ...
    return post
      .validate({ property: 'codeinjectionFoot' })
      .then(() => this.savePostTask.perform());
  }
  ...
  changeAuthors(newAuthors) {
    ...
    if (
      newAuthors.mapBy('id').join() === post.get('authors').mapBy('id').join()
    ) {
      return;
    }
