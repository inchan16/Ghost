...
export default class ModalTierPrice extends ModalBase {
  ...
  *saveTier() {
    ...
    this.tier.set(
      'benefits',
      this.benefits.filter((benefit) => !benefit.get('isBlank'))
    );
  ...
  validatePrices() {
    ...
    try {
      ...
      if (yearlyAmount > MAX_AMOUNT || monthlyAmount > MAX_AMOUNT) {
        throw new TypeError(
          `Subscription amount cannot be higher than ${symbol}${MAX_AMOUNT}`
        );
      }
    } catch (err) {
  ...
  calculateDiscount() {
    const discount = this.stripeMonthlyAmount
      ? 100 -
        Math.floor(
          ((this.stripeYearlyAmount / 12) * 100) / this.stripeMonthlyAmount
        )
      : 0;
