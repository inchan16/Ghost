...
const GhTaskButton = Component.extend({
  ...
  isRunning: computed(
    ...
    function () {
      ...
      let isRunning =
        taskName === lastTaskName && this.get('task.last.isRunning');
      if (
        this.hasRun &&
        taskName === lastTaskName &&
        this.get('task.last.value') &&
        !this.showSuccess
      ) {
        isRunning = true;
      }
  ...
  isFailure: computed(
    ...
    function () {
      ...
      return (
        taskName === lastTaskName &&
        this.get('task.last.error') !== undefined &&
        lastTaskValue !== 'canceled'
      );
    }
  ),
  ...
  handleReset: action(function () {
    const isTaskSuccess =
      this.get('task.last.isSuccessful') && this.get('task.last.value');
