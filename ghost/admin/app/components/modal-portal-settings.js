...
export default ModalComponent.extend({
  ...
  portalPreviewUrl: computed(
    'page',
    'model.tiers.[]',
    'changedTiers.[]',
    'membersUtils.{isFreeChecked,isMonthlyChecked,isYearlyChecked}',
    'settings.{portalName,portalButton,portalButtonIcon,portalButtonSignupText,portalSignupTermsHtml,portalSignupCheckboxRequired,portalButtonStyle,accentColor,portalPlans.[]}',
    function () {
      const options = this.getProperties(['page']);
      options.portalTiers = this.model.tiers
        ?.filter((tier) => {
          return (
            tier.get('visibility') === 'public' &&
  ...
  isFreeChecked: computed(
    'settings.{portalPlans.[],membersSignupAccess}',
    function () {
      const allowedPlans = this.settings.portalPlans || [];
      return (
        this.settings.membersSignupAccess === 'all' &&
        allowedPlans.includes('free')
      );
    }
  ),
  isMonthlyChecked: computed(
    'settings.portalPlans.[]',
    'membersUtils.paidMembersEnabled',
    function () {
      const allowedPlans = this.settings.portalPlans || [];
      return (
        this.membersUtils.paidMembersEnabled && allowedPlans.includes('monthly')
      );
    }
  ),
  isYearlyChecked: computed(
    'settings.portalPlans.[]',
    'membersUtils.paidMembersEnabled',
    function () {
      const allowedPlans = this.settings.portalPlans || [];
      return (
        this.membersUtils.paidMembersEnabled && allowedPlans.includes('yearly')
      );
    }
  ),
  tiers: computed(
    ...
    'isPreloading',
    function () {
      const paidTiers = this.model.tiers?.filter(
        (tier) => tier.type === 'paid' && tier.active === true
      );
  ...
  init() {
    ...
    this.set(
      'supportAddress',
      this.parseEmailAddress(this.settings.membersSupportAddress)
    );
  ...
  actions: {
    ...
    validateFreeSignupRedirect() {
      return this._validateSignupRedirect(
        this.freeSignupRedirect,
        'membersFreeSignupRedirect'
      );
    },

    validatePaidSignupRedirect() {
      return this._validateSignupRedirect(
        this.paidSignupRedirect,
        'membersPaidSignupRedirect'
      );
    },
    ...
    setSupportAddress(supportAddress) {
      ...
      if (
        this.config.emailDomain &&
        supportAddress === `noreply@${this.config.emailDomain}`
      ) {
  ...
  updateAllowedTier(tierId, isChecked) {
    ...
    let portalTiers = this.model.tiers
      .filter((p) => {
  ...
  finishPreloading: action(async function () {
    ...
    if (
      portalButtonIcon &&
      !this.membersUtils.defaultIconKeys.includes(portalButtonIcon)
    ) {
    ...
    try {
      ...
      if (error.payload && error.payload.errors) {
        for (const payloadError of error.payload.errors) {
          if (
            payloadError.type === 'ValidationError' &&
            payloadError.property &&
