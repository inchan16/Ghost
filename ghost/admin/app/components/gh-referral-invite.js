...
export default class GhReferralInvite extends Component {
  ...
  get showReferralInvite() {
    ...
    return (
      !this.args.hasThemeErrors &&
      this.isAdminOrOwner &&
      this.isReferralNotificationNotDismissed &&
      this.stripeLiveModeEnabled &&
      this.hasReachedMRR
    );
  }
  ...
  *loadCurrentMRR() {
  ...
  dismissReferralInvite(event) {
    ...
    if (!this.feature.referralInviteDismissed) {
      this.feature.referralInviteDismissed = moment().tz(
