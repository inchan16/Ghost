...
export default class GhTwitterUrlInput extends Component {
  ...
  setTwitterUrl(event) {
    ...
    if (
      newUrl.match(/(?:twitter\.com\/)(\S+)/) ||
      newUrl.match(/([a-z\d.]+)/i)
    ) {
      let username = [];
      ...
      if (newUrl.match(/(?:twitter\.com\/)(\S+)/)) {
        ...
        [username] = newUrl.match(/([^/]+)\/?$/im);
      }

      // check if username starts with http or www and show error if so
      if (
        username.match(/^(http|www)|(\/)/) ||
        !username.match(/^[a-z\d._]{1,15}$/im)
      ) {
        const message = !username.match(/^[a-z\d._]{1,15}$/im)
