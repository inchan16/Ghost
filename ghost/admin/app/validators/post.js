...
export default BaseValidator.create({
  ...
  canonicalUrl(model) {
    ...
    if (
      url.match(/\s/) ||
      (!validator.isURL(url, validatorOptions) && !url.match(urlRegex))
    ) {
      ...
      model.errors.add(
        'canonicalUrl',
        'Canonical URL is too long, max 2000 chars'
      );
      this.invalidate();
    }
  },
  ...
  _shouldValidatePublishedAtBlog(model) {
    return (
      isPresent(model.publishedAtUTC) ||
  ...
  publishedAtBlogTime(model) {
    ...
    if (
      !timeRegex.test(model.publishedAtBlogTime) &&
      this._shouldValidatePublishedAtBlog(model)
    ) {
  ...
  publishedAtBlogDate(model) {
    ...
    if (isEmpty(model.errors.errorsFor('publishedAtBlogTime'))) {
      ...
      if (
        ...
        publishedAtBlogTZ.isSameOrAfter(now)
      ) {
        ...
        (model.changedAttributes().status ||
          model.changedAttributes().publishedAtUTC) &&
        status === 'scheduled' &&
        !isInFuture
      ) {
  ...
  featureImageAlt(model) {
    if (!validator.isLength(model.featureImageAlt || '', 0, 125)) {
      model.errors.add(
        'featureImageAlt',
        'Feature image alt text cannot be longer than 125 characters.'
      );
      this.invalidate();
    }
  },
