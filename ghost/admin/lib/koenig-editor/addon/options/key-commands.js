...
import { CURSOR_AFTER, CURSOR_BEFORE } from '../components/koenig-editor';
...
const setHeader = function setHeader(headerTag, editor, koenig) {
  ...
  editor.run((postEditor) => {
    koenig.send('toggleHeaderSection', headerTag, postEditor, {
      force: true,
    });
    if (canKeepRange) {
...
export const DEFAULT_KEY_COMMANDS = [
  ...
  },
  {
    ...
    run(editor, koenig) {
      ...
      let sections = section.isListItem
        ? section.parent.parent.sections
      ...
      if (isCollapsed && marker && offset !== 0) {
        ...
        specialMarkupTagNames.forEach((tagName) => {
          ...
          let markup = marker.markups.find(
            (markerMarkup) => markerMarkup.tagName.toUpperCase() === tagName
          );
          if (markup) {
            ...
            if (!nextMarker || !nextMarker.hasMarkup(tagName)) {
              ...
              editor.run((postEditor) => {
                ...
                let range = editor.range.expandByMarker(
                  (markerToExpand) => !!markerToExpand.markups.includes(markup)
                );
  ...
  },
  {
    ...
    run(editor, koenig) {
      ...
      if (
        ...
        section.next &&
        section.next.type === 'card-section' &&
        !section.isBlank
      ) {
  ...
  },
  {
    ...
    run(editor, koenig) {
      ...
      let prevSection =
        section.isListItem && !section.prev
          ? section.parent.prev
          : section.prev;

      if (
        isCollapsed &&
        (offset === 0 || section.isCardSection) &&
        !prevSection
      ) {
...
export const BASIC_KEY_COMMANDS_WITH_BR = DEFAULT_KEY_COMMANDS.filter(
  (command) => {
    let basicCommands = [
      'BACKSPACE',
      'CTRL+K',
...
export const BASIC_TEXTAREA_KEY_COMMANDS = DEFAULT_KEY_COMMANDS.filter(
  (command) => {
    let basicCommands = [
      'BACKSPACE',
      'CTRL+K',
...
export const TEXT_REPLACEMENT_KEY_COMMANDS = DEFAULT_KEY_COMMANDS.filter(
  (command) => {
    let commands = [
      'BACKSPACE',
      'CTRL+K',
...
export default function registerKeyCommands(
  editor,
  koenig,
  commands = DEFAULT_KEY_COMMANDS
) {
