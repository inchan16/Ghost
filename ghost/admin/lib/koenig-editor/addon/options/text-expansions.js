  ...
  if (section.isListItem) {
    ...
    editor.run((postEditor) => {
      ...
      post.walkMarkerableSections(listRange, (listItem) => {
        ...
        let changedListItem = postEditor.changeSectionTagName(
          listItem,
          listTagName
        );
...
function _addMarkdownMarkup(_this, editor, matches, markupStr) {
  ...
  editor.run((postEditor) => {
    ...
    let endPos = postEditor.deleteRange(
      textRange.tail.toRange().extend(mdChars)
    );
...
function _matchLink(editor, text) {
  ...
  if (matches) {
    ...
    editor.run((postEditor) => {
      let startPos = postEditor.deleteRange(
        range.head.toRange().extend(hasLinkText ? 1 : 3)
      );
      ...
      let remainingRange = textRange.tail
        .toRange()
        .extend(hasLinkText ? (matches[2] || url).length + 3 : 1);
...
function _matchImage(editor, text) {
  ...
  if (matches) {
    ...
    editor.run((postEditor) => {
      let card = postEditor.builder.createCardSection('image', {
        src,
        alt,
      });
