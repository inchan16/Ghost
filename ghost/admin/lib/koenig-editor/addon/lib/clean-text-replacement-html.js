export default function cleanTextReplacementHtml(html = '', _options = {}) {
  ...
  if (!options.createDocument) {
    const Parser =
      (typeof DOMParser !== 'undefined' && DOMParser) ||
      (typeof window !== 'undefined' && window.DOMParser);

    if (!Parser) {
      throw new Error(
        'cleanTextReplacementHtml() must be passed a `createDocument` function as an option when used in a non-browser environment'
      );
    }
  ...
  if (cleanHtml) {
    ...
    doc.body.querySelectorAll('*').forEach((element) => {
      ...
      if (
        element.classList.contains('-mobiledoc-kit__atom') &&
