...
export default Service.extend({
  ...
  init() {
    ...
    this._rafUpdateGhostElementPosition = run.bind(
      this,
      this._updateGhostElementPosition
    );
  ...
  _onMouseDown(event) {
    if (
      ...
      this.grabbedElement = utils.getParent(
        event.target,
        constants.DRAGGABLE_SELECTOR
      );

      if (this.grabbedElement) {
        // some elements may have explicitly disabled dragging such as
        // captions where we want to allow text selection instead
        let dragDisabledElement = utils.getParent(
          event.target,
          constants.DRAG_DISABLED_SELECTOR
        );
        if (
          dragDisabledElement &&
          this.grabbedElement.contains(dragDisabledElement)
        ) {
          return;
        }

        let containerElement = utils.getParent(
          this.grabbedElement,
          constants.CONTAINER_SELECTOR
        );
        let container = this.containers.findBy('element', containerElement);
        this.sourceContainer = container;

        if (container.isDragEnabled) {
          this._waitForDragStart
            .perform(event)
            .then(() => {
  ...
  _initiateDrag(startEvent) {
    ...
    let draggableInfo = this.sourceContainer.getDraggableInfo(
      this.grabbedElement
    );
    ...
    let ghostElement = this.sourceContainer.createGhostElement(
      this.draggableInfo
    );
    if (ghostElement && ghostElement instanceof HTMLElement) {
      ...
      console.warn(
        'container.createGhostElement did not return an element',
        this.draggableInfo,
        { ghostElement }
      );
      this._resetDrag();
    ...
    this._elementsWithHoverRemoved =
      document.querySelectorAll('.kg-card-hover');
  ...
  _handleDrag() {
    ...
    let overContainerElem = utils.getParent(
      target,
      constants.CONTAINER_SELECTOR
    );
    let overDroppableElem = utils.getParent(
      target,
      constants.DROPPABLE_SELECTOR
    );
    ...
    let isLeavingContainer =
      this._currentOverContainerElem &&
      overContainerElem !== this._currentOverContainerElem;
    let isLeavingDroppable =
      this._currentOverDroppableElem &&
      overDroppableElem !== this._currentOverDroppableElem;
    let isOverContainer =
      overContainerElem && overContainerElem !== this._currentOverContainer;
  ...
  _showDropIndicator({ direction, position, beforeElems, afterElems }) {
    ...
    if (direction === 'horizontal') {
      ...
      if (position === 'left') {
        ...
        leftAdjustment +=
          parseInt(droppable.offsetWidth) +
          parseInt(droppableStyles.marginRight);
      }

      // account for indicator width
      leftAdjustment -= 2;

      let dropIndicatorParentRect =
        dropIndicator.parentNode.getBoundingClientRect();
      ...
      if (
        ...
        this._dropIndicatorTimeout = run.later(
          this,
          function () {
    ...
    if (direction === 'vertical') {
      ...
      if (position === 'top') {
        topElement = utils.getPreviousSibling(
          droppable,
          constants.DROPPABLE_SELECTOR
        );
        ...
        bottomElement = utils.getNextSibling(
          droppable,
          constants.DROPPABLE_SELECTOR
        );
      }
      ...
      let firstElement = (topElement || bottomElement).parentElement
        .children[0];
      ...
      if (topElement && bottomElement) {
        ...
        newTop =
          topElement.offsetTop + topElement.offsetHeight + firstTopMargin;
  ...
  _appendDropIndicator() {
    let dropIndicator = document.querySelector(
      `#${constants.DROP_INDICATOR_ID}`
    );
    if (!dropIndicator) {
      ...
      let scrollableElement =
        document.querySelector('.koenig-editor') ||
  ...
  _addGrabListeners() {
    this._addEventListener('mousedown', this._onMouseDown, {
      passive: false,
    });
  },
  ...
  _addMoveListeners() {
    this._addEventListener('mousemove', this._onMouseMove, {
      passive: false,
    });
  },
