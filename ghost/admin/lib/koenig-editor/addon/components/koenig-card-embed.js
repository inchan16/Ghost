...
export default class KoenigCardEmbed extends Component {
  ...
  insertAsLink(options = { linkOnError: false }) {
    ...
    this.editor.run((postEditor) => {
      ...
      postEditor.insertTextWithMarkup(p.toRange().head, this.payload.url, [
        link,
      ]);
  ...
  insertAsBookmark(payload) {
    ...
    this.editor.run((postEditor) => {
      let cardSection = this.env.postModel;
      let bookmarkCard = postEditor.builder.createCardSection(
        'bookmark',
        payload
      );
  ...
  _resizeIframe(iframe) {
    this._resizeDebounce = run.debounce(
      this,
      this.__debouncedResizeIframe,
      iframe,
      66
    );
  }
