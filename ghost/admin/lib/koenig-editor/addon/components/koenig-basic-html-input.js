...
import registerKeyCommands, {
  BASIC_KEY_COMMANDS,
  BASIC_KEY_COMMANDS_WITH_BR,
} from '../options/key-commands';
...
export default class KoenigBasicHtmlInput extends Component {
  ...
  get editorOptions() {
    ...
    return assign(
      {
        html: `<${this.defaultTag}>${this.cleanHTML || ''}</${
          this.defaultTag
        }>`,
  ...
  willRender() {
    ...
    registerKeyCommands(
      editor,
      this,
      this.allowBr ? BASIC_KEY_COMMANDS_WITH_BR : BASIC_KEY_COMMANDS
    );
  ...
  _getHTML() {
    if (this.editor && this.editor.element) {
      let firstParagraph =
        this.editor.element.querySelector(this.defaultTag) ||
        this.editor.element.querySelector('p');
