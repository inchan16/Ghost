...
export default class KoenigPlusMenu extends Component {
  ...
  didReceiveAttrs() {
    ...
    if (
      !this._ignoreRangeChange &&
      this.showMenu &&
      editorRange &&
      !editorRange.isBlank &&
      !editorRange.isEqual(this._lastEditorRange)
    ) {
      this._hideMenu();
    }
  ...
  itemClicked(item, event) {
    ...
    if (item.type === 'snippet') {
      let clickedSnippet = this.snippets.find(
  ...
  _positionMenu() {
    ...
    let {
      head: { section },
    } = this._editorRange || this.editorRange;
  ...
  _showMenu() {
    ...
    this._onWindowMousedownHandler = run.bind(
      this,
      this._handleWindowMousedown
    );
