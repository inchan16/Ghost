...
export default class KoenigCardImageTenorSelector extends Component {
  ...
  moveHighlightDown() {
    const nextGif =
      this.tenor.columns[this.highlightedGif.columnIndex][
        this.highlightedGif.columnRowIndex + 1
      ];
  ...
  _moveToNextHorizontalGif(direction) {
    const highlightedElem = document.querySelector(
      `[data-tenor-index="${this.highlightedGif.index}"]`
    );
    ...
    while (!foundGifElem) {
      let possibleMatch = document
        .elementFromPoint(x, y)
        ?.closest('[data-tenor-index]');
