...
export default class KoenigSnippetInputComponent extends Component {
  ...
  constructor() {
    ...
    let { node: headNode, offset: headOffset } =
      cursor._findNodeForPosition(head);
    let { node: tailNode, offset: tailOffset } =
      cursor._findNodeForPosition(tail);
  ...
  selectSnippet(snippetName) {
    ...
    const existingSnippet = this.args.snippets.find(
  ...
  replaceSnippet(snippet) {
    this.args.update(snippet, { mobiledoc: this.snippetMobiledoc });
  ...
  nameKeydown(event) {
    if (event.key === 'Enter') {
      ...
      let { snippetRange, editor } = this.args;
      let mobiledoc = editor.serializePost(
        editor.post.trimTo(snippetRange),
        'mobiledoc'
      );
  ...
  registerAndPositionElement(element) {
    ...
    let rangeRect =
      this.args.snippetRect || this._windowRange.getBoundingClientRect();
    ...
    newPosition = {
      ...
      left:
        rangeRect.left - containerRect.left + rangeRect.width / 2 - width / 2,
      right: null,
    };
    ...
    if (newPosition.left + width > containerRect.width) {
      ...
      // calculate the tick percentage position
      let absTickPosition =
        rangeRect.right - containerRect.right - rangeRect.width / 2;
    ...
    if (tickPosition !== 50) {
      this._addStyleElement(
        `left: calc(${tickPosition}% - ${TICK_ADJUSTMENT}px)`
      );
    }
  ...
  _handleMousedown(event) {
    const isOutsideElement =
      this.element && !event.target.closest(this.element.id);
    const isOutsideDropdown = !event.target.closest(
