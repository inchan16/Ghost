...
export default class KoenigLinkToolbar extends Component {
  ...
  didReceiveAttrs() {
    ...
    if (
      this.linkRange ||
      (this.selectedRange && !this.selectedRange.isCollapsed)
    ) {
  ...
  _getLinkRange() {
    ...
    try {
      ...
      const linkMarkup =
        position.marker && position.marker.markups.findBy('tagName', 'a');
      if (linkMarkup) {
        const linkRange = position
          .toRange()
          .expandByMarker((marker) => !!marker.markups.includes(linkMarkup));
        return linkRange;
      }
    } catch (e) {
  ...
  _handleMouseover(event) {
    if (this._canShowToolbar) {
      ...
      if (
        ...
        target.closest('[data-kg-has-link-toolbar=true]') === this.container
      ) {
        this._timeout = run.later(
          this,
          function () {
    ...
    if (this.showToolbar) {
      ...
      if (
        ...
        !(
          ...
          toElement === this._target ||
          toElement.closest(`#${this.elementId}`)
        )
      ) {
        this.set('showToolbar', false);
      }
  ...
  _positionToolbar(target, { x, y }) {
    ...
    if (x && y) {
      ...
      this._targetRect = rects.find((rect) => {
        return (
          rect.x - TOOLBAR_MARGIN <= x &&
          x <= rect.x + rect.width + TOOLBAR_MARGIN &&
          rect.y - TOOLBAR_MARGIN <= y &&
          y <= rect.y + rect.height + TOOLBAR_MARGIN
        );
      });
    }
    if (!this._targetRect) {
