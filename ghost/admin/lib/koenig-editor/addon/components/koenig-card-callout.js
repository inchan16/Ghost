...
export default class KoenigCardCalloutComponent extends Component {
  ...
  get isEmpty() {
    return (
      isBlank(this.args.payload.calloutText) &&
      isBlank(this.args.payload.calloutEmoji)
    );
  }
  ...
  get selectedBackgroundColor() {
    return this.backgroundColors.find(
      (option) => option.color === this.args.payload.backgroundColor
    );
  }
  ...
  constructor() {
    ...
    let emojiButtonContainer = document.getElementById(
  ...
  toggleEmoji() {
    this._updatePayloadAttr(
      'calloutEmoji',
      this.args.payload.calloutEmoji ? '' : this.defaultEmoji
    );
  }
