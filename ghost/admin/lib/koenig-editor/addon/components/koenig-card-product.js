...
export default class KoenigCardProductComponent extends Component {
  ...
  get isEmpty() {
    ...
    return (
      isBlank(productTitle) &&
      isBlank(productDescription) &&
      (!productButtonEnabled ||
        isBlank(productUrl) ||
        isBlank(productButton)) &&
      isBlank(productImageSrc) &&
      !productRatingEnabled
    );
  }
  ...
  handleTitleTab() {
    let contentInput = this.element.querySelector(
      '.kg-product-card-description .koenig-basic-html-textarea__editor'
    );
  ...
  _enter(modifier) {
    if (
      this.args.isEditing &&
      (modifier === 'meta' || (modifier === 'ctrl' && Browser.isWin()))
    ) {
      this.args.editCard();
    }
  }
  ...
  _focusInput() {
    let headingInput = this.element.querySelector(
      '.kg-product-card-title .koenig-basic-html-input__editor'
    );
  ...
  @action
  toggleProductButton() {
    this._updatePayloadAttr(
      'productButtonEnabled',
      !this.args.payload.productButtonEnabled
    );
  }
  ...
  toggleProductRating() {
    this._updatePayloadAttr(
      'productRatingEnabled',
      !this.args.payload.productRatingEnabled
    );
  }
