...
export default class KoenigCardHeaderComponent extends Component {
  ...
  get isEmpty() {
    const { header, subheader, buttonUrl, buttonText, buttonEnabled } =
      this.args.payload;

    return (
      isBlank(header) &&
      isBlank(subheader) &&
      (!buttonEnabled || isBlank(buttonUrl) || isBlank(buttonText))
    );
  }
  ...
  get hasSubheader() {
    return (
      this.args.payload.subheader &&
      Boolean(this.args.payload.subheader.replace(/(<br *\/?>)+$/gi, '').trim())
    );
  }
  ...
  _focusInput() {
    let headingInput = this.element.querySelector(
      '.kg-header-card-header .koenig-basic-html-input__editor'
    );
