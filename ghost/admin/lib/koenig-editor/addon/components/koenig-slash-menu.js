...
const createItemMatcher = (query) => {
  ...
  return (item) => {
    ...
    if (isArray(item.matches)) {
      return card
        ? item.matches.any((match) => match.indexOf(card.toLowerCase()) === 0)
        : true;
    }
...
export default class KoenigSlashMenuComponent extends Component {
  ...
  get itemMap() {
    ...
    this.itemSections.forEach((section) => {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        itemMap.push(
          section.items.slice(startIndex, startIndex + section.rowLength)
        );
      }
    });
  ...
  updateItemSections() {
    ...
    if (
      (matchedItem?.type === 'selector' ||
        matchedItem?.insertOnSpace === true) &&
  ...
  itemClicked(item, event) {
    ...
    if (item.type === 'snippet') {
      let clickedSnippet = this.args.snippets.find(
        (snippet) => snippet.name === item.label
      );
      if (clickedSnippet) {
        let post = mobiledocParsers.parse(
          this.args.editor.builder,
          clickedSnippet.mobiledoc
        );
  ...
  _handleWindowMousedown(event) {
    ...
    if (
      !event.target.closest(
        `#${this.containerElement.id}, .fullscreen-modal-container`
      )
    ) {
      this._hideMenu();
