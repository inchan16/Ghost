...
export default class KoenigCardGallery extends Component {
  ...
  uploadFailed(uploadResult) {
    ...
    this._buildAndSaveImagesPayload();
    let fileName =
      uploadResult.fileName.length > 20
        ? `${uploadResult.fileName.substr(0, 20)}...`
  ...
  handleErrors(errors) {
    let errorMssg =
      (errors[0] && errors[0].message) || 'Some images failed to upload';
  ...
  _buildAndSaveImagesPayload() {
    ...
    let isValidImage = (image) =>
      image.fileName && image.src && image.width && image.height;

    this.images.forEach((image, idx) => {
      if (isValidImage(image)) {
        let payloadImage = Object.assign({}, image, {
  ...
  _dragStart(draggableInfo) {
    ...
    let isImageDrag =
      draggableInfo.type === 'image' || draggableInfo.cardName === 'image';
    if (
      isImageDrag &&
      draggableInfo.payload.src &&
  ...
  _getDraggableInfo(draggableElement) {
    ...
    let image =
      this.images.findBy('src', src) || this.images.findBy('previewSrc', src);
    let payload =
      image &&
      image.getProperties(
        'fileName',
  ...
  _onDrop(draggableInfo /*, droppableElem, position*/) {
    ...
    if (this._isDropAllowed(draggableIndex, insertIndex)) {
      if (draggableIndex === -1) {
        ...
        let img = draggableInfo.element.querySelector(
          `img[src="${payload.src}"]`
        );
        ...
        let accountForRemoval =
          draggableIndex < insertIndex && insertIndex ? -1 : 0;
