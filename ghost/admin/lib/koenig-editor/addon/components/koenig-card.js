...
export default class KoenigCard extends Component {
  ...
  @computed(
    'showToolbar',
    'koenigUi.{captionHasFocus,isDragging,inputHasFocus}'
  )
  ...
  get selectedClass() {
    return this.isSelected && this.showSelectedOutline
      ? 'kg-card-selected'
      : '';
  }
  ...
  didReceiveAttrs() {
    ...
    if (
      isSelected &&
      this._lastIsSelected &&
      this.toolbar &&
      this.toolbar !== this._lastToolbar
    ) {
    ...
    if (
      ...
      this._lastIsSelected &&
      this.saveAsSnippet &&
      this.saveAsSnippet !== this._lastSaveAsSnippet
    ) {
      run.scheduleOnce('afterRender', this, this._showToolbar);
    }
  ...
  mouseDown(event) {
    ...
    if (!isSelected && !isEditing) {
      ...
      let allowClickthrough = !!event.target.closest(
  ...
  _onLeaveEdit() {
    ...
    if (
      ...
      JSON.stringify(this.payload) !==
        JSON.stringify(this.env.postModel.payload)
    ) {
      this.editor.run(() => {
  ...
  _setToolbarProperties() {
    ...
    let toolbar = this.element?.querySelector(
      ':scope > [data-kg-toolbar="true"]'
    );
  ...
  _handleKeydown(event) {
    if (
      (this.isEditing && event.key === 'Escape') ||
