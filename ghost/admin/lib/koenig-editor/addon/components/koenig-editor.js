...
import {
  canInsertCardsFromFiles,
  insertCardsFromFiles,
} from '../utils/insert-cards-from-files';
...
export function toggleSpecialFormatEditState(editor) {
  ...
  if (isCollapsed) {
    ...
    Object.keys(SPECIAL_MARKUPS).forEach((tagName) => {
      ...
      if (
        head.marker &&
        head.marker.hasMarkup(tagName) &&
        editor._editState.activeMarkups.findBy('tagName', tagName)
      ) {
        ...
        if (!nextMarker || !nextMarker.hasMarkup(tagName)) {
          ...
          editor._editState.activeMarkups
            .filterBy('tagName', tagName)
            .forEach((markup) => {
...
export default class KoenigEditor extends Component {
  ...
  get allComponentCardsRegistered() {
    return this.componentCards.every(
      (card) => typeof card.component === 'object'
    );
  }
  ...
  willRender() {
    ...
    let componentHooks = {
      ...
      [ADD_CARD_HOOK]: ({ env, options, payload }, koenigOptions) => {
        ...
        let { files } = payload;
        let payloadCopy = new TrackedObject(
      ...
      [ADD_ATOM_HOOK]: ({ env, options, value, payload }) => {
        ...
        const payloadCopy = new TrackedObject(
          JSON.parse(JSON.stringify(payload || null))
        );
    ...
    editorOptions.cardOptions = Object.assign(
      {},
      this.cardOptions,
      componentHooks
    );
  ...
  didInsertElement() {
    ...
    this._dropTarget =
      document.querySelector(this.dropTargetSelector) || this.element;
  ...
  replaceWithCardSection(cardName, range, payload) {
    ...
    run.schedule('afterRender', this, function () {
      ...
      if (!card) {
        captureMessage(
          'replaceWithCardSection: card was not present after first render'
        );
        console.warn(
          'replaceWithCardSection: card was not present after first render'
        ); // eslint-disable-line
        ...
        run.schedule('afterRender', this, function () {
          ...
          if (!card) {
            captureMessage(
              'replaceWithCardSection: card was not present after second render'
            );
            console.warn(
              'replaceWithCardSection: card was not present after second render'
            ); // eslint-disable-line
  ...
  postDidChange(editor) {
    ...
    updatedMobiledoc.ghostVersion =
      this._upstreamMobiledoc.ghostVersion || BLANK_DOC.ghostVersion;
  ...
  inputModeDidChange(editor) {
    ...
    let sectionParentTagNames = editor.activeSections.map((s) =>
      s.isNested ? s.parent.tagName : s.tagName
    );
  ...
  handleKeydown(event) {
    ...
    if (
      ...
      editor &&
      editor.cursor &&
      editor.cursor.isAddressable(event.target)
    ) {
      // ctrl+h = BACKSPACE
  ...
  handlePaste(event) {
    ...
    if (text && validator.isURL(text)) {
      ...
      if (
        range &&
        !range.isCollapsed &&
        range.headSection === range.tailSection &&
        range.headSection.isMarkerable
      ) {
        let linkMarkup = editor.builder.createMarkup('a', {
          href: text,
        });
        editor.run((postEditor) => {
      ...
      if (
        range &&
        range.isCollapsed &&
        range.headSection.isBlank &&
        !range.headSection.isListItem
      ) {
        if (!this._modifierKeys.shift) {
          editor.run((postEditor) => {
            let payload = new TrackedObject({
              url: text,
              linkOnError: true,
              isDirectUrl: true,
            });
  ...
  getCardFromSection(section) {
    ...
    let cardId =
      section.renderNode.element.querySelector('.__mobiledoc-card').firstChild
        .id;
  ...
  getCardFromElement(element) {
    ...
    let cardElement =
      element.querySelector('.__mobiledoc-card') ||
      getParent(element, '.__mobiledoc-card');
  ...
  moveCaretToSection(section, position, skipCursorChange = true) {
    let sectionPosition =
      position === 'head' ? section.headPosition() : section.tailPosition();
  ...
  @task(function* () {
    ...
    this.componentCards.forEach((card) => {
      if (
        typeof card.component.isEmpty === 'boolean' &&
        card.component.isEmpty
      ) {
  ...
  _scrollCursorIntoView(options = { jumpToCard: false }) {
    ...
    let element =
      ...
      range.head.section.renderNode.element;
    ...
    if (
      ...
      range &&
      range.head.section &&
      range.head.section.isCardSection
    ) {
      return;
    }
    ...
    if (windowRange) {
      ...
      let { top: cursorTop, height: cursorHeight } =
        windowRange.getBoundingClientRect();
      ...
      if (this.scrollOffsetBottomSelector) {
        let bottomElement = document.querySelector(
          this.scrollOffsetBottomSelector
        );
      ...
      if (cursorTop === 0 && cursorHeight === 0) {
        ...
        ({ top: cursorTop, height: cursorHeight } =
      ...
      if (cursorTop < 0 + offsetTop) {
        this._scrollContainer.scrollTop =
          scrollTop - offsetTop + cursorTop - 20;
        return;
      }
      ...
      if (
        ...
        paddingBottom = parseFloat(
          getComputedStyle(this.element.parentNode).getPropertyValue(
            'padding-bottom'
          )
        );
      }
  ...
  _registerCardReorderDragDropHandler() {
    let cardDragDropContainer = this.koenigDragDropHandler.registerContainer(
      this.editor.element,
      {
  ...
  _getDropIndicatorPosition(draggableInfo, droppableElem, position) {
    let droppables = Array.from(
      this.editor.element.querySelectorAll(':scope > *')
    );
  ...
  _onCardDrop(draggableInfo) {
    ...
    if (this._isCardDropAllowed(draggableIndex, draggableInfo.insertIndex)) {
      ...
      if (draggableInfo.type === 'image') {
        ...
        this.editor.run((postEditor) => {
          let imageCard = postEditor.builder.createCardSection(
            'image',
  ...
  _isCardDropAllowed(draggableIndex, droppableIndex, position = '') {
    ...
    if (
      ...
      typeof droppableIndex === 'undefined'
    ) {
      return false;
    }
  ...
  _triggerTextHandlers() {
    ...
    run.next(this, function () {
      let matchedHandler =
        editor._eventManager._textInputHandler._findHandler();
      if (matchedHandler) {
