...
describe('Unit: Helper: gh-count-characters', function () {
  ...
  it('counts remaining chars', function () {
    ...
    expect(result.string).to.equal(
      `<span class="word-count" style="${defaultStyle}">196</span>`
    );
  });
  ...
  it('warns when nearing limit', function () {
    ...
    expect(result.string).to.equal(
      `<span class="word-count" style="${errorStyle}">5</span>`
    );
  });
  ...
  it('counts multibyte correctly', function () {
    ...
    expect(result.string).to.equal(
      `<span class="word-count" style="${defaultStyle}">199</span>`
    );

    // emoji + modifier is still two chars
    result = countCharacters(['💃🏻']);
    expect(result.string).to.equal(
      `<span class="word-count" style="${defaultStyle}">198</span>`
    );
  });
});
