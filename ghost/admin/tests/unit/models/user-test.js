...
describe('Unit: Model: user', function () {
  ...
  it('isActive/isSuspended properties are correct', function () {
    ...
    ['warn-1', 'warn-2', 'warn-3', 'warn-4', 'locked'].forEach(function (
      status
    ) {
      run(() => {
  ...
  it('role property is correct', function () {
    ...
    run(() => {
      let role = store.push({
        data: { id: 1, type: 'role', attributes: { name: 'Author' } },
      });
    ...
    run(() => {
      let role = store.push({
        data: { id: 1, type: 'role', attributes: { name: 'Editor' } },
      });
      model.set('role', role);
    });
  ...
  it('isOwnerOnly property is correct', function () {
    ...
    run(() => {
      let role = store.push({
        data: { id: 1, type: 'role', attributes: { name: 'Owner' } },
      });
      model.set('role', role);
    });
