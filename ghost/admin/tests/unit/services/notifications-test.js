...
describe('Unit: Service: notifications', function () {
  ...
  beforeEach(function () {
    ...
    this.owner
      ...
      .set('delayedNotifications', emberA());
  });
  ...
  it('filters alerts/notifications', function () {
    ...
    expect(notifications.notifications.firstObject.message).to.equal(
      'Notification'
    );
  });
  ...
  it('#handleNotification defaults to notification if no status supplied', function () {
    ...
    expect(notifications.content).to.deep.include({
      message: 'Test',
      status: 'notification',
    });
  });
  ...
  it('#showAlert adds POJO alerts', function () {
    ...
    expect(notifications.alerts).to.deep.include({
      message: 'Test Alert',
      status: 'alert',
      type: 'error',
      key: undefined,
  ...
  it('#showAlert adds delayed notifications', function () {
    ...
    expect(notifications.delayedNotifications).to.deep.include({
      message: 'Test Alert',
      status: 'notification',
      type: 'error',
      key: undefined,
  ...
  it('#showAlert clears duplicates using keys', function () {
    ...
    expect(notifications.alerts.length).to.equal(2);
    expect(notifications.alerts.lastObject.message).to.equal(
      'Duplicate with new message'
    );
  });
  ...
  it('#showNotification adds POJO notifications', function () {
    ...
    expect(notifications.notifications).to.deep.include({
  ...
  it('#showAPIError sets correct key when passed a base key', function () {
    ...
    expect(notifications.alerts.firstObject.key).to.equal(
      'api-error.test.alert'
    );
  });
  ...
  it('#closeNotification removes POJO notifications', function () {
    ...
    expect(notifications.notifications).to.include(notification);

    run(() => {
      notifications.closeNotification(notification);
    });

    expect(notifications.notifications).to.not.include(notification);
  });
  ...
  it('#closeNotifications only removes notifications', function () {
    ...
    expect(notifications.notifications.length, 'notifications count').to.equal(
      2
    );
  ...
  it('#closeNotifications only closes notifications with specified key', function () {
    ...
    expect(
      notifications.notifications.firstObject.message,
      'notification message'
    ).to.equal('Second notification');
    expect(notifications.alerts.length, 'alerts count').to.equal(1);
  });
