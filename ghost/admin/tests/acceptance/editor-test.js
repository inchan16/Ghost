...
describe('Acceptance: Editor', function () {
  ...
  it('when logged in as a contributor, renders a save button instead of a publish menu & hides tags input', async function () {
    ...
    expect(find('[data-test-publishmenu-trigger]'), 'publish menu trigger').to
      .not.exist;
    ...
    expect(find('[data-test-token-input]'), 'tags input').to.not.exist;
  ...
  describe('when logged in', function () {
    ...
    describe('post settings menu', function () {
      it('can set publish date', async function () {
        ...
        expect(
          ...
          'inline error response for invalid time'
        ...
        // with the new time
        await fillIn(
          '[data-test-date-time-picker-datepicker] input',
          moment.tz('Etc/UTC').add(1, 'day').format('YYYY-MM-DD')
        );
        await blur('[data-test-date-time-picker-datepicker] input');
        await fillIn(
          '[data-test-date-time-picker-time-input]',
          futureTime.format('HH:mm')
        );
        await blur('[data-test-date-time-picker-time-input]');

        expect(
          find('[data-test-date-time-picker-error]').textContent.trim(),
          'inline error response for future time'
        ...
        expect(
          ...
          moment(post1.publishedAt).tz('Etc/UTC').format('YYYY-MM-DD')
        );
        ...
        expect(
          moment(post1.publishedAt).tz('Etc/UTC').format('YYYY-MM-DD HH:mm:ss')
        ).to.equal('2017-04-09 12:00:00');

        // go to settings to change the timezone
        await visit('/settings/general');
        await click('[data-test-toggle-timezone]');

        expect(currentURL(), 'currentURL for settings').to.equal(
          '/settings/general'
        );
        expect(
          find('#timezone option:checked').textContent.trim(),
          'default timezone'
        ).to.equal('(GMT) UTC');
    ...
    it.skip('handles validation errors when scheduling', async function () {
      ...
      await datepickerSelect(
        '[data-test-publishmenu-draft] [data-test-date-time-picker-datepicker]',
        plusTenMin.toDate()
      );
      await fillIn(
        '[data-test-publishmenu-draft] [data-test-date-time-picker-time-input]',
        plusTenMin.format('HH:mm')
      );
      await blur(
        '[data-test-publishmenu-draft] [data-test-date-time-picker-time-input]'
      );
    ...
    it('renders first countdown notification before scheduled time', async function () {
      ...
      this.server.create('post', {
        publishedAt: moment.utc().add(4, 'minutes'),
        status: 'scheduled',
        authors: [author],
      });
      ...
      expect(
        find('[data-test-schedule-countdown]').textContent.trim(),
    ...
    it('shows author token input and allows changing of authors in PSM', async function () {
      ...
      let tokens = findAll(
        '[data-test-input="authors"] .ember-power-select-multiple-option'
      );
    ...
    it('saves post settings fields', async function () {
      ...
      let headerCM = find(
        '[data-test-field="codeinjection-head"] .CodeMirror'
      ).CodeMirror;
      ...
      await headerCM.setValue(
        '<script src="http://example.com/inject-head.js"></script>'
      );
      await click(headerCM.getInputField());
      await blur(headerCM.getInputField());
      ...
      let footerCM = find(
        '[data-test-field="codeinjection-foot"] .CodeMirror'
      ).CodeMirror;
      ...
      await footerCM.setValue(
        '<script src="http://example.com/inject-foot.js"></script>'
      );
      await click(footerCM.getInputField());
      await blur(footerCM.getInputField());
