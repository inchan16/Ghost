...
describe('Acceptance: Members import', function () {
  ...
  it('can open and close import modal', async function () {
    ...
    expect(find('[data-test-modal="import-members"]'), 'members import modal')
      .to.exist;
...
`;
    ...
    await fileUpload('[data-test-fileinput="members-csv"]', [csv], {
      name: 'members.csv',
      type: 'text/csv',
    });

    expect(find('[data-test-csv-file-mapping]'), 'csv file mapper').to.exist;
    expect(find('[data-test-members-import-table]'), 'csv file mapper').to
      .exist;
    expect(
      findAll('[data-test-members-import-mapper]').length,
      '# of mapper rows'
    ).to.equal(6);
    ...
    expect(find('[data-test-modal="import-members"]')).to.contain.text(
      'Import complete'
    );
...
`;
    ...
    expect(
      findAll(`#${dropdownContentId} li.ember-power-select-option`).length,
      '# of label options'
    ).to.equal(1);

    // label input doesn't allow editing from the import modal
    expect(
      findAll(`#${dropdownContentId} [data-test-edit-label]`).length,
      '# of label edit buttons'
    ).to.equal(0);

    await click(find(`#${dropdownContentId} li.ember-power-select-option`));

    expect(
      findAll(`${labelInput} .ember-power-select-multiple-options li`).length,
      '# of selected labels'
    ).to.equal(1);
    expect(
      find(`${labelInput} .ember-power-select-multiple-options li`)
    ).to.contain.text(label1.name);
