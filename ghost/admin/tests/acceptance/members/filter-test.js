...
describe('Acceptance: Members filtering', function () {
  ...
  it('has a known base-state', async function () {
    ...
    expect(
      findAll('[data-test-list="members-list-item"]').length,
      '# of member rows'
    ).to.equal(7);

    // export is available
    expect(find('[data-test-button="export-members"]'), 'export members button')
      .to.exist;
    expect(
      find('[data-test-button="export-members"]'),
      'export members button'
    ).to.not.have.attribute('disabled');

    // bulk actions are hidden
    expect(
      find('[data-test-button="add-label-selected"]'),
      'add label to selected button'
    ).to.not.exist;
    expect(
      find('[data-test-button="remove-label-selected"]'),
      'remove label from selected button'
    ).to.not.exist;
    expect(
      find('[data-test-button="unsubscribe-selected"]'),
      'unsubscribe selected button'
    ).to.not.exist;
    expect(
      find('[data-test-button="delete-selected"]'),
    ...
    expect(
      find('[data-test-input="members-search"]'),
      'search input'
    ).to.not.have.class('active');
    expect(
      find('[data-test-button="members-filter-actions"] span'),
      'filter button'
    ).to.not.have.class('gh-btn-label-green');

    // standard columns are shown
    expect(
      findAll('[data-test-table="members"] [data-test-table-column]').length
    ).to.equal(4);
  });
  ...
  describe('filtering', function () {
    it('can filter by label', async function () {
      ...
      await fillIn(
        `${filterSelector} [data-test-select="members-filter"]`,
        'label'
      );

      // has the right operators
      const operatorOptions = findAll(
        `${filterSelector} [data-test-select="members-filter-operator"] option`
      );
      ...
      expect(
        findAll(`${filterSelector} [data-test-label-filter]`).length,
        '# of label options'
      ).to.equal(5);

      // selecting a value updates table
      await selectChoose(
        `${filterSelector} .gh-member-label-input`,
        label.name
      );

      expect(
        findAll('[data-test-list="members-list-item"]').length,
        `# of filtered member rows - ${label.name}`
      ).to.equal(3);
      ...
      expect(find('[data-test-table-data="label"]')).to.contain.text(
        label.name
      );
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows after delete'
      ).to.equal(7);
    });
    ...
    it('can filter by tier', async function () {
      ...
      expect(
        findAll(`${filterSelector} [data-test-tiers-segment]`).length,
        '# of label options'
      ).to.equal(5);
    ...
    it('can filter by offer redeemed', async function () {
      ...
      // create 3 offers
      const offer = this.server.create('offer', {
        tier: { id: tier.id },
        createdAt: moment.utc().subtract(1, 'day').valueOf(),
      });
      this.server.create('offer', {
        tier: { id: tier.id },
        createdAt: moment.utc().subtract(2, 'day').valueOf(),
      });
      this.server.create('offer', {
        tier: { id: tier.id },
        createdAt: moment.utc().subtract(3, 'day').valueOf(),
      });
      this.server.createList('member', 3, {
        status: 'paid',
        tiers: [tier],
      });
      const sub = this.server.create('subscription', {
        member: this.server.schema.members.first(),
        tier: tier,
        offer: offer,
      });
      ...
      expect(
        findAll(`${filterSelector} [data-test-offers-segment]`).length,
        '# of label options'
      ).to.equal(3);
      ...
      await visit(
        '/members?filter=' +
          encodeURIComponent(`offer_redemptions:'${offer.id}'`)
      ); // ensure that the id is parsed as a string and not an integer

      // only one redeemed offer so only 1 member should be shown
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows'
      ).to.equal(1);
    });
    ...
    it('can filter by specific newsletter subscription', async function () {
      ...
      const newslettersCount =
        this.server.schema.newsletters.all().models.length;
      let options = this.element.querySelectorAll('option');
      let matchingOptions = [...options].filter((option) =>
        option.value.includes('newsletters.slug')
      );
      ...
      await visit(
        '/members?filter=' +
          encodeURIComponent(`newsletters.slug:${newsletter.slug}`)
      );
    ...
    it('can filter by newsletter subscription', async function () {
      ...
      const valueOptions = findAll(
        `${filterSelector} [data-test-select="members-filter-value"] option`
      );
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - true'
      ).to.equal(3);

      // can change filter
      await fillIn(
        `${filterSelector} [data-test-select="members-filter-value"]`,
        'false'
      );
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - false'
      ).to.equal(4);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - true - from URL'
      ).to.equal(3);
      await click('[data-test-button="members-filter-actions"]');
      expect(
        find(`${filterSelector} [data-test-select="members-filter-value"]`)
      ).to.have.value('true');
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - false - from URL'
      ).to.equal(4);
      await click('[data-test-button="members-filter-actions"]');
      expect(
        find(`${filterSelector} [data-test-select="members-filter-value"]`)
    ...
    it('can filter by member status', async function () {
      ...
      expect(
        find(
          `${filterSelector} [data-test-select="members-filter"] option[value="status"]`
        ),
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - comped'
      ).to.equal(2);
      expect(find('[data-test-table-column="status"]')).to.exist;
    ...
    it('can filter by billing period', async function () {
      // add some members to filter
      this.server.createList('member', 3).forEach((member) =>
        this.server.create('subscription', {
          member,
          planInterval: 'month',
        })
      );
      this.server.createList('member', 4).forEach((member) =>
        this.server.create('subscription', {
          member,
          planInterval: 'year',
        })
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - year'
      ).to.equal(4);
      expect(find('[data-test-table-column="subscriptions.plan_interval"]')).to
        .exist;
    ...
    it('can filter by stripe subscription status', async function () {
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - active'
      ).to.equal(3);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - trialing'
      ).to.equal(4);
    ...
    it('can filter by emails opened', async function () {
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - default'
      ).to.equal(7);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - after blur'
      ).to.equal(7);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - input 5'
      ).to.equal(3);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - cleared'
      ).to.equal(7);
    ...
    it('can filter by open rate', async function () {
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - value 50'
      ).to.equal(3);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - operator is-greater'
      ).to.equal(4);

      // it does not add duplicate column
      expect(find('[data-test-table-column="email_open_rate"]')).to.exist;
      expect(
        findAll('[data-test-table-column="email_open_rate"]').length
      ).to.equal(1);
    ...
    it('can filter by last seen date', async function () {
      ...
      // add some members to filter
      this.server.createList('member', 3, {
        lastSeenAt: moment('2022-02-01 11:00:00').format('YYYY-MM-DD HH:mm:ss'),
      });
      this.server.createList('member', 4, {
        lastSeenAt: moment('2022-02-05 11:00:00').format('YYYY-MM-DD HH:mm:ss'),
      });
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - is before 2022-02-05'
      ).to.equal(3);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - is after 2022-02-01'
      ).to.equal(4);
      ...
      await datepickerSelect(
        valueDatePicker,
        moment.utc('2022-01-01').toDate()
      );
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - is after 2022-01-01'
      ).to.equal(7);

      // table shows last seen column+data
      expect(find('[data-test-table-column="last_seen_at"]')).to.exist;
      expect(findAll('[data-test-table-data="last_seen_at"]').length).to.equal(
        7
      );
      expect(
        find('[data-test-table-data="last_seen_at"]')
      ).to.contain.trimmed.text('1 Feb 2022');
      expect(
        find('[data-test-table-data="last_seen_at"]')
      ).to.contain.trimmed.text('4 days ago');
    });
    ...
    it('can filter by created at date', async function () {
      ...
      // add some members to filter
      this.server.createList('member', 3, {
        createdAt: moment('2022-02-01 12:00:00').format('YYYY-MM-DD HH:mm:ss'),
      });
      this.server.createList('member', 4, {
        createdAt: moment('2022-02-05 12:00:00').format('YYYY-MM-DD HH:mm:ss'),
      });
      ...
      expect(
        find(
          `${filterSelect} [data-test-select="members-filter"] option[value="created_at"]`
        ),
        'created_at filter option'
      ).to.exist;
      ...
      await datepickerSelect(
        valueDatePicker,
        moment.utc('2022-02-03').toDate()
      );
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - 2022-02-03'
      ).to.equal(3);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - is-greater'
      ).to.equal(4);
      ...
      expect(find(typeSelect), 'type select - from URL').to.have.value(
        'created_at'
      );
      expect(find(operatorSelect), 'operator select - from URL').to.have.value(
        'is-or-less'
      );
      expect(find(valueDateInput), 'date input - from URL').to.have.value(
        '2022-02-01'
      );

      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - from URL'
      ).to.equal(3);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - is-or-greater after URL change'
      ).to.equal(7);
    ...
    it('uses site timezone when filtering by date', async function () {
      ...
      this.server.createList('member', 3, {
        createdAt: moment
          .utc('2022-02-22 04:00:00.000Z')
          .format('YYYY-MM-DD HH:mm:ss'),
      });
      // 2022-02-22 signups
      this.server.createList('member', 4, {
        createdAt: moment
          .utc('2022-02-22 05:00:00.000Z')
          .format('YYYY-MM-DD HH:mm:ss'),
      });
      ...
      const createdAtFields = findAll(
        '[data-test-list="members-list-item"] [data-test-table-data="created-at"]'
      );
      expect(
        createdAtFields.filter((el) => el.textContent.match(/21 Feb 2022/))
          .length
      ).to.equal(3);
      expect(
        createdAtFields.filter((el) => el.textContent.match(/22 Feb 2022/))
          .length
      ).to.equal(4);
      ...
      const filterQuery = encodeURIComponent(
        `created_at:<='2022-02-22 04:59:59'`
      );
      await visit(`/members?filter=${filterQuery}`);

      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of member rows - post URL parse'
      ).to.equal(3);
    ...
    it('can filter by paid subscription start date', async function () {
      ...
      this.server.createList('member', 3).forEach((member) =>
        this.server.create('subscription', {
          member,
          startDate: moment('2022-02-01 12:00:00').format(
            'YYYY-MM-DD HH:mm:ss'
          ),
        })
      );
      this.server.createList('member', 4).forEach((member) =>
        this.server.create('subscription', {
          member,
          startDate: moment('2022-02-05 12:00:00').format(
            'YYYY-MM-DD HH:mm:ss'
          ),
        })
      );
      ...
      expect(
        find(
          `${filterSelect} [data-test-select="members-filter"] option[value="subscriptions.start_date"]`
        ),
        'subscriptions.start_date filter option'
      ).to.exist;
      ...
      expect(find('[data-test-table-column="subscriptions.start_date"]')).to
        .exist;
      expect(
        find('[data-test-table-column="subscriptions.start_date"]')
      ).to.contain.text('Paid start date');
      expect(
        findAll('[data-test-table-data="subscriptions.start_date"]').length
      ).to.equal(3);
      expect(
        find('[data-test-table-data="subscriptions.start_date"]')
      ).to.contain.text('1 Feb 2022');
      expect(
        find('[data-test-table-data="subscriptions.start_date"]')
      ).to.contain.text('a month ago');
    });
    ...
    it('can filter by name', async function () {
      ...
      expect(
        find(
          `${filterSelect} [data-test-select="members-filter"] option[value="name"]`
        ),
        'name filter option'
      ).to.exist;
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - is "hello"'
      ).to.equal(1);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - contains "hello"'
      ).to.equal(1);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - contains "test"'
      ).to.equal(2);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - starts with "tset"'
      ).to.equal(3);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - ends with "2"'
      ).to.equal(2);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - does not contain "2"'
      ).to.equal(6);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - contains "O\'Nolan"'
      ).to.equal(1);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - from URL contains "hello"'
      ).to.equal(1);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - from URL starts with "tset"'
      ).to.equal(3);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - from URL ends with "2"'
      ).to.equal(2);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - from URL does not contain "2"'
      ).to.equal(6);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - from URL contains "O\'Nolan"'
      ).to.equal(1);
      expect(find(operatorSelect)).to.have.value('contains');
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - from URL contains "test+test"'
      ).to.equal(0);
      expect(find(operatorSelect)).to.have.value('contains');
    ...
    it('can filter by email', async function () {
      ...
      expect(
        find(
          `${filterSelect} [data-test-select="members-filter"] option[value="email"]`
        ),
        'email filter option'
      ).to.exist;
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - is "hello@hi.com"'
      ).to.equal(1);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - starts with "test-2"'
      ).to.equal(2);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - ends with ".com"'
      ).to.equal(6);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - does not contain ".com"'
      ).to.equal(1);
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of filtered member rows - contains "with+plus"'
      ).to.equal(1);
    ...
    it('can filter by next billing date', async function () {
      ...
      this.server.createList('member', 3).forEach((member) =>
        this.server.create('subscription', {
          member,
          currentPeriodEnd: moment('2022-02-01 12:00:00').format(
            'YYYY-MM-DD HH:mm:ss'
          ),
        })
      );
      this.server.createList('member', 4).forEach((member) =>
        this.server.create('subscription', {
          member,
          currentPeriodEnd: moment('2022-02-05 12:00:00').format(
            'YYYY-MM-DD HH:mm:ss'
          ),
        })
      );
      ...
      expect(
        find(
          `${filterSelect} [data-test-select="members-filter"] option[value="subscriptions.current_period_end"]`
        ),
        'subscriptions.current_period_end filter option'
      ).to.exist;
      ...
      await visit(`/`);
      const filter = encodeURIComponent(
        `subscriptions.current_period_end:<='2022-02-01 23:59:59'`
      );
      await visit(`/members?filter=${filter}`);
      await click('[data-test-button="members-filter-actions"]');
      ...
      // it adds extra column to table
      expect(
        find('[data-test-table-column="subscriptions.current_period_end"]')
      ).to.exist;
      expect(
        find('[data-test-table-column="subscriptions.current_period_end"]')
      ).to.contain.text('Next billing date');
      expect(
        findAll('[data-test-table-data="subscriptions.current_period_end"]')
          .length
      ).to.equal(3);
      expect(
        find('[data-test-table-data="subscriptions.current_period_end"]')
      ).to.contain.text('1 Feb 2022');
      expect(
        find('[data-test-table-data="subscriptions.current_period_end"]')
      ).to.contain.text('a month ago');
    ...
    it('can handle multiple filters', async function () {
      // add some members to filter
      this.server.createList('member', 1).forEach((member) =>
        this.server.create('subscription', {
          member,
          status: 'active',
        })
      );
      this.server.createList('member', 2).forEach((member) =>
        this.server.create('subscription', {
          member,
          status: 'trialing',
        })
      ...
      this.server
        .createList('member', 3, { emailOpenRate: 50 })
        .forEach((member) =>
          this.server.create('subscription', {
            member,
            status: 'trialing',
      ...
      await fillIn(
        '[data-test-members-filter="0"] [data-test-select="members-filter"]',
        'email_open_rate'
      );
      await fillIn(
        '[data-test-members-filter="0"] [data-test-input="members-filter-value"]',
        '50'
      );
      await blur(
        '[data-test-members-filter="0"] [data-test-input="members-filter-value"]'
      ...
      await fillIn(
        `[data-test-members-filter="1"] [data-test-select="members-filter"]`,
        'subscriptions.status'
      );
      await fillIn(
        `[data-test-members-filter="1"] [data-test-select="members-filter-value"]`,
        'trialing'
      );

      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of members rows after filter'
      ).to.equal(3);
      ...
      expect(
        find('[data-test-button="members-filter-actions"] span'),
        'filter button'
      ).to.have.class('gh-btn-label-green');
      expect(
        find('[data-test-button="members-filter-actions"]'),
        'filter button'
      ).to.contain.text('(2)');

      // search is inactive
      expect(find('[data-test-input="members-search"]'), 'search input').to
        .exist;
      expect(
        find('[data-test-input="members-search"]'),
        'search input'
    ...
    it('has a no-match state', async function () {
      this.server.createList('member', 5).forEach((member) =>
        this.server.create('subscription', {
          member,
          status: 'active',
        })
      ...
      // export is disabled
      expect(find('[data-test-button="export-members"]')).to.have.attribute(
        'disabled'
      );
      ...
      await click(
        '[data-test-no-matching-members] [data-test-button="show-all-members"]'
      );
    ...
    it('resets filter operator when changing filter type', async function () {
      ...
      await fillIn(
        `${filter} [data-test-select="members-filter"]`,
        'email_count'
      );
      await fillIn(
        `${filter} [data-test-select="members-filter-operator"]`,
        'is-greater'
      );
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of members after email_count filter'
      ).to.equal(4);
      ...
      expect(
        find(`${filter} [data-test-select="members-filter-operator"]`)
      ).to.have.value('is');
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of members after email_count filter'
    ...
    it("hides paid filters when stripe isn't connected", async function () {
      ...
      expect(
        find(
          '[data-test-members-filter="0"] [data-test-select="members-filter"] optgroup[label="Subscription"]'
        ),
        "Subscription option group doesn't exist"
      ).to.not.exist;

      const filterOptions = findAll(
        '[data-test-members-filter="0"] [data-test-select="members-filter"] option'
    ...
    it('hides email filters when email is disabled', async function () {
      ...
      expect(
        find(
          '[data-test-members-filter="0"] [data-test-select="members-filter"] optgroup[label="Email"]'
        ),
  ...
  describe('search', function () {
    ...
    it('works', async function () {
      ...
      expect(find('[data-test-input="members-search"]')).to.have.class(
        'active'
      );
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of members after clearing search'
      ).to.equal(3);

      expect(find('[data-test-input="members-search"]')).to.not.have.class(
        'active'
      );
    });
    ...
    it('has a no-match state', async function () {
      ...
      expect(findAll('[data-test-list="members-list-item"]').length).to.equal(
        3
      );
    });
    ...
    it('can search + filter', async function () {
      this.server.create('member', {
        ...
        email: 'a@aaa.aaa',
        subscriptions: [
          this.server.create('subscription', { status: 'active' }),
        ],
      });
      ...
      await fillIn(
        '[data-test-members-filter="0"] [data-test-select="members-filter-value"]',
        'active'
      );
      await click('[data-test-button="members-apply-filter"]');
      ...
      expect(
        findAll('[data-test-list="members-list-item"]').length,
        '# of member rows after filter+search'
      ).to.equal(1);
