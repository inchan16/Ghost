...
describe('Acceptance: Member details', function () {
  ...
  it('has a known base-state', async function () {
    ...
    expect(
      findAll('[data-test-subscription]').length,
      'displays all member subscriptions'
    ).to.equal(2);
  ...
  it('can add and remove complimentary subscription', async function () {
    ...
    expect(
      findAll('[data-test-button="add-complimentary"]').length,
      '# of add complimentary buttons'
    ).to.equal(1);

    await click('[data-test-button="add-complimentary"]');
    expect(find('[data-test-modal="member-tier"]'), 'select tier modal').to
      .exist;
    expect(find('[data-test-text="select-tier-desc"]')).to.contain.text(
      'Comp Member Test'
    );
    expect(find('[data-test-tier-option="6213b3f6cb39ebdb03ebd810"]')).to.have
      .exist;
    expect(
      find('[data-test-tier-option="6213b3f6cb39ebdb03ebd810"]')
    ).to.have.class('active');
    await click('[data-test-button="save-comp-tier"]');

    expect(
      findAll('[data-test-subscription]').length,
      '# of subscription blocks - after add comped'
    ).to.equal(1);

    await click(
      '[data-test-tier="6213b3f6cb39ebdb03ebd810"] [data-test-button="subscription-actions"]'
    );
    await click(
      '[data-test-tier="6213b3f6cb39ebdb03ebd810"] [data-test-button="remove-complimentary"]'
    );

    expect(
      findAll('[data-test-subscription]').length,
      '# of subscription blocks - after remove comped'
    ).to.equal(0);
  });
  ...
  it('can add complimentary subscription when member has canceled subscriptions', async function () {
    ...
    expect(
      findAll('[data-test-button="add-complimentary"]').length,
      '# of add complimentary buttons - after add comped'
    ).to.equal(0);
  });
  ...
  it('handles multiple tiers', async function () {
    ...
    this.server.create('subscription', {
      ...
      tier,
      status: 'canceled',
      price: { id: '1', tier: { tier_id: tier.id } },
    });
    this.server.create('subscription', {
      member,
      tier,
    ...
    this.server.create('subscription', {
      ...
      tier: tier2,
      status: 'canceled',
      price: { id: '1', tier: { tier_id: tier2.id } },
    });
    ...
    expect(find(`${p1} [data-test-text="tier-name"]`)).to.contain.text(
      'Ghost Subscription'
    );
    expect(
      findAll(`${p1} [data-test-subscription]`).length,
      '# of tier 1 subscription blocks'
    ).to.equal(2);

    expect(find(`${p2} [data-test-text="tier-name"]`)).to.contain.text(
      'Second tier'
    );
    expect(
      findAll(`${p2} [data-test-subscription]`).length,
      '# of tier 2 subscription blocks'
    ).to.equal(1);
    ...
    expect(
      findAll(`${p2} [data-test-subscription]`).length,
      '# of tier 2 subscription blocks after comp added'
    ).to.equal(2);
  });
});
