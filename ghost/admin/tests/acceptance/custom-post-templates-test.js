...
describe('Acceptance: Custom Post Templates', function () {
  ...
  describe('with custom templates', function () {
    ...
    it('can change selected template', async function () {
      ...
      expect(select.value, 'selected value').to.equal(
        'custom-news-bulletin.hbs'
      );
      ...
      expect(select.options.item(1).value, 'first custom value').to.equal(
        'custom-big-images.hbs'
      );
      expect(select.options.item(1).text, 'first custom text').to.equal(
        'Big Images'
      );
      expect(select.options.item(2).value, 'second custom value').to.equal(
        'custom-news-bulletin.hbs'
      );
      expect(select.options.item(2).text, 'second custom text').to.equal(
        'News Bulletin'
      );
