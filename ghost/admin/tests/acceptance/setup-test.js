...
describe('Acceptance: Setup', function () {
  ...
  describe('with a new blog', function () {
    ...
    it('has a successful happy path', async function () {
      ...
      expect(
        findAll('[data-test-blog-title-input]')[0] === document.activeElement,
        'blog title has focus'
      ).to.be.true;
      ...
      expect(
        ...
        'number of in-line validation messages'
      ).to.equal(4);

      // it displays main error
      expect(findAll('.main-error').length, 'main error is displayed').to.equal(
        1
      );
      ...
      expect(currentURL(), 'url after submitting account details').to.equal(
        '/setup/done'
      );
    });
    ...
    it('handles validation errors in setup', async function () {
      ...
      expect(find('.main-error').textContent.trim(), 'error text').to.not.be
        .empty;
      ...
      expect(find('.main-error').textContent.trim(), 'error text').to.equal(
      ...
      expect(
        findAll('.main-error').length,
        'main error is not displayed'
      ).to.equal(0);

      expect(findAll('.gh-alert-red').length, 'number of alerts').to.equal(1);
    });
    ...
    it('handles invalid origin error on setup', async function () {
            ...
            errors: [
              {
                ...
                message:
                  'Access Denied from url: unknown.com. Please use the url configured in config.js.',
              },
            ],
          }
        );
      ...
      expect(
        findAll('.gh-btn-signup .spinner').length,
        'button has spinner'
      ).to.equal(0);
      // we should show an error message
      expect(find('.main-error').textContent, 'error text').to.have.string(
        'Access Denied from url: unknown.com. Please use the url configured in config.js.'
      );
    });
  });
