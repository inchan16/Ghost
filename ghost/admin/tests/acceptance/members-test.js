...
describe('Acceptance: Members', function () {
  ...
  it('redirects non-admins to site', async function () {
    ...
    expect(find('[data-test-nav="members"]'), 'sidebar link').to.not.exist;
  });
  ...
  describe('as owner', function () {
    ...
    it('it renders, can be navigated, can edit member', async function () {
      ...
      expect(
        member.querySelector('.gh-members-list-name').textContent,
        'member list item title'
      ).to.equal(member1.name);

      // it does not add ?include=email_recipients
      const membersRequests = this.server.pretender.handledRequests.filter(
        (r) => r.url.match(/\/members\/(\?|$)/)
      );
      ...
      expect(
        find('[data-test-input="member-name"]').value,
        'loads correct member into form'
      ).to.equal(member1.name);

      expect(
        find('[data-test-input="member-email"]').value,
        'loads correct email into form'
    ...
    it('can create a new member', async function () {
      ...
      expect(
        find('.gh-canvas-header h2').textContent,
        'settings pane title'
      ).to.contain('New');
      ...
      findAll('.gh-member-settings-primary .gh-input').forEach(function (elem) {
        expect(
          elem.value,
          `input field for ${elem.getAttribute('name')}`
        ).to.be.empty;
      });
    ...
    it('can bulk delete members', async function () {
      ...
      expect(exportQueryParams).to.deep.equal({
        filter: 'label:[label-0]',
        limit: 'all',
      });

      expect(find('[data-test-text="deleted-count"]')).to.have.text(
        '5 members'
      );
    ...
    it('can delete a member (via list)', async function () {
      ...
      await click(
        '[data-test-modal="delete-member"] [data-test-button="cancel"]'
      );
      ...
      await click('[data-test-button="delete-member"]');
      await click(
        '[data-test-modal="delete-member"] [data-test-button="confirm"]'
      );
    ...
    it('can delete a member (via url)', async function () {
      const newsletter = this.server.create('newsletter');
      const label = this.server.create('label');
      const [memberOne] = this.server.createList('member', 2, {
        newsletters: [newsletter],
        labels: [label],
      });
