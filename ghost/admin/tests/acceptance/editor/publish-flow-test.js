...
describe('Acceptance: Publish flow', function () {
  ...
  it('has minimal features for contributors', async function () {
    ...
    expect(find('[data-test-button="publish-flow"]'), 'publish button').to.not
      .exist;

    expect(
      find('[data-test-button="contributor-preview"]'),
      'contributor preview button'
    ).to.exist;
    expect(
      find('[data-test-button="contributor-save"]'),
    ...
    expect(post.title, 'post title after save').to.equal(
      'Contributor save test'
    );
  });
  ...
  it('triggers post validation before opening', async function () {
    ...
    expect(find('[data-test-modal="publish-flow"]'), 'publish flow modal').to
      .not.exist;
  });
  ...
  async function testEmailUnavailableFlow() {
    ...
    expect(find('[data-test-setting="publish-type"]'), 'publish type setting')
      .to.exist;
    expect(
      find('[data-test-setting="publish-type"] [data-test-setting-title]'),
      'publish type title'
    ...
    expect(find('[data-test-setting="publish-at"]'), 'publish time setting').to
      .exist;
    expect(
      find('[data-test-setting="publish-at"] [data-test-setting-title]'),
      'publish time title'
    ...
    expect(
      find('[data-test-text="confirm-details"]'),
      'confirmation text'
    ).to.have.rendered.text('Your post will be published on your site.');

    expect(
      find('[data-test-button="confirm-publish"]'),
      'publish button text'
    ...
    expect(find('[data-test-publish-flow="complete"]'), 'complete step').to
      .exist;
    expect(
      find('[data-test-complete-title]'),
      'complete title'
    ).to.have.rendered.text(
      'Boom. It’s out there. That’s 1 post published, keep going!'
    );
    expect(find('[data-test-complete-bookmark]'), 'bookmark card').to.exist;

    // "revert to draft" only shown for scheduled posts
    expect(
      find('[data-test-button="revert-to-draft"]'),
      'revert-to-draft button'
    ).to.not.exist;

    // publish/preview buttons are hidden on complete step
    expect(
      find('[data-test-button="publish-flow-preview"]'),
      'preview button on complete step'
    ).to.not.exist;
    expect(
      find('[data-test-button="publish-flow-publish"]'),
      'publish button on complete step'
    ).to.not.exist;

    await click('[data-test-button="back-to-editor"]');

    expect(
      find('[data-test-button="publish-flow"]'),
      'publish button after publishing'
    ).to.not.exist;
    expect(
      find('[data-test-button="update-flow"]'),
      'update button after publishing'
    ).to.exist;

    await click('[data-test-button="update-flow"]');

    expect(find('[data-test-modal="update-flow"]'), 'update flow modal').to
      .exist;
    expect(find('[data-test-update-flow-title]')).to.have.rendered.text(
      'This post has been published'
    );
    expect(
      find('[data-test-update-flow-confirmation]')
    ).to.contain.rendered.text('Your post was published on your site');
    const savedPublishAt = moment(post.publishedAt).utc();
    expect(
      find('[data-test-update-flow-confirmation]')
    ).to.contain.rendered.text(
      `on ${savedPublishAt.format('D MMM YYYY')} at ${savedPublishAt.format(
        'HH:mm'
      )}`
    );
    ...
    expect(
      find('[data-test-button="revert-to-draft"]')
    ).to.contain.rendered.text('Unpublish and revert to private draft');
  ...
  describe('members enabled', function () {
    ...
    it('can publish+send with single newsletter', async function () {
      ...
      expect(
        find(
          '[data-test-setting="email-recipients"] [data-test-setting-title]'
        ),
        'publish type title'
      ...
      expect(find('[data-test-select="newsletter"]'), 'newsletter select').to
        .not.exist;
      ...
      expect(specificCheckbox.checked, 'specific people checkbox checked').to.be
        .true;
      ...
      await removeMultipleOption(
        '[data-test-select="specific-members"]',
        'Label 0'
      );
      expect(specificCheckbox.checked, 'specific people checkbox checked').to.be
        .true;
      ...
      expect(
        ...
        'will be published on your site, and delivered to all 7 subscribers.'
      );

      expect(
        find('[data-test-button="confirm-publish"]')
      ).to.have.rendered.text('Publish & send, right now');

      await click('[data-test-button="confirm-publish"]');

      // complete text has right count
      expect(find('[data-test-complete-title]')).to.contain.rendered.text(
    ...
    it('can publish+send with multiple newsletters', async function () {
      ...
      expect(
        ...
        'publish type title'
      ).to.have.trimmed.rendered.text(
        'All 8 subscribers of Default newsletter'
      );
      ...
      expect(
        findAll('.ember-power-select-dropdown [data-test-select-option]').length
      ).to.equal(2);

      // selecting a different newsletter updates recipient count
      await selectChoose(
        '[data-test-select="newsletter"]',
        'Second newsletter'
      );
      expect(
        find(
      ...
      expect(
        ...
        'will be published on your site, and delivered to all 1 subscriber of Second newsletter.'
      );
    ...
    it('can schedule publish+send', async function () {
      ...
      expect(
        find('[data-test-setting="publish-at"] [data-test-setting-title]')
      ).to.have.rendered.text('Right now');
      ...
      expect(
        find(
          '[data-test-setting="publish-at"] [data-test-date-time-picker-datepicker]'
        ),
        'datepicker'
      ).to.exist;
      expect(
      ...
      ).to.have.value(newDate.format('HH:mm'));

      expect(
        find('[data-test-setting="publish-at"] [data-test-setting-title]'),
        'publish-at title after change'
      ).to.have.rendered.text('In 4 days');

      await click('[data-test-button="continue"]');

      // has correct confirm text
      expect(find('[data-test-text="confirm-details"]')).to.have.rendered.text(
        `On ${newDate.format('D MMM YYYY')} at ${newDate.format(
          'HH:mm'
        )} your post will be published on your site, and delivered to all 7 subscribers.`
      );

      expect(
        find('[data-test-button="confirm-publish"]')
      ).to.have.rendered.text(
        `Publish & send, on ${newDate.format('MMMM Do')}`
      );
      ...
      expect(moment.utc(post.publishedAt).format('YYYY-MM-DD HH:mm')).to.equal(
        moment(newDate).utc().format('YYYY-MM-DD HH:mm')
      );
    ...
    it('can send', async function () {
      ...
      await click(
        '[data-test-setting="publish-type"] [data-test-setting-title]'
      );
      await click('[data-test-publish-type="send"]');

      expect(
        find('[data-test-setting="publish-type"] [data-test-setting-title]')
      ).to.have.rendered.text('Email');

      await click('[data-test-button="continue"]');

      // has correct confirm text
      expect(find('[data-test-text="confirm-details"]')).to.have.rendered.text(
        `Your post will be delivered to all 7 subscribers, and will not be published on your site.`
      );

      expect(
        find('[data-test-button="confirm-publish"]')
      ).to.have.rendered.text(`Send email, right now`);
    ...
    it('can schedule send', async function () {
      ...
      await click('[data-test-setting="publish-at"] [data-test-setting-title]');
      await click(
        '[data-test-setting="publish-at"] [data-test-radio="schedule"]'
      );
      ...
      expect(
        find('[data-test-button="confirm-publish"]')
      ).to.have.rendered.text(`Send email, on ${newDate.format('MMMM Do')}`);
    ...
    it('respects default recipient settings - usually nobody', async function () {
      ...
      expect(
        find('[data-test-publish-type="publish+send"]')
      ).to.not.have.attribute('disabled');
      expect(find('[data-test-publish-type="send"]')).to.not.have.attribute(
        'disabled'
      );
    ...
    it('handles Mailgun not being set up', async function () {
      ...
      expect(find('[data-test-publish-type-error]'), 'publish type error').to
        .exist;
      expect(
        find('[data-test-publish-type-error="no-mailgun"]'),
        'publish type error text'
      ).to.exist;

      // email-related options are disabled
      expect(find('[data-test-publish-type="publish+send"]')).to.have.attribute(
        'disabled'
      );
      expect(find('[data-test-publish-type="send"]')).to.have.attribute(
        'disabled'
      );
    });
    ...
    it('handles no members present', async function () {
      ...
      expect(find('[data-test-publish-type-error]'), 'publish type error').to
        .exist;
      expect(
        find('[data-test-publish-type-error="no-members"]'),
        'publish type error text'
      ).to.exist;
    ...
    it('handles over-member limit before publish', async function () {
      ...
      config.hostSettings = {
        limits: {
          members: {
            max: 9,
            error:
              'Your plan supports up to {{max}} members. Please upgrade to reenable publishing.',
          },
        },
      };
      ...
      expect(find('[data-test-publish-type-error]'), 'publish disabled error')
        .to.exist;
      expect(
        find('[data-test-publish-type-error="publish-disabled"]'),
        'publish disabled error'
      ).to.have.trimmed.text(
        'Your plan supports up to 9 members. Please upgrade to reenable publishing.'
      );

      expect(find('[data-test-button="continue"]'), 'continue button').to.not
        .exist;
    });
          ...
          errors: [
            {
              ...
              context:
                'Your plan supports up to 1,000 members. Please upgrade to reenable publishing.',
              type: 'HostLimitError',
              details: {
                name: 'members',
                limit: 1000,
