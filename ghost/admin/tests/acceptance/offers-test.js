...
describe('Acceptance: Offers', function () {
  ...
  it('redirects non-admins to site', async function () {
    ...
    expect(find('[data-test-nav="offers"]'), 'sidebar link').to.not.exist;
  });
  ...
  describe('as owner', function () {
    ...
    it('it renders, can be navigated, can edit offer', async function () {
      ...
      // it lists all offers
      expect(
        findAll('[data-test-list="offers-list-item"]').length,
        'offers list count'
      ).to.equal(2);

      let offer = find('[data-test-list="offers-list-item"]');
      expect(
        offer.querySelector('[data-test-list="offer-name"] h3').textContent,
      ...
      expect(
        find('[data-test-input="offer-name"]').value,
        'loads correct offer into form'
      ).to.equal(offer1.name);
