...
describe('Acceptance: Signup', function () {
  ...
  it('can signup successfully', async function () {
    ...
    server.post(
      ...
      function ({ users }, { requestBody }) {
        ...
        expect(params.invitation[0].token).to.equal(
          'MTQ3MDM0NjAxNzkyOXxrZXZpbit0ZXN0MkBnaG9zdC5vcmd8MmNEblFjM2c3ZlFUajluTks0aUdQU0dmdm9ta0xkWGY2OEZ1V2dTNjZVZz0'
        );
    ...
    await visit(
      '/signup/MTQ3MDM0NjAxNzkyOXxrZXZpbit0ZXN0MkBnaG9zdC5vcmd8MmNEblFjM2c3ZlFUajluTks0aUdQU0dmdm9ta0xkWGY2OEZ1V2dTNjZVZz0'
    );
  ...
  it('redirects if already logged in', async function () {
    ...
    expect(currentRouteName()).to.equal('site');
    expect(find('.gh-alert-content').textContent).to.have.string(
