  ...
  describe('when logged in as admin', function () {
    ...
    beforeEach(async function () {
      ...
      this.server.create('invite', {
        expires: moment.utc().subtract(1, 'day').valueOf(),
        role: adminRole,
      });
    ...
    it('it renders and navigates correctly', async function () {
      ...
      expect(find(`[data-test-active-users] [data-test-user-id="${user1.id}"]`))
        .to.exist;
      expect(find(`[data-test-active-users] [data-test-user-id="${user2.id}"]`))
        .to.exist;
      expect(find(`[data-test-active-users] [data-test-user-id="${admin.id}"]`))
        .to.exist;
      ...
      expect(
        find(
          `[data-test-suspended-users] [data-test-user-id="${suspendedUser.id}"]`
        )
      ).to.exist;
      ...
      // url is correct
      expect(currentURL(), 'url after clicking user').to.equal(
        `/settings/staff/${user2.slug}`
      );

      // title is correct
      expect(document.title, 'title after clicking user').to.equal(
        'Staff - User - Test Blog'
      );

      // view title should exist and be linkable and active
      expect(
        find(
          '[data-test-breadcrumb] a[href="/ghost/settings/staff"]'
    ...
    it('can manage invites', async function () {
      ...
      expect(find('.view-actions .gh-btn-primary'), 'invite people button').to
        .exist;
      ...
      expect(
        find(
          '[data-test-user-id="1"] [data-test-role-name]'
        ).textContent.trim(),
      ...
      expect(
        find('[data-test-invite-id="1"] [data-test-invite-description]')
          .textContent,
      ...
      await fillIn(
        '.fullscreen-modal input[name="email"]',
        'invite1@example.com'
      );
      ...
      expect(
        find(
          '[data-test-invite-id="2"] [data-test-role-name]'
        ).textContent.trim(),
        'displayed role of first invite'
      ).to.equal('Contributor');

      expect(
        find('[data-test-invite-id="2"] [data-test-invite-description]')
          .textContent,
      ...
      expect(
        find(
          '[data-test-invite-id="3"] [data-test-role-name]'
        ).textContent.trim(),
      ...
      expect(
        find(
          '[data-test-invite-id]:last-of-type [data-test-email]'
        ).textContent.trim(),
      ...
      expect(
        find(
          '[data-test-invite-id]:first-of-type [data-test-email]'
        ).textContent.trim(),
      ...
      await click(
        '[data-test-invite-id]:first-of-type [data-test-resend-button]'
      );
      await click(
        '[data-test-invite-id]:first-of-type [data-test-revoke-button]'
      );
    ...
    it('can manage suspended users', async function () {
      ...
      expect(
        find(
          `[data-test-active-users] [data-test-user-id="${suspendedUser.id}"]`
        )
      ).to.exist;
    ...
    describe('existing user', function () {
      ...
      it('input fields reset and validate correctly', async function () {
        ...
        expect(
          find('[data-test-name-input]').value,
          'current user name'
        ).to.equal('Test User');
        ...
        expect(
          find('.user-details-bottom .first-form-group').classList.contains(
            'error'
          ),
          'username input is in error state with blank input'
        ).to.be.true;
        ...
        expect(
          ...
          'username input is in error state with too long input'
        ).to.be.true;
        ...
        expect(
          find('[data-test-slug-input]').value,
          'slug value is default'
        ).to.equal('test-1');
        ...
        expect(
          find('[data-test-slug-input]').value,
          'slug value is reset to original upon empty string'
        ).to.equal('test-1');
        ...
        expect(
          find('[data-test-slug-input]').value,
          'slug value is correctly dasherized'
        ).to.equal('white-space');
        ...
        expect(
          find(
            '.user-details-bottom .form-group:nth-of-type(3)'
          ).classList.contains('error'),
          'email input should be in error state with invalid email'
        ).to.be.true;
        ...
        let testSocialInput = async function (
          type,
          input,
          expectedValue,
          expectedError = ''
          ...
          expect(
            find(`[data-test-error="user-${type}"]`)
              .closest('.form-group')
              .classList.contains('error'),
        ...
        let testFacebookValidation = async (...args) =>
          testSocialInput('facebook', ...args);
        let testTwitterValidation = async (...args) =>
          testSocialInput('twitter', ...args);

        // Testing Facebook input

        // displays initial value
        expect(
          find('[data-test-facebook-input]').value,
          'initial facebook value'
        ...
        expect(
          find('[data-test-facebook-input]').value,
          'facebook value after blur with no change'
        ).to.equal('https://www.facebook.com/test');
        ...
        expect(
          find('[data-test-twitter-input]').value,
          'initial twitter value'
        ...
        expect(
          find('[data-test-twitter-input]').value,
          'twitter value after blur with no change'
        ).to.equal('https://twitter.com/test');
        ...
        expect(newRequest.url, 'password request URL').to.match(
          /\/users\/password/
        );
      ...
      it('warns when leaving without saving', async function () {
        ...
        expect(find('[data-test-slug-input]').value).to.be.equal(
          'another-slug'
        );
        ...
        expect(find('[data-test-facebook-input]').value).to.be.equal(
          'https://www.facebook.com/testuser'
        );
        ...
        await click(
          '[data-test-modal="unsaved-settings"] [data-test-leave-button]'
        );
      ...
      it('cannot see email alerts for other user', async function () {
        await visit('/settings/staff/test-1');
        expect(
          find('[data-test-checkbox="free-signup-notifications"]'),
          'free signup alert'
        ).to.not.exist;
        expect(
          find('[data-test-checkbox="paid-started-notifications"]'),
          'paid start alert'
        ).to.not.exist;
        expect(
          find('[data-test-checkbox="paid-canceled-notifications"]'),
          'paid cancel alert'
        ).to.not.exist;
      });
    });
    ...
    describe('own user', function () {
      ...
      it('can toggle email alerts for own user', async function () {
        await visit(`/settings/staff/${admin.slug}`);
        expect(find('[data-test-checkbox="free-signup-notifications"]')).to.not
          .be.checked;
        expect(find('[data-test-checkbox="paid-started-notifications"]')).to.not
          .be.checked;
        expect(find('[data-test-checkbox="paid-canceled-notifications"]')).to
          .not.be.checked;
        expect(find('[data-test-checkbox="mention-notifications"]')).to.not.be
          .checked;
        expect(find('[data-test-checkbox="milestone-notifications"]')).to.not.be
          .checked;
        ...
        expect(find('[data-test-checkbox="free-signup-notifications"]')).to.be
          .checked;
        expect(find('[data-test-checkbox="paid-started-notifications"]')).to.be
          .checked;
        expect(find('[data-test-checkbox="paid-canceled-notifications"]')).to.be
          .checked;
        expect(find('[data-test-checkbox="mention-notifications"]')).to.be
          .checked;
        expect(find('[data-test-checkbox="milestone-notifications"]')).to.be
          .checked;
      });
    });
