  ...
  describe('when attempting to signin', function () {
    beforeEach(function () {
      ...
      this.server.post('/session', function (schema, { requestBody }) {
        let { username, password } = JSON.parse(requestBody);
    ...
    it('errors correctly', async function () {
      ...
      expect(
        findAll('input[name="identification"]').length,
        'email input field'
      ).to.equal(1);
      expect(
        findAll('input[name="password"]').length,
      ...
      expect(
        find('.main-error').textContent.trim(),
        'main error text'
