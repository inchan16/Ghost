...
describe('Acceptance: Settings - Labs', function () {
  ...
  describe('when logged in', function () {
    ...
    it('can upload/download redirects', async function () {
      ...
      await fileUpload('[data-test-file-input="redirects"] input', ['test'], {
        name: 'redirects.json',
      ...
      expect(buttons[0].textContent, 'post-success button text').to.have.string(
        'Upload redirects'
      );
      ...
      expect(buttons[0].textContent, 'post-failure button text').to.have.string(
        'Upload redirects'
      );
    ...
    it('can upload/download routes.yaml', async function () {
      ...
      await fileUpload('[data-test-file-input="routes"] input', ['test'], {
        name: 'routes.yaml',
      ...
      let iframe = document.querySelector('#iframeDownload');
      expect(iframe.getAttribute('src')).to.have.string(
        '/settings/routes/yaml/'
      );
    });
  });
  ...
  describe('When logged in as Owner', function () {
    ...
    it.skip('sets the mailgunBaseUrl to the default', async function () {
      ...
      expect(
        params.settings.findBy('key', 'mailgun_base_url').value
      ).not.to.equal(null);
    });
  });
});
