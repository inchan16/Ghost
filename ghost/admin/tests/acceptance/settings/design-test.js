...
describe('Acceptance: Settings - Design', function () {
  ...
  it('renders with no custom theme settings', async function () {
    ...
    expect(document.title, 'page title').to.equal(
      'Settings - Design - Test Blog'
    );
    ...
    expect(
      findAll('[data-test-nav-toggle]'),
      'no of group toggles'
    ).to.have.lengthOf(1);
    expect(
      findAll('[data-test-nav-group]'),
      'no of groups open'
    ).to.have.lengthOf(1);

    // current theme is shown in nav menu
    expect(find('[data-test-text="current-theme"]')).to.contain.text(
      'casper - v1.0'
    );

    // defaults to "home" desktop preview
    expect(find('[data-test-button="desktop-preview"]')).to.have.class(
      'gh-btn-group-selected'
    );
    expect(find('[data-test-button="mobile-preview"]')).to.not.have.class(
      'gh-btn-group-selected'
    );
  });
  ...
  it('has unsaved-changes confirmation', async function () {
    ...
    await click(
      '[data-test-modal="unsaved-settings"] [data-test-button="close"]'
    );
    ...
    expect(find('[data-test-input="siteDescription"]')).to.not.have.value(
      'Changed'
    );
  });
  ...
  it('can install an official theme', async function () {
    ...
    expect(currentURL(), 'currentURL').to.equal(
      '/settings/design/change-theme/Journal'
    );

    await click('[data-test-button="install-theme"]');
    expect(find('[data-test-modal="install-theme"]'), 'install-theme modal').to
      .exist;
    ...
    expect(find('[data-test-state="installed-no-notes"]'), 'success state').to
      .exist;
    expect(findAll('[data-test-state]').length, 'state count').to.equal(1);
  ...
  it('can upload custom theme', async function () {
    ...
    expect(find('[data-test-modal="upload-theme"]'), 'upload-theme modal').to
      .exist;

    await fileUpload(
      '[data-test-modal="upload-theme"] input[type="file"]',
      ['test'],
      { name: 'valid-theme.zip', type: 'application/zip' }
    );

    expect(find('[data-test-state="installed-no-notes"]'), 'success state').to
      .exist;
    expect(currentURL(), 'url after upload').to.equal(
      '/settings/design/change-theme'
    );
  ...
  describe('limits', function () {
    it('displays upgrade notice when custom themes are not allowed', async function () {
      ...
      config.hostSettings = {
        limits: {
          customThemes: {
            ...
            error:
              'All our official built-in themes are available the Starter plan, if you upgrade to one of our higher tiers you will also be able to edit and upload custom themes for your site.',
          },
        },
      };
      ...
      expect(
        find('[data-test-modal="limits/custom-theme"]'),
        'limits/custom-theme modal'
      ).to.exist;
    });
  });
});
