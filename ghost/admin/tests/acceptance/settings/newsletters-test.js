...
async function checkValidationError(errors) {
  ...
  for (const selector of Object.keys(errors)) {
    expect(
      findAll(selector).length,
      'field ' + selector + ' is not visible'
    ).to.equal(1);
    expect(
      findAll(selector + ' + .response').length,
      'error message is displayed'
    ).to.equal(1);
    expect(find(selector + ' + .response').textContent).to.match(
      errors[selector]
    );
  }
  ...
  expect(
    find(
      '[data-test-button="save-newsletter"] > [data-test-task-button-state="failure"]'
    )
  ).to.exist;
}
...
async function checkSave(options) {
  ...
  // No errors
  expect(
    findAll('.error > .response').length,
    'error message is displayed'
  ).to.equal(0);

  if (options.verifyEmail) {
    expect(
      find('[data-test-modal="confirm-newsletter-email"]'),
      'Confirm email modal'
    ).to.exist;

    // Check message
    if (typeof verifyEmail !== 'boolean') {
      const t = find('[data-test-modal="confirm-newsletter-email"] p')
        .textContent.trim()
        .replace(/\s+/g, ' ');
  ...
  expect(
    find(`[data-test-modal="${name}-newsletter"]`),
    'Newsletter modal should disappear after saving'
  ).to.not.exist;
}
...
async function checkCancel(options) {
  ...
  if (options.shouldConfirm) {
    expect(
      ...
      'Confirm unsaved settings modal should be visible'
    ).to.exist;
  ...
  expect(
    find(`[data-test-modal="${name}-newsletter"]`),
    'Newsletter modal should disappear after canceling'
  ).to.not.exist;
}
...
describe('Acceptance: Settings - Newsletters', function () {
  ...
  describe('Creating newsletters', function () {
    it('can create new newsletter', async function () {
      ...
      expect(
        findAll('[data-test-newsletter]').length,
        'Total newsletters shown'
      ).to.equal(1);
      await click('[data-test-button="add-newsletter"]');

      // Check if modal opens
      expect(
        find('[data-test-modal="create-newsletter"]'),
      ...
      await fillIn(
        'textarea#newsletter-description',
      ...
      expect(
        findAll('[data-test-newsletter]').length,
        'Total newsletters shown afterwards'
      ).to.equal(2);
    });
    ...
    it('checks limits when creating a newsletter', async function () {
      ...
      config.hostSettings = {
        limits: {
          newsletters: {
            max: 1,
            error:
              'Your plan supports up to {{max}} newsletters. Please upgrade to add more.',
          },
        },
      };
      ...
      expect(
        find('[data-test-modal="limits/multiple-newsletters"]'),
        'Limits modal'
      ).to.exist;
  ...
  describe('Editing newsletters', function () {
    it('can edit via menu if multiple newsletters', async function () {
      ...
      // Check if modal opens
      expect(
        find('[data-test-modal="edit-newsletter"]'),
    ...
    it('shows current sender email in verify modal', async function () {
      ...
      await click(
        '[data-test-newsletter="test-newsletter"] [data-test-newsletter-menu-trigger]'
      );
      await click('[data-test-button="customize-newsletter"]');
    ...
    it('asks to confirm unsaved changes', async function () {
      async function doCheck(tabName, field) {
        ...
        if (field.input) {
          ...
          await click(
            `${field.dropdown} li.ember-power-select-option[aria-current="false"]`
          );
        }
      ...
      await doCheck.call(this, 'design.body', {
        toggle:
          '#newsletter-title-alignment button:not(.gh-btn-group-selected)',
      });
  ...
  describe('Archiving newsletters', function () {
    it('can archive newsletters', async function () {
      ...
      expect(
        find(
          '[data-test-dropdown="newsletter-status-filter"] .ember-power-select-trigger'
        )
      ).not.to.exist;
      ...
      expect(
        find('[data-test-modal="confirm-newsletter-archive"]'),
        'Archive newsletter modal'
      ).to.exist;
    ...
    it('can reactivate newsletters if only archived newsletter left', async function () {
      ...
      // Check title okay
      expect(
        find('.gh-newsletters .gh-expandable-title').textContent.trim(),
        'Title'
      ).to.equal('Archived newsletters');

      // Check total newsletters shown
      expect(
        findAll('[data-test-newsletter]').length,
        'Total archived newsletters shown'
      ).to.equal(1);
      ...
      // Check if confimation modal opens
      expect(
        find('[data-test-modal="confirm-newsletter-reactivate"]'),
        'Reactivate newsletter modal'
      ).to.exist;
