  ...
  describe('when logged in', function () {
    ...
    it('it renders, loads and saves editors correctly', async function () {
      ...
      expect(document.title, 'page title').to.equal(
        'Settings - Code injection - Test Blog'
      );
      ...
      expect(
        findAll('#ghost-head .CodeMirror').length,
        'ghost head codemirror element'
      ).to.equal(1);
      expect(
        find('#ghost-head .CodeMirror'),
        'ghost head editor theme'
      ).to.have.class('cm-s-xq-light');

      expect(
        findAll('#ghost-foot .CodeMirror').length,
        'ghost head codemirror element'
      ).to.equal(1);
      expect(
        find('#ghost-foot .CodeMirror'),
        'ghost head editor theme'
      ).to.have.class('cm-s-xq-light');
      ...
      expect(
        params.settings.findBy('key', 'codeinjection_head').value
      ).to.equal('Test');
