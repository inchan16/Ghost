...
describe('Acceptance: Settings - Integrations - Slack', function () {
  ...
  describe('when logged in', function () {
    ...
    it('it validates and saves slack settings properly', async function () {
      ...
      expect(
        find('[data-test-error="slack-url"]').textContent.trim(),
        'inline validation response'
      ).to.equal(
      ...
      await fillIn(
        '[data-test-slack-url-input]',
        'https://hooks.slack.com/services/1275958430'
      );
      ...
      let usernameResult = params.settings.findBy(
        'key',
        'slack_username'
      ).value;
      ...
      expect(
        find('[data-test-error="slack-url"]'),
        'inline validation response'
      ).to.not.exist;
      ...
      expect(
        findAll('.gh-notification').length,
      ...
      await fillIn(
        '[data-test-slack-url-input]',
        'https://hooks.slack.com/services/1275958431'
      );
      ...
      expect(
        findAll('.gh-notification').length,
        'check slack notification after api validation error'
      ).to.equal(0);
    });
