...
describe('Acceptance: Settings - Integrations - Custom', function () {
  ...
  describe('navigation', function () {
    ...
    it('renders defaults correctly', async function () {
      ...
      expect(
        find(
          '[data-test-app="slack"] [data-test-app-status]'
        ).textContent.trim(),
        'slack app status'
      ).to.equal('Configure');
    ...
    it('it redirects to Unsplash when clicking on the grid', async function () {
      ...
      // has correct url
      expect(currentURL(), 'currentURL').to.equal(
        '/settings/integrations/unsplash'
      );
    });
  });
  ...
  describe('custom integrations', function () {
    ...
    it('can add new integration', async function () {
      ...
      expect(currentURL(), 'url after clicking new').to.equal(
        '/settings/integrations/new'
      );
      expect(
        find('[data-test-modal="new-integration"]'),
        'modal after clicking new'
      ).to.exist;
      ...
      expect(currentURL(), 'url after integration creation').to.equal(
      ...
      expect(currentURL(), 'url after clicking "Back"').to.equal(
      ...
      expect(currentURL(), 'url after clicking integration in list').to.equal(
        '/settings/integrations/1'
      );
    });
    ...
    it('can manage an integration', async function () {
      ...
      expect(currentURL(), 'initial URL').to.equal('/settings/integrations/1');
      ...
      await fillIn(
        ...
        'Description for Test Integration'
      );
      ...
      expect(currentURL(), 'url after saving and clicking "back"').to.equal(
        '/settings/integrations'
      );

      expect(
        find(
          '[data-test-integration="1"] [data-test-text="name"]'
        ).textContent.trim(),
      ...
      expect(
        find(
          '[data-test-integration="1"] [data-test-text="description"]'
        ).textContent.trim(),
      ...
      expect(currentURL(), 'url after clicking "stay"').to.equal(
        '/settings/integrations/1'
      );
      ...
      expect(currentURL(), 'url after clicking "leave"').to.equal(
        '/settings/integrations'
      );
    ...
    it("can manage an integration's webhooks", async function () {
      ...
      expect(
        find('[data-test-modal="webhook-form"] [data-test-text="title"]')
          .textContent
      ...
      await fillIn(
        ...
        'https://example.com/first-webhook'
      );
      ...
      expect(
        row.querySelector('[data-test-text="name"]').textContent
      ).to.have.string('First webhook');
      expect(
        row.querySelector('[data-test-text="event"]').textContent
      ).to.have.string('Site changed (rebuild)');
      expect(
        row.querySelector('[data-test-text="targetUrl"]').textContent
      ).to.have.string('https://example.com/first-webhook');
      expect(
        row.querySelector('[data-test-text="last-triggered"]').textContent
      ).to.have.string('Not triggered');

      // click edit webhook link
      await click(
        '[data-test-webhook-row="1"] [data-test-newsletter-menu-trigger]'
      );
    ...
    it('can delete integration', async function () {
      ...
      await click(
        '[data-test-modal="delete-integration"] [data-test-button="confirm"]'
      );
