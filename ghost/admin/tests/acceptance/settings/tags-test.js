...
describe('Acceptance: Tags', function () {
  ...
  describe('when logged in', function () {
    ...
    it('lists public and internal tags separately', async function () {
      ...
      expect(
        find('[data-test-nav="tags"]'),
        'highlights nav menu item'
      ).to.have.class('active');

      // it defaults to public tags
      expect(find('[data-test-tags-nav="public"]')).to.have.attr(
        'data-test-active'
      );
      expect(find('[data-test-tags-nav="internal"]')).to.not.have.attr(
        'data-test-active'
      );
      ...
      expect(
        tags[0].querySelector('[data-test-tag-name]')
      ).to.have.trimmed.text('A - First');
      expect(
        tags[1].querySelector('[data-test-tag-name]')
      ).to.have.trimmed.text('!A - Second');
      expect(
        tags[2].querySelector('[data-test-tag-name]')
      ).to.have.trimmed.text('B - Third');
      expect(
        tags[3].querySelector('[data-test-tag-name]')
      ).to.have.trimmed.text('Z - Last');
    ...
    it('can edit tags', async function () {
      ...
      expect(find('[data-test-input="tag-name"]')).to.have.value(
        'To be edited'
      );
      expect(find('[data-test-input="tag-slug"]')).to.have.value(
        'to-be-edited'
      );
      ...
      expect(
        tagListItem.querySelector('[data-test-tag-name]')
      ).to.have.trimmed.text('New tag name');
      expect(
        tagListItem.querySelector('[data-test-tag-slug]')
      ).to.have.trimmed.text('new-tag-slug');
    });
    ...
    it('can delete tags', async function () {
      ...
      expect(
        find(`${tagModal} [data-test-text="posts-count"]`)
