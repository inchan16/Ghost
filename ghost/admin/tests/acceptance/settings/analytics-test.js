...
describe('Acceptance: Settings - Analytics', function () {
  ...
  it('can manage open rate tracking', async function () {
    ...
    expect(
      this.server.db.settings.findBy({ key: 'email_track_opens' }).value
    ).to.equal(false);
  });
  ...
  it('can manage click tracking', async function () {
    ...
    expect(
      this.server.db.settings.findBy({ key: 'email_track_clicks' }).value
    ).to.equal(false);
  });
  ...
  it('can manage source tracking', async function () {
    ...
    expect(find('[data-test-checkbox="members-track-sources"]')).to.not.be
      .checked;

    await click('[data-test-button="save-analytics-settings"]');

    expect(
      this.server.db.settings.findBy({ key: 'members_track_sources' }).value
    ).to.equal(false);
  });
  ...
  it('can manage outbound link tagging', async function () {
    ...
    expect(find('[data-test-checkbox="outbound-link-tagging"]')).to.not.be
      .checked;

    await click('[data-test-button="save-analytics-settings"]');

    expect(
      this.server.db.settings.findBy({ key: 'outbound_link_tagging' }).value
    ).to.equal(false);
  });
});
