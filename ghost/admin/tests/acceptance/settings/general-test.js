  ...
  describe('when logged in', function () {
    ...
    it('it renders, handles image uploads', async function () {
      ...
      expect(document.title, 'page title').to.equal(
        'Settings - General - Test Blog'
      );

      // highlights nav menu
      expect(
        find('[data-test-nav="settings"]'),
      ...
      expect(document.title, 'page title').to.equal(
        'Settings - General - New Blog Title'
      );
      ...
      expect(params.settings.findBy('key', 'title').value).to.equal(
        'CMD-S Test'
      );
    });
    ...
    it('renders timezone selector correctly', async function () {
      ...
      expect(
        findAll('#timezone option').length,
        'available timezones'
      ).to.equal(66);
      expect(find('#timezone option:checked').textContent.trim()).to.equal(
        '(GMT) UTC'
      );
    ...
    it('handles private blog settings correctly', async function () {
      ...
      expect(find('[data-test-private-checkbox]').checked, 'isPrivate checkbox')
        .to.be.false;
      ...
      expect(find('[data-test-private-checkbox]').checked, 'isPrivate checkbox')
        .to.be.true;
      expect(
        findAll('[data-test-password-input]').length,
        'password input'
      ).to.equal(1);
      expect(
        find('[data-test-password-input]').value,
      ...
      expect(
        find('[data-test-password-error]').textContent.trim(),
