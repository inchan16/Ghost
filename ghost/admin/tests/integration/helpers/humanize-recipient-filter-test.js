...
describe('Integration: Helper: humanize-recipient-filter', function () {
  ...
  it('handles free subscribers and labels array', async function () {
    await render(
      hbs`{{humanize-recipient-filter "status:free,labels:[one,two]"}}`
    );
    expect(this.element.textContent.trim()).to.equal(
      'Free subscribers & Labels: One, Two'
    );
  });

  it('handles free subscribers and individual labels', async function () {
    await render(
      hbs`{{humanize-recipient-filter "status:free,label:one,label:two"}}`
    );
    expect(this.element.textContent.trim()).to.equal(
      'Free subscribers & Labels: One, Two'
    );
  ...
  it('handles paid subscribers and labels array', async function () {
    ...
    expect(this.element.textContent.trim()).to.equal(
      'Paid subscribers & Labels: One, Two'
    );
  });
  ...
  it('handles paid subscribers and products array', async function () {
    await render(
      hbs`{{humanize-recipient-filter "status:-free,products:[one,two]"}}`
    );
    expect(this.element.textContent.trim()).to.equal(
      'Paid subscribers & Products: One, Two'
    );
  });

  it('handles paid subscribers and individual products', async function () {
    await render(
      hbs`{{humanize-recipient-filter "status:-free,product:one,product:two"}}`
    );
    expect(this.element.textContent.trim()).to.equal(
      'Paid subscribers & Products: One, Two'
    );
