...
describe('Integration: Service: feature', function () {
  ...
  it('notifies for server errors on labs save', async function () {
    ...
    return service.fetch().then(() => {
      ...
      return settled().then(() => {
        expect(server.handlers[2].numberOfCalls, 'PUT call is made').to.equal(
          1
        );
  ...
  it('notifies for server errors on accessibility save', async function () {
    ...
    return service.fetch().then(() => {
      ...
      return settled().then(() => {
        expect(server.handlers[3].numberOfCalls, 'PUT call is made').to.equal(
          1
        );
  ...
  it('has correct labs flags when accessed before and after settings load', async function () {
    ...
    expect(featureService.testFlag, 'testFlag before settings fetch').to.be
      .false;

    await settingsService.fetch();

    expect(
      featureService.settings.labs,
      'feature.settings.labs after settings fetch'
    ).to.equal('{"testFlag":true}');
    expect(
      featureService.labs,
      'feature.labs after settings fetch'
    ).to.deep.equal({ testFlag: true });
    expect(featureService.testFlag, 'feature.testFlag after settings fetch').to
      .be.true;
  });
});
