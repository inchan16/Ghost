...
describe('Integration: Component: gh-cm-editor', function () {
  ...
  it('handles change event', async function () {
    ...
    await render(
      hbs`<GhCmEditor @value={{this.text}} @classNames="gh-input" @update={{this.onUpdate}} />`
    );
    ...
    expect(this.text, 'text value after CM editor change').to.equal('Testing');
  });
  ...
  it('can autofocus', async function () {
    ...
    let onFocus = async () => {
      ...
      await settled();
      expect(
        find('.gh-input').classList.contains('focus'),
        'has focused class on first render with autofocus'
      ).to.be.true;
    };
    ...
    await render(
      hbs`<GhCmEditor @value={{this.text}} @classNames="gh-input" @update={{this.onUpdate}} @autofocus={{true}} @focus-in={{this.onFocus}} />`
    );
  });
});
