...
describe('Integration: Component: gh-date-time-picker', function () {
  ...
  it('renders', async function () {
    ...
    expect(
      find('[data-test-component="gh-date-time-picker"]'),
      'component wrapper'
    ).to.exist;
    expect(
      find('[data-test-date-time-picker-datepicker]'),
      'datepicker trigger'
    ).to.exist;
    expect(find('[data-test-date-time-picker-date-input]'), 'datepicker input')
      .to.exist;
    expect(find('[data-test-date-time-picker-time-input]'), 'time input').to
      .exist;
    expect(find('[data-test-date-time-picker-timezone]')).to.contain.text(
      'UTC'
    );
  });
  ...
  it('defaults to now when @date is empty', async function () {
    ...
    await render(hbs`<GhDateTimePicker />`);
    expect(
      find('[data-test-date-time-picker-date-input]'),
      'date input'
    ).to.have.value('2022-02-22');
    expect(
      find('[data-test-date-time-picker-time-input]'),
      'time input'
    ).to.have.value('22:22');
  });
  ...
  it('uses separate @time value', async function () {
    ...
    await render(
      hbs`<GhDateTimePicker @date={{this.date}} @time={{this.time}} />`
    );
    expect(
      find('[data-test-date-time-picker-date-input]'),
      'date input'
  ...
  it('can update date via date input', async function () {
    ...
    this.set('updateDate', (newDate) => {
      ...
      expect(moment.utc(newDate).toISOString()).to.equal(
    ...
    await render(
      hbs`<GhDateTimePicker @date={{this.date}} @time={{this.time}} @setDate={{this.updateDate}} @setTime={{this.updateTime}} />`
    );
  ...
  it('can update date via datepicker', async function () {
    ...
    await datepickerSelect(
      '[data-test-date-time-picker-datepicker]',
      moment('2022-02-27T13:00:00.000Z').toDate()
    );
  });
  ...
  it('handles invalid date input', async function () {
    ...
    expect(find('[data-test-date-time-picker-error]')).to.have.text(
      'Invalid date'
    );
    ...
    expect(find('[data-test-date-time-picker-error]')).to.contain.text(
      'Invalid date format'
    );
  ...
  describe('min/max', function () {
    it('disables datepicker dates outside of range', async function () {
      ...
      await render(
        hbs`<GhDateTimePicker @date={{this.date}} @time={{this.time}} @minDate={{this.minDate}} @maxDate={{this.maxDate}} />`
      );
    ...
    it('Handles timezone of minimum date correctly', async function () {
      ...
      await render(
        hbs`<GhDateTimePicker @date={{this.date}} @time={{this.time}} @minDate={{this.minDate}} />`
      );
      await click('[data-test-date-time-picker-datepicker]');

      expect(find('[data-date="2021-12-30"]')).to.have.attribute('disabled');
      expect(find('[data-date="2021-12-31"]')).not.to.have.attribute(
        'disabled'
      );
    });
    ...
    it('Handles timezone of maximum date correctly', async function () {
      ...
      await render(
        hbs`<GhDateTimePicker @date={{this.date}} @time={{this.time}} @maxDate={{this.maxDate}} />`
      );
      await click('[data-test-date-time-picker-datepicker]');
      expect(find('[data-date="2021-12-30"]')).not.to.have.attribute(
        'disabled'
      );
