...
describe('Integration: Component: gh-uploader', function () {
  ...
  describe('uploads', function () {
    ...
    it('triggers onStart when upload starts', async function () {
      ...
      await render(
        hbs`<GhUploader @files={{files}} @onStart={{this.uploadStarted}}></GhUploader>`
      );
      this.set('files', [createFile(), createFile()]);
      await settled();
    ...
    it('triggers onUploadSuccess when a file uploads', async function () {
      ...
      await render(
        hbs`<GhUploader @files={{files}} @onUploadSuccess={{this.fileUploaded}}></GhUploader>`
      );
      this.set('files', [
    ...
    it('triggers onComplete when all files uploaded', async function () {
      ...
      await render(
        hbs`<GhUploader @files={{files}} @onComplete={{this.uploadsFinished}}></GhUploader>`
      );
      this.set('files', [
        createFile(['test'], { name: 'file1.png' }),
    ...
    it('onComplete only passes results for last upload', async function () {
      ...
      this.set('files', [createFile(['test'], { name: 'file1.png' })]);
      await settled();

      this.set('files', [createFile(['test'], { name: 'file2.png' })]);
    ...
    it('yields progressBar component with total upload progress', async function () {
      ...
      let finalProgressWidth = parseInt(
        find('[data-test-progress-bar]').style.width
      );
    ...
    it('uploads to supplied `uploadUrl`', async function () {
      ...
      await render(
        hbs`<GhUploader @files={{files}} @uploadUrl="/images/"></GhUploader>`
      );
      this.set('files', [createFile()]);
      await settled();
    ...
    it('passes supplied paramName in request', async function () {
      await render(
        hbs`<GhUploader @files={{files}} @paramName="testupload"></GhUploader>`
      );
      this.set('files', [createFile()]);
      await settled();
  ...
  describe('validation', function () {
    it('validates file extensions by default', async function () {
      ...
      expect(onFailedResult[0].fileName, 'onFailed file name').to.equal(
        'test.png'
      );
      expect(onFailedResult[0].message, 'onFailed message').to.match(
        /not supported/
      );
    });
    ...
    it('accepts custom validation method', async function () {
      ...
      expect(onFailedResult[0].message).to.equal(
        'test.png failed test validation'
      );
    });
  ...
  describe('server errors', function () {
    ...
    it('yields errors when uploads fail', async function () {
      ...
      expect(find('.error-fileName').textContent).to.equal('test.png');
      expect(find('.error-message').textContent).to.equal(
        'Error: No upload for you'
      );
    });
  });
});
