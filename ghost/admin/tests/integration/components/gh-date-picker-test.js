...
describe('Integration: Component: gh-date-picker', function () {
  ...
  it('renders', async function () {
    ...
    expect(find('[data-test-date-picker-trigger]'), 'datepicker trigger').to
      .exist;
  ...
  it('defaults to now when @value is empty', async function () {
    ...
    await render(hbs`<GhDatePicker />`);
    expect(find('[data-test-date-picker-input]'), 'date input').to.have.value(
      '2022-02-22'
    );
  });
  ...
  it('updates date via input blur', async function () {
    ...
    await render(
      hbs`<GhDatePicker @value={{this.date}} @onChange={{this.onChange}} />`
    );
    ...
    expect(changeSpy.firstCall.args[0].toISOString()).to.equal(
      moment('2022-02-28T00:00:00.000').toISOString()
    );
  });
  ...
  it('updates date via datepicker selection', async function () {
    ...
    await datepickerSelect(
      ...
      moment('2022-02-27T13:00:00.000').toDate()
    );
    ...
    expect(changeSpy.firstCall.args[0]).to.be.an.instanceof(Date);
    expect(changeSpy.firstCall.args[0].toISOString()).to.equal(
      moment('2022-02-27T00:00:00.000').toISOString()
    );
  });
  ...
  it('calls @onInput on input events', async function () {
    ...
    await render(
      hbs`<GhDatePicker @value={{this.date}} @onInput={{this.onInput}} />`
    );
  ...
  it('calls @onKeydown on input keydown events', async function () {
    ...
    await render(
      hbs`<GhDatePicker @value={{this.date}} @onKeydown={{this.onKeydown}} />`
    );
    await typeIn('[data-test-date-picker-input]', 'lo');
  ...
  it('calls @onBlur on input blur events', async function () {
    ...
    await render(
      hbs`<GhDatePicker @value={{this.date}} @onBlur={{this.onBlur}} />`
    );
  ...
  it('handles invalid date input', async function () {
    ...
    await render(
      hbs`<GhDatePicker @value={{this.date}} @onChange={{this.onChange}} @onError={{this.onError}} />`
    );
  ...
  it('handles invalid date format input', async function () {
    ...
    expect(find('[data-test-date-picker-error]')).to.contain.text(
    ...
    expect(errorSpy.callCount, '@onError call count').to.equal(1);
    expect(errorSpy.firstCall.args[0]).to.be.instanceof(Error);
    expect(errorSpy.firstCall.args[0].message).to.contain(
      'Date must be YYYY-MM-DD'
    );
  });
  ...
  describe('min/max', function () {
    it('disables datepicker dates outside of range', async function () {
      ...
      await render(
        hbs`<GhDatePicker @value={{this.date}} @minDate={{this.minDate}} @maxDate={{this.maxDate}} />`
      );
    ...
    it('errors when date input is earlier than min', async function () {
      ...
      await render(
        hbs`<GhDatePicker @value={{this.date}} @minDate={{this.minDate}} @onChange={{this.onChange}} @onError={{this.onError}} />`
      );
      await fillIn('[data-test-date-picker-input]', '2022-02-10');
      await blur('[data-test-date-picker-input]');

      expect(find('[data-test-date-picker-error]')).to.have.text(
        'Must be on or after 2022-02-11'
      );

      expect(changeSpy.callCount, '@onChange call count').to.equal(0);
      expect(errorSpy.callCount, '@onError call count').to.equal(1);
      expect(errorSpy.firstCall.args[0]).to.be.instanceof(Error);
      expect(errorSpy.firstCall.args[0].message).to.equal(
        'Must be on or after 2022-02-11'
      );
    ...
    it('allows for min date error override', async function () {
      ...
      await render(
        hbs`<GhDatePicker @value={{this.date}} @minDate={{this.minDate}} @minDateError="Must be in the future" @onChange={{this.onChange}} />`
      );
    ...
    it('errors when date input is later than max', async function () {
      ...
      await render(
        hbs`<GhDatePicker @value={{this.date}} @maxDate={{this.maxDate}} @onChange={{this.onChange}} @onError={{this.onError}} />`
      );
      await fillIn('[data-test-date-picker-input]', '2022-02-28');
      await blur('[data-test-date-picker-input]');

      expect(find('[data-test-date-picker-error]')).to.have.text(
        'Must be on or before 2022-02-25'
      );
    ...
    it('allows for max date error override', async function () {
      ...
      await render(
        hbs`<GhDatePicker @value={{this.date}} @maxDate={{this.maxDate}} @maxDateError="Must be in the past" @onChange={{this.onChange}} />`
      );
  ...
  describe('block invocation', function () {
    it('exposes Nav and Days components', async function () {
      ...
      await render(
        hbs`<GhDatePicker @value={{this.date}} @maxDate={{this.maxDate}} as |dp|><dp.Nav /><dp.Days /></GhDatePicker>`
      );
