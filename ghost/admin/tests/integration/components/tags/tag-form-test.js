...
describe.skip('Integration: Component: tags/tag-form', function () {
  ...
  it('has the correct title', async function () {
    await render(hbs`
        ...
        `);
    expect(
      find('.tag-settings-pane h4').textContent,
      'existing tag title'
    ).to.equal('Tag settings');

    this.set('tag.isNew', true);
    expect(find('.tag-settings-pane h4').textContent, 'new tag title').to.equal(
      'New tag'
    );
  });
  ...
  it('renders main settings', async function () {
    ...
    expect(
      ...
      'displays image uploader'
    ).to.equal(1);
    expect(find('input[name="name"]').value, 'name field value').to.equal(
      'Test'
    );
    expect(find('input[name="slug"]').value, 'slug field value').to.equal(
      'test'
    );
    expect(
      find('textarea[name="description"]').value,
      'description field value'
    ).to.equal('Description.');
    expect(
      find('input[name="metaTitle"]').value,
      'metaTitle field value'
    ).to.equal('Meta Title');
    expect(
      find('textarea[name="metaDescription"]').value,
      'metaDescription field value'
    ).to.equal('Meta description');
  });
  ...
  it('can switch between main/meta settings', async function () {
    ...
    expect(
      find('.tag-settings-pane').classList.contains('settings-menu-pane-in'),
      'main settings are displayed by default'
    ).to.be.true;
    expect(
      find('.tag-meta-settings-pane').classList.contains(
        'settings-menu-pane-out-right'
      ),
      'meta settings are hidden by default'
    ).to.be.true;

    await click('.meta-data-button');

    expect(
      find('.tag-settings-pane').classList.contains(
        'settings-menu-pane-out-left'
      ),
      'main settings are hidden after clicking Meta Data button'
    ).to.be.true;
    expect(
      find('.tag-meta-settings-pane').classList.contains(
        'settings-menu-pane-in'
      ),
      'meta settings are displayed after clicking Meta Data button'
    ).to.be.true;
    ...
    expect(
      find('.tag-settings-pane').classList.contains('settings-menu-pane-in'),
      'main settings are displayed after clicking "back"'
    ).to.be.true;
    expect(
      find('.tag-meta-settings-pane').classList.contains(
        'settings-menu-pane-out-right'
      ),
      'meta settings are hidden after clicking "back"'
    ).to.be.true;
  });
  ...
  it('has one-way binding for properties', async function () {
    ...
    expect(this.get('tag.description'), 'tag description').to.equal(
      'Description.'
    );
    expect(this.get('tag.metaTitle'), 'tag metaTitle').to.equal('Meta Title');
    expect(this.get('tag.metaDescription'), 'tag metaDescription').to.equal(
      'Meta description'
    );
  });
  ...
  it('displays error messages for validated fields', async function () {
    ...
    expect(nameFormGroup, 'name form group has error state').to.have.class(
      'error'
    );
    expect(
      nameFormGroup.querySelector('.response'),
      'name form group has error message'
    ).to.exist;

    let slugFormGroup = find('input[name="slug"]').closest('.form-group');
    expect(slugFormGroup, 'slug form group has error state').to.have.class(
      'error'
    );
    expect(
      slugFormGroup.querySelector('.response'),
      'slug form group has error message'
    ).to.exist;

    let descriptionFormGroup = find('textarea[name="description"]').closest(
      '.form-group'
    );
    expect(
      descriptionFormGroup,
      'description form group has error state'
    ).to.have.class('error');

    let metaTitleFormGroup = find('input[name="metaTitle"]').closest(
      '.form-group'
    );
    expect(
      metaTitleFormGroup,
      'metaTitle form group has error state'
    ).to.have.class('error');
    expect(
      metaTitleFormGroup.querySelector('.response'),
      'metaTitle form group has error message'
    ).to.exist;

    let metaDescriptionFormGroup = find(
      'textarea[name="metaDescription"]'
    ).closest('.form-group');
    expect(
      metaDescriptionFormGroup,
      'metaDescription form group has error state'
    ).to.have.class('error');
    expect(
      metaDescriptionFormGroup.querySelector('.response'),
      'metaDescription form group has error message'
    ).to.exist;
  });
  ...
  it('displays char count for text fields', async function () {
    ...
    expect(
      descriptionFormGroup.querySelector('.word-count'),
      'description char count'
    ).to.have.trimmed.text('12');

    let metaDescriptionFormGroup = find(
      'textarea[name="metaDescription"]'
    ).closest('.form-group');
    expect(
      metaDescriptionFormGroup.querySelector('.word-count'),
      'description char count'
    ).to.have.trimmed.text('16');
  });
  ...
  it('renders SEO title preview', async function () {
    await render(hbs`
        ...
        `);
    expect(
      find('.seo-preview-title').textContent,
      'displays meta title if present'
    ).to.equal('Meta Title');

    this.set('tag.metaTitle', '');
    expect(
      find('.seo-preview-title').textContent,
      'falls back to tag name without metaTitle'
    ).to.equal('Test');

    this.set('tag.name', new Array(151).join('x'));
    let expectedLength = 70 + '…'.length;
    expect(
      find('.seo-preview-title').textContent.length,
      'cuts title to max 70 chars'
  ...
  it('renders SEO URL preview', async function () {
    await render(hbs`
        ...
        `);
    expect(
      find('.seo-preview-link').textContent,
      'adds url and tag prefix'
    ).to.equal('http://localhost:2368/tag/test/');

    this.set('tag.slug', new Array(151).join('x'));
    let expectedLength = 70 + '…'.length;
    expect(
      find('.seo-preview-link').textContent.length,
      'cuts slug to max 70 chars'
    ).to.equal(expectedLength);
  });
  ...
  it('renders SEO description preview', async function () {
    await render(hbs`
        ...
        `);
    expect(
      find('.seo-preview-description').textContent,
      'displays meta description if present'
    ).to.equal('Meta description');

    this.set('tag.metaDescription', '');
    expect(
      find('.seo-preview-description').textContent,
      'falls back to tag description without metaDescription'
    ).to.equal('Description.');

    this.set('tag.description', new Array(500).join('x'));
    let expectedLength = 156 + '…'.length;
    expect(
      find('.seo-preview-description').textContent.length,
      'cuts description to max 156 chars'
    ).to.equal(expectedLength);
  });
  ...
  it('resets if a new tag is received', async function () {
    ...
    expect(
      ...
      'meta data pane is shown'
    ).to.be.true;

    this.set('tag', EmberObject.create({ id: '2' }));
    expect(
      find('.tag-settings-pane').classList.contains('settings-menu-pane-in'),
      'resets to main settings'
    ).to.be.true;
  });
  ...
  it('shows tags arrow link on mobile', async function () {
    ...
    expect(
      findAll(
        '.tag-settings-pane .settings-menu-header .settings-menu-header-action'
      ).length,
      'tags link is shown'
    ).to.equal(1);
  });
