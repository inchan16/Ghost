...
describe('Integration: Component: gh-notification', function () {
  ...
  it('renders', async function () {
    this.set(
      'message',
      new Message({ message: 'Test message', type: 'success' })
    );
  ...
  it('maps message types to CSS classes', async function () {
    ...
    await settled();
    expect(notification, 'success class is red').to.have.class(
      'gh-notification-red'
    );

    this.message.type = 'warn';
    await settled();
    expect(notification, 'success class is yellow').to.have.class(
      'gh-notification-yellow'
    );
  });
