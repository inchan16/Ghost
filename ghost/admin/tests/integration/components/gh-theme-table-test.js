...
describe('Integration: Component: gh-theme-table', function () {
  ...
  it('renders', async function () {
    ...
    expect(
      findAll('[data-test-themes-list]').length,
      'themes list is present'
    ).to.equal(1);
    expect(findAll('[data-test-theme-id]').length, 'number of rows').to.equal(
      4
    );

    let packageNames = findAll('[data-test-theme-title]').map((name) =>
    ...
    expect(
      find('[data-test-theme-active="true"]').querySelector(
        '[data-test-theme-title]'
      ),
  ...
  it('has download button in actions dropdown for all themes', async function () {
    ...
    for (const theme of themes) {
      await click(
        `[data-test-theme-id="${theme.name}"] [data-test-button="actions"]`
      );
      expect(
        find(
          `[data-test-actions-for="${theme.name}"] [data-test-button="download"]`
        )
      ).to.exist;
    }
  });
  ...
  it('has delete button for non-active, non-default, themes', async function () {
    ...
    for (const theme of themes) {
      ...
      expect(
        find(
          `[data-test-actions-for="${theme.name}"] [data-test-button="delete"]`
        )
      ).to.exist;
    }
  });
  ...
  it('does not show delete action for casper', async function () {
    ...
    expect(find(`[data-test-actions-for="casper"] [data-test-button="delete"]`))
      .to.not.exist;
  });
  ...
  it('does not show delete action for active theme', async function () {
    ...
    expect(find(`[data-test-actions-for="Daring"] [data-test-button="delete"]`))
      .to.not.exist;
  });
