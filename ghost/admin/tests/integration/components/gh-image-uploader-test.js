...
const stubSuccessfulUpload = function (server, delay = 0) {
  server.post(
    `${ghostPaths().apiRoot}/images/upload/`,
    function () {
      return [
        200,
...
describe('Integration: Component: gh-image-uploader', function () {
  ...
  it('renders form with supplied alt text', async function () {
    await render(
      hbs`<GhImageUploader @image={{this.image}} @altText="text test" />`
    );
    expect(find('[data-test-file-input-description]')).to.have.trimmed.text(
      'Upload image of "text test"'
    );
  });

  it('renders form with supplied text', async function () {
    await render(
      hbs`<GhImageUploader @image={{this.image}} @text="text test" />`
    );
    expect(find('[data-test-file-input-description]')).to.have.trimmed.text(
      'text test'
    );
  ...
  it('generates request to correct endpoint', async function () {
    ...
    await render(
      hbs`<GhImageUploader @image={{this.image}} @update={{this.update}} />`
    );
    ...
    expect(server.handledRequests[0].requestHeaders.Authorization).to.be
      .undefined;
  });
  ...
  it('fires fileSelected action on file selection', async function () {
    ...
    await render(
      hbs`<GhImageUploader @image={{this.image}} @fileSelected={{this.fileSelected}} @update={{this.update}} />`
    );
    await fileUpload('input[type="file"]', ['test'], { name: 'test.png' });
  ...
  it('fires uploadStarted action on upload start', async function () {
    ...
    await render(
      hbs`<GhImageUploader @image={{this.image}} @uploadStarted={{this.uploadStarted}} @update={{this.update}} />`
    );
    await fileUpload('input[type="file"]', ['test'], { name: 'test.png' });
  ...
  it('fires uploadFinished action on successful upload', async function () {
    ...
    await render(
      hbs`<GhImageUploader @image={{this.image}} @uploadFinished={{this.uploadFinished}} @update={{this.update}} />`
    );
    await fileUpload('input[type="file"]', ['test'], { name: 'test.png' });
  ...
  it('displays invalid file type error', async function () {
    ...
    expect(find('.failed').textContent).to.match(
      /The image type you uploaded is not supported/
    );
    expect(
      findAll('.gh-btn-green').length,
  ...
  it('displays file too large for server error', async function () {
    ...
    expect(findAll('.failed').length, 'error message is displayed').to.equal(1);
    expect(find('.failed').textContent).to.match(
      /The image you uploaded was larger/
    );
  });
  ...
  it('triggers file upload on file drop', async function () {
    ...
    stubSuccessfulUpload(server);
    await render(
      hbs`<GhImageUploader @uploadSuccess={{this.uploadSuccess}} />`
    );
    ...
    expect(uploadSuccess.firstCall.args[0]).to.equal(
