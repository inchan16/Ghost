...
describe('Integration: Component: gh-members-import-table', function () {
  ...
  it('renders members data with all the properties', async function () {
    ...
    expect(findAll('table tbody tr td')[2].textContent).to.match(
      /Not imported/
    );
    expect(findAll('table tbody tr td')[3].textContent).to.equal('email');
    expect(findAll('table tbody tr td')[4].textContent).to.equal(
      'kevin@example.com'
    );
    expect(findAll('table tbody tr td')[5].textContent).to.match(
      /Not imported/
    );
  });
  ...
  it('cannot navigate through data when only one data item is present', async function () {
    it('renders members data with all the properties', async function () {
      ...
      expect(findAll('table tbody tr td')[3].textContent).to.equal(
        'egg@example.com'
      );
