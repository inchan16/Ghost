...
const stubSuccessfulUpload = function (server, delay = 0) {
  server.post(
    `${ghostPaths().apiRoot}/members/upload/`,
    function () {
      return [
        200,
...
describe('Integration: Component: modal-import-members-test', function () {
  ...
  it('renders', async function () {
    ...
    expect(find('h1').textContent.trim(), 'default header').to.equal(
      'Import members'
    );
    expect(find('.description').textContent.trim(), 'upload label').to.equal(
  ...
  it('generates request to supplied endpoint', async function () {
    ...
    expect(find('label').textContent.trim(), 'labels label').to.equal(
      'Label these members'
    );
