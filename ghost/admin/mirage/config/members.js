...
export function mockMembersStats(server) {
  server.get(
    '/members/stats/count',
    withPermissionsCheck(ALLOWED_ROLES, function (db, { queryParams }) {
      let { days } = queryParams;

      let firstSubscriberDays = faker.datatype.number({
        min: 30,
        max: 600,
      });
...
export default function mockMembers(server) {
  server.post(
    '/members/',
    withPermissionsCheck(ALLOWED_ROLES, function ({ members }) {
        ...
        if (filter) {
          try {
            ...
            collection = collection.filter((member) => {
              ...
              ['labels', 'tiers', 'subscriptions', 'newsletters'].forEach(
                (association) => {
                  ...
                  member[association].models.forEach((associatedModel) => {
                    ...
                    Object.keys(associatedModel.attrs).forEach((key) => {
                      serializedAssociation[underscore(key)] =
                        associatedModel.attrs[key];
                    });
              ...
              errors: [
                {
                  ...
                  message:
                    'DELETE /members/ must be used with a filter, search, or all=true query parameter',
                },
              ],
            }
          );
  ...
  server.get(
    '/members/:id/',
    withPermissionsCheck(ALLOWED_ROLES, function ({ members }, { params }) {
      let { id } = params;
  ...
  server.put(
    ...
    withPermissionsCheck(
      ALLOWED_ROLES,
      function ({ members, tiers, subscriptions }, { params }) {
        const attrs = this.normalizedRequestAttrs();
        ...
        member.tiers.models.forEach((tier) => {
          if (!tierIds.includes(tier.id)) {
            member.subscriptions.models
              .filter((sub) => sub.tier.id === tier.id)
              .forEach((sub) => {
  ...
  server.del(
    '/members/:id/',
    withPermissionsCheck(ALLOWED_ROLES, function ({ members }, request) {
  ...
  server.get(
    '/members/upload/',
    withPermissionsCheck(ALLOWED_ROLES, function () {
      return new Response(
        200,
        {
  ...
  server.post(
    '/members/upload/',
    withPermissionsCheck(ALLOWED_ROLES, function ({ labels }, request) {
  ...
  server.get(
    ...
    withPermissionsCheck(
      ALLOWED_ROLES,
      function ({ memberActivityEvents }, { queryParams }) {
