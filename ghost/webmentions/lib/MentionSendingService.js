...
module.exports = class MentionSendingService {
  ...
  async sendForPost(post, options) {
    ...
    try {
      ...
      if (
        post.get('status') === post.previous('status') &&
        post.get('html') === post.previous('html')
      ) {
        // Not changed
        return;
      }
      ...
      let previousHtml =
        post.previous('status') === 'published' ? post.previous('html') : null;
      ...
      logging.error(
        'Error in webmention sending service post update event handler:'
      );
      logging.error(e);
    }
  }
  ...
  async send({ source, target, endpoint }) {
    logging.info(
      '[Webmention] Sending webmention from ' +
        source.href +
        ' to ' +
        target.href +
        ' via ' +
    ...
    throw new errors.BadRequestError({
      message:
        'Webmention sending failed with status code ' + response.statusCode,
  ...
  async sendAll(resource) {
    ...
    if (links.length) {
      logging.info(
        '[Webmention] Sending all webmentions for ' + resource.url.href
      );
    }
