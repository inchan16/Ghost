...
class SubscriptionStatsService {
  ...
  async getSubscriptionHistory() {
    ...
    for (
      let index = subscriptionDeltaEntries.length - 1;
      index >= 0;
      index -= 1
    ) {
    ...
    const rows = await knex('members_paid_subscription_events')
      .join('stripe_prices AS price', function () {
        this.on(
          ...
          '=',
          'members_paid_subscription_events.from_plan'
        ).orOn(
          'price.stripe_price_id',
          '=',
          'members_paid_subscription_events.to_plan'
        );
      })
      .join(
        'stripe_products AS product',
      ...
      .leftJoin(
        'stripe_prices AS from_price',
        'from_price.stripe_price_id',
        '=',
        'members_paid_subscription_events.from_plan'
      )
      .leftJoin(
        'stripe_prices AS to_price',
        'to_price.stripe_price_id',
        '=',
        'members_paid_subscription_events.to_plan'
      )
      .select(
