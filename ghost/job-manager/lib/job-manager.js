...
class JobManager {
  ...
  addJob({ name, at, job, data, offloaded = true }) {
    if (offloaded) {
      ...
      logging.info(
        `Adding one-off job to queue with current length = ${this.queue.length()} called '${
          name || 'anonymous'
        }'`
      );
  ...
  async awaitOneOffCompletion(name) {
    ...
    if (
      !persistedJob ||
      ![ALL_STATUSES.finished, ALL_STATUSES.failed].includes(
        persistedJob.get('status')
      )
    ) {
