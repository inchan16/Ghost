...
const messages = {
  checkingForUpdatesFailedError:
    'Checking for updates failed, your site will continue to function.',
  checkingForUpdatesFailedHelp:
    'If you get this error repeatedly, please seek help from {url}',
};
...
class UpdateCheckService {
  ...
  updateCheckError(err) {
    ...
    err.help = tpl(messages.checkingForUpdatesFailedHelp, {
  ...
  async updateCheckData() {
    ...
    try {
      const hash = (
        await this.api.settings.read(_.extend({ key: 'db_hash' }, internal))
      ).settings[0];
      const theme = (
        await this.api.settings.read(
          _.extend({ key: 'active_theme' }, internal)
        )
      ).settings[0];
      ...
      const blogId =
        parsedBlogUrl.hostname +
        parsedBlogUrl.pathname.replace(/\//, '') +
        hash.value;
      ...
      data.post_count =
        posts && posts.meta && posts.meta.pagination
          ? posts.meta.pagination.total
          : 0;
      data.user_count =
        users && users.users && users.users.length ? users.users.length : 0;
      data.blog_created_at =
        users && users.users && users.users[0] && users.users[0].created_at
          ? moment(users.users[0].created_at).unix()
          : '';
  ...
  async updateCheckRequest() {
    ...
    if (checkMethod === 'POST') {
      reqObj.json = true;
      reqObj.body = reqData;
      reqObj.headers['Content-Length'] = Buffer.byteLength(
  ...
  async updateCheckResponse(response) {
    let notifications = [];
    let notificationGroups = (this.config.notificationGroups || []).concat([
      'all',
    ]);
    ...
    if (_.isArray(response)) {
      ...
      Object.prototype.hasOwnProperty.call(response, 'notifications') &&
      _.isArray(response.notifications)
    ) {
    ...
    if (notificationGroups.length) {
      notifications = notifications.filter(function (notification) {
        ...
        return (
          _.includes(
            notificationGroups.map(function (groupIdentifier) {
              if (
                notification &&
                notification.version &&
                notification.version.match(new RegExp(groupIdentifier))
              ) {
                return true;
              }
  ...
  async createCustomNotification(notification) {
    if (
      ...
      debug(
        `Skipping notification creation as there are no messages to process`
      );
      return;
    }

    debug(
      `creating custom notifications for ${notification.messages.length} notifications`
    );
    ...
    for (const message of notification.messages) {
      const toAdd = {
        ...
        id: message.id,
        dismissible: Object.prototype.hasOwnProperty.call(
          message,
          'dismissible'
        )
          ? message.dismissible
  ...
  async check() {
    try {
      const result = await this.api.settings.read(
        _.extend({ key: 'next_update_check' }, internal)
      );
      ...
      if (
        ...
        nextUpdateCheck &&
        nextUpdateCheck.value &&
        nextUpdateCheck.value > moment().unix()
      ) {
        return;
      }
