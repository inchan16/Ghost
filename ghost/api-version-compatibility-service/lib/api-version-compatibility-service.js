...
class APIVersionCompatibilityService {
  ...
  async handleMismatch({
    ...
    if (
      !(await this.versionNotificationsDataService.fetchNotification(
        acceptVersion
      ))
    ) {
      const { name: integrationName, type: integrationType } =
        await this.versionNotificationsDataService.getIntegration(
          apiKeyValue,
          apiKeyType
        );
      ...
      const emails =
        await this.versionNotificationsDataService.getNotificationEmails();

      for (const email of emails) {
        const template =
          trimmedUseAgent === 'Zapier' ? 'zapier-mismatch' : 'generic-mismatch';

        const subject =
          trimmedUseAgent === 'Zapier'
      ...
      await this.versionNotificationsDataService.saveNotification(
        acceptVersion
      );
    }
  }
