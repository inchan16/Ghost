...
const messages = {
  hasPrimaryKeySQLiteError:
    'Must use hasPrimaryKeySQLite on an SQLite3 database',
...
function addTableColumn(
  ...
  if (
    ...
    Object.prototype.hasOwnProperty.call(columnSpec, 'fieldtype')
  ) {
  ...
  if (
    Object.prototype.hasOwnProperty.call(columnSpec, 'primary') &&
    columnSpec.primary === true
  ) {
    column.primary();
  }
  if (
    Object.prototype.hasOwnProperty.call(columnSpec, 'unique') &&
    columnSpec.unique
  ) {
    column.unique();
  }
  if (
    Object.prototype.hasOwnProperty.call(columnSpec, 'unsigned') &&
    columnSpec.unsigned
  ) {
    column.unsigned();
  }
  ...
  if (
    Object.prototype.hasOwnProperty.call(columnSpec, 'index') &&
    columnSpec.index === true
  ) {
    column.index();
  }
}
...
async function addColumn(tableName, column, transaction = db.knex, columnSpec) {
  const addColumnBuilder = transaction.schema.table(
    tableName,
    function (table) {
...
async function dropColumn(
  ...
  const dropColumnBuilder = transaction.schema.table(
    tableName,
    function (table) {
 ...
 */
async function hasForeignSQLite({
  fromTable,
  fromColumn,
  toTable,
  toColumn,
...
async function addForeign({
  ...
  if (DatabaseInfo.isSQLite(transaction)) {
    const foreignKeyExists = await hasForeignSQLite({
      fromTable,
      fromColumn,
      toTable,
      toColumn,
  ...
  try {
    logging.info(
      `Adding foreign key from ${fromTable}.${fromColumn} to ${toTable}.${toColumn}`
    );
...
async function dropForeign({
  ...
  try {
    logging.info(
      `Dropping foreign key from ${fromTable}.${fromColumn} to ${toTable}.${toColumn}`
    );
...
async function hasPrimaryKeySQLite(tableName, transaction = db.knex) {
  ...
  const rawConstraints = await transaction.raw(
...
async function addPrimaryKey(tableName, columns, transaction = db.knex) {
  ...
  try {
    logging.info(
      `Adding primary key constraint for '${columns}' in table '${tableName}'`
    );
    return await transaction.schema.table(tableName, function (table) {
...
function createColumnMigration(...migrations) {
  async function runColumnMigration(conn, migration) {
    ...
    if (isInCorrectState) {
      logging.warn(
        `${operationVerb} ${table}.${column} column - skipping as table is correct`
      );
    } else {
