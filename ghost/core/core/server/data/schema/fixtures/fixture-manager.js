...
class FixtureManager {
  ...
  async addAllFixtures(options) {
    ...
    const userRolesRelation = this.fixtures.relations.find(
      (r) => r.from.relation === 'roles'
    );
  ...
  findPermissionRelationsForObject(objName, role) {
    ...
    const foundRelation = _.cloneDeep(
  ...
  fetchRelationData(relation, options) {
    const fromOptions = _.extend({}, options, {
      withRelated: [relation.from.relation],
    });
      ...
      _.each(entry, (value, entryKey) => {
        ...
        toItems = _.reject(toItems, (item) => {
          return fromItem
            ...
            .find((model) => {
              const objectToMatch = FixtureManager.matchObj(
                relationFixture.to.match,
                item
              );
  ...
  async removeFixturesForModel(modelFixture, options) {
    const results = await sequence(
      modelFixture.entries.map((entry) => async () => {
        ...
        if (found) {
          return models[modelFixture.name].destroy(
            _.extend(options, { id: found.id })
          );
        }
      })
    );
