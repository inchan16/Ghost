...
const messages = {
  ...
  valueMustBeBoolean:
    'Value in [{tableName}.{columnKey}] must be one of true, false, 0 or 1.',
  ...
  _.each(columns, function each(columnKey) {
    ...
    if (model.get(columnKey) !== null && model.get(columnKey) !== undefined) {
      ...
      if (
        ...
        if (
          !validator.isLength(strVal, 0, schema[tableName][columnKey].maxlength)
        ) {
      ...
      if (
        ...
        if (
          schema[tableName][columnKey].type === 'integer' &&
          !validator.isInt(strVal)
        ) {
