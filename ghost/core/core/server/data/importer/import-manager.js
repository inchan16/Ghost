...
const messages = {
  ...
  invalidZipFileNameEncodingContext:
    'The filename was too long or contained invalid characters',
...
class ImportManager {
  ...
  getContentTypes() {
    return _.union(
      _.flatMap(this.handlers, 'contentTypes'),
  ...
  getDirectories() {
    return _.union(
      _.flatMap(this.handlers, 'directories'),
  ...
  getDirectoryGlob(directories, level) {
    const prefix =
      level === ALL_DIRS ? '**/' : level === ROOT_OR_SINGLE_DIR ? '{*/,}' : '';
  ...
  isValidZip(directory) {
    ...
    throw new errors.UnsupportedMediaTypeError({
      message: tpl(messages.invalidZipStructure),
    });
  }
  ...
  getFilesFromZip(handler, directory) {
    ...
    return _.map(
      glob.sync(globPattern, { cwd: directory, nocase: true }),
      function (file) {
  ...
  getBaseDirectory(directory) {
    ...
    const extMatches = glob.sync(
      this.getExtensionGlob(this.getExtensions(), ROOT_ONLY),
      { cwd: directory, nocase: true }
    );

    const dirMatches = glob.sync(
      this.getDirectoryGlob(this.getDirectories(), ROOT_ONLY),
      { cwd: directory, nocase: true }
    );
    let extMatchesAll;
    ...
    if (extMatchesAll.length < 1 || extMatchesAll[0].split('/').length < 1) {
      throw new errors.ValidationError({
        message: tpl(messages.invalidZipFileBaseDirectory),
      });
    }
  ...
  async processFile(file, ext) {
    ...
    await Promise.all(
      fileHandlers.map(async (fileHandler) => {
        ...
        importData[fileHandler.type] = await fileHandler.loadFile([
          _.pick(file, 'name', 'path'),
        ]);
      })
    );
