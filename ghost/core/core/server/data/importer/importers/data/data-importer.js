...
DataImporter = {
  ...
  init: function init(importData) {
    ...
    importers.custom_theme_settings = new CustomThemeSettingsImporter(
      importData.data
    );
  ...
  doImport: async function doImport(importData, importOptions) {
    ...
    if (
      !Object.prototype.hasOwnProperty.call(importOptions, 'returnImportedData')
    ) {
      ...
      _.each(importers, function (importer) {
        ops.push(async function doModelImport() {
          ...
          if (importOptions.returnImportedData) {
            importedData[importer.dataKeyToImport] =
              importer.importedDataToReturn;
          }
        });
      });
