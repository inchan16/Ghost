...
class ProductsImporter extends BaseImporter {
  ...
  fetchExisting(modelOptions) {
    return models.Product.findAll(
      _.merge({ columns: ['products.id as id', 'name', 'slug'] }, modelOptions)
    ).then((existingData) => {
  ...
  populatePriceData() {
    ...
    _.each(this.dataToImport, (row) => {
      ...
      if (!row.monthly_price || !row.currency) {
        const monthlyStripePrice =
          _.find(this.requiredFromFile.stripe_prices, {
            id: row.monthly_price_id,
          }) ||
          _.find(this.requiredExistingData.stripe_prices, {
            id: row.monthly_price_id,
          });
    ...
    this.dataToImport = this.dataToImport.filter(
      (item) => !invalidRows.includes(item.id)
    );
  }
  ...
  validateStripePrice() {
    ...
    _.each(['monthly_price_id', 'yearly_price_id'], (field) => {
      _.each(this.dataToImport, (objectInFile) => {
        ...
        const existingObject = _.find(this.requiredExistingData.stripe_prices, {
          id: objectInFile[field],
        });
    ...
    this.dataToImport = this.dataToImport.filter(
      (item) => !invalidProducts.includes(item.id)
    );
  }
  ...
  preventDuplicates() {
    ...
    this.dataToImport = this.dataToImport.filter(
      (item) => !duplicateProducts.includes(item.id)
    );
  }
