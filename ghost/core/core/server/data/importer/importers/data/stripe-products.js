...
class StripeProductsImporter extends BaseImporter {
  ...
  fetchExisting(modelOptions) {
    return models.StripeProduct.findAll(
      _.merge({ columns: ['id', 'stripe_product_id'] }, modelOptions)
    ).then((existingData) => {
  ...
  replaceIdentifiers() {
    ...
    _.each(this.dataToImport, (objectInFile) => {
      ...
      if (importedObject) {
        debug(
          `replaced product_id ${objectInFile.product_id} with ${importedObject.id}`
        );
        objectInFile.product_id = importedObject.id;
        return;
      }

      const existingObjectById = _.find(this.requiredExistingData.products, {
      ...
      const productFromFile = _.find(this.requiredFromFile.products, {
        id: objectInFile.product_id,
      });
      if (productFromFile) {
        ...
        if (existingObjectByNameAndSlug) {
          debug(
            `resolved ${objectInFile.product_id} to ${existingObjectByNameAndSlug.name}`
          );
