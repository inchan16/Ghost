...
class Base {
  constructor(allDataFromFile, options) {
    ...
    this.dataToImport = _.cloneDeep(
      allDataFromFile[this.dataKeyToImport] || []
    );
    this.originalIdMap = {};
  ...
  sanitizeValues() {
    _.each(this.dataToImport, (obj) => {
      _.each(
        _.pick(obj, ['updated_at', 'created_at', 'published_at']),
        (value, key) => {
          ...
          if (isNaN(temporaryDate)) {
            this.problems.push({
              message:
                'Date is in a wrong format and invalid. It was replaced with the current timestamp.',
              help: this.modelName,
    ...
    _.each(errs, (err) => {
      if (err.code && err.message.toLowerCase().indexOf('unique') !== -1) {
        ...
        if (this.errorConfig.showNotFoundWarning) {
          problems.push({
            message:
              'Entry was not imported and ignored. Could not find entry.',
            help: this.modelName,
            context: JSON.stringify(obj),
            err: err,
          });
  ...
  replaceIdentifiers() {
    ...
    const handleObject = (obj, key) => {
      ...
      let userFromFile = _.find(this.requiredFromFile.users, {
        id: obj[key],
      });

      if (!userFromFile) {
        // CASE: if user does not exist in file, try to lookup the existing db users
        let existingUser = _.find(this.requiredExistingData.users, {
          id: obj[key].toString(),
        });
      ...
      if (
        this.modelName === 'User' &&
        !this.requiredImportedData.users.length
      ) {
        let userToImport = _.find(this.dataToImport, {
          slug: userFromFile.slug,
        });
      ...
      let importedUser = _.find(this.requiredImportedData.users, {
      ...
      if (!existingUser) {
        ...
        existingUser = _.find(this.requiredExistingData.users, {
          id: userFromFile.id.toString(),
        });
    ...
    _.each(this.dataToImport, (obj) => {
      _.each(
        ['author_id', 'published_by', 'created_by', 'updated_by'],
        (key) => {
    ...
    _.each(this.dataToImport, (obj, index) => {
      ops.push(async () => {
        try {
          ...
          this.importedData.push(this.mapImportedData(obj, importedModel));
