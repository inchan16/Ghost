...
class SettingsImporter extends BaseImporter {
  ...
  beforeImport() {
    ...
    for (const key in deprecatedSupportedSettingsOneToManyMap) {
      const deprecatedSetting = this.dataToImport.find(
        (setting) => setting.key === key
      );

      // NOTE: filtering before mapping in case the key name is the same
      this.dataToImport = _.filter(
        this.dataToImport,
        (data) => key !== data.key
      );
      ...
      if (deprecatedSetting) {
        ...
        if (deprecatedSettingValue) {
          deprecatedSupportedSettingsOneToManyMap[key].forEach(
            ({ from, to }) => {
    ...
    if (
      oldIsPrivate &&
      newIsPrivate &&
      isFalse(oldIsPrivate.value) &&
      isTrue(newIsPrivate.value)
    ) {
      this.problems.push({
        message:
          'IMPORTANT: Content in this import was previously published on a private Ghost install, but the current site is public. Are your privacy settings up to date?',
        help: this.modelName,
  ...
  fetchExisting(modelOptions) {
    return models.Settings.findAll(modelOptions).then((existingData) => {
