...
class PostsImporter extends BaseImporter {
    ...
    _.each(this.dataToImport, (obj) => {
      ...
      if (_.has(obj, 'send_email_when_published')) {
        if (obj.send_email_when_published) {
          obj.email_recipient_filter =
            obj.visibility === 'paid' ? 'status:-free' : 'all';
  ...
  sanitizePostsMeta(model) {
    let postsMetaFromFile =
      _.find(this.requiredFromFile.posts_meta, { post_id: model.id }) ||
      _.pick(model, metaAttrs);
  ...
  addNestedRelations() {
    ...
    this.requiredFromFile.posts_authors = _.orderBy(
      this.requiredFromFile.posts_authors,
      ['post_id', 'sort_order'],
      ['asc', 'asc']
    );
    this.requiredFromFile.posts_products = _.orderBy(
      this.requiredFromFile.posts_products,
      ['post_id', 'sort_order'],
      ['asc', 'asc']
    );
    ...
    const run = (relations, target, fk) => {
      _.each(relations, (relation) => {
        ...
        let postToImport = _.find(this.dataToImport, {
          id: relation.post_id,
        });
  ...
  replaceIdentifiers() {
    ...
    const run = (postToImport, postIndex, targetProperty, tableName) => {
      if (
        ...
        !postToImport[targetProperty].length
      ) {
        return;
      }
      ...
      _.each(postToImport[targetProperty], (object, index) => {
        ...
        let objectInFile = _.find(this.requiredFromFile[tableName], {
          id: object.id,
        });
        ...
        if (!objectInFile) {
          ...
          if (!existingObject) {
            ...
            this.dataToImport[postIndex][targetProperty][index].id =
              existingObject.id;
            return;
          }
        }
        ...
        if (objectInFile.id) {
          importedObject = _.find(this.requiredImportedData[tableName], {
            originalId: objectInFile.id,
          });
        }
      ...
      this.dataToImport[postIndex][targetProperty] = _.filter(
      ...
      if (
        ...
        !this.dataToImport[postIndex][targetProperty].length
      ) {
        this.dataToImport[postIndex][targetProperty] = [
          {
            id: ownerUserId,
    ...
    _.each(this.dataToImport, (objectInFile) => {
      ...
      const importedObject = _.find(this.requiredImportedData.newsletters, {
        originalId: objectInFile.newsletter_id,
      });
      // CASE: we've imported the newsletter
      if (importedObject) {
        debug(
          `replaced newsletter_id ${objectInFile.newsletter_id} with ${importedObject.id}`
        );
        objectInFile.newsletter_id = importedObject.id;
        return;
      }
      const existingObject = _.find(this.requiredExistingData.newsletters, {
        id: objectInFile.newsletter_id,
      });
  ...
  beforeImport() {
    ...
    _.each(this.dataToImport, (model) => {
      ...
      if (model.mobiledoc) {
        ...
        model.html = mobiledocLib.mobiledocHtmlRenderer.render(
          JSON.parse(model.mobiledoc)
        );
      } else if (model.html) {
        model.mobiledoc = JSON.stringify(
          mobiledocLib.htmlToMobiledocConverter(model.html)
        );
        model.html = mobiledocLib.mobiledocHtmlRenderer.render(
          JSON.parse(model.mobiledoc)
        );
    ...
    this.dataToImport = _.filter(this.dataToImport, (post) => {
      if (!!post.slug && slugs.indexOf(post.slug) !== -1) {
        this.problems.push({
          message:
            'Entry was not imported and ignored. Detected duplicated entry.',
          help: this.modelName,
