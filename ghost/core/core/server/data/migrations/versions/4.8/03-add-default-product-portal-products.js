...
module.exports = createTransactionalMigration(
  async function up(knex) {
    ...
    if (products.length === 0) {
      logging.warn(
        `Skipping adding default product to portal_products, no default product exists`
      );
      return;
    }
    ...
    if (!portalProductsSetting) {
      logging.info(
        `Adding "portal_products" record to "settings" table with product - ${defaultProduct.id}`
      );
    ...
    logging.info(
      `Setting portal_products setting to have product - ${defaultProduct.id}`
    );
  ...
  async function down(knex) {
    ...
    if (!portalProductSetting) {
      logging.info(
        'Skipping revert of portal_products setting, setting does not exist'
      );
      return;
    }
