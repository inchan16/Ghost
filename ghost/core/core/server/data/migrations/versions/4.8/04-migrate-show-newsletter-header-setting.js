...
module.exports = createTransactionalMigration(
  async function up(connection) {
    ...
    if (!oldSetting) {
      logging.info(
        'Skipping migration of newsletter_show_header setting. Does not exist'
      );
      return;
    }
    ...
    if (oldSetting.value === 'false') {
      logging.info(
        'Migrating newsletter_show_header setting. Disabling title and logo'
      );

      const newSettingKeys = [
      ...
      logging.info(
        'Skipping migration of newsletter_show_header setting. Matches new defaults'
      );
    }

    logging.info('Deleting newsletter_show_header setting');
    await connection('settings').where('key', 'newsletter_show_header').del();
  },
  ...
  async function down(connection) {
    ...
    if (oldSetting) {
      logging.warn(
        'Skipping rollback of newsletter_show_header setting. Already exists'
      );
    } else {
      ...
      if (showTitle && showIcon) {
        showHeader =
          showTitle.value === 'false' && showIcon.value === 'false'
            ? 'false'
            : 'true';
      }
