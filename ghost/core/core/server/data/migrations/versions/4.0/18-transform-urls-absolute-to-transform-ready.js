...
module.exports = createIrreversibleMigration(async (knex) => {
  ...
  await knex.transaction(async (trx) => {
    ...
    for (const postIdRow of postIdRows) {
      ...
      const codeinjection_head = urlUtils.htmlToTransformReady(
        post.codeinjection_head
      );
      const codeinjection_foot = urlUtils.htmlToTransformReady(
        post.codeinjection_foot
      );
      const feature_image = urlUtils.toTransformReady(post.feature_image);
      const canonical_url = urlUtils.toTransformReady(post.canonical_url, {
    ...
    for (const userIdRow of userIdRows) {
      ...
      const [user] = await knex('users')
        .transacting(trx)
        .where({ id })
        .select(['profile_image', 'cover_image']);
    ...
    for (const tagIdRow of tagIdRows) {
      ...
      const codeinjection_head = urlUtils.htmlToTransformReady(
        tag.codeinjection_head
      );
      const codeinjection_foot = urlUtils.htmlToTransformReady(
        tag.codeinjection_foot
      );
      const canonical_url = urlUtils.toTransformReady(tag.canonical_url, {
        ignoreProtocol: false,
      });

      await knex('tags').transacting(trx).where({ id }).update({
        feature_image,
        og_image,
        twitter_image,
        codeinjection_head,
    ...
    for (const snippetIdRow of snippetIdRows) {
      ...
      await knex('snippets').transacting(trx).where({ id }).update({
        mobiledoc,
      });
    }
    ...
    for (const settingRow of settingsRows) {
      ...
      await knex('settings').transacting(trx).where({ key }).update({ value });
    }
