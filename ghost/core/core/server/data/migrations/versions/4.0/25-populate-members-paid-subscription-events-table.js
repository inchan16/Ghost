...
const messages = {
  unknownSubscriptionIntervalError:
    'Unknown Subscription interval "{interval}" found.',
};

module.exports = createTransactionalMigration(
  async function up(knex) {
    logging.info(
      'Populating members_paid_subscription_events from members_stripe_customers_subscriptions'
    );
    ...
    const allEvents = allSubscriptions.reduce((allEventsAcc, subscription) => {
      ...
      if (
        !['year', 'month', 'week', 'day'].includes(subscription.plan_interval)
      ) {
        return allEventsAcc;
      }
