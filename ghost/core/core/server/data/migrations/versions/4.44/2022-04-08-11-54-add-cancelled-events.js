...
module.exports = createTransactionalMigration(
  async function up(knex) {
    const cancelledSubscriptions = await knex
      ...
      .where(
        'members_stripe_customers_subscriptions.cancel_at_period_end',
        true
      )
  ...
  async function down(knex) {
    logging.info(
      'Deleting all members_paid_subscription_events with a "type" of "cancelled"'
    );
    await knex('members_paid_subscription_events')
      .where({ type: 'canceled', source: 'migration' })
      .del();
  }
);
