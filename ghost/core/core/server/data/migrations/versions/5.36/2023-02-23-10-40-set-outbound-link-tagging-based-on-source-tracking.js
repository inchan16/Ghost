...
module.exports = createTransactionalMigration(
  async function up(connection) {
    ...
    if (!reuseValueOfSetting) {
      logging.warn(
        `Skipped setting outbound_link_tagging to current value of members_track_sources - members_track_sources not found`
      );
      return;
    }
    ...
    if (affectedRows === 1) {
      logging.info(
        `Set outbound_link_tagging to ${reuseValueOfSetting.value} (current members_track_sources value)`
      );
    } else {
      logging.warn(
        `Tried setting outbound_link_tagging to ${reuseValueOfSetting.value} — ${affectedRows} changes`
      );
    }
  },
  async function down() {
