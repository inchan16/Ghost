...
module.exports = createTransactionalMigration(
  async function up(knex) {
    if (!DatabaseInfo.isMySQL(knex)) {
      logging.warn(
        'Skipping cleanup of duplicate offer redemptions - database is not MySQL'
      );
      return;
    }
    ...
    for (const duplicate of duplicates) {
      ...
      const [offerRedemptionToKeep, ...offerRedemptionsToDelete] =
        offerRedemptions;
      ...
      await knex('offer_redemptions')
        .whereIn(
          'id',
          offerRedemptionsToDelete.map((x) => x.id)
        )
        .del();
    }
  },
