...
module.exports = createTransactionalMigration(
  async function up(connection) {
    ...
    if (!currentSetting) {
      logging.info(
        'Skipping update of members_signup_access setting. Does not exist'
      );
      return;
    }
    ...
    if (currentSetting.value !== 'invite') {
      logging.info(
        `Skipping update of members_signup_access setting. Not set to 'invite', keeping current value`
      );
      return;
    }
    ...
    if (!hasStripe) {
      logging.info(
        'Skipping update of members_signup_access setting. Stripe is not configured, staying as invite-only'
      );
      return;
    }
    ...
    if (!portalPlansSetting) {
      ...
      logging.info(
        'Skipping update of members_signup_access setting. Portal plans setting does not exist'
      );
      return;
    }
    ...
    if (currentPlans.length === 0) {
      logging.info(
        'Skipping update of members_signup_access setting. Portal configured as invite-only, staying as invite-only'
      );
      return;
    }
    ...
    if (currentPlans.includes('free')) {
      ...
      logging.info(
        `Removing free plan from portal plans setting to match "allow free members signup = false" behaviour`
      );
      await connection('settings')
        .where('key', 'portal_plans')
  ...
  async function down(connection) {
    ...
    if (!accessSetting) {
      logging.info(
        'Skipping rollback of members_signup_access setting. Does not exist.'
      );
      return;
    }
    ...
    if (accessSetting.value !== 'all') {
      logging.info(
        `Skipping rollback of members_signup_access setting. Not set 'all', nothing to roll back`
      );
      return;
    }
    ...
    logging.info(
      `Skipping rollback of members_signup_access setting. Value did not match \`up\` requirements`
    );
  }
);
