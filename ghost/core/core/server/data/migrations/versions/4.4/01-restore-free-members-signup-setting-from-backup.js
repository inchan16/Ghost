...
module.exports = createTransactionalMigration(
  async function up(connection) {
    ...
    if (!currentSetting) {
      logging.info(
        'Skipping restore of "allow free signups", `members_signup_access` setting doesn\'t exist'
      );
      return;
    }
    ...
    if (currentSetting.value !== 'all') {
      logging.info(
        'Skipping restore of "allow free signups", setting changed since bad migration'
      );
      return;
    }
    ...
    const backupFileRegex =
      /ghost\.([\d]{4}-[\d]{2}-[\d]{2})-([\d]{2}-[\d]{2}-[\d]{2})\.json$/;

    const backupFileDate = function (filename) {
      const dateMatch = filename.match(backupFileRegex);
      return new Date(
        `${dateMatch[1]}T${dateMatch[2].replace(/-/g, ':')}.000Z`
      );
    };

    logging.info(
      'Restoring "allow free signups" value from backup files if needed'
    );
    ...
    for (const backupFile of backupFiles) {
      try {
        ...
        if (semver.satisfies(backup.meta.version, '4.3.x')) {
          const oldValue = backup.data.settings.find(
            (s) => s.key === 'members_allow_free_signup'
          );

          if (oldValue && oldValue.value === 'false') {
            logging.info(
              "Setting `members_signup_access` to 'invite' to match previous `members_allow_free_signup` setting"
            );
