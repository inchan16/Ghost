  ...
  async function up(knex) {
    ...
    const offerRedemptions = await knex
      .select(
        ...
        'o.discount_type',
        'o.discount_amount',
        'o.interval AS discount_interval',
        's.mrr AS mrr',
      ...
      .join(
        'members_stripe_customers_subscriptions AS s',
        'or.subscription_id',
        '=',
        's.id'
      )
    ...
    if (offerRedemptions.length === 0) {
      ...
      logging.info(
        `Adjusting MRR for ${offerRedemptions.length} Offer Redemptions`
      );
    }

    const memberIds = uniq(
      offerRedemptions.map((redemption) => redemption.member_id)
    );
    ...
    function storeEventOnMemberId(storage, event) {
      return {
        ...storage,
        [event.member_id]: storage[event.member_id]
          ? storage[event.member_id].concat(event)
          : [event],
      };
    }

    const mrrCreatedEventsByMemberId = mrrCreatedEvents.reduce(
      storeEventOnMemberId,
      {}
    );
    ...
    function calculateMRR(subscription, redemption) {
      if (
        redemption &&
        subscription.interval !== redemption.discount_interval
      ) {
      ...
      if (redemption.discount_type === 'percent') {
        return calculateMRR({
          ...
          amount:
            (subscription.amount * (100 - redemption.discount_amount)) / 100,
        });
      }
