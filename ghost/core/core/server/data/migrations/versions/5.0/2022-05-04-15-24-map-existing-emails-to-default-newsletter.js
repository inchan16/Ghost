...
module.exports = createTransactionalMigration(
  async function up(knex) {
    logging.info(
      'Linking existing emails and related posts to the default newsletter'
    );
    ...
    if (!newsletter) {
      ...
      logging.warn(
        `Original default newsletter not found - using first in sort order`
      );
    ...
    logging.info(
      `Assigning existing emails to newsletter ID ${newsletter.id} (${newsletter.slug})`
    );
    ...
    await knex('posts')
      ...
      .whereIn(
        'id',
        knex.raw(
          'SELECT post_id FROM emails WHERE emails.newsletter_id IS NULL'
        )
      );
