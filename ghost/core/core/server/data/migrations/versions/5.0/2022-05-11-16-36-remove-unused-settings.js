...
module.exports = createTransactionalMigration(
  async function up(knex) {
    ...
    const settingsInDatabase = settingsToRemove.filter((s) =>
      existingSettings.includes(s)
    );
    if (settingsInDatabase.length) {
      logging.info(`Deleting settings: ${settingsInDatabase.join(', ')}`);
      await knex('settings').whereIn('key', settingsInDatabase).del();
    }

    const settingsNotInDatabase = settingsToRemove.filter(
      (s) => !existingSettings.includes(s)
    );
    if (settingsNotInDatabase.length) {
      logging.warn(
        `Unable to delete missing settings: ${settingsNotInDatabase.join(', ')}`
      );
    }
  },
  async function down() {
