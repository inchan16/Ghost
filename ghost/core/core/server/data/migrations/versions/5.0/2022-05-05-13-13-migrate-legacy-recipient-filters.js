...
module.exports = createTransactionalMigration(
  async function up(transaction) {
    ...
    logging.info(
      `Migrated ${postsFreeCount} posts with 'email_recipient_filter' = 'free' to 'status:free'`
    );
    ...
    const postsPaidCount = await transaction('posts')
      ...
      .update('email_recipient_filter', 'status:-free');
    logging.info(
      `Migrated ${postsPaidCount} posts with 'email_recipient_filter' = 'paid' to 'status:-free'`
    );
    ...
    logging.info(
      `Migrated ${emailsFreeCount} emails with 'recipient_filter' = 'free' to 'status:free'`
    );
    ...
    const emailsPaidCount = await transaction('emails')
      ...
      .update('recipient_filter', 'status:-free');
    logging.info(
      `Migrated ${emailsPaidCount} emails with 'recipient_filter' = 'paid' to 'status:-free'`
    );
  },
  async function down() {
