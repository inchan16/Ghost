...
module.exports = createTransactionalMigration(
  async function up(knex) {
    const subscriptionsToUpdate = await knex(
      ...
      .select(
        's.*',
        'o.discount_type AS offer_type',
        'o.discount_amount AS offer_amount'
      );
    ...
    const toInsert = subscriptionsToUpdate.map((subscription) => {
      ...
      if (subscription.offer_type === 'percent') {
        discountedAmount =
          (subscription.plan_amount * (100 - subscription.offer_amount)) / 100;
      } else {
    ...
    logging.info(
      `Replacing ${toDelete.length} subscriptions with updated MRR based on Offers`
    );
