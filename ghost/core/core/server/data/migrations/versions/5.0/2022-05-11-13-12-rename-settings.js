  ...
  async function up(knex) {
    ...
    for (const renameMapping of renameMappings) {
      if (
        ...
        let updatedValue = _.find(settings, {
          key: renameMapping.from,
        }).value;
        // CASE: default settings were added already, update them with old values & remove old settings
        logging.info(
          `Updating ${renameMapping.to} with value from ${renameMapping.from}`
        );
        await knex('settings')
        ...
        await knex('settings').where('key', renameMapping.from).del();
        ...
        logging.warn(
          `Setting ${renameMapping.from} is missing, skipping update`
        );
      }
    }
  },
