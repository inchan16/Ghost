...
module.exports = createTransactionalMigration(
  async function up(knex) {
    ...
    const canceledSubscriptions = await knex(
    ...
    if (canceledSubscriptions.length === 0) {
      ...
      logging.info(
        `Found ${canceledSubscriptions.length} canceled subscriptions, updating MRR events`
      );
    }
    ...
    const canceledEvents = await knex('members_paid_subscription_events')
      ...
      .whereIn(
        'subscription_id',
        canceledSubscriptions.map((x) => x.id)
      )
    ...
    for (const subscription of canceledSubscriptions) {
      const event = canceledEvents.find(
    ...
    logging.info(
      `Inserting ${toInsert.length} MRR events for canceled subscriptions`
    );
    await knex.batchInsert('members_paid_subscription_events', toInsert);

    logging.info(
      `Updating ${toUpdate.length} MRR events for canceled subscriptions`
    );
