...
module.exports = createTransactionalMigration(
  async function up(knex) {
    ...
    if (!portalProductSetting) {
      logging.warn(
        'Could not find portal_products setting - skipping migration'
      );
      return;
    }
    ...
    try {
      ...
      logging.info(
        `Updating ${settingData.length} products to visible, ${settingData}`
      );
      await knex('products')
        .update('visibility', 'public')
        .whereIn('id', settingData);
    } catch (err) {
  ...
  async function down(knex) {
    const visibleTiers = await knex('products')
      .select('id')
