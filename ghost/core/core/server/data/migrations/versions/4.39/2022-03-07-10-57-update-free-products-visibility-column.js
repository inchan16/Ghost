...
module.exports = createTransactionalMigration(
  async function up(knex) {
    const portalPlanSetting = await knex('settings')
      .select('value')
    ...
    try {
      ...
      if (!settingData.includes('free')) {
        logging.warn(
          `portal_plans does not include "free" - skipping migration`
        );
        return;
      }
  ...
  async function down(knex) {
    const freeTier = await knex('products')
      .select('id')
      .where('type', 'free')
      .first();
    const portalPlanSetting = await knex('settings')
    ...
    try {
      ...
      if (freeTier.visibility === 'public') {
        if (existingSettingData.includes('free')) {
          logging.info(
            'portal_plans setting already contains "free" - skipping update'
          );
          return;
        } else {
        ...
        settingData = JSON.stringify(
          existingSettingData.filter((value) => value !== 'free')
        );
      }

      logging.info(`Updating portal_plans to ${settingData}`);
      await knex('settings')
        .update('value', settingData)
        .where('key', 'portal_plans');
    } catch (err) {
      logging.error(err);
      logging.warn('portal_plans setting is invalid - skipping migration');
