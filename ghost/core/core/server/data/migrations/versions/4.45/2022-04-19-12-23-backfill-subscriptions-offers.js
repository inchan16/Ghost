...
module.exports = createTransactionalMigration(
  async function up(knex) {
    logging.info(
      'Backfilling "offer_id" column in "members_stripe_customers_subscriptions" by matching tier and cadence'
    );
    ...
    if (DatabaseInfo.isSQLite(knex)) {
      ...
      logging.info(
        `Setting the offer_id for ${updatedRows} members_stripe_customers_subscriptions`
      );
      ...
      logging.info(
        `Updated ${updatedRows} members_stripe_customers_subscriptions with an offer_id`
      );
    }
  },
  async function down() {
