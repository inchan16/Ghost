...
module.exports = createTransactionalMigration(
  async function up(knex) {
    ...
    if (!role) {
      throw new InternalServerError(
        'Could not find "Self-Serve Migration Integration" Role'
      );
    }
    ...
    if (existingKey) {
      logging.warn(
        'Admin API key for "Self-Serve Migration Integration" already exists'
      );
      return;
    }
  ...
  async function down(knex) {
    ...
    if (!role || !integration) {
      logging.warn(
        'Could not delete "Self-Serve Migration Integration" API key'
      );
      return;
    }
