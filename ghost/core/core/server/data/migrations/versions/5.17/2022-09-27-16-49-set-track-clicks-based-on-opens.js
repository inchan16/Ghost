...
module.exports = createTransactionalMigration(
  async function up(connection) {
    ...
    if (!reuseValueOfSetting) {
      logging.warn(
        `Skipped setting email_track_clicks to current value of email_track_opens - email_track_opens not found`
      );
      return;
    }
    ...
    if (affectedRows === 1) {
      logging.info(
        `Set email_track_clicks to ${reuseValueOfSetting.value} (current email_track_opens value)`
      );
    } else {
      logging.warn(
        `Tried setting email_track_clicks to ${reuseValueOfSetting.value} — ${affectedRows} changes`
      );
    }
  },
  async function down() {
