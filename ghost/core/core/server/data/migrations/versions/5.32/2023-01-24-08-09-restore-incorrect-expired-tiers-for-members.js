...
module.exports = createTransactionalMigration(
  async function up(knex) {
    ...
    try {
      ...
      const memberWithTiers = await knex
        .select('m.id as member_id', 'mpe.product_id as product_id')
        .from('members as m')
        .leftJoin('members_products as mp', 'm.id', 'mp.member_id')
        .leftJoin('members_product_events as mpe', function () {
          this.on('m.id', 'mpe.member_id').andOn(
            knex.raw(
              'mpe.created_at = (SELECT max(created_at) FROM members_product_events WHERE member_id = mpe.member_id and action = "added")'
            )
          );
        })
        .where({
      ...
      const toInsert = memberWithTiers
        .map((memberTier) => {
          return {
            ...memberTier,
        ...
        .filter((memberTier) => {
          ...
          if (!memberTier.product_id) {
            logging.warn(
              `Invalid record found - member_id: ${memberTier.member_id} is without product_id`
            );
            return false;
          }
          return true;
      ...
      logging.info(
        `Inserting ${toInsert.length} records into members_products`
      );
      await knex.batchInsert('members_products', toInsert);
    } catch (err) {
      logging.warn(
        'Failed to restore member<>tier mapping for members with paid status'
      );
      logging.warn(err);
    }
  },
