...
module.exports = createTransactionalMigration(
  async function up(knex) {
    ...
    const eventsToUpdate = Object.keys(eventsByMember).reduce(
      (incorrectEvents, memberId) => {
        ...
        if (
          mostRecentStatusEvent &&
          mostRecentStatusEvent.to_status !== 'free'
        ) {
    ...
    logging.info(
      `Found ${eventsToUpdate.length} member status events that need updating`
    );
