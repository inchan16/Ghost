  ...
  async function up(knex) {
    ...
    const compedMemberIds = (
      await knex('members')
        ...
        .innerJoin('members_stripe_customers_subscriptions', function () {
          this.on(
            'members_stripe_customers.customer_id',
            'members_stripe_customers_subscriptions.customer_id'
          ).onIn('members_stripe_customers_subscriptions.status', [
            'active',
            'trialing',
            'past_due',
            'unpaid',
        ...
        .andWhere('members.status', '!=', 'comped')
    ).map(({ id }) => id);
    ...
    if (!compedMemberIds.length) {
      ...
      logging.info(
        `Found ${compedMemberIds.length} Complimentary members with the incorrect status`
      );
    }
    ...
    for (const memberId of compedMemberIds) {
      ...
      if (!mostRecentStatusEvent) {
        logging.warn(
          `Could not find a status event for member ${memberId} - skipping this member`
        );
      } else if (mostRecentStatusEvent.to_status !== 'comped') {
        logging.info(
          `Updating members_status_event ${mostRecentStatusEvent.id}`
        );
