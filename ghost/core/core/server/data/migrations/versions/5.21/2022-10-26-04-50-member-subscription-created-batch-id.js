...
module.exports = createTransactionalMigration(
  async function up(knex) {
    if (DatabaseInfo.isSQLite(knex)) {
      logging.info(
        'Skipped linking members_created_events and members_subscription_created_events on SQLite'
      );
      return;
    }
    ...
    const rows = await knex('members_created_events as m')
      ...
      .join(
        'members_subscription_created_events AS s',
        's.member_id',
        'm.member_id'
      )
      .whereRaw(
        'TIMESTAMPDIFF(MINUTE, s.created_at, m.created_at) between -15 and 15'
      );

    if (!rows.length) {
      logging.info(
        'Did not find linkable members_created_events and members_subscription_created_events'
      );
      return;
    }
    ...
    for (const batch of batches) {
      ...
      const response1 = await knex('members_created_events')
        .insert(
          batch.map((r) => {
            return {
              id: r.m_id,
      ...
      if (response1[0] !== 0) {
        logging.error(
          `Inserted ${response1[0]} members_created_events, expected 0`
        );
        throw new Error('Rolling back');
      }

      const response2 = await knex('members_subscription_created_events')
        .insert(
          batch.map((r) => {
            return {
              id: r.s_id,
      ...
      if (response2[0] !== 0) {
        logging.error(
          `Inserted ${response1[0]} members_subscription_created_events, expected 0`
        );
        throw new Error('Rolling back');
      }
    }
    logging.info(
      `Linked ${rows.length} members_created_events and members_subscription_created_events`
    );
  },
  async function down() {
    // noop
