...
module.exports = createTransactionalMigration(
  async function up(knex) {
    ...
    const product = await knex.select('id', 'name').from('products').first();
    ...
    logging.info(
      `Attaching product ${product.name} to ${membersWithProduct.length} members`
    );
    ...
    const chunkSize = 333;
    const memberProductRelationsChunks = chunk(
      memberProductRelations,
      chunkSize
    );
