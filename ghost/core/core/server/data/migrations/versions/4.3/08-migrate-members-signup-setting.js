...
module.exports = createTransactionalMigration(
  async function up(connection) {
    ...
    if (!oldSetting) {
      logging.warn(
        'Could not find setting `members_allow_free_signup`, skipping update of `members_signup_access` setting'
      );
      return;
    }
    ...
    if (newSetting) {
      ...
      logging.info(
        'Updating `members_signup_access` setting with value from `members_allow_free_signup`'
      );
      ...
      logging.info(
        'Creating `members_signup_access` setting with value from `members_allow_free_signup`'
      );
  ...
  async function down(connection) {
    ...
    if (!newSetting) {
      logging.warn(
        'Could not find setting `members_signup_access`, skipping rollback of `members_allow_free_signup` setting'
      );
      return;
    }
    ...
    if (oldSetting) {
      logging.info(
        'Updating `members_allow_free_signup` based on value from `members_signup_access`'
      );
      await connection('settings')
        .where('key', 'members_allow_free_signup')
      ...
      logging.info(
        'Creating `members_allow_free_signup` based on value from `members_signup_access`'
      );
    ...
    await connection('settings').where('key', 'members_signup_access').del();
  }
);
