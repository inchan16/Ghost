  ...
  async function up(knex) {
    ...
    try {
      ...
      if (currPortalProductsValue.length > 0) {
        logging.warn(
          `Ignoring - portal_products setting is not empty, - ${currPortalProductsValue}`
        );
        return;
      }
      ...
      logging.warn(
        `Ignoring, unable to parse portal_products setting value - ${portalProductsSetting.value}`
      );
      logging.warn(e);
    }
  },
