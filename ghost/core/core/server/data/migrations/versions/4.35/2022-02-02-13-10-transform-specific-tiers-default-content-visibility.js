...
module.exports = createTransactionalMigration(
  async function up(knex) {
    ...
    const contentVisibilitySetting = settings.find(
      (d) => d.key === 'default_content_visibility'
    );
    const visibilityTiersSetting = settings.find(
      (d) => d.key === 'default_content_visibility_tiers'
    );
    ...
    if (['public', 'members', 'paid'].includes(contentVisibility)) {
      logging.info(
        `Ignoring default_content_visibility change as already set to ${contentVisibility}.`
      );
      return;
    }
    // Transform visibility to tiers when stored as nql string
    const isValidProductNqlFilter = /^(?:product:[\w-]+,?)+$/.test(
      contentVisibility
    );
    ...
    if (!isValidProductNqlFilter) {
      logging.warn(
        `Found invalid default_content_visibility value - ${contentVisibility}, resetting to paid`
      );
      await knex('settings')
        .where({
  ...
  async function down(knex) {
    ...
    const visibilityValue =
      contentVisibilitySetting && contentVisibilitySetting.value;
    const visibilityTiersValue =
      visibilityTiersSetting && visibilityTiersSetting.value;

    if (visibilityValue !== 'tiers') {
      logging.info(
        `Ignoring default_content_visibility as is set to ${visibilityValue}.`
      );
      return;
    }
    ...
    if (!visibilityTiersValue) {
      logging.warn(
        `Ignoring, found empty default_content_visibility_tiers value`
      );
      return;
    }
    ...
    try {
      ...
      logging.warn(
        `Invalid default_content_visibility_tiers value - ${visibilityTiersValue}`
      );
      logging.warn(e);
      return;
    }
