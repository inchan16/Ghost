...
module.exports = createTransactionalMigration(
  async function up(knex) {
    ...
    if (paidSignupRedirect) {
      logging.info(
        `Setting paid Tiers welcome_page_url to ${paidSignupRedirect.value}`
      );
    ...
    if (freeSignupRedirect) {
      logging.info(
        `Setting free Tiers welcome_page_url to ${freeSignupRedirect.value}`
      );
  ...
  async function down(knex) {
    ...
    await knex('products').update('welcome_page_url', null);
  }
);
