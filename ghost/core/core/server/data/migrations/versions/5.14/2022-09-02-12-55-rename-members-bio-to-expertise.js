...
module.exports = createNonTransactionalMigration(
  async function up(knex) {
    ...
    if (!hasBio || hasExpertise) {
      logging.warn(
        `Database is in the wrong state - skipping rename (bio=${hasBio}, expertise=${hasExpertise})`
      );
      return;
    }
    ...
    if (DatabaseInfo.isMySQL(knex)) {
      await knex.raw(
        `ALTER TABLE members CHANGE bio expertise VARCHAR(191) NULL`
      );
      return;
    }
  ...
  async function down(knex) {
    ...
    if (DatabaseInfo.isMySQL(knex)) {
      await knex.raw(
        `ALTER TABLE members CHANGE expertise bio VARCHAR(191) NULL`
      );
      return;
    }
