...
const messages = {
  ...
  onlyOneRolePerUserSupported:
    'Only one role per user is supported at the moment.',
  methodDoesNotSupportOwnerRole:
    'This method does not support assigning the owner role',
  invalidRoleValue: {
  ...
  onlyOwnerCanTransferOwnerRole:
    'Only owners are able to transfer the owner role.',
  onlyAdmCanBeAssignedOwnerRole:
    'Only administrators can be assigned the owner role.',
  onlyActiveAdmCanBeAssignedOwnerRole:
    'Only active administrators can be assigned the owner role.',
  userUpdateError: {
...
User = ghostBookshelf.Model.extend(
  {
    ...
    onCreated: function onCreated(model, options) {
      ...
      if (
        !model.get('status') ||
        _.includes(activeStates, model.get('status'))
      ) {
    ...
    extraFilters: function extraFilters(options) {
      ...
      if (options.status !== 'all') {
        options.status =
          allStates.indexOf(options.status) > -1 ? options.status : 'active';
      }
  ...
  },
  {
    ...
    permittedOptions: function permittedOptions(methodName, options) {
      let permittedOptionsToReturn = ghostBookshelf.Model.permittedOptions.call(
        this,
        methodName
      );
      ...
      if (options && options.context && options.context.public) {
        if (
          options.withRelated &&
          options.withRelated.indexOf('roles') !== -1
        ) {
    ...
    getEmailAlertUsers(type, options) {
      ...
      const updatedOptions = _.merge({}, options, {
        filter,
        withRelated: ['roles'],
      });
    ...
    isSetup: function isSetup(options) {
      return this.getOwnerUser(options).then(function (owner) {
    ...
    permissible: async function permissible(
      ...
      if (
        _.isObject(userModelOrId) &&
        !_.isObject(userModelOrId.related('roles'))
      ) {
      ...
      if (action === 'edit') {
        ...
        if (context.user === userModel.get('id')) {
          ...
          hasUserPermission =
            userModel.hasRole('Author') || userModel.hasRole('Contributor');
        }
      }
      ...
      if (action === 'destroy') {
        ...
        if (
          ...
          hasUserPermission =
            context.user === userModel.get('id') ||
            userModel.hasRole('Author') ||
            userModel.hasRole('Contributor');
        }
      }
    ...
    transferOwnership: function transferOwnership(object, unfilteredOptions) {
      const options = ghostBookshelf.Model.filterOptions(
        unfilteredOptions,
        'transferOwnership'
      );
      let ownerRole;
