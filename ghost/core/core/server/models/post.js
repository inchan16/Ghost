...
const messages = {
  isAlreadyPublished:
    'Your post is already published, please reload your page.',
  valueCannotBeBlank: 'Value in {key} cannot be blank.',
  expectedPublishedAtInFuture:
...
Post = ghostBookshelf.Model.extend(
  {
    ...
    defaults: function defaults() {
      ...
      let tiers = [];
      const defaultContentVisibility = settingsCache.get(
        'default_content_visibility'
      );
      if (defaultContentVisibility) {
        if (defaultContentVisibility === 'tiers') {
          const tiersData =
            settingsCache.get('default_content_visibility_tiers') || [];
          (visibility = 'tiers'),
    ...
    formatOnWrite(attrs) {
      ...
      Object.entries(urlTransformMap).forEach(
        ([attrToTransform, transform]) => {
          ...
          if (attrs[attrToTransform]) {
            attrs[attrToTransform] = urlUtils[method](
              attrs[attrToTransform],
              transformOptions
            );
          }
        }
      ...
      if (
        attrs.visibility &&
        !['public', 'members', 'paid', 'tiers'].includes(attrs.visibility)
      ) {
        if (attrs.visibility === 'status:-free') {
          ...
          if (
            visibilityNql.queryJSON({ status: 'free' }) &&
            visibilityNql.queryJSON({ status: '-free' })
          ) {
    ...
    permittedAttributes: function permittedAttributes() {
      let filteredKeys =
        ghostBookshelf.Model.prototype.permittedAttributes.apply(
          this,
          arguments
        );
    ...
    orderAttributes: function orderAttributes() {
      let keys = ghostBookshelf.Model.prototype.orderAttributes.apply(
        this,
        arguments
      );
    ...
    onUpdated: function onUpdated(model, options) {
      ...
      if (model.resourceTypeChanging) {
        if (model.wasPublished) {
          model.emitChange(
            'unpublished',
            Object.assign({ usePreviousAttribute: true }, options)
          );
        }
    ...
    handleAttachedModels: function handleAttachedModels(model) {
      ...
      model
        .related('tags')
        .once('detaching', function detachingTags(collection, tag) {
          model
            .related('tags')
            .once(
              'detached',
              function detachedTags(detachedCollection, response, options) {
      ...
      model
        .related('tags')
        .once('attaching', function tagsAttaching(collection, tags) {
          model
            .related('tags')
            .once(
              'attached',
              function tagsAttached(detachedCollection, response, options) {
                tags.forEach((tag) => {
      ...
      model
        .related('authors')
        .once('detaching', function authorsDetaching(collection, author) {
          model
            .related('authors')
            .once(
              'detached',
              function authorsDetached(detachedCollection, response, options) {
      ...
      model
        .related('authors')
        .once('attaching', function authorsAttaching(collection, authors) {
          model
            .related('authors')
            .once(
              'attached',
              function authorsAttached(detachedCollection, response, options) {
     ...
     * these models
     */
    handleStatusForAttachedModels: function handleStatusForAttachedModels(
      model,
      options
    ) {
    ...
    onSaving: async function onSaving(model, attrs, options) {
      ...
      const publishedAtHasChanged = this.hasDateChanged('published_at', {
        beforeWrite: true,
      });
      ...
      if (
        newStatus !== olderStatus &&
        newStatus === 'scheduled' &&
        olderStatus === 'published'
      ) {
        return Promise.reject(
          new errors.ValidationError({
            message: tpl(messages.isAlreadyPublished, {
              key: 'status',
            }),
          })
        );
      ...
      if (!_.isUndefined(this.get('tags')) && !_.isNull(this.get('tags'))) {
        ...
        loopTags: for (const tag of this.get('tags')) {
          if (!tag.id && !tag.tag_id && tag.slug) {
            ...
            tag.slug = await ghostBookshelf.Model.generateSlug(Tag, tag.slug, {
          ...
          for (i = 0; i < tagsToSave.length; i = i + 1) {
            if (
              tagsToSave[i].name &&
              tag.name &&
              tagsToSave[i].name.toLocaleLowerCase() ===
       ...
       */
      if (
        !_.isUndefined(this.get('posts_meta')) &&
        !_.isNull(this.get('posts_meta'))
      ) {
      ...
      if (options.force_rerender) {
        this.set(
          'mobiledoc',
          await mobiledocLib.populateImageSizes(this.get('mobiledoc'))
        );
      }
      ...
      if (
        ...
        try {
          this.set(
            'html',
            mobiledocLib.mobiledocHtmlRenderer.render(
              JSON.parse(this.get('mobiledoc'))
            )
          );
        } catch (err) {
      ...
      if (
        (newStatus === 'published' || newStatus === 'sent') &&
        ...
        if (this.hasChanged('published_by') && !options.importing) {
          this.set(
            'published_by',
            this.previous('published_by')
              ? String(this.previous('published_by'))
              : null
          );
        }
      }
      ...
      const hasEmailOnlyFlag =
        _.get(attrs, 'posts_meta.email_only') ||
        model.related('posts_meta').get('email_only');
      if (
        hasEmailOnlyFlag &&
        newStatus === 'published' &&
        this.hasChanged('status')
          ...
          return ghostBookshelf
            ...
            .then((revisions) => {
              ...
              if (!revisions.length && options.method !== 'insert') {
                ...
                const revisionsJSON = revisions
                  .toJSON()
                  .slice(0, MOBILEDOC_REVISIONS_COUNT - 1);

                model.set(
                  'mobiledoc_revisions',
                  revisionsJSON.concat([
                    {
                      post_id: model.id,
                      mobiledoc: model.get('mobiledoc'),
                      created_at_ts: Date.now(),
      ...
      if (!labs.isSet('postHistory')) {
            ...
            return ghostBookshelf
              ...
              .then((revisions) => {
                ...
                if (!revisions.length && options.method !== 'insert') {
                  ...
                  const revisionsJSON = revisions
                    .toJSON()
                    .slice(0, POST_REVISIONS_COUNT - 1);
        ...
        if (
          ...
          ops.push(async function updateRevisions() {
            const revisionModels = await ghostBookshelf
              .model('PostRevision')
              .findAll(
                Object.assign(
                  {
            ...
            const newRevisions = await postRevisions.getRevisions(
              previous,
              current,
              revisions
            );
    ...
    wasChanged() {
      ...
      const postMetaChanged =
        this.relations.posts_meta &&
        this.relations.posts_meta._changed &&
        Object.keys(this.relations.posts_meta._changed).length;
    ...
    enforcedFilters: function enforcedFilters(options) {
      return options.context && options.context.public
        ? 'status:published'
        : null;
    },
    ...
    extraFilters: function extraFilters(options) {
      ...
      if (options.status && options.status !== 'all') {
        options.status = _.includes(ALL_STATUSES, options.status)
          ? options.status
          : 'published';
  ...
  },
  {
    ...
    orderDefaultRaw: function (options) {
      ...
      if (
        options.filter &&
        options.filter.match(/(authors|author):\s?\[.*\]/)
      ) {
    ...
    permittedOptions: function permittedOptions(methodName) {
      let options = ghostBookshelf.Model.permittedOptions.call(
        this,
        methodName
      );
    ...
    defaultRelations: function defaultRelations(methodName, options) {
      if (['edit', 'add', 'destroy'].indexOf(methodName) !== -1) {
        options.withRelated = _.union(
          ['authors', 'tags', 'post_revisions', 'post_revisions.author'],
          options.withRelated || []
        );
      }

      const META_ATTRIBUTES = _.without(
        ghostBookshelf.model('PostsMeta').prototype.permittedAttributes(),
        'id',
        'post_id'
      );
      ...
      if (
        !options.columns ||
        (options.columns &&
          _.intersection(META_ATTRIBUTES, options.columns).length)
      ) {
        options.withRelated = _.union(
          ['posts_meta'],
          options.withRelated || []
    ...
    filterData: function filterData(data) {
      const filteredData = ghostBookshelf.Model.filterData.apply(
        this,
        arguments
      );
    ...
    edit: function edit(data, unfilteredOptions) {
      ...
      const editPost = () => {
        ...
        return ghostBookshelf.Model.edit
            ...
            return this.findOne(
              ...
              if (found) {
                ...
                if (found.relations.posts_meta) {
                  found.relations.posts_meta._changed =
                    post.relations.posts_meta._changed;
                }
    ...
    add: function add(data, unfilteredOptions) {
      ...
      const addPost = () => {
        return ghostBookshelf.Model.add
          .call(this, data, options)
          .then((post) => {
            return this.findOne(
              {
    ...
    permissible: async function permissible(
      postModel,
      action,
      context,
      unsafeAttrs,
      ...
      isContributor =
        loadedPermissions.user &&
        _.some(loadedPermissions.user.roles, { name: 'Contributor' });
      isOwner =
        loadedPermissions.user &&
        _.some(loadedPermissions.user.roles, { name: 'Owner' });
      isAdmin =
        loadedPermissions.user &&
        _.some(loadedPermissions.user.roles, { name: 'Administrator' });
      isEditor =
        loadedPermissions.user &&
        _.some(loadedPermissions.user.roles, { name: 'Editor' });
      isIntegration =
        loadedPermissions.apiKey &&
        _.some(loadedPermissions.apiKey.roles, {
      ...
      if (limitService.isLimited('members')) {
        ...
        if (
          (isEdit && isChanging('status') && isDraft()) ||
          (isAdd && isPublished())
        ) {
        ...
        clicks(modelOrCollection) {
          modelOrCollection.query('columns', 'posts.*', (qb) => {
            qb.countDistinct('members_click_events.member_id')
              ...
              .join(
                'redirects',
                'members_click_events.redirect_id',
                'redirects.id'
              )
        ...
        negative_feedback(modelOrCollection) {
          modelOrCollection.query('columns', 'posts.*', (qb) => {
            qb.count('*')
              .from('members_feedback')
              .whereRaw(
                'posts.id = members_feedback.post_id AND members_feedback.score = 0'
              )
