...
const MemberPaidSubscriptionEvent = ghostBookshelf.Model.extend(
      ...
      if (options.aggregateMRRDeltas) {
        if (options.limit || options.filter) {
          throw new errors.IncorrectUsageError({
            message:
              'aggregateMRRDeltas does not work when passed a filter or limit',
          });
        }
  ...
  },
  {
    permittedOptions(methodName) {
      const options = ghostBookshelf.Model.permittedOptions.call(
        this,
        methodName
      );
    ...
    async edit() {
      throw new errors.IncorrectUsageError({
        message: 'Cannot edit MemberPaidSubscriptionEvent',
      });
    },

    async destroy() {
      throw new errors.IncorrectUsageError({
        message: 'Cannot destroy MemberPaidSubscriptionEvent',
      });
    },
  }
);
...
module.exports = {
  MemberPaidSubscriptionEvent: ghostBookshelf.model(
    'MemberPaidSubscriptionEvent',
    MemberPaidSubscriptionEvent
  ),
  MemberPaidSubscriptionEvents: ghostBookshelf.collection(
    'MemberPaidSubscriptionEvents',
    MemberPaidSubscriptionEvents
  ),
};
