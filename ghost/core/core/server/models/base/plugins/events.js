  ...
  Bookshelf.Model = Bookshelf.Model.extend({
    ...
    onCreating: function onCreating(model, attr, options) {
      if (
        ...
        if (
          !options.importing ||
          (options.importing && !this.get('created_by'))
        ) {
      ...
      return Promise.resolve(this.onValidate(model, attr, options)).then(() => {
        /**
         * @NOTE:
         *
         ...
         */
        _.each(
          Object.keys(schema.tables[this.tableName]).filter(
            (key) => key.indexOf('@@') === -1
          ),
          (columnKey) => {
    ...
    onUpdating: function onUpdating(model, attr, options) {
      ...
      if (
        ...
        if (
          ...
          if (
            model.hasDateChanged('created_at', {
              beforeWrite: true,
            })
          ) {
      ...
      if (
        ...
        if (options.migrating) {
          ...
          Object.keys(model.changed).length === 1 &&
          model.changed.updated_at
        ) {
          model.set('updated_at', model.previous('updated_at'));
