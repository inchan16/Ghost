  ...
  Bookshelf.Model = Bookshelf.Model.extend(
    ...
    },
    {
      ...
      findPage: async function findPage(unfilteredOptions) {
        ...
        itemCollection.debug =
          unfilteredOptions.debug && process.env.NODE_ENV !== 'production';
        ...
        if (options.columns) {
          ...
          options.columns = _.union(
            options.columns,
            this.prototype.defaultColumnsToFetch()
          );
        }

        if (options.order) {
          const { order, orderRaw, eagerLoad } =
            itemCollection.parseOrderOption(options.order, options.withRelated);
        ...
        const data = response.collection.models.map((model) => {
          if (requestedColumns) {
            ...
            model._previousAttributes = _.pick(
