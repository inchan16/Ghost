...
const Comment = ghostBookshelf.Model.extend(
  ...
  },
  {
    destroy: function destroy(unfilteredOptions) {
      ...
      const softDelete = () => {
        return ghostBookshelf.Model.edit.call(
          this,
          { status: 'deleted' },
          options
        );
    ...
    async permissible(
      ...
      if (
        action === 'edit' &&
        commentModelOrId.get('member_id') !== context.member.id
      ) {
    ...
    defaultRelations: function defaultRelations(methodName, options) {
      ...
      if (
        ['findAll', 'findPage', 'edit', 'findOne', 'destroy'].indexOf(
          methodName
        ) !== -1
      ) {
    ...
    async findPage(options) {
      ...
      for (const model of result.data) {
        await model.load(
          relationsToLoadIndividually,
          _.omit(options, 'withRelated')
        );
      }
