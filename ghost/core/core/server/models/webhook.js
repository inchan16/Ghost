...
Webhook = ghostBookshelf.Model.extend(
  ...
  },
  {
    ...
    getByEventAndTarget: function getByEventAndTarget(
      event,
      targetUrl,
      unfilteredOptions
    ) {
      ...
      return Webhooks.forge()
        .fetch(options)
        .then(function then(webhooks) {
          const webhookWithEventAndTarget = webhooks.find(function findWebhook(
            webhook
          ) {
            return (
              webhook.get('event').toLowerCase() === event.toLowerCase() &&
              webhook.get('target_url').toLowerCase() ===
