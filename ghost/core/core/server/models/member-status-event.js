...
const MemberStatusEvent = ghostBookshelf.Model.extend(
      ...
      if (options.aggregateStatusCounts) {
        if (options.limit || options.filter) {
          throw new errors.IncorrectUsageError({
            message:
              'aggregateStatusCounts does not work when passed a filter or limit',
          });
        }
  ...
  },
  {
    ...
    async edit() {
      throw new errors.IncorrectUsageError({
        message: 'Cannot edit MemberStatusEvent',
      });
    },

    async destroy() {
      throw new errors.IncorrectUsageError({
        message: 'Cannot destroy MemberStatusEvent',
      });
    },
  }
);
...
module.exports = {
  MemberStatusEvent: ghostBookshelf.model(
    'MemberStatusEvent',
    MemberStatusEvent
  ),
  MemberStatusEvents: ghostBookshelf.collection(
    'MemberStatusEvents',
    MemberStatusEvents
  ),
};
