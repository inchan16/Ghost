...
const Member = ghostBookshelf.Model.extend(
  {
    ...
    productEvents() {
      return this.hasMany('MemberProductEvent', 'member_id', 'id').query(
        'orderBy',
        'created_at',
        'DESC'
      );
    ...
    offerRedemptions() {
      return this.hasMany('OfferRedemption', 'member_id', 'id').query(
        'orderBy',
        'created_at',
        'DESC'
      );
    ...
    async updateTierExpiry(products = [], options = {}) {
      for (const product of products) {
        if (product?.id) {
          ...
          await this.products().updatePivot(
            { expiry_at: expiry },
            queryOptions
          );
        }
    ...
    serialize(options) {
      const defaultSerializedObject =
        ghostBookshelf.Model.prototype.serialize.call(this, options);

      if (defaultSerializedObject.stripeSubscriptions) {
        defaultSerializedObject.subscriptions =
          defaultSerializedObject.stripeSubscriptions;
        ...
        _.each(this.get('labels'), function each(item) {
          ...
          for (let i = 0; i < labelsToSave.length; i = i + 1) {
            if (
              labelsToSave[i].name &&
              item.name &&
              labelsToSave[i].name.toLocaleLowerCase() ===
      ...
      return ghostBookshelf
        ...
        .then((labels) => {
          labelsToSave.forEach((label) => {
            ...
            label.name =
              (existingLabel && existingLabel.get('name')) || label.name;
    ...
    handleAttachedModels: function handleAttachedModels(model) {
      ...
      model
        .related('labels')
        .once('detaching', function onDetaching(collection, label) {
          model
            .related('labels')
            .once(
              'detached',
              function onDetached(detachedCollection, response, options) {
      ...
      model
        .related('labels')
        .once('attaching', function onDetaching(collection, labels) {
          model
            .related('labels')
            .once(
              'attached',
    ...
    toJSON(unfilteredOptions) {
      ...
      if (attrs.email && !config.isPrivacyDisabled('useGravatar')) {
        attrs.avatar_image = gravatar.url(attrs.email, {
          size: 250,
          default: 'blank',
        });
      }
  ...
  },
  {
    ...
    add(data, unfilteredOptions = {}) {
      if (!unfilteredOptions.transacting) {
        return ghostBookshelf.transaction((transacting) => {
          return this.add(
            data,
            Object.assign({ transacting }, unfilteredOptions)
          );
        });
      }

      return ghostBookshelf.Model.add
        .call(this, data, unfilteredOptions)
        .then(async (member) => {
          if (data.products) {
            await member.updateTierExpiry(
              data.products,
              _.pick(unfilteredOptions, 'transacting')
            );
          }
          return member;
        });
    ...
    fetchAllSubscribed(unfilteredOptions = {}) {
      ...
      const query = ghostBookshelf
        ...
        .join(
          'newsletters',
          'members_newsletters.newsletter_id',
          '=',
          'newsletters.id'
        )
