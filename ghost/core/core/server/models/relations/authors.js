  ...
  const Model = Post.extend(
    {
      _handleOptions: function _handleOptions(fnName) {
        ...
        return function innerHandleOptions(model, attrs, options) {
          model._originalOptions = _.cloneDeep(
            _.pick(options, ['withRelated'])
          );
          ...
          if (options.withRelated.indexOf('author') !== -1) {
            options.withRelated.splice(
              options.withRelated.indexOf('author'),
              1
            );
      ...
      onFetchingCollection: function onFetchingCollection(
        collection,
        attrs,
        options
      ) {
        return this._handleOptions('onFetchingCollection')(
          collection,
          attrs,
          options
        );
      ...
      onFetchedCollection: function (collection, attrs, options) {
        ...
        return proto.onFetchingCollection.call(
          this,
          collection,
          attrs,
          options
            ...
            authors.map((author, index) => {
              ...
              return ghostBookshelf
                ...
                .then((user) => {
                  ...
                  const userExists = _.find(authorsToSet, {
                    id: userId.id,
                  });
    ...
    },
    {
      ...
      reassignByAuthor: async function reassignByAuthor(unfilteredOptions) {
        ...
        const reassignPost = async () => {
          ...
          try {
            ...
            const authorsPrimaryPosts = authorsPosts.filter(
              (ap) => ap.sort_order === 0
            );
            const primaryPostsWithOwnerCoauthor = _.intersectionBy(
              authorsPrimaryPosts,
              ownersPosts,
              'post_id'
            );
            const primaryPostsWithOwnerCoauthorIds =
              primaryPostsWithOwnerCoauthor.map((post) => post.post_id);
            ...
            const postsWithoutOwnerCoauthorIds =
              primaryPostsWithoutOwnerCoauthor.map((post) => post.post_id);
      ...
      permissible: function permissible(
        ...
        isContributor =
          ...
          _.some(loadedPermissions.user.roles, {
            name: 'Contributor',
          });
        isAuthor =
          loadedPermissions.user &&
          _.some(loadedPermissions.user.roles, { name: 'Author' });
        isEdit = action === 'edit';
        isAdd = action === 'add';
