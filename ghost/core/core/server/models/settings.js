...
function parseDefaultSettings() {
  const defaultSettingsInCategories =
    require('../data/schema/').defaultSettings;
...
Settings = ghostBookshelf.Model.extend(
  {
    ...
    onDestroyed: function onDestroyed(model, options) {
      ...
      model.emitChange(
        model._previousAttributes.key + '.' + 'deleted',
        options
      );
    },
    ...
    async onValidate(model, attr, options) {
      await ghostBookshelf.Model.prototype.onValidate.call(
        this,
        model,
        attr,
        options
    ...
    format() {
      const attrs = ghostBookshelf.Model.prototype.format.apply(
        this,
        arguments
      );
    ...
    parse() {
      ...
      if (
        settingType === 'boolean' &&
        (attrs.value === 'false' || attrs.value === 'true')
      ) {
        attrs.value = JSON.parse(attrs.value);
      }
  ...
  },
  {
    findOne: function (data, options) {
      ...
      return Promise.resolve(
        ghostBookshelf.Model.findOne.call(this, data, options)
      );
    },
          ...
          .then(function then(setting) {
            ...
            return Promise.reject(
              new errors.NotFoundError({
                message: tpl(messages.unableToFindSetting, {
                  key: item.key,
                }),
              })
            );
    ...
    populateDefaults: async function populateDefaults(unfilteredOptions) {
      ...
      _.each(
        getDefaultSettings(),
        function forEachDefault(defaultSetting, defaultSettingKey) {
      ...
      if (settingsToInsert.length > 0) {
        ...
        const columnInfo = await ghostBookshelf.knex
          .table('settings')
          .columnInfo();
