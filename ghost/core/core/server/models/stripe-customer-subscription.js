    ...
    serialize(options) {
      ...
      const serialized = {
        ...
        customer: {
          ...
          name: defaultSerializedObject.customer
            ? defaultSerializedObject.customer.name
            : null,
          email: defaultSerializedObject.customer
            ? defaultSerializedObject.customer.email
            : null,
        },
        plan: {
          ...
          currency: String.prototype.toUpperCase.call(
            defaultSerializedObject.plan_currency
          ),
        },
        status: defaultSerializedObject.status,
        start_date: defaultSerializedObject.start_date,
        default_payment_card_last4:
          defaultSerializedObject.default_payment_card_last4,
      ...
      if (!_.isEmpty(defaultSerializedObject.stripePrice)) {
        serialized.price = {
          ...
          currency: String.prototype.toUpperCase.call(
            defaultSerializedObject.stripePrice.currency
          ),
        };

        if (defaultSerializedObject.stripePrice.stripeProduct) {
          const productData =
            defaultSerializedObject.stripePrice.stripeProduct.product || {};
          serialized.price.product = {
            id: defaultSerializedObject.stripePrice.stripeProduct
              .stripe_product_id,
            name: productData.name,
            product_id:
              defaultSerializedObject.stripePrice.stripeProduct.product_id,
          };
        }
      }
  ...
  },
  {
    async upsert(data, unfilteredOptions) {
      ...
      if (model) {
        return this.edit(
          data,
          Object.assign({}, unfilteredOptions, {
            id: model.id,
          })
        );
      }
...
module.exports = {
  StripeCustomerSubscription: ghostBookshelf.model(
