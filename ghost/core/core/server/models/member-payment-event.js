...
const MemberPaymentEvent = ghostBookshelf.Model.extend(
      ...
      if (options.aggregatePaymentVolume) {
        if (options.limit || options.filter) {
          throw new errors.IncorrectUsageError({
            message:
              'aggregatePaymentVolume does not work when passed a filter or limit',
          });
        }
  ...
  },
  {
    ...
    async edit() {
      throw new errors.IncorrectUsageError({
        message: 'Cannot edit MemberPaymentEvent',
      });
    },

    async destroy() {
      throw new errors.IncorrectUsageError({
        message: 'Cannot destroy MemberPaymentEvent',
      });
    },
  }
);
...
module.exports = {
  MemberPaymentEvent: ghostBookshelf.model(
    'MemberPaymentEvent',
    MemberPaymentEvent
  ),
  MemberPaymentEvents: ghostBookshelf.collection(
    'MemberPaymentEvents',
    MemberPaymentEvents
  ),
};
