      ...
      if (options.aggregateSubscriptionDeltas) {
        if (options.limit || options.filter) {
          throw new errors.IncorrectUsageError({
            message:
              'aggregateSubscriptionDeltas does not work when passed a filter or limit',
          });
        }
        ...
        return qb
          ...
          .select(
            knex.raw(
              `SUM(CASE WHEN subscribed THEN 1 ELSE -1 END) as subscribed_delta`
            )
          )
...
module.exports = {
  MemberSubscribeEvent: ghostBookshelf.model(
    'MemberSubscribeEvent',
    MemberSubscribeEvent
  ),
  MemberSubscribeEvents: ghostBookshelf.collection(
    'MemberSubscribeEvents',
    MemberSubscribeEvents
  ),
};
