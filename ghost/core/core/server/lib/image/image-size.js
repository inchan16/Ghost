...
const FETCH_ONLY_FORMATS = ['cur', 'icns', 'dds'];
...
class ImageSize {
  constructor({ config, storage, storageUtils, validator, urlUtils, request }) {
    ...
    this.NEEDLE_OPTIONS = {
      ...
      response_timeout:
        this.config.get('times:getImageSizeTimeoutInMS') || 10000,
    };
  }
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (dimensions.images) {
          dimensions.width = _.maxBy(
            dimensions.images,
            (img) => img.width
          ).width;
          dimensions.height = _.maxBy(
            dimensions.images,
            (img) => img.height
          ).height;
        }
  ...
  getImageSizeFromUrl(imagePath) {
    ...
    if (imagePath.match(/^\/assets/)) {
      imagePath = this.urlUtils.urlJoin(
        this.urlUtils.urlFor('home', true),
        this.urlUtils.getSubdir(),
        '/',
        imagePath
      );
    }
    ...
    return this._imageSizeFromUrl(imagePath)
      .then((dimensions) => {
  ...
  async getOriginalImagePath(imagePath) {
    ...
    const preferredUnoptimizedImagePathExists = await storageInstance.exists(
      preferredUnoptimizedImagePath
    );
    ...
    const [imageNameMatched, imageName, imageNumber] = name.match(
      /^(.+?)(-\d+)?$/
    ) || [null];
    ...
    const legacyOriginalImagePath = path.join(
      dir,
      `${imageName}_o${imageNumber || ''}${ext}`
    );
    const legacyOriginalImageExists = await storageInstance.exists(
      legacyOriginalImagePath
    );
  ...
  async getOriginalImageSizeFromStoragePath(imagePath) {
    return this.getImageSizeFromStoragePath(
      await this.getOriginalImagePath(imagePath)
    );
  }
  ...
  getOriginalImageSizeFromStorageUrl(imageUrl) {
    return this.getOriginalImageSizeFromStoragePath(
      this._getPathFromUrl(imageUrl)
    );
  }
