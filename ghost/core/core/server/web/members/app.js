...
module.exports = function setupMembersApp() {
  ...
  membersApp.post(
    '/api/member/email',
    bodyParser.json({ limit: '50mb' }),
    (req, res) => membersService.api.middleware.updateEmailAddress(req, res)
  );

  // Remove email from suppression list
  membersApp.delete(
    '/api/member/suppression',
    labs.enabledMiddleware('suppressionList'),
  ...
  membersApp.post(
    ...
    (req, res, next) =>
      membersService.api.middleware.sendMagicLink(req, res, next)
  );
  membersApp.post('/api/create-stripe-checkout-session', (req, res, next) =>
    membersService.api.middleware.createCheckoutSession(req, res, next)
  );
  membersApp.post('/api/create-stripe-update-session', (req, res, next) =>
    membersService.api.middleware.createCheckoutSetupSession(req, res, next)
  );
  membersApp.put('/api/subscriptions/:id', (req, res, next) =>
    membersService.api.middleware.updateSubscription(req, res, next)
  );
