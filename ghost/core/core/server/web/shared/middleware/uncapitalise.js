...
const uncapitalise = (req, res, next) => {
  ...
  const isAPIRegExp = new RegExp(
    `^(.*${urlUtils.BASE_API_PATH}(/(v[\\d.]+|canary))?/.*?/)`,
    'i'
  );
  ...
  if (/[A-Z]/.test(decodedURI)) {
    redirectPath = localUtils.removeOpenRedirectFromUrl(
      (req.originalUrl || req.url).replace(pathToTest, pathToTest.toLowerCase())
    );
