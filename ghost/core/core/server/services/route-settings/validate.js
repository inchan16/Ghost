...
const messages = {
  ...
  badDataHelp: 'Example:\n data:\n  my-tag:\n    resource: tags\n    ...\n',
  authorDeprecatedError:
    'Please choose a different name. We recommend not using author.',
};
...
_private.validateData = function validateData(object) {
  ...
  const shortToLongForm = (shortForm, options = {}) => {
    ...
    if (
      !RESOURCE_CONFIG.QUERY[resourceKey] ||
      (Object.prototype.hasOwnProperty.call(
        RESOURCE_CONFIG.QUERY[resourceKey],
        'internal'
      ) &&
        RESOURCE_CONFIG.QUERY[resourceKey].internal === true)
    ) {
      throw new errors.ValidationError({
    ...
    longForm.query[options.resourceKey || resourceKey] = _.cloneDeep(
      _.omit(RESOURCE_CONFIG.QUERY[resourceKey], 'resourceAlias')
    );

    // redirect is enabled by default when using the short form
    longForm.router = {
      [RESOURCE_CONFIG.QUERY[resourceKey].resourceAlias ||
      RESOURCE_CONFIG.QUERY[resourceKey].resource]: [
        {
          slug: slug,
          redirect: true,
        },
  ...
  if (typeof object.data === 'string') {
    ...
    _.each(object.data, (value, key) => {
      ...
      if (typeof object.data[key] === 'string') {
        const longForm = shortToLongForm(object.data[key], {
          resourceKey: key,
        });
        ...
        _.each(Object.keys(longForm.router), (routerKey) => {
          if (data.router[routerKey]) {
            data.router[routerKey] = data.router[routerKey].concat(
              longForm.router[routerKey]
            );
          } else {
      ...
      _.each(requiredQueryFields, (option) => {
        ...
        if (
          allowedQueryValues[option] &&
          allowedQueryValues[option].indexOf(object.data[key][option]) === -1
        ) {
          throw new errors.ValidationError({
            message: tpl(messages.validationError, {
              at: JSON.stringify(object.data[key]),
              reason: `${
                object.data[key][option]
              } not supported. Please use ${_.uniq(
                allowedQueryValues[option]
              )}.`,
            }),
          });
      ...
      const DEFAULT_RESOURCE = _.find(RESOURCE_CONFIG.QUERY, {
        resource: data.query[key].resource,
      });

      data.query[key].resource = DEFAULT_RESOURCE.resource;

      data.query[key] = _.defaults(
        data.query[key],
        _.omit(DEFAULT_RESOURCE, ['options', 'resourceAlias'])
      );

      data.query[key].options = _.pick(object.data[key], allowedQueryOptions);
      if (data.query[key].type === 'read') {
        data.query[key].options = _.defaults(
          data.query[key].options,
      ...
      if (
        !Object.prototype.hasOwnProperty.call(
          data.router,
          DEFAULT_RESOURCE.resourceAlias || DEFAULT_RESOURCE.resource
        )
      ) {
        data.router[
          DEFAULT_RESOURCE.resourceAlias || DEFAULT_RESOURCE.resource
...
_private.validateRoutes = function validateRoutes(routes) {
  ...
  _.each(routes, (routingTypeObject, routingTypeObjectKey) => {
    ...
    routes[routingTypeObjectKey] = _private.validateData(
      routes[routingTypeObjectKey]
    );
  });
...
_private.validateCollections = function validateCollections(collections) {
  ...
  _.each(collections, (routingTypeObject, routingTypeObjectKey) => {
    ...
    if (
      routingTypeObject.permalink &&
      routingTypeObject.permalink.match(/\/:\w+/)
    ) {
      throw new errors.ValidationError({
        message: tpl(messages.validationError, {
          at: routingTypeObject.permalink,
    ...
    if (routingTypeObject.permalink.match(/{.*}/)) {
      routingTypeObject.permalink = routingTypeObject.permalink.replace(
        /{(\w+)}/g,
        ':$1'
      );
    }

    collections[routingTypeObjectKey] =
      _private.validateTemplate(routingTypeObject);
    collections[routingTypeObjectKey] = _private.validateData(
      collections[routingTypeObjectKey]
    );
  });
