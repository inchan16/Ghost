...
const debug = require('@tryghost/debug')(
  'frontend:services:settings:ensure-settings'
);
...
class DefaultSettingsManager {
  ...
  async ensureSettingsFileExists() {
    ...
    return Promise.resolve(fs.readFile(destinationFilePath, 'utf8')).catch(
      (err) => {
        if (err.code === 'ENOENT') {
          // CASE: file doesn't exist, copy it from our defaults
          return fs.copy(defaultFilePath, destinationFilePath).then(() => {
            debug(
              `'${defaultFileName}' copied to ${this.destinationFolderPath}.`
            );
          });
        } else {
