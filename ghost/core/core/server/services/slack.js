...
const messages = {
  requestFailedError:
    'The {service} service was unable to send a ping request, your site will continue to function.',
  requestFailedHelp:
    'If you get this error repeatedly, please seek help on {url}.',
};
...
function ping(post) {
  ...
  if (slackSettings && slackSettings.url && slackSettings.url !== '') {
    slackSettings.username = slackSettings.username
      ? slackSettings.username
      : 'Ghost';
        ...
        attachments: [
          {
            ...
            image_url: post
              ? urlUtils.urlFor('image', { image: post.feature_image }, true)
              : null,
            color: '#008952',
            fields: [
              {
          ...
          },
          {
            ...
            color: '#008952',
            thumb_url: author
              ? urlUtils.urlFor('image', { image: author.profile_image }, true)
              : null,
            fields: [
              {
                title: 'Author',
                value: author
                  ? `<${urlService.getUrlByResourceId(author.id, {
                      absolute: true,
                    })} | ${author.name}>`
                  : null,
                short: true,
    ...
    return request(slackSettings.url, {
      ...
      logging.error(
        new errors.InternalServerError({
          ...
          help: tpl(messages.requestFailedHelp, {
