        ...
        objTypeHandlers[objType] = function (modelOrId, unsafeAttrs) {
          ...
          return permissionLoad.then(function (loadedPermissions) {
            ...
            const apiKeyPermissions = loadedPermissions.apiKey
              ...
              : null;
            const memberPermissions = loadedPermissions.member
              ? loadedPermissions.member.permissions
              : null;
            ...
            const checkPermission = function (perm) {
              ...
              if (
                perm.get('action_type') !== actType ||
                perm.get('object_type') !== objType
              ) {
                return false;
              }
  ...
  beginCheck(context) {
    ...
    if (actionsMap.empty()) {
      throw new errors.InternalServerError({
        message: tpl(messages.noActionsMapFoundError),
      });
    }
    ...
    permissionsLoad = Promise.all([
      userPermissionLoad,
      apiKeyPermissionLoad,
      memberPermissionLoad,
