...
module.exports = {
  ...
  async getLastSeenEventTimestamp() {
    ...
    let { maxDeliveredAt } =
      (await db
        ...
        .select(db.knex.raw('MAX(delivered_at) as maxDeliveredAt'))
        .first()) || {};
    let { maxOpenedAt } =
      (await db
        .knex('email_recipients')
        .select(db.knex.raw('MAX(opened_at) as maxOpenedAt'))
        .first()) || {};
    let { maxFailedAt } =
      (await db
        .knex('email_recipients')
        .select(db.knex.raw('MAX(failed_at) as maxFailedAt'))
        .first()) || {};
  ...
  async aggregateEmailStats(emailId) {
    await db
      .knex('emails')
      .update({
        delivered_count: db.knex.raw(
          `(SELECT COUNT(id) FROM email_recipients WHERE email_id = ? AND delivered_at IS NOT NULL)`,
          [emailId]
        ),
        opened_count: db.knex.raw(
          `(SELECT COUNT(id) FROM email_recipients WHERE email_id = ? AND opened_at IS NOT NULL)`,
          [emailId]
        ),
        failed_count: db.knex.raw(
          `(SELECT COUNT(id) FROM email_recipients WHERE email_id = ? AND failed_at IS NOT NULL)`,
          [emailId]
        ),
      })
      .where('id', emailId);
  ...
  async aggregateMemberStats(memberId) {
    ...
    const updateQuery = {
      email_count: db.knex.raw(
        '(SELECT COUNT(id) FROM email_recipients WHERE member_id = ?)',
        [memberId]
      ),
      email_opened_count: db.knex.raw(
        '(SELECT COUNT(id) FROM email_recipients WHERE member_id = ? AND opened_at IS NOT NULL)',
        [memberId]
      ),
    };
    ...
    await db.knex('members').update(updateQuery).where('id', memberId);
  },
};
