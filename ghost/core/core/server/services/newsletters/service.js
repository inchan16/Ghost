...
class NewslettersService {
  ...
  async add(attrs, options = {}) {
    ...
    const { cleanedAttrs, emailsToVerify } =
      await this.prepAttrsForEmailVerification(attrs);

    // add this newsletter last
    const sortOrder = await this.NewsletterModel.getNextAvailableSortOrder(
      options
    );
    ...
    newsletter = await this.NewsletterModel.findOne(
      { id: newsletter.id },
  ...
  async edit(attrs, options) {
    ...
    const originalNewsletter = await this.NewsletterModel.findOne(
      { id: options.id },
      { ...sharedOptions, require: true }
    );

    const { cleanedAttrs, emailsToVerify } =
      await this.prepAttrsForEmailVerification(attrs, originalNewsletter);

    if (
      originalNewsletter.get('status') !== 'active' &&
      cleanedAttrs.status === 'active'
    ) {
      await this.limitService.errorIfWouldGoOverLimit(
        'newsletters',
        sharedOptions
    ...
    try {
      updatedNewsletter = await this.NewsletterModel.edit(
        cleanedAttrs,
        options
      );
    } catch (error) {
    ...
    updatedNewsletter = await this.NewsletterModel.findOne(
  ...
  async respondWithEmailVerification(newsletter, emailsToVerify) {
    if (emailsToVerify.length > 0) {
      for (const { email, property } of emailsToVerify) {
        await this.sendEmailVerificationMagicLink({
      ...
      newsletter.meta = newsletter.meta || {};
      newsletter.meta.sent_email_verification = emailsToVerify.map(
        (v) => v.property
      );
    }
   ...
   * @private
   */
  async sendEmailVerificationMagicLink({
    id,
    email,
    property = 'sender_from',
  }) {
    ...
    return this.magicLinkService.sendMagicLink({
      email,
      tokenData: { id, property, value: email },
    });
  }
}
