...
const messages = {
  ...
  overrideCasper:
    "Please rename your zip, it's not allowed to override the default casper theme.",
...
module.exports = {
  ...
  setFromZip: async (zip) => {
    const themeName = getStorage().getSanitizedFileName(
      zip.name.split('.zip')[0]
    );
    ...
    try {
      ...
      if (overrideTheme) {
        ...
        await activator.activateFromAPIOverride(
          themeName,
          loadedTheme,
          checkedTheme
        );
