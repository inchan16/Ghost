...
const messages = {
  ...
  themeCannotBeActivated:
    '{themeName} cannot be activated because it was not found in the theme directory.',
};
...
module.exports.loadAndActivate = async (themeName) => {
  ...
  try {
    ...
    if (!validate.canActivate(checkedTheme)) {
      logging.error(
        validate.getThemeValidationError(
...
module.exports.activate = async (themeName) => {
  ...
  if (!loadedTheme) {
    throw new errors.ValidationError({
      message: tpl(messages.themeCannotBeActivated, {
        themeName: themeName,
      }),
