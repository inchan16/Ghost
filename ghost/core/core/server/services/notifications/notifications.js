...
class Notifications {
    ...
    allNotifications = allNotifications.filter((notification) => {
      if (notification.createdAtVersion && !this.wasSeen(notification, user)) {
        ...
        if (
          ...
          (!notification.custom ||
            notification.message.match(ghostMajorRegEx) ||
            notification.message.match(ghostSec43))
        ) {
          let notificationVersion =
            notification.message.match(/(\d+\.)(\d+\.)(\d+)/);
          ...
          if (
            ghostMajorMatch &&
            ghostMajorMatch.groups &&
            ghostMajorMatch.groups.major
          ) {
          ...
          if (
            notificationVersion &&
            blogVersion &&
            semver.gt(notificationVersion, blogVersion[0])
          ) {
            return true;
          } else {
            return false;
  ...
  add({ notifications }) {
    ...
    notificationsToCheck.forEach((notification) => {
      ...
      if (!isDuplicate) {
        notificationsToAdd.push(
          Object.assign({}, defaults, notification, overrides)
        );
      }
    });
    ...
    const releaseNotificationsToAdd = notificationsToAdd.filter(
      (notification) => {
        return !notification.custom;
      }
    );
    ...
    if (releaseNotificationsToAdd.length > 1) {
      ...
      notificationsToAdd.push(
        _.orderBy(releaseNotificationsToAdd, 'created_at', 'desc')[0]
      );
    }
  ...
  destroy({ notificationId, user }) {
    ...
    const notificationToMarkAsSeenIndex = allNotifications.findIndex(
      (notification) => {
        return notification.id === notificationId;
      }
    );
    ...
    if (
      notificationToMarkAsSeenIndex > -1 &&
      !notificationToMarkAsSeen.dismissible
    ) {
