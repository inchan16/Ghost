...
class Queue extends EventEmitter {
  ...
  run(options) {
    ...
    debug(
      ...
      action,
      event,
      this.queue[event].subscribers.length,
      this.toNotify[action].notified.length
    );
    ...
    if (
      ...
      const fn =
        this.queue[event].subscribers[this.toNotify[action].notified.length];
      ...
      if (this.queue[event].tolerance === 0) {
        ...
        this.queue[options.event].subscribers.length >=
          this.queue[options.event].requiredSubscriberCount &&
        ...
        this.toNotify[action].timeoutInMS =
          this.toNotify[action].timeoutInMS * 1.1;
