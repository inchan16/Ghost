...
class Resources {
  ...
  _fetch(resourceConfig, options = { offset: 0, limit: 999 }) {
    ...
    return models.Base.Model.raw_knex.fetchAll(modelOptions).then((objects) => {
      debug('fetched', resourceConfig.type, objects.length);

      _.each(objects, (object) => {
        this.data[resourceConfig.type].push(
          new Resource(resourceConfig.type, object)
        );
      });

      if (objects.length && isSQLite) {
        options.offset = options.offset + options.limit;
        return this._fetch(resourceConfig, {
    ...
    if (withRelatedFields) {
      ...
      if (withRelatedPrimary) {
        _.each(withRelatedPrimary, (relation, primaryKey) => {
          ...
          const targetTagKeys = Object.keys(
            obj[relation].find((item) => {
  ...
  _containsRoutingAffectingChanges(model, ignoredProperties) {
    if (model._changed && Object.keys(model._changed).length) {
      return (
        _.difference(Object.keys(model._changed), ignoredProperties).length !==
        0
      );
    }
  ...
  async _onResourceUpdated(type, model) {
    ...
    const ignoredProperties = [
      ...resourceConfig.modelOptions.exclude,
      'updated_at',
    ];
    if (!this._containsRoutingAffectingChanges(model, ignoredProperties)) {
      ...
      if (
        cachedResource &&
        model._changed &&
        Object.keys(model._changed).includes('updated_at')
      ) {
        DomainEvents.dispatch(
          URLResourceUpdatedEvent.create(
            Object.assign(cachedResource.data, {
      ...
      debug(
        'skipping _onResourceUpdated because only non-route-related properties changed'
      );
      return false;
    }
