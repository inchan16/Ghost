...
module.exports = class LinkRedirectRepository {
  ...
  fromModel(model) {
    ...
    const edited =
      model.get('updated_at')?.getTime() >
      model.get('created_at')?.getTime() + 1000;

    return new LinkRedirect({
      id: model.id,
      from: new URL(
        this.#trimLeadingSlash(model.get('from')),
  ...
  async getFilteredIds(options) {
    const linkRows = await this.#LinkRedirect
      .getFilteredCollectionQuery(options)
      .select('redirects.id')
      .distinct();
  ...
  stripSubdirectoryFromPath(path) {
    ...
    return this.#urlUtils.absoluteToRelative(absolute, {
