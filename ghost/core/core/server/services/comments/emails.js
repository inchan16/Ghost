...
class CommentsServiceEmails {
  ...
  async notifyPostAuthors(comment) {
    ...
    for (const author of post.related('authors')) {
      ...
      const templateData = {
        ...
        postUrl: this.urlService.getUrlByResourceId(post.get('id'), {
          absolute: true,
        }),
        commentHtml: comment.get('html'),
        commentDate: moment(comment.get('created_at'))
          .tz(this.settingsCache.get('timezone'))
          .format('D MMM YYYY'),
        ...
        staffUrl: this.urlUtils.urlJoin(
          ...
          '#',
          `/settings/staff/${author.get('slug')}`
        ),
      };
  ...
  async notifyParentCommentAuthor(reply) {
    const parent = await this.models.Comment.findOne({
      id: reply.get('parent_id'),
    });
    const parentMember = parent.related('member');

    if (
      parent?.get('status') !== 'published' ||
      !parentMember.get('enable_comment_notifications')
    ) {
      return;
    }
    ...
    const subject =
      '↪️ New reply to your comment on ' + this.settingsCache.get('title');

    const post = await this.models.Post.findOne({
      id: reply.get('post_id'),
    });
    const member = await this.models.Member.findOne({
      id: reply.get('member_id'),
    });
    ...
    const templateData = {
      ...
      replyDate: moment(reply.get('created_at'))
        .tz(this.settingsCache.get('timezone'))
        .format('D MMM YYYY'),
      memberName: memberName,
      memberExpertise: member.get('expertise'),
      ...
      fromEmail: this.notificationFromAddress,
      toEmail: to,
      profileUrl: emailService.renderer.createUnsubscribeUrl(
        member.get('uuid'),
        { comments: true }
      ),
    };
