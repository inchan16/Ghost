...
class WebhookTrigger {
  ...
  getAll(event) {
    return this.models.Webhook.findAllByEvent(event, {
  ...
  update(webhook, data) {
    this.models.Webhook.edit(
      ...
      logging.warn(
        `Unable to update "last_triggered" for webhook: ${webhook.id}`
      );
    });
  }

  destroy(webhook) {
    return this.models.Webhook.destroy(
      { id: webhook.id },
      { context: { internal: true } }
    ).catch(() => {
  ...
  onError(webhook) {
    return (err) => {
      if (err.statusCode === 410) {
        logging.info(
          `Webhook destroyed (410 response) for "${webhook.get(
            'event'
          )}" with url "${webhook.get('target_url')}".`
        );
      ...
      logging.warn(
        `Request to ${webhook.get('target_url') || null} failed because of: ${
          err.code || ''
        }.`
      );
    };
  ...
  async trigger(event, model) {
    ...
    for (const webhook of hooks.models) {
      ...
      if (secret !== '') {
        headers['X-Ghost-Signature'] = `sha256=${crypto
          .createHmac('sha256', secret)
          .update(reqPayload)
          .digest('hex')}, t=${Date.now()}`;
      }
      ...
      logging.info(
        `Triggering webhook for "${webhook.get('event')}" with url "${url}"`
      );
