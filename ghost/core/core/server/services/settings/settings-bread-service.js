...
const messages = {
  ...
  accessCoreSettingFromExtReq:
    'Attempted to access core setting from external request',
};
...
class SettingsBREADService {
  ...
  read(key, context) {
    ...
    if (key === 'slack') {
      const slackURL = this.settingsCache.get('slack_url', {
        resolve: false,
      });
      const slackUsername = this.settingsCache.get('slack_username', {
        resolve: false,
      });
  ...
  async edit(settings, options, stripeConnectData) {
    ...
    const getSetting = (setting) =>
      this.settingsCache.get(setting.key, { resolve: false });

    const firstUnknownSetting = filteredSettings.find(
    ...
    const modelArray = await this.SettingsModel.edit(
      refilteredSettings,
      options
    ).then((result) => {
      ...
      const labsSetting = result.find(
        (setting) => setting.get('key') === 'labs'
      );
      if (labsSetting) {
      ...
      return this._formatBrowse(
        _.keyBy(_.invokeMap(result, 'toJSON'), 'key'),
        options.context
      );
    });
  ...
  async getStripeConnectData(
    ...
    if (stripeConnectIntegrationToken && stripeConnectIntegrationToken.value) {
      try {
        return await getStripeConnectTokenData(
          stripeConnectIntegrationToken.value,
          getSessionProp
        );
      } catch (err) {
