...
module.exports = class BookshelfMentionRepository {
  ...
  async getPage(options) {
    ...
    if (options.unique) {
      _options.whereRaw =
        'NOT EXISTS (select id from mentions as m where m.id > mentions.id and m.source = mentions.source)';
    }
    ...
    return {
      data: await Promise.all(
        page.data.map((model) => this.#modelToMention(model))
      ),
      meta: page.meta,
    };
  }
  ...
  async getAll(options) {
    ...
    return await Promise.all(
      models.map((model) => this.#modelToMention(model))
    );
  }
  ...
  async save(mention) {
    ...
    const existing = await this.#MentionModel.findOne(
      { id: data.id },
      { require: false }
    );
