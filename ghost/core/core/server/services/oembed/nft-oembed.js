...
class NFTOEmbedProvider {
  ...
  async canSupportRequest(url) {
    return (
      url.host === 'opensea.io' && OPENSEA_ETH_PATH_REGEX.test(url.pathname)
    );
  }
  ...
  async getOEmbedData(url, externalRequest) {
    const [match, transaction, asset] = url.pathname.match(
      OPENSEA_ETH_PATH_REGEX
    );
    if (!match) {
      return null;
    }
    ...
    const result = await externalRequest(
      `https://api.opensea.io/api/v1/asset/${transaction}/${asset}/?format=json`,
      {
        headers,
      }
    ).json();
