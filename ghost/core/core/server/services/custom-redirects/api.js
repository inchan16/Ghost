...
const messages = {
  ...
  yamlPlainString:
    'YAML input cannot be a plain string. Check the format of your YAML file.',
...
class CustomRedirectsAPI {
    ...
    try {
      ...
      if (filePath !== null) {
        ...
        for (const redirect of redirects) {
          this.redirectManager.addRedirect(redirect.from, redirect.to, {
  ...
  async setFromFilePath(filePath, ext = '.json') {
    ...
    if (ext === '.json') {
      await fs.writeFile(
        this.createRedirectsFilePath('.json'),
