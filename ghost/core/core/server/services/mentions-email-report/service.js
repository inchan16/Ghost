  ...
  async init() {
    ...
    const mentionReportEmailView = {
      ...
      async renderSubject(report) {
        const sourceSiteTitles = report?.mentions?.map(
          (mention) => mention.sourceSiteTitle
        );
      ...
      async renderHTML(report, recipient) {
        ...
        const uniqueMentions = report.mentions.filter(
          (mention, index, self) => {
            return (
              self.findIndex(
                (m) => m.sourceUrl.href === mention.sourceUrl.href
              ) === index
            );
          }
        );
    ...
    const mentionReportHistoryService = {
      async getLatestReportDate() {
        const setting = settingsCache.get(
