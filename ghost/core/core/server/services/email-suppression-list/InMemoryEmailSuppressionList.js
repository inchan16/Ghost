...
} = require('@tryghost/email-suppression-list');

module.exports = class InMemoryEmailSuppressionList extends (
  AbstractEmailSuppressionList
) {
  ...
  async removeEmail(email) {
    if (
      (email === 'fail@member.test' || email === 'spam@member.test') &&
      this.store.includes(email)
    ) {
