...
class MembersStats {
  ...
  async getTotalMembers() {
    const result = await this._db.knex.raw(
  ...
  async getTotalMembersOnDatesInRange({ days, totalMembers, siteTimezone }) {
    ...
    if (this._isSQLite) {
      const dateModifier = `${
        Math.sign(tzOffsetMins) === -1 ? '' : '+'
      }${tzOffsetMins} minutes`;

      result = await this._db
        .knex('members')
        .select(
          this._db.knex.raw(
            'DATE(created_at, ?) AS created_at, COUNT(DATE(created_at, ?)) AS count',
      ...
      result = await this._db
        ...
        .select(
          this._db.knex.raw(
            "DATE(CONVERT_TZ(created_at, '+00:00', ?)) AS created_at, COUNT(CONVERT_TZ(created_at, '+00:00', ?)) AS count",
            [utcOffset, utcOffset]
          )
        )
        .where((builder) => {
    ...
    const totalInRange = Object.values(resultObject).reduce(
      (acc, value) => acc + value,
      0
    );
    ...
    if (days === 'all-time') {
      ...
      currentRangeDate = moment(
        moment(result[0].created_at).format('YYYY-MM-DD')
      ).tz(siteTimezone);
    } else {
  ...
  async fetch(days) {
    ...
    const [total, totalInRange, totalOnDate, newToday] = await Promise.all([
      ...
      this.getTotalMembersOnDatesInRange({
        days,
        totalMembers,
        siteTimezone,
      }),
