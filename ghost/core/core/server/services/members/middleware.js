...
const getIdentityToken = async function (req, res) {
  try {
    const token = await membersService.ssr.getIdentityTokenForMemberFromSession(
      req,
      res
    );
    res.writeHead(200);
...
const updateMemberNewsletters = async function (req, res) {
  try {
    ...
    const updatedMember = await membersService.api.members.update(
      data,
      options
    );
...
const updateMemberData = async function (req, res) {
  try {
    ...
    if (member) {
      ...
      const updatedMember = await membersService.ssr.getMemberDataFromSession(
        req,
        res
      );
