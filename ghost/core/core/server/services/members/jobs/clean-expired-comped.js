...
function cancel() {
  if (parentPort) {
    parentPort.postMessage(
      'Expired complimentary subscriptions cleanup cancelled before completion'
    );
...
(async () => {
  ...
  const expiredCompedRows = await db
  ...
  debug(
    `Removed ${deletedExpiredSubs} expired subscriptions, updated ${updatedMembers} members in ${
      cleanupEndDate.valueOf() - cleanupStartDate.valueOf()
    }ms`
  );
