...
const initVerificationTrigger = () => {
  return new VerificationTrigger({
    ...
    isVerified: () =>
      config.get('hostSettings:emailVerification:verified') === true,
    isVerificationRequired: () =>
      settingsCache.get('email_verification_required') === true,
    sendVerificationEmail: async ({ subject, message, amountTriggered }) => {
      const escalationAddress = config.get(
        'hostSettings:emailVerification:escalationAddress'
      );
...
module.exports = {
  async init() {
    ...
    if (!env?.startsWith('testing')) {
      ...
      if (
        !(await jobsService.hasExecutedSuccessfully(membersMigrationJobName))
      ) {
