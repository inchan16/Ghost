...
class Users {
  ...
  async resetAllPasswords(frameOptions) {
    return this.models.Base.transaction(async (t) => {
      ...
      for (const user of users) {
        const token = await this.auth.passwordreset.generateToken(
          user.get('email'),
          this.apiSettings,
          t
        );
        await this.auth.passwordreset.sendResetNotification(
          token,
          this.apiMail
        );
      }
  ...
  async assignTagToUserPosts({ id, context, transacting }) {
    ...
    for (const postId of usersPostIds) {
      ...
      const existingTagSlugs = post.relations.tags.models.map((t) =>
        t.get('slug')
      );
  ...
  async destroyUser(frameOptions) {
    ...
    return this.models.Base.transaction(async (t) => {
      ...
      await this.models.User.destroy(
        Object.assign({ status: 'all' }, frameOptions)
      );
