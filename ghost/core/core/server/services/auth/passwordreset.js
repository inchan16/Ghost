...
function generateToken(email, settingsAPI, transaction) {
  ...
  return settingsAPI
    .read(_.merge({ key: 'db_hash' }, options))
    .then((response) => {
...
function extractTokenParts(options) {
  options.data.password_reset[0].token = security.url.decodeBase64(
    options.data.password_reset[0].token
  );
...
function protectBruteForce({ options, tokenParts }) {
  if (
    tokenSecurity[`${tokenParts.email}+${tokenParts.expires}`] &&
...
function doReset(options, tokenParts, settingsAPI) {
  ...
  return settingsAPI
    .read(_.merge({ key: 'db_hash' }, _.omit(options, 'data')))
    .then((response) => {
      dbHash = response.settings[0].value;
    ...
    .then((user) => {
      ...
      if (!compareResult.correct) {
        let error;
        if (
          compareResult.reason === 'expired' ||
          compareResult.reason === 'invalid_expiry'
        ) {
