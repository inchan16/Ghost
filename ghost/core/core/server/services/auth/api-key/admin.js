...
const {
  legacyApiPathMatch,
} = require('../../../services/api-version-compatibility');
const tpl = require('@tryghost/tpl');
const _ = require('lodash');

const messages = {
  incorrectAuthHeaderFormat:
...
const authenticate = (req, res, next) => {
  ...
  return authenticateWithToken(req, res, next, {
    token,
    JWT_OPTIONS: JWT_OPTIONS_DEFAULTS,
  });
};
...
const authenticateWithUrl = (req, res, next) => {
  ...
  if (!token) {
    return next(
      new errors.UnauthorizedError({
        message: tpl(messages.invalidTokenWithMessage, {
  ...
  return authenticateWithToken(req, res, next, {
    token,
    JWT_OPTIONS: _.omit(JWT_OPTIONS_DEFAULTS, 'maxAge'),
  });
};
