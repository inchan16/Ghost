...
async function setupUser(userData) {
  ...
  const user = await models.User.setup(
    userData,
    _.extend({ id: owner.id }, context)
  );
...
async function doSettings(data, settingsAPI) {
  ...
  const description = data.userData.description
    ? data.userData.description.trim()
    : null;
  ...
  userSettings = [
    ...
    {
      key: 'description',
      value: description || tpl(messages.sampleBlogDescription),
    },
  ];
  ...
  if (data.userData.accentColor) {
    userSettings.push({
      key: 'accent_color',
      value: data.userData.accentColor,
    });
  }
...
async function doTiersAndNewsletter(data, api) {
  ...
  try {
    ...
    const defaultTier = tierPage.tiers.find(
      (p) => p.slug === 'default-product'
    );
    const defaultNewsletter = newsletterPage.newsletters.find(
      (p) => p.slug === 'default-newsletter'
    );
...
function sendWelcomeEmail(email, mailAPI) {
  if (config.get('sendWelcomeEmail')) {
    ...
    return mail.utils
      .generateContent({ data: data, template: 'welcome' })
      .then(async (content) => {
        const message = {
          to: email,
        ...
        await mailAPI
          .send(payload, { context: { internal: true } })
          .catch((err) => {
...
async function installTheme(data, api) {
  ...
  try {
    ...
    logging.warn(
      tpl(messages.failedThemeInstall, {
        themeName,
        error: error.message,
      })
    );
