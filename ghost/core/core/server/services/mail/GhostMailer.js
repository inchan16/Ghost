...
const messages = {
  ...
  checkEmailConfigInstructions:
    'Please see {url} for instructions on configuring email.',
...
function getFromAddress(requestedFromAddress) {
  ...
  if (validator.isEmail(address, { require_tld: false })) {
    const defaultSiteTitle = settingsCache.get('title')
      ? settingsCache.get('title').replace(/"/g, '\\"')
...
function createMailError(
  { message, err, ignoreDefaultMessage } = { message: '' }
) {
...
module.exports = class GhostMailer {
  constructor() {
    ...
    let transport =
      (config.get('mail') && config.get('mail').transport) || 'direct';
    transport = transport.toLowerCase();

    // nodemailer mutates the options passed to createTransport
    const options =
      (config.get('mail') && _.clone(config.get('mail').options)) || {};
