  ...
  constructor({
    ...
    if (!apiUrl) {
      throw new errors.IncorrectUsageError({
        message: 'post-scheduling: no apiUrl was provided',
      });
    }
    ...
    if (Object.keys(scheduledResources).length) {
      ...
      Object.keys(scheduledResources).forEach((resourceType) => {
        scheduledResources[resourceType].forEach((model) => {
          adapter.unschedule(
            this.normalize(
              { model, apiUrl, integration, resourceType },
              'unscheduled'
            ),
            { bootstrap: true }
          );
    ...
    SCHEDULED_RESOURCES.forEach((resource) => {
      ...
      events.on(`${resource}.rescheduled`, (model) => {
        adapter.unschedule(
          this.normalize(
            { model, apiUrl, integration, resourceType: resource },
            'unscheduled'
          )
        );
        adapter.schedule(
