...
async function fetchOrCreatePersonalToken(userId) {
  ...
  if (!token) {
    const newToken = await models.ApiKey.add({
      user_id: userId,
      type: 'admin',
    });
    return newToken;
...
module.exports = {
  ...
  edit: {
    ...
    query(frame) {
      return models.User.edit(frame.data.users[0], frame.options).then(
        (model) => {
          if (!model) {
            return Promise.reject(
              new errors.NotFoundError({
  ...
  regenerateToken: {
    ...
    query(frame) {
      ...
      return fetchOrCreatePersonalToken(targetId).then((model) => {
        return models.ApiKey.refreshSecret(
          model.toJSON(),
          Object.assign({}, { id: model.id })
        );
      });
    },
  },
