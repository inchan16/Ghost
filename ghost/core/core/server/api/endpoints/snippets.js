...
module.exports = {
  ...
  browse: {
    options: ['limit', 'order', 'page', 'formats', 'filter'],
    permissions: true,
    validation: {
      options: {
        formats: {
  ...
  read: {
    ...
    permissions: true,
    query(frame) {
      return models.Snippet.findOne(frame.data, frame.options).then((model) => {
        if (!model) {
  ...
  add: {
    ...
    permissions: true,
    query(frame) {
      return models.Snippet.add(frame.data.snippets[0], frame.options).catch(
        (error) => {
          if (
            error.code &&
            ...
            throw new errors.ValidationError({
              message: tpl(messages.snippetAlreadyExists),
            });
          }
  ...
  edit: {
    ...
    permissions: true,
    query(frame) {
      return models.Snippet.edit(frame.data.snippets[0], frame.options).then(
        (model) => {
          if (!model) {
            return Promise.reject(
              new errors.NotFoundError({
