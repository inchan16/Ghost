...
const messages = {
  ...
  invalidAnnouncementVisibilityValueReceived:
    'Please enter a valid announcement visibility value',
  invalidAnnouncementBackgroundValueReceived:
    'Please enter a valid announcement background value',
};
  ...
  edit(apiConfig, frame) {
    ...
    _.each(frame.data.settings, (setting) => {
      ...
      if (emailTypeSettings.includes(setting.key)) {
        ...
        if (
          typeof email !== 'string' ||
          (!validator.isEmail(email) && email !== 'noreply')
        ) {
          const typeError = new ValidationError({
      ...
      if (setting.key === 'announcement_visibility') {
        ...
        if (
          !['public', 'visitors', 'members', 'paid'].includes(visibilityValue)
        ) {
      ...
      if (setting.key === 'announcement_background') {
        ...
        if (
          !['accent', 'dark', 'light'].includes(announcementBackgroundValue)
        ) {
          const visibilityError = new ValidationError({
