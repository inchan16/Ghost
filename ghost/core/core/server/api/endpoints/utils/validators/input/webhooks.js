...
const messages = {
  ...
  noIntegrationIdProvidedContext:
    "You may only create webhooks with 'integration_id' when using session authentication.",
};

module.exports = {
  add(apiConfig, frame) {
    if (
      !_.get(frame, 'options.context.integration.id') &&
      !_.get(frame.data, 'webhooks[0].integration_id')
    ) {
