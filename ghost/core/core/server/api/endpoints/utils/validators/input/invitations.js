...
const debug = require('@tryghost/debug')(
  'api:endpoints:utils:validators:input:invitation'
);
...
module.exports = {
  acceptInvitation(apiConfig, frame) {
    ...
    if (!data.token) {
      return Promise.reject(
        new errors.ValidationError({
          message: tpl(messages.noTokenProvided),
        })
      );
    }
    ...
    if (!data.email) {
      return Promise.reject(
        new errors.ValidationError({
          message: tpl(messages.noEmailProvided),
        })
      );
    }
    ...
    if (!data.password) {
      return Promise.reject(
        new errors.ValidationError({
          message: tpl(messages.noPasswordProvided),
        })
      );
    }
    ...
    if (!data.name) {
      return Promise.reject(
        new errors.ValidationError({
          message: tpl(messages.noNameProvided),
        })
      );
    }
  },
