...
const nonePublicAuth = (apiConfig, frame) => {
  ...
  let unsafeAttrObject =
    apiConfig.unsafeAttrs && _.has(frame, `data.[${apiConfig.docName}][0]`)
      ? _.pick(frame.data[apiConfig.docName][0], apiConfig.unsafeAttrs)
      : {};
  ...
  const permsPromise = permissions
    .canThis(frame.options.context)
    [apiConfig.method][singular](permissionIdentifier, unsafeAttrObject);
  ...
  return permsPromise
    .then((result) => {
      ...
      if (
        result &&
        result.excludedAttrs &&
        _.has(frame, `data.[${apiConfig.docName}][0]`)
      ) {
        frame.data[apiConfig.docName][0] = _.omit(
          frame.data[apiConfig.docName][0],
          result.excludedAttrs
        );
      }
    })
