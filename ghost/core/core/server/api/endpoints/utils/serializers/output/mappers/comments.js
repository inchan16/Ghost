...
const commentFields = ['id', 'status', 'html', 'created_at', 'edited_at'];

const memberFields = ['id', 'uuid', 'name', 'expertise', 'avatar_image'];
...
const postFields = ['id', 'uuid', 'title', 'url'];

const countFields = ['replies', 'likes'];
...
const commentMapper = (model, frame) => {
  ...
  if (jsonModel.member) {
    response.member = _.pick(
      jsonModel.member,
      utils.isMembersAPI(frame) ? memberFields : memberFieldsAdmin
    );
  } else {
  ...
  if (jsonModel.replies) {
    response.replies = jsonModel.replies.map((reply) =>
      commentMapper(reply, frame)
    );
  }
