...
const post = (attrs, frame) => {
  ...
  if (
    columns &&
    columns.includes('email_segment') &&
    fields &&
    !fields.includes('email_segment')
  ) {
    delete attrs.email_segment;
  }
...
const action = (attrs) => {
  if (attrs.actor) {
    if (attrs.actor_type === 'user') {
      attrs.actor = _.pick(attrs.actor, [
        'id',
        'name',
        'slug',
        'profile_image',
  ...
  if (attrs.resource) {
    ...
    attrs.resource = _.pick(attrs.resource, [
      'id',
      'title',
      'slug',
      'feature_image',
