...
module.exports = (model, frame) => {
  ...
  if (jsonModel.html && emailService.renderer && emailService.service) {
    ...
    const replacements = emailService.renderer.buildReplacementDefinitions({
      html: jsonModel.html,
      newsletterUuid: 'preview',
    });
    const exampleMember = emailService.service.getDefaultExampleMember();

    jsonModel.html = emailService.service.replaceDefinitions(
      jsonModel.html,
      replacements,
      exampleMember
    );
    ...
    if (jsonModel.plaintext) {
      jsonModel.plaintext = emailService.service.replaceDefinitions(
        jsonModel.plaintext,
        replacements,
        exampleMember
      );
