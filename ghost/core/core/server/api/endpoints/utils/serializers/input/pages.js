...
const debug = require('@tryghost/debug')(
  'api:endpoints:utils:serializers:input:pages'
);
...
function removeSourceFormats(frame) {
  if (
    frame.options.formats?.includes('mobiledoc') ||
    frame.options.formats?.includes('lexical')
  ) {
...
function setDefaultOrder(frame) {
  ...
  if (frame.options.withRelated) {
    ...
    includesOrderedRelations =
      _.intersection(orderedRelations, frame.options.withRelated).length > 0;
  }
