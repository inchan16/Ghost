...
module.exports = {
  ...
  bulkDestroy: {
    ...
    async query(frame) {
      const bulkDestroyResult = await membersService.api.members.bulkDestroy(
        frame.options
      );
  ...
  bulkEdit: {
    ...
    async query(frame) {
      return membersService.api.members.bulkEdit(
        frame.data.bulk,
        frame.options
      );
    },
  ...
  importCSV: {
    statusCode(result) {
      if (
        ...
        result.meta &&
        result.meta.stats &&
        result.meta.stats.imported !== null
      ) {
        return 201;
      } else {
        return 202;
  ...
  memberStats: {
    ...
    async query() {
      ...
      let totalMembers = _.last(memberStats)
        ? _.last(memberStats).paid +
          _.last(memberStats).free +
          _.last(memberStats).comped
        : 0;
