...
const messages = {
  ...
  noPermissionToEdit: {
    ...
    context:
      'You may only {method} webhooks that belong to the authenticated integration. Check the supplied Admin API Key.',
  },
};
...
module.exports = {
  ...
  edit: {
    permissions: {
      before: (frame) => {
        if (
          ...
          frame.options.context.integration &&
          frame.options.context.integration.id
        ) {
          return models.Webhook.findOne({
            id: frame.options.id,
          }).then((webhook) => {
            if (!webhook) {
    ...
    data: ['name', 'event', 'target_url', 'secret', 'api_version'],
    options: ['id'],
    validation: {
      options: {
        id: {
