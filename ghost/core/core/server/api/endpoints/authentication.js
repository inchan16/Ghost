...
module.exports = {
  ...
  setup: {
    ...
    query(frame) {
      return Promise.resolve()
        ...
        .then((user) => {
          return auth.setup
            .sendWelcomeEmail(user.get('email'), api.mail)
            .then(() => user);
        });
    },
  },
  ...
  updateSetup: {
    ...
    permissions: (frame) => {
      return models.User.findOne({ role: 'Owner', status: 'all' }).then(
        (owner) => {
  ...
  generateResetToken: {
    ...
    query(frame) {
      return Promise.resolve()
        ...
        .then(() => {
          return auth.passwordreset.generateToken(
      ...
      return Promise.resolve()
        ...
        .then(({ options, tokenParts }) => {
          ...
          return auth.passwordreset
            .doReset(options, tokenParts, api.settings)
            .then((params) => {
