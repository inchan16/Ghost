...
const postSchedulingService =
  require('../../services/posts/post-scheduling-service')();
...
module.exports = {
  ...
  publish: {
    ...
    async query(frame) {
      ...
      const { scheduledResource, preScheduledResource } =
        await postSchedulingService.publish(
          resourceType,
          frame.options.id,
          frame.data.force,
          options
        );
      const cacheInvalidate = postSchedulingService.handleCacheInvalidation(
        scheduledResource,
        preScheduledResource
      );
  ...
  getScheduled: {
    ...
    query(frame) {
      ...
      return models[resourceModel].findAll(cleanOptions).then((result) => {
        let response = {};
