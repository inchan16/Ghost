...
${chalk.red(
  'warning'
)} This is a destructive operation for testing purposes only. DO NOT run it against a database you care about.`;

module.exports = class TimeTravel extends Command {
  setup() {
    this.help(helpText);
    this.argument('--offset', {
      type: 'number',
      desc: 'Specify a date offset (in days)',
    });
    this.argument('--force', {
      type: 'boolean',
      desc: 'Continue without confirmation',
    });
  }
  ...
  async handle(argv = {}) {
    ...
    const confirm =
      argv.force || (await this.confirm('Are you sure you want to continue?'));
    if (!confirm) {
      this.warn('Aborting');
      process.exit(1);
    }
    ...
    for (const table in dateTimeFields) {
      for (const column of dateTimeFields[table]) {
        ...
        await db(table).update(
          column,
          db.raw(`DATE_ADD(${column}, interval ${dateOffset} day)`)
        );
