...
module.exports = class DataGeneratorCommand extends Command {
  setup() {
    this.help(
      'Generates random data to populate the database for development & testing'
    );
    this.argument('--base-data-pack', {
      type: 'string',
      defaultValue: '',
      desc: 'Base data pack file location, imported instead of random content',
    });
    this.argument('--scale', {
      type: 'string',
      defaultValue: 'small',
      desc: 'Scale of the data to generate. `small` for a quick run, `large` for more content',
    });
    this.argument('--single-table', {
      type: 'string',
      desc: 'Import a single table',
    });
    this.argument('--quantity', {
      type: 'number',
      desc: 'When importing a single table, the quantity to import',
    });
    this.argument('--clear-database', {
      type: 'boolean',
      defaultValue: false,
      desc: 'Clear all entries in the database before importing',
    });
  }
  ...
  async handle(argv = {}) {
    ...
    try {
      if (argv['single-table']) {
        await dataGenerator.importSingleTable(
          argv['single-table'],
          argv.quantity ?? undefined
        );
      } else {
