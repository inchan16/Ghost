...
function getTitle(data, root, options = {}) {
  ...
  if (pagination && pagination.total > 1) {
    pageString = _.has(options.hash, 'page')
  ...
  if (data.meta_title) {
    ...
    title =
      data.tag[optionsPropertyName] ||
      data.tag.meta_title ||
      data.tag.name + dashSiteTitle;
    // Post title
  } else if (_.includes(context, 'post') && data.post) {
    title =
      data.post[optionsPropertyName] || data.post.meta_title || data.post.title;
    ...
    title =
      data.page[optionsPropertyName] || data.page.meta_title || data.page.title;
    // Fallback
  } else {
