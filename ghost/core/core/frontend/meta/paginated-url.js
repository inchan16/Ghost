...
function getPaginatedUrl(page, data, absolute) {
  ...
  if (page === 'next' && data.pagination.next) {
    ...
    newRelativeUrl =
      data.pagination.prev > 1
        ? urlUtils.urlJoin(pagePath, data.pagination.prev, '/')
        : '/';
  ...
  newRelativeUrl = baseUrl
    ? urlUtils.urlJoin(baseUrl, newRelativeUrl)
    : newRelativeUrl;
