...
function getDescription(data, root, options = {}) {
  ...
  if (data.meta_description) {
    ...
    const siteDescription =
      settingsCache.get('meta_description') || settingsCache.get('description');

    if (options.property) {
      // options.property = null/'og'/'twitter'
      const optionsPropertyName = `${options.property || 'meta'}_description`;
      description =
        settingsCache.get(optionsPropertyName) || siteDescription || '';
    } else {
    ...
    if (!options.property && _.includes(context, 'paged')) {
      ...
      description =
        data.tag[`${options.property}_description`] ||
    ...
    if (options.property) {
      description =
        data.post[`${options.property}_description`] ||
      ...
      description =
        data.post.meta_description || data.post.custom_excerpt || '';
    }
    ...
    if (options.property) {
      description =
        data.page[`${options.property}_description`] ||
      ...
      description =
        data.page.meta_description || data.page.custom_excerpt || '';
    }
  }
