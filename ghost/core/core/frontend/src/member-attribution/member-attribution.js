...
(async function () {
  try {
    ...
    try {
      ...
      console.error(
        '[Member Attribution] Parsing referrer from querystring failed',
        e
      );
    }
    ...
    try {
      ...
      console.error(
        '[Member Attribution] Parsing attribution from querystring failed',
        error
      );
    }
    ...
    if (
      history.length === 0 ||
      history[history.length - 1].path !== currentPath
    ) {
      history.push({
        path: currentPath,
        time: currentTime,
