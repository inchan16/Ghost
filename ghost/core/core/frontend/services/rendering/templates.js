 ...
 * @returns {String[]}
 */
_private.getErrorTemplateHierarchy = function getErrorTemplateHierarchy(
  statusCode
) {
 ...
 * @returns {String[]}
 */
_private.getEntriesTemplateHierarchy = function getEntriesTemplateHierarchy(
  routerOptions,
  requestOptions
) {
  ...
  if (
    ...
    (requestOptions.path === '/' ||
      (requestOptions.path === '/' && requestOptions.page === 1))
  ) {
 ...
 * @returns {String[]}
 */
_private.getEntryTemplateHierarchy = function getEntryTemplateHierarchy(
  postObject,
  context
) {
...
_private.getTemplateForEntries = function getTemplateForEntries(
  routerOptions,
  requestOptions
) {
  const templateList = _private.getEntriesTemplateHierarchy(
    routerOptions,
    requestOptions
  );
  const fallback = templateList[templateList.length - 1];
...
module.exports.setTemplate = function setTemplate(req, res, data) {
  ...
  if (['channel', 'collection'].indexOf(res.routerOptions.type) !== -1) {
    ...
    res._template = _private.pickTemplate(
