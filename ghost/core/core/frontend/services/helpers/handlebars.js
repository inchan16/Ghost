...
function asyncHelperWrapper(hbsInstance, name, fn) {
  hbsInstance.registerAsyncHelper(
    name,
    async function returnAsync(context, options, cb) {
      ...
      try {
        ...
        const wrappedErr = errors.utils.isGhostError(error)
          ? error
...
module.exports.registerAsyncThemeHelper = function registerAsyncThemeHelper(
  name,
  fn
) {
