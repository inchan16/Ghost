...
module.exports = function entryController(req, res, next) {
  ...
  return dataService
    .entryLookup(req.path, res.routerOptions, res.locals)
    .then(function then(lookup) {
      ...
      if (lookup.isEditURL) {
        ...
        const resourceType = res.routerOptions?.context?.includes('page')
          ? 'page'
          : 'post';

        return urlUtils.redirectToAdmin(
          302,
          res,
       ...
       */
      if (
        routerManager.getResourceById(entry.id).config.type !==
        res.routerOptions.resourceType
      ) {
       ...
       */
      if (
        urlUtils.absoluteToRelative(entry.url, {
