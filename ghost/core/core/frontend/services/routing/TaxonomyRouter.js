...
class TaxonomyRouter extends ParentRouter {
  ...
  _registerRoutes() {
    ...
    this.mountRoute(
      urlUtils.urlJoin(this.permalinks.value, 'page', ':page(\\d+)'),
      controllers.channel
    );
    ...
    if (config.get('admin:redirects')) {
      this.mountRoute(
        urlUtils.urlJoin(this.permalinks.value, 'edit'),
        this._redirectEditOption.bind(this)
      );
    }
  ...
  _prepareContext(req, res, next) {
    res.routerOptions = {
      ...
      data: {
        [this.taxonomyKey]: this.RESOURCE_CONFIG.QUERY[this.taxonomyKey],
      },
  ...
  _redirectEditOption(req, res) {
    urlUtils.redirectToAdmin(
      302,
      res,
      this.RESOURCE_CONFIG.TAXONOMIES[this.taxonomyKey].editRedirect.replace(
        ':slug',
        req.params.slug
      )
    );
