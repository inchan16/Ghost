...
function updateLocalTemplateOptions(req, res, next) {
  ...
  const previewData = preview.handle(
    req,
    Object.keys(customThemeSettingsCache.getAll())
  );
  ...
  const member = req.member
    ? {
        ...
        subscriptions:
          req.member.subscriptions &&
          req.member.subscriptions.map((sub) => {
            return Object.assign({}, sub, {
              default_payment_card_last4:
                sub.default_payment_card_last4 || '****',
            });
          }),
