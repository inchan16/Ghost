...
module.exports = function (req, res, next) {
  ...
  const redirectToOriginal = () => {
    ...
    let url = req.originalUrl.replace(`/size/${requestedDimension}`, '');
  ...
  if (
    ...
    (!imageDimensionConfig.width && !imageDimensionConfig.height)
  ) {
  ...
  if (
    !format &&
    !imageTransform.shouldResizeFileExtension(requestUrlFileExtension)
  ) {
    return redirectToOriginal();
  }
  ...
  storageInstance
    ...
    .then((exists) => {
      ...
      return imageSize
        .getOriginalImagePath(imagePath)
        .then((storagePath) => {
    ...
    .catch(function (err) {
      if (
        err.code === 'SHARP_INSTALLATION' ||
        err.code === 'IMAGE_PROCESSING' ||
        err.errorType === 'NoContentError'
      ) {
        return redirectToOriginal();
      }
      next(err);
