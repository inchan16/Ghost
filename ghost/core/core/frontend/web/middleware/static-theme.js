...
function isAllowedFile(file) {
  ...
  return (
    allowedFiles.includes(base) ||
    (normalizedFilePath.startsWith(allowedPath) && !alwaysDeny.includes(ext))
  );
}
...
function forwardToExpressStatic(req, res, next) {
  ...
  express.static(themeEngine.getActive().path, {
    maxAge:
      configMaxAge || configMaxAge === 0 ? configMaxAge : constants.ONE_YEAR_MS,
  })(req, res, next);
}
...
function staticTheme() {
  return function denyStatic(req, res, next) {
    if (
      !isAllowedFile(req.path.toLowerCase()) &&
      isDeniedFile(req.path.toLowerCase())
    ) {
      return next();
    }
