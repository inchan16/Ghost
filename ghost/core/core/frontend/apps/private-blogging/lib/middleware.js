...
const privateBlogging = {
  ...
  filterPrivateRoutes: function filterPrivateRoutes(req, res, next) {
    ...
    let isPrivateRSS = new RegExp(
      `/${settingsCache.get('public_hash')}/rss(/)?$`
    );
    ...
    privateBlogging.authenticatePrivateSession(
      req,
      res,
  ...
  authenticatePrivateSession: function authenticatePrivateSession(
    req,
    res,
    next
  ) {
  ...
  redirectPrivateToHomeIfLoggedIn: function redirectPrivateToHomeIfLoggedIn(
    req,
    res,
    next
  ) {
  ...
  handle404: function handle404(err, req, res, next) {
    ...
    return privateBlogging.authenticatePrivateSession(
      req,
      res,
      function onSessionVerified() {
