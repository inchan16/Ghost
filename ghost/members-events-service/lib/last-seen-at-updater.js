...
class LastSeenAtUpdater {
   ...
   */
  constructor({ services: { settingsCache }, getMembersApi, db }) {
    if (!getMembersApi) {
  ...
  subscribe(domainEvents) {
    domainEvents.subscribe(MemberPageViewEvent, async (event) => {
      try {
        await this.updateLastSeenAt(
          event.data.memberId,
          event.data.memberLastSeenAt,
          event.timestamp
        );
      } catch (err) {
        logging.error(
          `Error in LastSeenAtUpdater.MemberPageViewEvent listener for member ${event.data.memberId}`
        );
        logging.error(err);
      }
    });
    ...
    domainEvents.subscribe(MemberLinkClickEvent, async (event) => {
      try {
        ...
        logging.error(
          `Error in LastSeenAtUpdater.MemberLinkClickEvent listener for member ${event.data.memberId}`
        );
        logging.error(err);
      }
    });
    ...
    domainEvents.subscribe(MemberCommentEvent, async (event) => {
      try {
        ...
        logging.error(
          `Error in LastSeenAtUpdater.MemberCommentEvent listener for member ${event.data.memberId}`
        );
        logging.error(err);
      }
    });
    ...
    domainEvents.subscribe(EmailOpenedEvent, async (event) => {
      try {
        await this.updateLastSeenAtWithoutKnownLastSeen(
          event.memberId,
          event.timestamp
        );
      } catch (err) {
        logging.error(
          `Error in LastSeenAtUpdater.EmailOpenedEvent listener for member ${event.memberId}, emailRecipientId ${event.emailRecipientId}`
        );
        logging.error(err);
      }
    });
  ...
  async updateLastSeenAtWithoutKnownLastSeen(memberId, timestamp) {
    ...
    const startOfDayInSiteTimezone = moment
      .utc(timestamp)
      .tz(timezone)
      .startOf('day')
      .utc()
  ...
  async updateLastSeenAt(memberId, memberLastSeenAt, timestamp) {
    ...
    if (
      ...
      moment(moment.utc(timestamp).tz(timezone).startOf('day')).isAfter(
        memberLastSeenAt
      )
    ) {
  ...
  async updateLastCommentedAt(memberId, timestamp) {
    const membersApi = this._getMembersApi();
    const member = await membersApi.members.get(
      { id: memberId },
      { require: true }
    );
    const timezone = this._settingsCacheService.get('timezone');
    ...
    if (
      ...
      memberLastCommentedAt === null ||
      moment(moment.utc(timestamp).tz(timezone).startOf('day')).isAfter(
        memberLastCommentedAt
      )
    ) {
      await membersApi.members.update(
        {
          last_seen_at: moment.utc(timestamp).format('YYYY-MM-DD HH:mm:ss'),
          last_commented_at: moment
            .utc(timestamp)
            .format('YYYY-MM-DD HH:mm:ss'),
        },
        {
