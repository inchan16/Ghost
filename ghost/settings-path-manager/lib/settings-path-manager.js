...
const messages = {
  incorrectPathsParameter:
    'Attempted to setup settings path manager without paths values.',
};
...
class SettingsPathManager {
  ...
  getDefaultFilePath() {
    ...
    return path.join(
      settingsFolder,
      `${this.filename}.${this.defaultExtension}`
    );
  }
  ...
  getBackupFilePath() {
    ...
    return path.join(
      settingsFolder,
      `${this.filename}-${dateStamp}.${this.defaultExtension}`
    );
  }
}
