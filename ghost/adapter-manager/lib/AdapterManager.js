  ...
  getAdapter(adapterName, adapterClassName, config) {
    if (!adapterName || !adapterClassName) {
      throw new errors.IncorrectUsageError({
        message:
          'getAdapter must be called with a adapterName and a adapterClassName.',
      });
    }
    ...
    if (!(adapter instanceof this.baseClasses[adapterType])) {
      if (
        Object.getPrototypeOf(Adapter).name !==
        this.baseClasses[adapterType].name
      ) {
        throw new errors.IncorrectUsageError({
