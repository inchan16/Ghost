...
class StaffService {
  ...
  /** @private */
  async getDataFromIds({
    memberId,
    tierId = null,
    subscriptionId = null,
    offerId = null,
  }) {
    const memberModel = memberId
      ? await this.models.Member.findOne({ id: memberId })
      : null;
    const tierModel = tierId
      ? await this.models.Product.findOne({ id: tierId })
      : null;
    const subscriptionModel = subscriptionId
      ? await this.models.StripeCustomerSubscription.findOne({
          id: subscriptionId,
        })
      : null;
    const offerModel = offerId
      ? await this.models.Offer.findOne({ id: offerId })
      : null;
  ...
  async handleEvent(type, event) {
    ...
    if (type === MemberCreatedEvent && member.status === 'free') {
      ...
      try {
        ...
        this.logging.warn(
          `Failed to get attribution for member - ${event?.data?.memberId}`
        );
      }
  ...
  subscribeEvents() {
    ...
    this.DomainEvents.subscribe(MemberCreatedEvent, async (event) => {
      try {
        ...
        this.logging.error(
          e,
          `Failed to notify free member signup - ${event?.data?.memberId}`
        );
      }
    });
    ...
    this.DomainEvents.subscribe(SubscriptionActivatedEvent, async (event) => {
      try {
        ...
        this.logging.error(
          e,
          `Failed to notify paid member subscription start - ${event?.data?.memberId}`
        );
      }
    });
