...
class StaffServiceEmails {
  ...
  async notifyFreeMemberSignup({ member, attribution }, options) {
    const users = await this.models.User.getEmailAlertUsers(
      'free-signup',
      options
    );
    ...
    for (const user of users) {
      ...
      const templateData = {
        ...
        toEmail: to,
        staffUrl: this.urlUtils.urlJoin(
          this.urlUtils.urlFor('admin', true),
          '#',
          `/settings/staff/${user.slug}`
        ),
      };
  ...
  async notifyPaidSubscriptionStarted(
    { member, subscription, offer, tier, attribution },
    options = {}
  ) {
    const users = await this.models.User.getEmailAlertUsers(
      'paid-started',
  ...
  async notifyPaidSubscriptionCanceled(
    { member, tier, subscription },
    options = {}
  ) {
    const users = await this.models.User.getEmailAlertUsers(
      'paid-canceled',
  ...
  async notifyMilestoneReceived({ milestone }) {
    ...
    const formattedValue = this.getFormattedAmount({
      currency: milestone?.currency,
      amount: milestone.value,
      maximumFractionDigits: 0,
    });
    const milestoneEmailConfig = require('./milestone-email-config')(
      this.settingsCache.get('title'),
      formattedValue
    );
  ...
  getMemberData(member) {
    ...
    return {
      ...
      adminUrl: this.urlUtils.urlJoin(
        this.urlUtils.urlFor('admin', true),
        '#',
        `/members/${member.id}`
      ),
  ...
  getFormattedAmount({ amount = 0, currency, maximumFractionDigits = 2 }) {
    if (!currency) {
      return amount > 0
        ? Intl.NumberFormat('en', { maximumFractionDigits }).format(amount)
        : '';
    }
  ...
  getOfferData(offer) {
    if (offer) {
      ...
      if (offer.type === 'percent') {
        ...
        offAmount = `${this.getFormattedAmount({
  ...
  get siteDomain() {
    const [, siteDomain] = this.urlUtils
      .getSiteUrl()
  ...
  registerPartials() {
    ...
    const files = glob.sync('*.hbs', {
      cwd: path.join(__dirname, rootDirname),
    });
