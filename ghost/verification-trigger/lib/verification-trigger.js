...
class VerificationTrigger {
  ...
  async _handleMemberCreatedEvent(event) {
    ...
    if (['api', 'admin'].includes(source) && isFinite(sourceThreshold)) {
      ...
      const membersTotal = (
        await this._eventRepository.getSignupEvents(
          {},
          {
            source: 'member',
          }
      ...
      if (
        events.meta.pagination.total > Math.max(sourceThreshold, membersTotal)
      ) {
  ...
  async testImportThreshold() {
    ...
    const importThreshold = Math.max(
      membersTotal - events.meta.pagination.total,
      this._importTriggerThreshold
    );
    if (
      isFinite(importThreshold) &&
      events.meta.pagination.total > importThreshold
    ) {
      await this._startVerificationProcess({
        amount: events.meta.pagination.total,
        throwOnTrigger: false,
   ...
   */
  async _startVerificationProcess({ amount, throwOnTrigger, source }) {
    if (!this._isVerified()) {
