...
module.exports = class EventRepository {
  ...
  async getEventTimeline(options = {}) {
    ...
    const totalEvents = allEventPages.reduce(
      (accumulator, page) => accumulator + page.meta.pagination.total,
      0
    );
    ...
    return {
      ...
      meta: {
        pagination: {
          ...
          total: totalEvents,
          pages:
            options.limit > 0 ? Math.ceil(totalEvents / options.limit) : null,
  ...
  async getSubscriptionEvents(options = {}, filter) {
          ...
          return expandFilters(f, [
            {
              ...
              expansion: {
                'subscriptionCreatedEvent.attribution_type': 'post',
                type: 'created',
              },
            },
          ]);
    ...
    const { data: models, meta } =
      await this._MemberPaidSubscriptionEvent.findPage(options);

    const data = models.map((model) => {
      const tierName =
        model.related('stripeSubscription') &&
      ...
      const d = {
        ...
        attribution:
          ...
          model.related('subscriptionCreatedEvent').id
            ? this._memberAttributionService.getEventAttribution(
                model.related('subscriptionCreatedEvent')
              )
            : null,
        signup:
          ...
          model.related('subscriptionCreatedEvent').id &&
          model
            .related('subscriptionCreatedEvent')
            .related('memberCreatedEvent') &&
          model
            .related('subscriptionCreatedEvent')
            .related('memberCreatedEvent').id
            ? true
            : false,
        tierName,
      };
  ...
  async getPaymentEvents(options = {}, filter) {
    ...
    const { data: models, meta } = await this._MemberPaymentEvent.findPage(
      options
    );
  ...
  async getLoginEvents(options = {}, filter) {
    ...
    const { data: models, meta } = await this._MemberLoginEvent.findPage(
      options
    );
  ...
  async getSignupEvents(options = {}, filter) {
    ...
    const { data: models, meta } = await this._MemberCreatedEvent.findPage(
      options
    );
    ...
    const data = models.map((model) => {
      return {
        ...
        data: {
          ...model.toJSON(options),
          attribution:
            this._memberAttributionService.getEventAttribution(model),
        },
      };
    });
  ...
  async getClickEvents(options = {}, filter) {
    ...
    const { data: models, meta } = await this._MemberLinkClickEvent.findPage(
      options
    );
  ...
  async getEmailSpamComplaintEvents(options = {}, filter) {
    ...
    const { data: models, meta } = await this._EmailSpamComplaintEvent.findPage(
      options
    );
        ...
        [result.currency]: accumulator[result.currency].concat([
          {
            date: result.date,
            mrr:
              result.mrr_delta + accumulator[result.currency].slice(-1)[0].mrr,
            currency: result.currency,
          },
        ]),
      };
  ...
  async getStatuses() {
    ...
    const cumulativeResults = resultsJSON.reduce(
