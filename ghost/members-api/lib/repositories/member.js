...
const messages = {
  ...
  addProductWithActiveSubscription:
    'Cannot add comped Products to a Member with active Subscriptions',
  deleteProductWithActiveSubscription:
  ...
  bulkActionRequiresFilter:
    'Cannot perform {action} without a filter or all=true',
...
module.exports = class MemberRepository {
  ...
  isComplimentarySubscription(subscription) {
    return (
      ...
      subscription.plan.nickname.toLowerCase() === 'complimentary'
    );
  }
  ...
  getMRR({
    ...
    if (
      ...
      if (discount.coupon.amount_off !== null) {
        amountWithDiscount = Math.max(
          0,
          amountWithDiscount - discount.coupon.amount_off
        );
      } else {
        amountWithDiscount = Math.round(
          (amountWithDiscount * (100 - discount.coupon.percent_off)) / 100
        );
      }
    }
  ...
  async create(data, options) {
    ...
    if (memberData.products && memberData.products.length > 1) {
      throw new errors.BadRequestError({
        message: tpl(messages.moreThanOneProduct),
      });
    }
    ...
    if (memberData.subscribed !== false && !memberData.newsletters) {
      ...
      memberData.newsletters = await this.getSubscribeOnSignupNewsletters(
        browseOptions
      );
    }
  ...
  async getSubscribeOnSignupNewsletters(browseOptions) {
    ...
    browseOptions.filter =
      'status:active+subscribe_on_signup:true+visibility:members';
  ...
  async update(data, options) {
    ...
    const needsNewsletters =
      memberData.newsletters || typeof memberData.subscribed === 'boolean';
    ...
    if (!initialMember) {
      throw new NotFoundError({
        message: tpl(messages.memberNotFound, { id: options.id }),
      });
    }

    // Throw error if email is invalid and it's been changed
    if (
      initialMember?.get('email') &&
      memberData.email &&
    ...
    if (needsProducts) {
      ...
      if (
        incomingProductIds.length > 1 &&
        incomingProductIds.length > existingProductIds.length
      ) {
        throw new errors.BadRequestError({
          message: tpl(messages.moreThanOneProduct),
        });
      ...
      productsToRemove = _.differenceWith(
        existingProductIds,
        incomingProductIds
      );
      ...
      if (productsToModify.length !== 0) {
        ...
        const exisitingSubscriptions =
          initialMember.related('stripeSubscriptions')?.models ?? [];
        ...
        if (productsToRemove.length > 0) {
          ...
          for (const deleteId of productsToRemove) {
            if (dontAllowToRemoveProductsIds.includes(deleteId)) {
              throw new errors.BadRequestError({
                message: tpl(messages.deleteProductWithActiveSubscription),
              });
            }
          }
        ...
        if (productsToAdd.length > 0) {
          ...
          // subscription
          const existingActiveSubscriptions = exisitingSubscriptions.filter(
            (subscription) => {
              return this.isActiveSubscriptionStatus(
          ...
          if (existingActiveSubscriptions.length) {
            throw new errors.BadRequestError({
              message: tpl(messages.addProductWithActiveSubscription),
            });
          }
    ...
    if (needsNewsletters) {
      ...
      if (!memberData.newsletters) {
        if (memberData.subscribed === false) {
          ...
          memberData.subscribed === true &&
          !existingNewsletters.find((n) => n.get('status') === 'active')
        ) {
          const browseOptions = _.pick(options, 'transacting');
          memberData.newsletters = await this.getSubscribeOnSignupNewsletters(
            browseOptions
      ...
      if (memberData.newsletters) {
        ...
        const incomingNewsletterIds = memberData.newsletters.map(
          (newsletter) => newsletter.id
        );

        newslettersToAdd = _.differenceWith(
          incomingNewsletterIds,
          existingNewsletterIds
        );
        newslettersToRemove = _.differenceWith(
          existingNewsletterIds,
          incomingNewsletterIds
        );
      }
      ...
      const archivedNewsletters = existingNewsletters.filter(
  ...
  async destroy(data, options) {
    ...
    if (
      this._stripeAPIService.configured &&
      options.cancelStripeSubscriptions
    ) {
      ...
      for (const subscription of subscriptions.models) {
        if (subscription.get('status') !== 'canceled') {
          const updatedSubscription =
            await this._stripeAPIService.cancelSubscription(
  ...
  async bulkDestroy(options) {
    ...
    if (!filter && !search && (!all || all !== true)) {
      throw new errors.IncorrectUsageError({
        message: tpl(messages.bulkActionRequiresFilter, {
          action: 'bulk delete',
        }),
      });
    }
    ...
    if (all !== true) {
      ...
      Object.assign(
        filterOptions,
        _.pick(options, ['filter', 'search', 'mongoTransformer'])
      );
    }

    const memberRows = await this._Member
    ...
    if (data.action === 'unsubscribe') {
      ...
      if (hasNewsletterSelected) {
        ...
        const unsubscribeRows =
          await this._MemberNewsletter.getFilteredCollectionQuery({
      ...
      if (!hasNewsletterSelected) {
        return await this._Member.bulkDestroy(
          memberIds,
          'members_newsletters',
          { column: 'member_id' }
        );
  ...
  async linkSubscription(data, options = {}) {
    if (!this._stripeAPIService.configured) {
      throw new errors.BadRequestError({
        message: tpl(messages.noStripeConnection, {
          action: 'link Stripe Subscription',
        }),
      });
    }
    ...
    if (!customer) {
      ...
      throw new errors.NotFoundError({
        message: tpl(messages.subscriptionNotFound),
      });
    }
    ...
    const paymentMethod = paymentMethodId
      ? await this._stripeAPIService.getCardPaymentMethod(paymentMethodId)
      : null;

    const model = await this.getSubscriptionByStripeID(subscription.id, {
      ...options,
      forUpdate: true,
    });
    ...
    try {
      ...
      if (ghostProduct) {
            ...
            stripe_prices: [
              {
                ...
                interval:
                  (subscriptionPriceData.recurring &&
                    subscriptionPriceData.recurring.interval) ||
                  null,
              },
            ],
          },
        ...
        logging.error(
          `There was an error linking subscription - ${subscription.id}, no Products exist.`
        );
      }
    } catch (e) {
      logging.error(
        `Failed to handle prices and product for - ${subscription.id}.`
      );
      logging.error(e);
    }
    ...
    if (stripeCouponId) {
      ...
      offer = await this._offerRepository.getByStripeCouponId(stripeCouponId, {
        transacting: options.transacting,
      });
      if (offer) {
        offerId = offer.id;
        ...
        logging.error(
          `Received an unknown stripe coupon id (${stripeCouponId}) for subscription - ${subscription.id}.`
        );
      }
    } else if (offerId) {
      offer = await this._offerRepository.getById(offerId, {
        transacting: options.transacting,
      });
    }
    ...
    const subscriptionData = {
      ...
      cancellation_reason:
        (subscription.metadata && subscription.metadata.cancellation_reason) ||
        null,
      ...
      plan_nickname:
        subscriptionPriceData.nickname ||
        _.get(subscriptionPriceData, 'recurring.interval'),
    ...
    if (model) {
      ...
      const updated = await this._StripeCustomerSubscription.edit(
        subscriptionData,
        {
          ...options,
          id: model.id,
      ...
      if (
        model.get('mrr') !== updated.get('mrr') ||
        model.get('plan_id') !== updated.get('plan_id') ||
        model.get('status') !== updated.get('status') ||
        model.get('cancel_at_period_end') !==
      ...
      const subscriptionModel = await this._StripeCustomerSubscription.add(
        subscriptionData,
        options
      );
      await this._MemberPaidSubscriptionEvent.add(
    ...
    let memberProducts = (
      await member.related('products').fetch(options)
    ).toJSON();
    ...
    if (this.isActiveSubscriptionStatus(subscription.status)) {
      ...
      if (model) {
        if (model.get('stripe_price_id') !== subscriptionData.stripe_price_id) {
          ...
          const subscriptions = await member
          ...
          for (const subscriptionModel of subscriptions.models) {
            if (
              this.isActiveSubscriptionStatus(subscriptionModel.get('status'))
            ) {
              try {
                const subscriptionProduct = await this._productRepository.get(
                  {
                    stripe_price_id: subscriptionModel.get('stripe_price_id'),
                  },
                  options
                );
                if (
                  subscriptionProduct &&
                  changedProduct &&
                  subscriptionProduct.id === changedProduct.id
                ) {
                  activeSubscriptionForChangedProduct = true;
                }
              } catch (e) {
                logging.error(
                  `Failed to attach products to member - ${data.id}`
                );
                logging.error(e);
              }
            }
        ...
        if (this.isActiveSubscriptionStatus(subscriptionModel.get('status'))) {
          ...
          try {
            ...
            if (
              subscriptionProduct &&
              ghostProduct &&
              subscriptionProduct.id === ghostProduct.id
            ) {
    ...
    try {
      ...
      updatedMember = await this._Member.edit(
        { status: status, products: memberProducts },
        { ...options, id: data.id }
      );
    } catch (e) {
      logging.error(
        `Failed to update member - ${data.id} - with related products`
      );
      logging.error(e);
      updatedMember = await this._Member.edit(
        { status: status },
    ...
    const productsToAdd = _.differenceWith(
      newMemberProductIds,
      oldMemberProductIds
    );
    const productsToRemove = _.differenceWith(
      oldMemberProductIds,
      newMemberProductIds
    );
    ...
    if (
      updatedMember.attributes.status !==
      updatedMember._previousAttributes.status
    ) {
      await this._MemberStatusEvent.add(
        {
          member_id: data.id,
  ...
  async cancelSubscription(data, options) {
    ...
    if (!this._stripeAPIService.configured) {
      throw new errors.BadRequestError({
        message: tpl(messages.noStripeConnection, {
          action: 'update Stripe Subscription',
        }),
      });
    }
  ...
  async updateSubscription(data, options) {
    ...
    if (data.subscription.price) {
      ...
      if (data.subscription.price !== subscription.price) {
        ...
        updatedSubscription =
          await this._stripeAPIService.removeCouponFromSubscription(
            subscription.id
          );
      }
    }
  ...
  async createSubscription(data, options) {
    ...
    for (const customer of member.related('stripeCustomers').models) {
      try {
        const fetchedCustomer = await this._stripeAPIService.getCustomer(
          customer.get('customer_id')
        );
  ...
  async setComplimentarySubscription(data, options = {}) {
    ...
    if (!this._stripeAPIService.configured) {
      throw new errors.BadRequestError({
        message: tpl(messages.noStripeConnection, {
          action: 'create Complimentary Subscription',
        }),
      });
    }
    const member = await this._Member.findOne(
    ...
    if (activeSubscriptions.length) {
      for (const subscription of activeSubscriptions) {
        ...
        let zeroValuePrice = zeroValuePrices.find((p) => {
          return (
            p.currency.toLowerCase() === price.get('currency').toLowerCase()
          );
        });
          ...
          zeroValuePrice = product.stripePrices.find((p) => {
            return (
              p.currency.toLowerCase() ===
                price.get('currency').toLowerCase() && p.amount === 0
            );
          });
  ...
  async cancelComplimentarySubscription({ id }, options) {
    if (!this._stripeAPIService.configured) {
      throw new errors.BadRequestError({
        message: tpl(messages.noStripeConnection, {
          action: 'cancel Complimentary Subscription',
        }),
      });
    }
    ...
    for (const subscription of subscriptions.models) {
      if (subscription.get('status') !== 'canceled') {
        try {
          ...
          logging.error(
            `There was an error cancelling subscription ${subscription.get(
              'subscription_id'
            )}`
          );
          logging.error(err);
