...
class ProductRepository {
  ...
  async get(data, options = {}) {
    ...
    if ('stripe_price_id' in data) {
      ...
      const stripeProduct = await stripePrice
    ...
    throw new NotFoundError({
      message:
        'Missing id, slug, stripe_product_id or stripe_price_id from data',
    });
  }
  ...
  async create(data, options = {}) {
    if (
      ...
      (data.stripe_prices || data.monthly_price || data.yearly_price)
    ) {
      throw new UpdateCollisionError({
        message:
          'The requested functionality requires Stripe to be configured. See https://ghost.org/integrations/stripe/',
    ...
    if (
      data.yearly_price &&
      data.monthly_price &&
      data.yearly_price.currency !== data.monthly_price.currency
    ) {
    ...
    if (this._stripeAPIService.configured) {
      ...
      if (data.monthly_price || data.yearly_price) {
        if (data.monthly_price) {
          ...
          await this._Product.edit(
            { monthly_price_id: stripePrice.id },
            { id: product.id, transacting: options.transacting }
          );
        }
        ...
        if (data.yearly_price) {
          ...
          await this._Product.edit(
            { yearly_price_id: stripePrice.id },
            { id: product.id, transacting: options.transacting }
          );
        }
  ...
  async update(data, options = {}) {
    ...
    const existingProduct = await this._Product.findOne(
      { id: productId },
      options
    );
    ...
    if (
      existingProduct.get('active') === true &&
      ...
      const updatedProducts = portalProducts.filter(
    ...
    if (this._stripeAPIService.configured && product.get('type') !== 'free') {
      ...
      if (!product.related('stripeProducts').first()) {
        ...
        if (product.attributes.name !== product._previousAttributes.name) {
          ...
          await this._stripeAPIService.updateProduct(
      ...
      if (data.monthly_price || data.yearly_price) {
        if (data.monthly_price) {
          ...
          if (existingPrice) {
            ...
            await this._stripeAPIService.updatePrice(
              priceModel.get('stripe_price_id'),
              {
                active: true,
              }
            );
          ...
          product = await this._Product.edit(
            { monthly_price_id: priceModel.id },
        ...
        if (data.yearly_price) {
          ...
          product = await this._Product.edit(
            { yearly_price_id: priceModel.id },
            { ...options, id: product.id }
          );
        }
      } else if (data.stripe_prices) {
        const newPrices = data.stripe_prices.filter(
        ...
        for (const existingPrice of existingPrices) {
          ...
          let stripeProduct = await this._StripeProduct.findOne(
        ...
        for (const newPrice of newPrices) {
          ...
          const stripeProduct = newProductId
            ? await this._StripeProduct.findOne(
                { stripe_product_id: newProductId },
                options
              )
            : defaultStripeProduct;
  ...
  async destroy() {
    throw new MethodNotAllowedError({
      message: 'Cannot destroy products, yet...',
    });
  }
}
