...
module.exports = class RouterController {
  ...
  async createCheckoutSetupSession(req, res) {
    ...
    if (!req.body.subscription_id) {
      customer =
        await this._stripeAPIService.getCustomerForMemberCheckoutSession(
          member
        );
    } else {
      ...
      if (!subscription) {
        ...
        return res.end(
          `Could not find subscription ${req.body.subscription_id}`
        );
      }
      customer = await this._stripeAPIService.getCustomer(
        subscription.get('customer_id')
      );
    }

    const session = await this._stripeAPIService.createCheckoutSetupSession(
      customer,
      {
  ...
  async createCheckoutSession(req, res) {
    ...
    if (metadata.urlHistory) {
      ...
      const attribution = await this._memberAttributionService.getAttribution(
        urlHistory
      );
  ...
  async sendMagicLink(req, res) {
    ...
    try {
      if (emailType === 'signup' || emailType === 'subscribe') {
        ...
        tokenData.attribution =
          await this._memberAttributionService.getAttribution(
            req.body.urlHistory
          );
      ...
      throw new errors.BadRequestError({
        message: this._allowSelfSignup()
          ? tpl(messages.memberNotFoundSignUp)
          : tpl(messages.memberNotFound),
      });
    } catch (err) {
