...
module.exports = class MemberBREADService {
  ...
  attachSubscriptionsToMember(member) {
    ...
    const subscriptionProducts = (member.subscriptions || [])
      .filter((sub) =>
        this.memberRepository.isActiveSubscriptionStatus(sub.status)
      )
  ...
  async fetchSubscriptionOffers(subscriptions) {
    ...
    try {
      ...
      logging.error(
        `Failed to load offers for subscriptions - ${subscriptions
          .map((s) => s.id)
          .join(', ')}.`
      );
      logging.error(e);
  ...
  async attachAttributionsToMember(member, subscriptionIdMap) {
    // Created attribution
    member.attribution =
      await this.memberAttributionService.getMemberCreatedAttribution(
        member.id
      );
    ...
    for (const subscription of member.subscriptions) {
      ...
      subscription.attribution =
        await this.memberAttributionService.getSubscriptionCreatedAttribution(
          id
        );
    }
  }
  ...
  async read(data, options = {}) {
    ...
    const withRelated = new Set(
      (options.withRelated || []).concat(defaultWithRelated)
    );
    ...
    if (this.labsService.isSet('suppressionList')) {
      const suppressionData =
        await this.emailSuppressionList.getSuppressionData(member.email);
  ...
  async add(data, options) {
    if (
      !this.stripeService.configured &&
      (data.comped || data.stripe_customer_id)
    ) {
    ...
    try {
      const attribution =
        await this.memberAttributionService.getAttributionFromContext(
          options?.context
        );
      if (attribution) {
    ...
    try {
      ...
      if (isStripeLinkingError) {
        if (
          error.message.indexOf('customer') &&
          error.code === 'resource_missing'
        ) {
          error.message = `Member not imported. ${error.message}`;
          error.context = 'Missing Stripe Customer';
          error.help =
            "Make sure you're connected to the correct Stripe Account";
        }
  ...
  async edit(data, options) {
    ...
    if (this.stripeService.configured) {
      const hasCompedSubscription = !!model
        ...
        .find(
          (sub) =>
            sub.get('plan_nickname') === 'Complimentary' &&
  ...
  async browse(options) {
    ...
    if (this.labsService.isSet('suppressionList')) {
      bulkSuppressionData =
        await this.emailSuppressionList.getBulkSuppressionData(
