...
const offerSetup = async ({ site, member = null, offer }) => {
  ...
  const utils = appRender(<App api={ghostApi} />);
  ...
  const submitButton = within(popupIframeDocument).queryByRole('button', {
    name: 'Continue',
  });
  const chooseBtns = within(popupIframeDocument).queryAllByRole('button', {
    name: 'Choose',
  });
  const signinButton = within(popupIframeDocument).queryByRole('button', {
    name: 'Sign in',
  });
  const siteTitle = within(popupIframeDocument).queryByText(site.title);
  const offerName = within(popupIframeDocument).queryByText(
    offer.display_title
  );
  const offerDescription = within(popupIframeDocument).queryByText(
  ...
  const fullAccessTitle =
    within(popupIframeDocument).queryByText('Full access');
  return {
    ghostApi,
    popupIframeDocument,
    popupFrame,
...
const multiTierSetup = async ({ site, member = null }) => {
  ...
  const utils = appRender(<App api={ghostApi} />);
  const freeTierDescription = site.products?.find(
    (p) => p.type === 'free'
  )?.description;
  const triggerButtonFrame = await utils.findByTitle(/portal-trigger/i);
  const popupFrame = utils.queryByTitle(/portal-popup/i);
  ...
  const freePlanDescription =
    within(popupIframeDocument).queryAllByText(freeTierDescription);
  const monthlyPlanTitle = within(popupIframeDocument).queryByText('Monthly');
  const yearlyPlanTitle = within(popupIframeDocument).queryByText('Yearly');
  const fullAccessTitle =
    within(popupIframeDocument).queryByText('Full access');
...
describe('Signup', () => {
  describe('as free member on single tier site', () => {
    test('with default settings', async () => {
      ...
      const continueButton = within(popupIframeDocument).queryAllByRole(
        'button',
        { name: 'Continue' }
      );
      ...
      const magicLink = await within(popupIframeDocument).findByText(
        /now check your email/i
      );
    ...
    test('with only free plan', async () => {
      ...
      submitButton = within(popupIframeDocument).queryByRole('button', {
        name: 'Sign up',
      });
  ...
  describe('as paid member on single tier site', () => {
    test('with default settings on monthly plan', async () => {
      ...
      const monthlyPlanContainer =
        within(popupIframeDocument).queryByText(/Monthly$/);
      const singleTierProduct = FixtureSite.singleTier.basic.products.find(
        (p) => p.type === 'paid'
      );
    ...
    test('with default settings on yearly plan', async () => {
      ...
      const yearlyPlanContainer =
        within(popupIframeDocument).queryByText(/Yearly$/);
      const singleTierProduct = FixtureSite.singleTier.basic.products.find(
        (p) => p.type === 'paid'
      );
    ...
    test('with only paid plans available', async () => {
      ...
      const submitButton = within(popupIframeDocument).queryAllByRole(
        'button',
        { name: 'Continue' }
      );
    ...
    test('to an offer via link', async () => {
      ...
      let planId = FixtureSite.singleTier.basic.products.find(
        (p) => p.type === 'paid'
      ).monthlyPrice.id;
      let tier = FixtureSite.singleTier.basic.products.find(
        (p) => p.type === 'paid'
      );
...
describe('Signup', () => {
  ...
  describe('as paid member on multi tier site', () => {
    test('with default settings', async () => {
      ...
      const firstPaidTier = FixtureSite.singleTier.basic.products.find(
        (p) => p.type === 'paid'
      );
      ...
      const labelText = popupIframeDocument.querySelector(
    ...
    test('to an offer via link with portal disabled', async () => {
      ...
      const singleTier = FixtureSite.singleTier.basic.products.find(
        (p) => p.type === 'paid'
      );
      let planId = FixtureSite.singleTier.basic.products.find(
        (p) => p.type === 'paid'
