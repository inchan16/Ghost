...
describe('Member Data attributes:', () => {
  ...
  describe('data-members-form', () => {
    test('allows free signup', () => {
      ...
      expect(window.fetch).toHaveBeenCalledWith(
        'https://portal.localhost/members/api/send-magic-link/',
        {
          body: expectedBody,
  ...
  describe('data-members-plan', () => {
    test('allows new member paid signup via direct checkout', async () => {
      const { event, errorEl, siteUrl, clickHandler, site, member, element } =
        getMockData();

      const paidTier = site.products.find((p) => p.type === 'paid');

      await planClickHandler({
        event,
        errorEl,
        siteUrl,
        clickHandler,
  ...
  describe('data-members-plan', () => {
    test('allows free member upgrade via direct checkout', async () => {
      let { event, errorEl, siteUrl, clickHandler, site, member, element } =
        getMockData();
...
const setup = async ({ site, member = null, showPopup = true }) => {
  ...
  const utils = appRender(<App api={ghostApi} showPopup={showPopup} />);
...
describe('Portal Data attributes:', () => {
  ...
  describe('data-portal=signin', () => {
    test('opens Portal signin page', async () => {
      ...
      popupFrame = await utils.findByTitle(/portal-popup/i);
      expect(popupFrame).toBeInTheDocument();
      const loginTitle = within(popupFrame.contentDocument).queryByText(
        /sign in/i
      );
  ...
  describe('data-portal=account', () => {
    test('opens Portal account home page', async () => {
      ...
      popupFrame = await utils.findByTitle(/portal-popup/i);
      expect(popupFrame).toBeInTheDocument();
      const accountHomeTitle = within(popupFrame.contentDocument).queryByText(
        /your account/i
      );
  ...
  describe('data-portal=account/plans', () => {
    test('opens Portal account plan page', async () => {
      ...
      popupFrame = await utils.findByTitle(/portal-popup/i);
      expect(popupFrame).toBeInTheDocument();
      const accountPlanTitle = within(popupFrame.contentDocument).queryByText(
        /choose a plan/i
      );
  ...
  describe('data-portal=account/profile', () => {
    test('opens Portal account profile page', async () => {
      ...
      popupFrame = await utils.findByTitle(/portal-popup/i);
      expect(popupFrame).toBeInTheDocument();
      const accountProfileTitle = within(
        popupFrame.contentDocument
      ).queryByText(/account settings/i);
