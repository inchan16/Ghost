...
const setup = async ({ site, member = null }) => {
  ...
  const fullAccessTitle =
    within(popupIframeDocument).queryByText('Full access');
  const accountHomeTitle =
    within(popupIframeDocument).queryByText('Your account');
  const viewPlansButton = within(popupIframeDocument).queryByRole('button', {
    name: 'View plans',
  });
  return {
    ghostApi,
...
describe('Logged-in free member', () => {
  describe('can upgrade on single tier site', () => {
    test('with default settings on monthly plan', async () => {
      ...
      const monthlyPlanContainer = await within(popupIframeDocument).findByText(
        'Monthly'
      );
    ...
    test('with default settings on yearly plan', async () => {
      ...
      const yearlyPlanContainer = await within(popupIframeDocument).findByText(
        'Yearly'
      );
    ...
    test('to an offer via link', async () => {
      ...
      let singleTierProduct = FixtureSite.singleTier.basic.products.find(
        (p) => p.type === 'paid'
      );
...
describe('Logged-in free member', () => {
  describe('can upgrade on multi tier site', () => {
    test('with default settings', async () => {
      ...
      const singleTierProduct = FixtureSite.multipleTiers.basic.products.find(
        (p) => p.type === 'paid'
      );
    ...
    test('to an offer via link', async () => {
      ...
      let planId = FixtureSite.multipleTiers.basic.products.find(
        (p) => p.type === 'paid'
      ).monthlyPrice.id;
      let singleTierProduct = FixtureSite.multipleTiers.basic.products.find(
        (p) => p.type === 'paid'
      );
      let offerId = FixtureOffer.id;
      expect(popupFrame).toBeInTheDocument();
