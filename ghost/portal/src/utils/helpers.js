...
export function hasOnlyFreePlan({ plans, site = {} }) {
  ...
  return (
    !plans ||
    plans.length === 0 ||
    (plans.length === 1 && plans[0].type === 'free')
  );
}
...
export function hasPrice({ site = {}, plan }) {
  ...
  if (plan === 'free') {
    return (
      !prices || prices.length === 0 || prices.find((p) => p.type === 'free')
    );
  } else if (plan === 'monthly') {
    return (
      prices && prices.length > 0 && prices.find((p) => p.name === 'Monthly')
    );
  } else if (plan === 'yearly') {
    return (
      prices && prices.length > 0 && prices.find((p) => p.name === 'Yearly')
    );
  } else if (plan) {
...
export function isInviteOnlySite({ site = {}, pageQuery = '' }) {
  ...
  return (
    prices.length === 0 || (site && site.members_signup_access === 'invite')
  );
}
...
export function transformApiSiteData({ site }) {
  try {
    ...
    if (site.products?.[0]?.visibility) {
      ...
      site.portal_products = site.products
        .filter((p) => {
...
export function getAvailableProducts({ site }) {
  ...
  return products
    ...
    .filter((product) => {
      return !!(
        Object.keys(product.monthlyPrice).length > 0 &&
        Object.keys(product.yearlyPrice).length > 0
      );
    })
    .filter((product) => {
...
export function getProductFromPrice({ site, priceId }) {
  ...
  return products.find((product) => {
    return (
      product?.monthlyPrice?.id === priceId ||
      product?.yearlyPrice?.id === priceId
    );
  });
}
...
export const getDefaultNewsletterSender = ({ site }) => {
  ...
  if (defaultNewsletter) {
    return (
      defaultNewsletter.sender_email || `noreply@${getSiteDomain({ site })}`
    );
  }
};
...
export const getCurrencySymbol = (currency) => {
  return Intl.NumberFormat('en', { currency, style: 'currency' })
    .format(0)
    .replace(/[\d\s.]/g, '');
};
...
export const getUpdatedOfferPrice = ({
  offer,
  price,
  useFormatted = false,
}) => {
  ...
  if (
    offer.type === 'fixed' &&
    isSameCurrency(offer.currency, price.currency)
  ) {
    updatedAmount = (originalAmount - offer.amount) / 100;
    updatedAmount = updatedAmount > 0 ? updatedAmount : 0;
  } else if (offer.type === 'percent') {
    updatedAmount =
      (originalAmount - (originalAmount * offer.amount) / 100) / 100;
  } else {
