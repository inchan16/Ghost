...
function setupGhostApi({ siteUrl = window.location.origin, apiUrl, apiKey }) {
  ...
  function contentEndpointFor({ resource, params = '' }) {
    if (apiUrl && apiKey) {
      return `${apiUrl.replace(
        /\/$/,
        ''
      )}/${resource}/?key=${apiKey}&limit=all${params}`;
    }
    return '';
  }
  ...
  api.feedback = {
    async add({ uuid, postId, score }) {
      ...
      if (res.ok) {
        ...
        throw (
          (await HumanReadableError.fromApiResponse(res)) ??
  ...
  api.member = {
    ...
    async checkoutPlan({
      ...
      if (!cancelUrl) {
        const checkoutCancelUrl = window.location.href.startsWith(
          siteUrlObj.href
        )
    ...
    async updateSubscription({
      ...
      const identity = await api.member.identity();
      const url =
        endpointFor({ type: 'members', resource: 'subscriptions' }) +
        subscriptionId +
        '/';
      const body = {
