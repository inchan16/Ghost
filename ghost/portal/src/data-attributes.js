  ...
  fetch(`${siteUrl}/members/api/send-magic-link/`, {
    ...
    .catch((err) => {
      if (errorEl) {
        ...
        errorEl.innerText = HumanReadableError.getMessageFromError(
          err,
...
export function planClickHandler({
  ...
  let requestData = getCheckoutSessionDataFromPlanAttribute(
    site,
    plan.toLowerCase()
  );
    ...
    function (el) {
      ...
      function clickHandler(event) {
        ...
        fetch(`${siteUrl}/members/api/session`, {
          ...
          .then(function (identity) {
            return fetch(
              `${siteUrl}/members/api/create-stripe-update-session/`,
              {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
