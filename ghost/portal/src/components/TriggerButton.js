...
class TriggerButtonContent extends React.Component {
  ...
  componentDidMount() {
    if (this.container) {
      this.height =
        this.container.current && this.container.current.offsetHeight;
  ...
  renderTriggerIcon() {
    const {
      portal_button_icon: buttonIcon = '',
      portal_button_style: buttonStyle = '',
    } = this.context.site || {};
    const Style = Styles({ brandColor: this.context.brandColor });
    const memberGravatar =
      this.context.member && this.context.member.avatar_image;
  ...
  hasText() {
    ...
    return (
      ['icon-and-text', 'text-only'].includes(buttonStyle) &&
      !this.context.member &&
      buttonText
    );
  }
    ...
    return (
      ...
      >
        <TriggerButtonContent
          ...
          updateWidth={(width) => this.onWidthChange(width)}
        />
      </Frame>
    );
  }
