...
const PlanConfirmationSection = ({ plan, type, onConfirm }) => {
  ...
  const priceLabel = hasMultipleProductsFeature({ site })
    ? product?.name
    : t('Price');
  if (type === 'changePlan') {
    return (
    ...
    return (
      <div className="gh-portal-logged-out-form-container gh-portal-cancellation-form">
        <p>
          If you cancel your subscription now, you will continue to have access
          until{' '}
          <strong>{getDateString(subscription.current_period_end)}</strong>.
        </p>
  ...
  return (
    <section>
      ...
      <CancelSubscriptionButton
        {...{ member, onCancelSubscription, action, brandColor }}
      />
    </section>
  );
};

function PlansOrProductSection({
  showLabel,
  plans,
  selectedPlan,
  onPlanSelect,
...
const UpgradePlanSection = ({
  ...
  return (
    <section>
      <div
        className={`gh-portal-section gh-portal-accountplans-main ${singlePlanClass}`}
      >
        <PlansOrProductSection
          showLabel={false}
          plans={plans}
...
const PlansContainer = ({
  ...
  const allowUpgrade =
    allowCompMemberUpgrade({ member }) && isComplimentaryMember({ member });
...
export default class AccountPlanPage extends React.Component {
  ...
  onPlanCheckout(e, priceId) {
    ...
    const restrictCheckout = allowCompMemberUpgrade({ member })
  ...
  onPlanSelect = (e, priceId) => {
    ...
    const allowCompMember = allowCompMemberUpgrade({ member })
      ? isComplimentaryMember({ member })
      : false;
