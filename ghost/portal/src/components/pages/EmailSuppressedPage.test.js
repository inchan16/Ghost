...
const setup = (overrides) => {
  const { mockOnActionFn, ...utils } = render(<EmailSuppressedPage />);
  const resubscribeBtn = utils.queryByRole('button', {
    name: 'Re-enable emails',
  });
...
describe('Email Suppressed Page', () => {
  ...
  test('can call resubscribe button', () => {
    ...
    expect(mockOnActionFn).toHaveBeenCalledWith(
