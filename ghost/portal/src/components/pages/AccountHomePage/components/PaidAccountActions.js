...
const PaidAccountActions = () => {
  ...
  const PlanLabel = ({ price, isComplimentary, subscription }) => {
    const { offer, start_date: startDate } = subscription || {};
    let label = '';
    if (price) {
  ...
  const PlanUpdateButton = ({ isComplimentary }) => {
    const hideUpgrade = allowCompMemberUpgrade({ member })
      ? false
      : isComplimentary;
  ...
  if (subscription || isComplimentary) {
    const { price, default_payment_card_last4: defaultCardLast4 } =
      subscription || {};
    let planLabel = 'Plan';
  ...
  if (offer) {
    ...
    if (discountDuration === 'forever') {
      ...
      let offerEndDate = new Date(
        offerStartDate.setMonth(offerStartDate.getMonth() + durationInMonths)
      );
    ...
    offerLabel = `${getUpdatedOfferPrice({
      offer,
      price,
      useFormatted: true,
    })}/${price.interval}${durationLabel ? ` — ${durationLabel}` : ``}`;
  }
  return offerLabel;
}
