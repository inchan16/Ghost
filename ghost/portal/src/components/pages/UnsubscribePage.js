...
async function updateMemberNewsletters({
  ...
  try {
    return await api.member.updateNewsletters({
      uuid: memberUuid,
      newsletters,
      enableCommentNotifications,
    });
...
export default function UnsubscribePage() {
  ...
  const [subscribedNewsletters, setSubscribedNewsletters] =
    useState(defaultNewsletters);
  const [showPrefs, setShowPrefs] = useState(false);
  const { comments_enabled: commentsEnabled } = site;
  const { enable_comment_notifications: enableCommentNotifications = false } =
    member || {};

  useEffect(() => {
    const ghostApi = setupGhostApi({ siteUrl: site.url });
    (async () => {
      const memberData = await ghostApi.member.newsletters({
        uuid: pageData.uuid,
      });
  ...
  if (siteNewsletters?.length === 1 && !commentsEnabled && !showPrefs) {
    return (
      <div className="gh-portal-content gh-portal-unsubscribe with-footer">
        ...
        <div>
          <p className="gh-portal-text-center">
            <strong>{member?.email}</strong> will no longer receive this
            newsletter.
          </p>
          <p className="gh-portal-text-center">
  ...
  const HeaderNotification = () => {
    if (pageData.comments && commentsEnabled) {
      ...
      return (
        <>
          <p
            className={`gh-portal-text-center gh-portal-header-message ${hideClassName}`}
          >
            <strong>{member?.email}</strong> will no longer receive emails when
    ...
    return (
      <>
        <p
          className={`gh-portal-text-center gh-portal-header-message ${hideClassName}`}
        >
          <strong>{member?.email}</strong> will no longer receive{' '}
          <strong>{unsubscribedNewsletter?.name}</strong> newsletter.
        </p>
      </>
    );
  };
  ...
  return (
    <NewsletterManagement
      ...
      updateCommentNotifications={async (enabled) => {
        const updatedMember = await api.member.updateNewsletters({
          uuid: pageData.uuid,
          enableCommentNotifications: enabled,
        });
