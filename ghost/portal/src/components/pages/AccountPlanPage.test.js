...
import {
  generateAccountPlanFixture,
  getSiteData,
  getProductsData,
} from '../../utils/fixtures-generator';
import { render, fireEvent } from '../../utils/test-utils';
import AccountPlanPage from './AccountPlanPage';

const setup = (overrides) => {
  const { mockOnActionFn, context, ...utils } = render(<AccountPlanPage />, {
    overrideContext: {
      ...overrides,
    },
  });
...
describe('Account Plan Page', () => {
  ...
  test('can choose plan and continue', async () => {
    ...
    expect(mockOnActionFn).toHaveBeenCalledWith('checkoutPlan', {
  ...
  test('can cancel subscription for member on hidden tier', async () => {
    ...
    const confirmCancelButton = queryByRole('button', {
