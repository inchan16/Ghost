...
const {
  isReqResUserSpecific,
  cacheControlValues,
} = require('@tryghost/http-cache-utils');
...
const messages = {
  ...
  userMessages: {
    ...
    DatabaseVersionError:
      'Database version compatibility error, cannot {action}.',
...
module.exports.prepareStack = (err, req, res, next) => {
 ...
 */
module.exports.jsonErrorRenderer = (err, req, res, next) => {
  // eslint-disable-line no-unused-vars
  ...
  if (docName && method) {
    ...
    if (action) {
      ...
      if (_.get(messages.userMessages, err.name)) {
        userError.message = tpl(messages.userMessages[err.name], {
          action: action,
        });
      } else {
        userError.message = tpl(messages.UnknownError, {
          action,
          name: err.name,
        });
      }
