...
class MembersStripeCustomersSubscriptionsImporter extends TableImporter {
  ...
  constructor(
    knex,
    { membersStripeCustomers, products, stripeProducts, stripePrices }
  ) {
  ...
  generate() {
    const customer = this.membersStripeCustomers.find(
      (c) => this.model.member_id === c.member_id
    );
    const isMonthly = this.model.cadence === 'month';
    const ghostProduct = this.products.find(
      (product) => product.id === this.model.tier_id
    );
    const stripeProduct = this.stripeProducts.find(
      (product) => product.product_id === this.model.tier_id
    );
    const stripePrice = this.stripePrices.find((price) => {
      return (
        price.stripe_product_id === stripeProduct.stripe_product_id &&
        (isMonthly ? price.interval === 'month' : price.interval === 'year')
      );
    });
    const mrr = isMonthly
      ? stripePrice.amount
      : Math.floor(stripePrice.amount / 12);
    return {
