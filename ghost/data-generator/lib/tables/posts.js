...
class PostsImporter extends TableImporter {
  ...
  async addNewsletters({ posts }) {
    for (const { id, visibility } of posts) {
      await this.knex('posts')
        .update({
          newsletter_id: luck(90)
            ? visibility === 'paid'
              ? this.newsletters[0].id
              : this.newsletters[1].id
            : null,
        })
  ...
  generate() {
    ...
    return {
      ...
      published_at:
        status === 'published'
          ? dateToDatabaseString(faker.date.soon(5, timestamp))
          : null,
      ...
      mobiledoc: JSON.stringify({
        ...
        cards: [],
        markups: [['em']],
        sections: content.map((paragraph) => [1, 'p', [[0, [], 0, paragraph]]]),
      }),
      html: content.map((paragraph) => `<p>${paragraph}</p>`).join(''),
      email_recipient_filter: 'all',
      newsletter_id:
        this.type === 'post' && status === 'published' && luck(90)
          ? visibility === 'paid'
            ? this.newsletters[0].id
            : this.newsletters[1].id
          : null,
