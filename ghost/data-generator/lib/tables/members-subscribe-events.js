...
class MembersSubscribeEventsImporter extends TableImporter {
  ...
  generate() {
    ...
    let createdAt = dateToDatabaseString(
      faker.date.between(new Date(this.model.created_at), new Date())
    );
    ...
    if (this.model.status === 'paid' && count === 0) {
      ...
      createdAt = this.subscriptions.find(
        (s) => s.member_id === this.model.id
      ).created_at;
      subscribed = luck(98);
    }
