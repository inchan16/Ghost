  ...
  async addOpenRate({ emailRecipients }) {
    ...
    for (const [memberId, emailInfo] of Object.entries(memberData)) {
      const openRate = Math.round(
        100 * (emailInfo.openedCount / emailInfo.emailCount)
      );
  ...
  generate() {
    ...
    return {
      ...
      geolocation: JSON.stringify({
        ...
        organization: `${faker.random.alpha({
          count: 2,
          casing: 'upper',
        })}${faker.random.numeric(4)} ${faker.company.name()}`,
      ...
      last_seen_at: luck(40)
        ? dateToDatabaseString(faker.date.recent(7))
        : dateToDatabaseString(faker.date.between(timestamp, new Date())),
