...
class EmailRecipientsImporter extends TableImporter {
    ...
    this.events =
      this.membersList.length > 0
        ? generateEvents({
            ...
            endTime:
              currentTime < latestOpenTime ? currentTime : latestOpenTime,
          })
        : [];
    this.emailMeta = {
  ...
  generate() {
    ...
    return {
      ...
      delivered_at:
        status === emailStatus.opened
          ? dateToDatabaseString(deliveredTime)
          : status === emailStatus.delivered
          ? dateToDatabaseString(timestamp)
          : null,
      opened_at:
        status === emailStatus.opened ? dateToDatabaseString(timestamp) : null,
      failed_at:
        status === emailStatus.failed ? dateToDatabaseString(timestamp) : null,
