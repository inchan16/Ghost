...
class DataGenerator {
  ...
  constructor({
    ...
    this.schema = schema;
    this.logger = logger;
    this.modelQuantities = Object.assign(
      {},
      defaultQuantities,
      modelQuantities
    );
  ...
  async importData() {
    ...
    this.logger.info(
      'Starting import process, this has two parts: base data and member data. It can take a while...'
    );
    ...
    if (this.useBaseData) {
      ...
      try {
        baseData = JSON.parse(
          await (await fs.readFile(baseDataPack)).toString()
        );
      ...
      this.logger.info(
        'No base data pack specified, starting random base data generation'
      );
      const newslettersImporter = new NewslettersImporter(transaction);
      // First newsletter is paid, second is free
      newsletters = await newslettersImporter.import({
        amount: 2,
        rows: ['sort_order'],
      });
      newsletters.sort((a, b) => a.sort_order - b.sort_order);
      ...
      tags = await tagsImporter.import({
      ...
      products = await productsImporter.import({
        amount: 4,
        rows: ['name', 'monthly_price', 'yearly_price'],
      });

      const stripeProductsImporter = new StripeProductsImporter(transaction);
      stripeProducts = await stripeProductsImporter.importForEach(
        products.filter((product) => product.name !== 'Free'),
        {
          amount: 1,
          rows: ['product_id', 'stripe_product_id'],
        }
      ...
      const stripePricesImporter = new StripePricesImporter(transaction, {
        products,
      });
      ...
      const productsBenefitsImporter = new ProductsBenefitsImporter(
        transaction,
        { benefits }
      );
      // Up to 5 benefits for each product
      await productsBenefitsImporter.importForEach(products, {
        amount: 5,
      });
    ...
    const members = await membersImporter.import({
      amount: this.modelQuantities.members,
      rows: ['status', 'created_at', 'name', 'email', 'uuid'],
    });
    ...
    const membersProductsImporter = new MembersProductsImporter(transaction, {
      products: products.filter((product) => product.name !== 'Free'),
    });
    const membersProducts = await membersProductsImporter.importForEach(
      members.filter((member) => member.status !== 'free'),
      {
        amount: 1,
        rows: ['product_id', 'member_id'],
      }
    );
    const membersFreeProductsImporter = new MembersProductsImporter(
      transaction,
      { products: products.filter((product) => product.name === 'Free') }
    );
    await membersFreeProductsImporter.importForEach(
      members.filter((member) => member.status === 'free'),
      {
        amount: 1,
        rows: ['product_id', 'member_id'],
    ...
    const postsProductsImporter = new PostsProductsImporter(transaction, {
      products: products.slice(1),
    });
    // Paid newsletters
    await postsProductsImporter.importForEach(
      posts.filter(
        (post) =>
          newsletters.findIndex(
            (newsletter) => newsletter.id === post.newsletter_id
          ) === 0
      ),
      {
    ...
    const membersCreatedEventsImporter = new MembersCreatedEventsImporter(
      transaction,
      { baseUrl: this.baseUrl, posts }
    );
    await membersCreatedEventsImporter.importForEach(members, { amount: 1 });

    const membersLoginEventsImporter = new MembersLoginEventsImporter(
      transaction
    );
    // Will create roughly 1 login event for every 3 days, up to a maximum of 100.
    await membersLoginEventsImporter.importForEach(members, {
      amount: this.modelQuantities.membersLoginEvents,
    });

    const membersStatusEventsImporter = new MembersStatusEventsImporter(
      transaction
    );
    ...
    const subscriptionsImporter = new SubscriptionsImporter(transaction, {
      members,
      stripeProducts,
      stripePrices,
    });
    const subscriptions = await subscriptionsImporter.importForEach(
      membersProducts,
      {
        amount: 1,
        rows: [
    ...
    const membersStripeCustomersImporter = new MembersStripeCustomersImporter(
      transaction
    );
    const membersStripeCustomers =
      await membersStripeCustomersImporter.importForEach(members, {
        amount: 1,
        rows: ['customer_id', 'member_id'],
      });

    const membersStripeCustomersSubscriptionsImporter =
      new MembersStripeCustomersSubscriptionsImporter(transaction, {
        membersStripeCustomers,
        products,
        stripeProducts,
        stripePrices,
      });
    const membersStripeCustomersSubscriptions =
      await membersStripeCustomersSubscriptionsImporter.importForEach(
        subscriptions,
        {
          amount: 1,
    ...
    const membersSubscribeEventsImporter = new MembersSubscribeEventsImporter(
      transaction,
      {
        newsletters,
        subscriptions,
    ...
    const membersSubscribeEvents =
      await membersSubscribeEventsImporter.importForEach(members, {
        amount: 2,
        rows: ['member_id', 'newsletter_id', 'created_at'],
      });

    const membersNewslettersImporter = new MembersNewslettersImporter(
      transaction
    );
    await membersNewslettersImporter.importForEach(membersSubscribeEvents, {
      amount: 1,
    });

    const membersPaidSubscriptionEventsImporter =
      new MembersPaidSubscriptionEventsImporter(transaction, {
        membersStripeCustomersSubscriptions,
      });
    await membersPaidSubscriptionEventsImporter.importForEach(subscriptions, {
      amount: 1,
    });

    const membersSubscriptionCreatedEventsImporter =
      new MembersSubscriptionCreatedEventsImporter(transaction, {
        subscriptions,
        posts,
      });
    await membersSubscriptionCreatedEventsImporter.importForEach(
      membersStripeCustomersSubscriptions,
      { amount: 1 }
    );

    const mentionsImporter = new MentionsImporter(transaction, {
      baseUrl: this.baseUrl,
    });
    // Generate up to 4 webmentions per post
    await mentionsImporter.importForEach(posts, { amount: 4 });

    const emailsImporter = new EmailsImporter(transaction, {
      newsletters,
      members,
      membersSubscribeEvents,
    });
    const emails = await emailsImporter.importForEach(
      posts.filter((post) => post.newsletter_id),
      {
        amount: 1,
        rows: [
          'created_at',
    ...
    const emailRecipientsImporter = new EmailRecipientsImporter(transaction, {
      emailBatches,
      members,
      membersSubscribeEvents,
    });
    const emailRecipients = await emailRecipientsImporter.importForEach(
      emails,
      {
        amount: this.modelQuantities.members,
    ...
    const redirects = await redirectsImporter.importForEach(
      posts.filter((post) => post.newsletter_id),
      {
        amount: 10,
        rows: ['post_id'],
    ...
    const membersClickEventsImporter = new MembersClickEventsImporter(
      transaction,
      { redirects, emails }
    );
    await membersClickEventsImporter.importForEach(emailRecipients, {
      amount: 2,
    });

    const offersImporter = new OffersImporter(transaction, {
      products: products.filter((product) => product.name !== 'Free'),
    });
    await offersImporter.import({ amount: 2 });

    const membersLabelsImporter = new MembersLabelsImporter(transaction, {
      labels,
    });
    ...
    const rolesUsersImporter = new RolesUsersImporter(transaction, {
      roles,
    });
    await rolesUsersImporter.importForEach(users, { amount: 1 });

    const membersFeedbackImporter = new MembersFeedbackImporter(transaction, {
      emails,
    });
    await membersFeedbackImporter.importForEach(emailRecipients, {
      amount: 1,
    });
  ...
  async importSingleTable(tableName, quantity) {
    ...
    const importMembers = async () => {
      this.logger.info(
        `Importing ${quantity ?? this.modelQuantities.members} members`
      );
      const membersImporter = new MembersImporter(transaction);
      await membersImporter.import({
        amount: quantity ?? this.modelQuantities.members,
      });
    };
