...
class SlackNotifications {
  ...
  async notifyMilestoneReceived({ milestone, meta }) {
    ...
    const hasImportedMembers =
      meta?.reason === 'import' ? 'has imported members' : null;
    const lastEmailTooSoon =
      meta?.reason === 'email' ? 'last email too recent' : null;
    const emailNotSentReason = hasImportedMembers || lastEmailTooSoon;
    const milestoneTypePretty = milestone.type === 'arr' ? 'ARR' : 'Members';
    const valueFormatted = this.#getFormattedAmount({
      amount: milestone.value,
      currency: milestone?.currency,
    });
    const emailSentText = milestone?.emailSentAt
      ? this.#getFormattedDate(milestone?.emailSentAt)
      : `no / ${emailNotSentReason}`;
    ...
    const blocks = [
      ...
      },
      {
        ...
        text: {
          type: 'mrkdwn',
          text: `New *${milestoneTypePretty} Milestone* achieved for <${
            this.#siteUrl
          }|${this.#siteUrl}>`,
        },
      },
